...
는 괜찮은 성능과 확장성을 얻기 위해 파티셔닝을 적용해야 하는 필요를
강조했으므로, 파티셔닝 가능성은 데이터 구조를 고를 때 첫번째 고려사항이
되어야만 합니다.
이 고려사항은 병렬성에서 널리 사용되는 해쉬 테이블에 의해 잘 만족됩니다.
해쉬 테이블은 컨셉상으로 간단하여, \emph{해쉬 버킷} 의 배열로 구성됩니다.
하나의 \emph{해쉬 함수} 가 주어진 원소의 \emph{키} 를 이 원소가 저장되어 있는
해쉬 버킷으로 매핑합니다.
따라서 각 해쉬 버킷은 \emph{해쉬 체인} 이라 불리는 원소들의 링크드 리스트를
갖습니다.
올바르게 구성되었을 경우, 이 해쉬 체인은 상당히 짧아서, 해쉬 테이블이 그
원소들에 굉장히 효율적으로 접근할 수 있게 합니다.
...
\QuickQuiz{
	하지만 체인으로 연결된 해쉬 테이블은 많은 종류 중 하나입니다.
	왜 체인으로 연결된 해쉬 테이블에 집중하죠?
	...
	체인으로 연결된 해쉬 테이블은 완벽히 파티셔닝 가능하며 따라서 동시성
	사용에 잘 맞습니다.
	예를 들어 split-ordered list~\cite{OriShalev2006SplitOrderListHash}
	같은 다른 완벽하게 파티셔닝 가능한 해쉬 테이블도 존재하지만, 상당히 더
	복잡합니다.
	그러므로 우리는 체인으로 연결된 해쉬 테이블에서 시작하겠습니다.
...
추가적으로, 각 버킷은 각자의 락을 가지고 있어서 이 해쉬 테이블에서의 다른
...
\clnrefrange{tab:b}{tab:e}) 네개의 \co{ht_bucket} 구조체를
(Listing~\ref{lst:datastruct:Hash-Table Data Structures} 의
\clnrefrange{bucket:b}{bucket:e}) 가지며, \co{->ht_nbuckets} 필드는 버킷의
갯수를 제어하고 \co{->ht_cmp} 필드는 키 비교 함수로의 포인터를 갖습니다.
그런 각 버킷은 리스트 헤더 \co{->htb_head} 와 하나의 락 \co{->htb_lock} 을
갖습니다.
그것들의 \co{->hte_next} 필드를 통한 이 리스트 헤더는 \co{ht_elem} 구조체를
(Listing~\ref{lst:datastruct:Hash-Table Data Structures} 의
\clnrefrange{elem:b}{elem:e}) 연결하며, 각 \co{ht_elem} 구조체는 또한 연관된
원소의 해쉬 값을 \co{->hte_hash} 필드에 캐쉬해 둡니다.
\co{ht_elem} 구조체는 복잡한 키를 가지고 있을수도 있는 더 커다란 구조체에
포함됩니다.
\end{fcvref}
...
은 매핑과 락킹 함수들을 보입니다.
라인~\lnref{b} 와~\lnref{e} 는 \co{HASH2BKT()} 매크로를 보이는데, 이 매크로는
해쉬 값을 연관된 \co{ht_bucket} 구조체로 매핑합니다.
이 매크로는 간단한 모듈로 연산을 사용합니다: 더 강력한 해싱이 필요하다면,
호출자는 키에서 해쉬 값으로의 매핑 시에 그것을 구현해야 합니다.
남아있는 두 함수는 명시된 해쉬 값에 연관된 \co{->htb_lock} 을 획득하고
해제합니다.
\end{fcvref}
...
은 \co{hashtab_lookup()} 을 보이는데, 이 함수는 명시된 해쉬와 키를 가지고 그에
연관된 원소가 존재한다면 그 원소로의 포인터를, 아니라면 \co{NULL} 을
리턴합니다.
이 함수는 해쉬 값과 키로의 포인터를 둘 다 받는데 이는 이 함수의 사용자들이
임의의 키와 임의의 해쉬 함수를 사용할 수 있게 하기 위함입니다.
라인~\lnref{map} 은 이 해쉬 값에서 연관된 해쉬 버킷으로의 포인터로의 매핑을
합니다.
\Clnrefrange{loop:b}{loop:e} 의 루프의 각 패스는 이 버킷의 해쉬 체인의 원소를
하나씩 검사합니다.
라인~\lnref{hashmatch} 는 이 해쉬 값이 맞는지 검사하고, 그렇지 않다면
라인~\lnref{next} 에서 다음 원소로 넘어갑니다.
라인~\lnref{keymatch} 는 실제 키가 들어맞는지 확인하고, 그렇다면
라인~\lnref{return} 에서 이 맞는 원소로의 포인터를 리턴합니다.
어떤 원소도 들어맞지 않으면, 라인~\lnref{ret_NULL} 이 \co{NULL} 을 리턴합니다.
\end{fcvref}
...
\QuickQuiz{
	\begin{fcvref}[ln:datastruct:hash_bkt:lookup]
	하지만
	Listing~\ref{lst:datastruct:Hash-Table Lookup} 의
	\clnrefrange{hashmatch}{return} 에서의 두번의 비교는 키가 unsigned long
	에 들어갈 때에는 비효율적이지 않나요?
	\end{fcvref}
	...
	실제로 그렇습니다!
	그렇지만, 해쉬 테이블은 상당히 자주 unsinged long 에 들어맞지는 않는
	문자열과 같은 키를 사용해 정보를 저장합니다.
	이 해쉬 테이블 구현을 키가 항상 unsinged long 에 들어가는 경우를 위해
	단순화 하는 것은 독자 여러분의 연습으로 남겨두겠습니다.
...
은 해쉬 테이블로부터 원소를 각각 더하고 빼는 \co{hashtab_add()} 와
\co{hashtab_del()} 함수들을 보입니다.

\begin{fcvref}[ln:datastruct:hash_bkt:add_del:add]
\co{hashtab_add()} 함수는 단순히 원소의 해쉬 값을 라인~\lnref{set} 에서
설정하고, 라인~\lnref{add:b} 와~\lnref{add:e} 에서 연관된 버킷에 이를 더합니다.
\end{fcvref}
\co{hashtab_del()} 함수는 단순히 명시된 원소를 그것이 어디였든 연결되어 있던
곳에서 제거하는데, 해쉬 체인 리스트의 양방향 링크드 리스트 덕입니다.
이 두 함수를 호출하기 전에, 호출자는 어떤 다른 쓰레드도 이 같은 버킷을
접근하거나 수정하고 있지 않다는 것을 분명히 해야 하는데, 예를 들면
\co{hashtab_lock()} 을 호출할 수 있습니다.
...
은 \co{hashtab_alloc()} 과 \co{hashtab_free()} 를 보이는데, 이것들은 각각 해쉬
테이블 메모리 할당과 해제를 합니다.
\begin{fcvref}[ln:datastruct:hash_bkt:alloc_free:alloc]
할당은 \clnrefrange{alloc:b}{alloc:e} 에서 메모리 할당과 함께 시작됩니다.
라인~\lnref{chk_NULL} 이 메모리가 부족함을 확인하면, 라인~\lnref{ret_NULL} 이
호출자에게 \co{NULL} 을 리턴합니다.
그렇지 않다면, 라인~\lnref{set_nbck} 와~\lnref{set_cmp} 가 버킷의 갯수와 키
비교 함수로의 포인터를 초기화 하며, \clnrefrange{loop:b}{loop:e} 의 루프는 버킷
자체를 초기화 하는데, 라인~\lnref{init_head} 의 체인 리스트 헤더와
라인~\lnref{init_lock} 의 락 초기화를 포함합니다.
마지막으로, 라인~\lnref{return} 은 새로 할당된 해쉬 테이블로의 포인터를
리턴합니다.
\end{fcvref}
\begin{fcvref}[ln:datastruct:hash_bkt:alloc_free:free]
\Clnrefrange{b}{e} 에서의 \co{hashtab_free()} 함수는 직관적입니다.
\end{fcvref}
...
버킷 락킹 기반 해쉬 테이블을 262,144 버킷을 가지고 사용한 2.1\,GHz 28 코어 싱글
...
그리고 더 많은 CPU 에서는 상황이 더 나빠질 뿐인데,
Figure~\ref{fig:datastruct:Read-Only Hash-Table Performance For Schroedinger's Zoo; 448 CPUs}
에 보인 것과 같습니다.
우린 이상적 성능을 보일 필요는 없습니다: 29~CPU 와 그 뒤에서의 성능은
지옥보다도 나쁠 것이 너무 분명합니다.
이는 적은 수의 CPU 에서의 성능만 사용한 외삽의 위험성을 분명히 강조합니다.

물론, 이 성능 추락의 한가지 가능한 이유는 더 많은 해쉬 버킷이 필요해서일 수도
있습니다.
우린 해쉬 버킷의 수를 늘림으로써 이를 테스트 할 수 있습니다.
...
\QuickQuiz{
	단순히 해쉬 버킷의 수를 늘리는 대신, 존재하는 해쉬 버킷을 캐쉬 얼라인
	시키는게 낫지 않을까요?
	...
	그에 대한 답은 많은 것에 의존적입니다.
	이 해쉬 테이블이 버킷마다 많은 수의 원소를 가지고 있다면, 해쉬 버킷의
	갯수를 늘리는게 분명 나을 겁니다.
	반면에, 이 해쉬 테이블이 큰 로드를 가지고 있지 않다면, 그 답은
	하드웨어, 해쉬 함수의 효율성, 그리고 워크로드에 종속적일 겁니다.
	흥미 있는 독자 여러분은 실험해 보시기 바랍니다.
...
\QuickQuiz{
	소켓을 가로지르는 경우의 Schr\"odinger 의 동물원의 나쁜 확장성을
	생각하면, 이 어플리케이션의 여러 복사본을 각 복사본은 동물들의
	부분집합만을 가지고 각 복사본을 단일 소켓으로 가둬서 수행시키는 건
	어떤가요?
	...
	그럴 수 있습니다!
	실제로, 여러분은 이 아이디어를 이 어플리케이션의 한 복사본을 클러스터의
	각 노드에서 수행시키는 커다란 클러스터 시스템으로 확장시킬 수 있습니다.
	이 방법은 ``sharding'' 이라 불리며, 커다란 웹 기반 상점들에서 많이
	사용되고 있습니다~\cite{DeCandia:2007:DAH:1323293.1294281}.

	하지만, 여러분이 멀티소켓 시스템에서 소켓별 방식으로 sharding 을
	할거라면, 분리된 더 작고 저렴한 싱글 소켓 시스템을 여럿 사서 이 데이터
	베이스의 한shard 를 각 시스템에서 돌리는게 어떻습니까?
...
지금까지 이야기된 Schr\"odinger 의 동물원의 한가지 핵심 요소는 읽기 전용이라는
것입니다.
...
확장성을 모두 얻을 수 있습니다.
비슷한 결과가 해저드 포인터를 (\path{hazptr.c})~\cite{MagedMichael04a}
...
\co{HASH2BKT()}, \co{hashtab_lock()}, 그리고 \co{hashtab_unlock()} 함수는
...
에 보인, \co{hashtab_lock_lookup()} 과 \co{hashtab_unlock_lookup()} 에 포함된
더 가벼운 동시성 제어를 사용합니다.
...
\co{hashtab_lookup()} 을 보입니다.
이는 \co{cds_list_for_each_entry()} 가 \co{cds_list_for_each_entry_rcu()} 로
대체되었다는 것을 제외하면
Listing~\ref{lst:datastruct:Hash-Table Lookup} 의 것과 동일합니다.
이 두 함수들 모두 \co{htb->htb_head} 로 참조되는 해쉬 체인을 순회합니다만
\co{cds_list_for_each_entry_rcu()} 는 동시의 삽입 시에 메모리 순서를 올바르게
강제합니다.
이게 이 두 해쉬 테이블 구현 사이의 중요한 차이점입니다:
순수한 버킷별 락킹 구현과 달리, RCU 로 보호되는 구현은 탐색이 삽입과 삭제와
동시에 이루어질 수 있게 하며, \co{cds_list_for_each_entry_rcu()} 와 같은 RCU 를
인지하는 기능들은 이 추가된 동시성을 올바르게 처리할 필요를 갖습니다.
또한 \co{hashtab_lookup()} 의 호출자는 RCU read-side 크리티컬 섹션 내에
있어야만 하는데, 예를 들면 호출자는 \co{hashtab_lookup()} 호출 전에
\co{hashtab_lock_lookup()} 을 (그리고 물론 얼마 후에는
\co{hashtab_unlock_lookup()} 을) 호출해야만 함을 알아두시기 바랍니다.
...
\QuickQuiz{
	하지만 해쉬 테이블 내의 원소들이 탐색과 동시에 제거될 수 있다는 것은,
	탐색이 찾아진 직후 제거된 데이터 원소로의 참조를 리턴할 수 있다는 의미
	아닌가요?
	...
	맞아요 그럴 수 있습니다!
	이게 왜 \co{hashtab_lookup()} 이 RCU read-side 크리티컬 섹션 내에서
	호출되어야 하는지에 대한 이유이며, \co{hashtab_add()} 와
	\co{hashtab_del()} 이 RCU 를 인지하는 리스트 조정 기능을 써야만 하는
	이유입니다.
	마지막으로, 이는 왜 \co{hashtab_del()} 호출자가 제거한 원소를 메모리
	해제하기 전에 하나의 grace period 를 기다려야만 (예:
	\co{synchronize_rcu()} 를 호출함으로써) 하는 이유입니다.
	이는 모든 RCU 읽기 쓰레드가 제거된 원소들을 메모리 해제되기 전에
	참조함을 보장할 겁니다.
...
은 \co{hashtab_add()} 와 \co{hashtab_del()} 을 보이는데, 둘 다
Listing~\ref{lst:datastruct:Hash-Table Modification} 에 보인 RCU 기반이 아닌
해쉬 테이블의 것들과 상당히 비슷합니다.
\co{hashtab_add()} 함수는 어떤 원소를 탐색되고 있는 사이 이 해쉬 테이블에
추가하는 경우 올바른 순서를 보장하기 위해 \co{cds_list_add()} 대신
\co{cds_list_add_rcu()} 를 사용합니다.
\co{hashtab_del()} 함수는 원소가 삭제되기 전에 탐색되는 경우를 허용하기 위해
\co{cds_list_del_init()} 대신 \co{cds_list_del_rcu()} 를 사용합니다.
\co{cds_list_del_init()} 와 달리, \co{cds_list_del_rcu()} 는 앞으로의 포인터를
원래대로 두어서 \co{hashtab_lookup()} 이 새로이 제거된 원소의 다음 원소를
따라갈 수 있게 합니다.

물론, \co{hashtab_del()} 을 호출한 다음에는, 해당 원소의 메모리를 해제하기 전에
하나의 RCU grace period 를 기다려야 하며 (예: \co{synchronize_rcu()} 호출을
통해), 그러지 않으면 이 새로이 삭제된 원소의 메모리를 재사용하게 되어버립니다.
...
하지만 RCU 의 성능은 왜 이상적인 경우보다 다섯배나 적을까요?
한가지 가능성은 \co{rcu_read_lock()} 과 \co{rcu_read_unlock()} 으로 조절되는
...
각 해쉬 버킷은 (\co{struct ht_bucket}) 56~바이트를 사용하며 각 원소는
(\co{struct zoo_he}) RCU 와 QSBR 의 경우 72~바이트를 사용합니다.
Figure~\ref{fig:datastruct:Read-Only RCU-Protected Hash-Table Performance For Schroedinger's Zoo including QSBR and Unsynchronized, Linear Scale}
...
\QuickQuiz{
	...
	Figure~\ref{fig:datastruct:Read-Only Hash-Table Performance For Schroedinger's Zoo, Varying Buckets}
	가 해쉬 테이블 크기를 변화시키는게 거의 아무 효과도 내지 못하는 걸
	보이는데 해쉬 테이블 크기가 문제라고 여기서 확신할 수 있나요?
	문제는 그게 아니라 false sharing 일수도 있지 않을까요?
	...
	False sharing 은 쓰기를 필요로 하는데, 이 탐색만 하는 벤치마크의
	동기화되지 않은 수행이나 RCU 수행에서는 사용되지 않습니다.
	따라서 문제는 false sharing 이 아닙니다.
...
\QuickQuiz{
	이 커다란 시스템에서는 메모리 시스템이 상당한 병목입니다.
	유용한 뭔가를 하기 충분한 메모리 대역폭을 주지 않고서 시스템에 448~CPU
	를 장착해서는 왜 고생할까요?
	...
	이 커다랗고 비싼 시스템을 작은 데이터 원소들로 구성된 간단한 해쉬
	테이블 탐색에만 사용하는 것은 실제로 나쁜 생각일 겁니다.
	하지만, 이 시스템은 더 많은 계산과 적은 메모리 액세스를 사용하는 무척
	많은 워코로드에 매우 유용합니다.
	예를 들어, 어떤 인메모리 데이터베이스는 이런 종류의 시스템에서 무척 잘
	동작하는데, 이 챕터의 벤치마크에서 수행하는 것에 비해 훨씬 복잡한 쿼리
	집합을 사용할 때 뿐이긴 합니다.
	예를 들어, 그런 시스템은 각 원소에 저장된 이미지나 비디오를 처리해서
	그로 인해 생겨나는 순차적 메모리 액세스가 가용한 메모리 대역폭의 더
	나은 사용을 하여서 순전히 포인터 따라가기만 하는 워크로드에 비해 추가적
	성능 이득을 얻을 겁니다.

	하지만 이걸 여러분의 교훈으로 삼기 바랍니다.
	현대의 컴퓨터 시스템은 매우 다양한 형태와 크기로 나오며, 여러분의
	어플리케이션에 맞는 것을 고르는데는 상당한 주의가 종종 필요합니다.
	그리고 여러분의 어플리케이션을 특정 컴퓨터 시스템에 맞춰 조정하는데는
	아마 그보다도 자주 상당한 주의와 작업이 필요합니다.
...
우리가 데이터를 읽기만 할거라면, 애초에 동시성 제어가 필요 없을 겁니다.
따라서
...
탐색 성능의 차이점이 업데이트 비율에 의해 영향을 받았을 가능성이 상당히
있습니다.
이를 확인하는 한가지 방법은 인위적으로 버킷별 락킹과 해저드 포인터의 업데이트
비율을 RCU 의 그것과 맞게 조정하는 것입니다.
그렇게 하는 것이 버킷별 락킹의 탐색 성능을 눈에 띄게 개선하지는 않으며, 해저드
포인터와 RCU 사이의 차이를 메우지도 않습니다.
하지만, 해저드 포인터의 읽기 쪽 메모리 배리어를 제거하는 것은 (따라서 안전치
않은 구현이 되는 것은) 해저드 포인터와 RCU 사이의 차이를 거의 메웁니다.
이 안전하지 않은 해저드 포인터 구현은 벤치마킹 목적에는 충분히 안전하겠지만
제품에서의 사용에는 결코 추천되지 않습니다.
...
\QuickQuiz{
	28~CPU 에서의 결과를 448~CPU 로 외삽하는 것의 위험성은
	Section~\ref{sec:datastruct:Hash-Table Performance} 에서 분명히
	보여졌습니다.
	하지만 왜 448~CPU 에서의 결과로 그 이상을 외삽하는 것은 안전한가요?
	...
	이론상 이는 결코 안전하지 않으며 유용한 행위는 이 프로그램을 더 큰
	시스템에서 돌려보는 것일 겁니다.
	실용적으로는, 448~CPU 이상의 시스템보다 28~CPU 이상의 것이 훨씬
	많습니다.
	더해서, 다른 테스트가 RCU read-side 기능들이 1024~CPU 까지의 일관적인
	성능과 확장성을 보임을 보인 바 있습니다.
...
\pplsur{Weiner}{Heisenberg} 는 우리에게 이런 종류의 불확실성을 가지고 살라고
...
\ppl{Herbert}{Xu}~\cite{HerbertXu2010RCUResizeHash} 에 의해 개발되었으며, 다음
섹션에서 설명됩니다.
나머지 두개는
Section~\ref{sec:datastruct:Other Resizable Hash Tables}
에서 간단히 다룹니다.

첫번째 해쉬 테이블 구현의 핵심 아이디어는 각 데이터 원소가 두개의 리스트
...
크기 재조정은 고전적인 간접 조정 단계를 추가하는 접근법으로 행해지는데, 이 경우
Listing~\ref{lst:datastruct:Resizable Hash-Table Data Structures}
(\path{hash_resize.c}) 의
\clnrefrange{ht:b}{ht:e} 에 보여진 \co{ht} 구조체입니다.
\Clnrefrange{hashtab:b}{hashtab:e} 에 보여진 \co{hashtab} 구조체는 현재의
\co{ht} 구조체로의 구조체와 함께 동시의 해쉬 테이블을 크기 조정들을 직렬화 하기
위한 스핀락을 갖습니다.
우리가 고전적인 락 또는 어토믹 오퍼레이션 기반 구현을 사용하기로 했다면, 이
\co{hashtab} 구조체는 성능과 확장성 측면에서 상당한 병목이 될 수 있었을 겁니다.
하지만, 크기 재조정 오퍼레이션은 상대적으로 흔히 행해지지 않기 때문에, RCU 를
잘 사용할 수 있을 겁니다.
...
\co{ht} 구조체는
라인~\lnref{ht:nbuckets} 의 \co{->ht_nbuckets} 에 보여진 것처럼 해쉬 테이블의
구체적 크기를 표현합니다.
이 크기는 버킷 배열을 담는 같은 구조체 (line~\lnref{ht:bkt} 의 \co{->ht_bkt[]})
에도 크기와 배열 사이의 어긋남을 막기 위해 저장됩니다.
라인~\lnref{ht:resize_cur} 의 \co{->ht_resize_cur} 필드는 크기 재조정
오퍼레이션이 진행 중이 아니라면 $-1$ 과 동일한데, 원소들이 새로운 해쉬 테이블에
삽입되고 있는 버킷의 인덱스를 가리키는데, 라인~\lnref{ht:new} 의 \co{->ht_new}
를 통해 참조됩니다.
크기 재조정 오퍼레이션이 진행중이지 않다면, \co{->ht_new} 는 \co{NULL} 입니다.
따라서, 크기 재조정은 새로운 \co{ht} 구조체를 할당하고 그것을 \co{->ht_new}
포인터를 통해 참조하며, 이어서 \co{->ht_resize_cur} 를 기존 테이블의 버킷을
통해 진행시킴으로써 이루어집니다.
모든 원소가 이 새 테이블에 더해진 후에는 새 테이블이 \co{hashtab} 구조체의
\co{->ht_cur} 필드로 연결됩니다.
모든 기존 읽기 쓰레드들이 완료된 후에는 기존 해쉬 테이블의 \co{ht} 구조체는
메모리 해제 될 수 있습니다.
...
라인~\lnref{ht:idx} 의 \co{->ht_idx} 필드는 이 두개의 리스트 포인터 중 어느
것이 이 해쉬 테이블 현현물에 의해 사용되고 있는지를 나타내며,
라인~\lnref{ht_elem:next} 의 \co{ht_elem} 구조체의 \co{->ht_next[]} 배열을
인덱스 합니다.

\Clnrefrange{ht:cmp}{ht:getkey} 의
\co{->ht_cmp()}, \co{->ht_gethash()}, 그리고 \co{->ht_getkey()} 필드는 원소별
키와 해쉬 함수를 정의합니다.
\co{->ht_cmp()} 함수는 명시된 키를 명시된 원소와 비교하고, \co{->ht_gethash()}
는 명시된 키의 해쉬를 계산하며, \co{->ht_getkey()} 는 감싸는 데이터 원소로부터
키를 얻어옵니다.

\Clnrefrange{hls:b}{hls:e} 에 보인
\co{ht_lock_state} 는 새로운 \co{hashtab_lock_mod()} 로부터의 락 상태를
\co{hashtab_add()}, \co{hashtab_del()} 그리고 \co{hashtab_unlock_mod()} 에
전달하기 위해 사용됩니다.
...
\co{ht_bucket} 구조체는 이전과 같으며, \co{ht_elem} 구조체는 앞의 구현에서는
하나의 리스트 포인터 집합만 제공했던데 반해 리스트 포인터 집합의 두개 원소
배열을 제공한다는 점만이 다릅니다.

고정크기 해쉬 테이블에서 버킷 현택은 상당히 간단했습니다:
단순히 해쉬 값을 연관된 버킷 인덱스로 변환했습니다.
반대로, 크기를 재조정 할 때에는 버킷의 기존과 새 것들 중 어떤 것에서 선택을
할지 결정해야 합니다.
기존 테이블에서 선택될 수 있는 버킷이 이미 새 테이블로 분산되었다면, 이 버킷은
새 테이블에서는 물론이고 기존 테이블에서도 선택되어야 합니다.
반대로 이 기존 테이블에서 선택되었을 수 있는 버킷이 아직 분산되지 않았다면, 이
버킷은 기존 테이블에서 선택되어야 합니다.
\end{fcvref}
...
\clnrefrange{single:b}{single:e} 에 \co{ht_get_bucket()} 를,
\clnrefrange{hsb:b}{hsb:e} 에 \co{ht_search_bucket()} 를 보이는
Listing~\ref{lst:datastruct:Resizable Hash-Table Bucket Selection} 에 버킷
선택이 보여져 있습니다.
\co{ht_get_bucket()} 함수는 크기 재조정을 허용하지 않으면서 명시된 해쉬 테이블
내에서 명시된 키에 연관된 버킷으로의 참조를 반환합니다.
이 함수는 또한 라인~\lnref{single:gethash} 에서 이 키에 연관된 버킷 인덱스를
패러미터~\co{b} 로 참조되는 위치에 저장하며, 라인~\lnref{single:h} 에서
패러미터~\co{h} 로 참조되는 위치에 (\co{h} 가 \co{NULL} 이 아니라면) 이 키에
연관되는 해쉬 값을 저장합니다.

\co{ht_search_bucket()} 함수는 명시된 해쉬 테이블 버전 내에서 명시된 키를 위한
탐색을 합니다.
라인~\lnref{hsb:get_curbkt} 은 명시된 키에 연관된 버킷으로의 참조를 얻습니다.
\Clnrefrange{hsb:loop:b}{hsb:loop:e} 의 반복문은 그 버킷을 찾아서
라인~\lnref{hsb:match} 가 들어맞음을 발견하면 라인~\lnref{hsb:ret_match} 는
이를 감싸는 데이터 원소로의 포인터를 반환하게 합니다.
그렇지 않고 들어맞는 경우가 없다면 라인~\lnref{hsb:ret_NULL} 은 실패를 알리기
위해 \co{NULL} 을 반환합니다.
\end{fcvref}
...
\QuickQuiz{
	Listing~\ref{lst:datastruct:Resizable Hash-Table Bucket Selection}
	의 코드는 크기 재조정 프로세스가 선택된 버킷을 지나가는 걸 어떻게
	보호합니까?
	...
	그런 보호는 제공하지 않습니다.
	그건 이어서 설명될 업데이트 쪽 동시성 제어 기능의 역할입니다.
...
이 \co{ht_get_bucket()} 과 \co{ht_search_bucket()} 의 구현은 탐색과 수정이 크기
재조정 오퍼레이션과 동시에 이루어질 수 있게 합니다.
...
읽기 쪽 동시성 제어는
Listing~\ref{lst:datastruct:RCU-Protected Hash-Table Read-Side Concurrency Control}
에 보여졌듯 RCU 에 의해 제공됩니다만, 업데이트 쪽 동시성 제어 기능인
...
에 보이듯 동시의 크기 재조정 오퍼레이션의 가능성을 처리해야만 합니다.
...
\co{hashtab_lock_mod()} 는 이 리스트의 \clnrefrange{b}{e} 에 있습니다.
라인~\lnref{rcu_lock} 은 데이터 구조가 순회 도중 메모리 해제되는 걸 막기 위해
RCU read-side 크리티컬 섹션에 들어가고, 라인~\lnref{refhashtbl} 은 현재 해쉬
테이블로의 참조를 획득하며, 라인~\lnref{refbucket} 은 이 키에 연관되는 해쉬
테이블의 버킷으로의 참조를 얻습니다.
라인~\lnref{acq_bucket} 은 이 버킷의 락을 획득하는데, 이는 이 버킷이 동시의
크기 재조정 오퍼레이션에 의해 분산되는 것을 막을 것인데, 물론 이 버킷이 이미
분산되어 있다면 아무 영향을 끼치지 못할 것이긴 합니다.
\Clnrefrange{lsp0b}{lsp0e} 는 버킷 포인터와 포인터 집합 인덱스를
\co{ht_lock_state} 구조체의 연관된 필드에 저장하는데, 이는 \co{hashtab_add()},
\co{hashtab_del()}, 그리고 \co{hashtab_unlock_mod()} 에게 이 정보를 전달하는
것입니다.
라인~\lnref{ifresized} 는 동시의 크기 재조정 오퍼레이션이 이미 이 버킷을 새
해쉬 테이블로 분산시켰는지 검사하고, 그렇지 않다면 라인~\lnref{lsp1_1} 에서
이미 크기 재조정된 해쉬 버킷이 없음을 알리고 라인~\lnref{fastret1} 에서 선택된
...
그렇지 않고 동시의 크기 재조정 오퍼레이션이 이미 이 버킷을 분산시켰다면,
라인~\lnref{new_hashtbl} 은 새 해쉬 테이블로 넘어가서 라인~\lnref{get_newbkt}
이 이 키에 연관된 버킷을 선택하고 라인~\lnref{acq_newbkt} 에서 이 버킷의 락을
획득합니다.
\Clnrefrange{lsp1b}{lsp1e} 는 이 버킷 포인터와 포인터 집합 인텍스를
\co{ht_lock_state} 구조체의 연관된 필드에 저장하는데, 다시 말하지만 이 정보를
\co{hashtab_add()}, \co{hashtab_del()}, 그리고 \co{hashtab_unlock_mod()} 에게
전달하기 위함입니다.
이 버킷은 이미 크기 재조정 되었고 \co{hashtab_add()} 와 \co{hashtab_del()} 이
기존과 새로운 \co{ht_bucket} 구조체 모두에 영향을 끼쳤으므로, 두개의 버킷 각각
하나씩 두개의 락이 잡혀 있습니다.
더해서, \co{ht_lock_state} 구조체의 각 배열의 원소 둘 다 사용되었는데, \co{[0]}
원소는 기존 \co{ht_bucket} 구조체를 따르고 있고 \co{[1]} 원소는 새 구조체를
따르고 있습니다.
다시 말하지만, \co{hashtab_lock_mod()} 는 RCU read-side 크리티컬 섹션에
있습니다.
\end{fcvref}
...
\co{hashtab_unlock_mod()} 함수는 \co{hashtab_lock_mod()} 에 의해 획득된 락들을
해제합니다.
라인~\lnref{relbkt0} 는 기존 \co{ht_bucket} 구조체의 락을 해제합니다.
라인~\lnref{ifbkt1} 이 크기 재조정 오퍼레이션이 진행 중이라는 드문 이벤트를
감지하면 라인~\lnref{relbkt1} 은 새 \co{ht_bucket} 구조체의 락을 해제합니다.
어떤 경우든 라인~\lnref{rcu_unlock} 은 RCU read-side 크리티컬 섹션을
빠져나갑니다.
\end{fcvref}
...
\QuickQuiz{
	한 쓰레드가 크기 재조정 오퍼레이션 중에 이 해쉬 테이블에 원소를 하나
	넣는다고 생각해 봅시다.
	뒤따르는 크기 재조정 오퍼레이션이 이 삽입 전에 완료됨으로 인해 이
	삽입이 사라지는 것은 어떻게 방지됩니까?
	...
	이 두번째 크기 재조정 오퍼레이션은 이 버킷을 이 삽입이 위치하는 곳으로
	옮길 수 없을텐데 이 삽입은 이 해쉬 테이블의 해쉬 버킷 하나 또는 둘
	모두의 락을 잡을 것이기 때문입니다.
	더 나아가, 이 삽입 오퍼레이션은 RCU read-side 크리티컬 섹션 내에 위치해
	있습니다.
	\co{hashtab_resize()} 함수를 살펴 볼 때 보게 되겠지만, 이는 각 크기
	재조정 오퍼레이션이 이 삽입의 read-side 크리티컬 섹션이 완료되길
	기다리기 위해 \co{synchronize_rcu()} 를 호출함을 의미합니다.
...
이제 우린 버킷 선택과 동시성 제어를 가지고 있으므로, 이 크기 재조정 가능한 해쉬
테이블을 탐색하고 업데이트할 준비가 되었습니다.
\co{hashtab_lookup()}, \co{hashtab_add()}, 그리고 \co{hashtab_del()} 함수가
Listing~\ref{lst:datastruct:Resizable Hash-Table Access Functions} 에 있습니다.

\begin{fcvref}[ln:datastruct:hash_resize:access:lkp]
이 리스트의 \clnrefrange{b}{e} 에 있는 \co{hashtab_lookup()} 함수가 해쉬 탐색을
합니다.
라인~\lnref{get_curtbl} 은 현재 해쉬 테이블을 가져오고 라인~\lnref{get_curbkt}
이 이 키에 연관된 버킷을 찾습니다.
라인~\lnref{ret} 는 탐색된 원소로의 포인터를, 탐색이 실패한 경우 \co{NULL} 을
반환합니다.
호출자는 RCU read-side 크리티컬 섹션 내에 있어야만 합니다.
\end{fcvref}
...
\QuickQuiz{
	Listing~\ref{lst:datastruct:Resizable Hash-Table Access Functions}
	의 \co{hashtab_lookup()} 함수는 동시의 크기 재조정 오퍼레이션을
	무시합니다.
	이는 읽기 쓰레드가 크기 재조정 오퍼레이션 중에 추가된 원소를 놓칠 수도
	있음을 의미하지 않습니까?
	...
	아니요.
	곧 보게 되겠지만, \co{hashtab_add()} 와 \co{hashtab_del()} 함수는 기존
	해쉬 테이블을 크기 재조정 오퍼레이션이 진행 중인 동안 최신 상태로
	유지합니다.
...
이 리스트의 \clnrefrange{b}{e} 에 있는 \co{hashtab_add()} 함수는 이 해쉬
테이블에 새 데이터 원소를 추가합니다.
라인~\lnref{htbp} 는 이 새 원소가 추가될 현재의 \co{ht_bucket} 구조체를 고르고,
라인~\lnref{i} 는 이 포인터 상의 인덱스를 가져옵니다.
라인~\lnref{add} 는 이 새 원소를 현재의 해쉬 버킷에 더합니다.
라인~\lnref{ifnew} 가 이 버킷이 새 버전의 해쉬 테이블로 분산되었음을 파악하면
라인~\lnref{addnew} 가 이 새 원소를 연관된 새 버킷에도 더합니다.
호출자는 동시성을 제어할 책임을 갖는데, 예를 들어 \co{hashtab_add()}의 호출
전에는 \co{hashtab_lock_mod()} 를, 후에는 \co{hashtab_unlock_mod()} 를 호출하는
식입니다.
\end{fcvref}
...
이 리스트의 \clnrefrange{b}{e} 에 있는 \co{hashtab_del()} 함수는 이 해쉬
테이블에서 존재하는 원소 하나를 제거합니다.
라인~\lnref{i} 는 포인터 쌍의 인덱스를 가져오고 라인~\lnref{del} 에서 명시된
원소를 현재 테이블에서 제거합니다.
라인~\lnref{ifnew} 가 이 버킷이 새 버전의 해쉬 테이블로 분산되었음을 파악하면
라인~\lnref{delnew} 가 명시된 원소를 연관된 새 버킷에서도 제거합니다.
\co{hashtab_add()} 에서와 마찬가지로 호출자는 동시성 제어의 책임을 가지며 이
...
\QuickQuiz{
	Listing~\ref{lst:datastruct:Resizable Hash-Table Access Functions}
	의 \co{hashtab_add()} 와 \co{hashtab_del()} 함수는 크기 재조정
	오퍼레이션이 진행 중인 사이에 두개의 해쉬 버킷을 업데이트 할 수
	있습니다.
	이는 크기 재조정 오퍼레이션의 빈도가 무시할 만한 수준이 아니라면 성능
	저하를 일으킬 수도 있을 겁니다.
	그런 경우에 업데이트의 비용을 줄일 수 있지 않습니까?
	...
	최소한 \co{hashtab_lookup()} 비용의 약간의 상승이 받아들여질 수 있다면
	그렇습니다.
	그런 접근법이
	Listings~\ref{lst:datastruct:Resizable Hash-Table Access Functions (Fewer Updates)}
	과~\ref{lst:datastruct:Resizable Hash-Table Update-Side Locking Function (Fewer Updates)}
	(\path{hash_resize_s.c}) 에 보여져 있습니다.
...
\end{listing}
	...
	이 버전의 \co{hashtab_add()} 는 원소를 아직 크기 재조정이 되지 않았다면
	기존 버킷에, 이미 크기 재조정이 되었다면 새 버킷에 저장하며
	\co{hashtab_del()} 은 명시된 원소를 어디든 그게 삽입된 버킷에서
	제거합니다.
	\co{hashtab_lookup()} 함수는 기존 버킷 탐색이 실패하면 새 버킷을
	탐색하는데, 이는 탐색 fastpath 에 오버헤드를 더하는 단점을 갖습니다.
	대안적인 \co{hashtab_lock_mod()} 는 크기 재조정 오퍼레이션이 진행
	중이라면 새 버킷의 \co{->hbp[0]} 와 \co{->hls_idx[0]} 의 새 버킷으로의
	락킹 상태를 아마도 그보다 자연스런 선택일 \co{->hbp[1]} 과
	\co{->hls_idx[1]} 대신 리턴합니다.
	그러나, 이 덜 자연스러운 선택은 \co{hashtab_add()} 를 단순화 시키는
	이익을 갖습니다.
...
에 보여져 있습니다.
라인~\lnref{trylock} 은 조건적으로 꼭대기 단계의 \co{->ht_lock} 을 획득하고, 이
획득이 실패하면 라인~\lnref{ret_busy} 에서 크기 재조정이 이미 진행 중임을
알리는 \co{-EBUSY} 를 리턴합니다.
그렇지 않다면, 라인~\lnref{get_curtbl} 이 현재 해쉬 테이블로의 레퍼런스를
가져오고, \clnrefrange{alloc:b}{alloc:e} 에서 원하는 크기의 새 해쉬 테이블을
할당합니다.
새로운 해쉬/키 함수가 명시되었다면, 그것들이 새 해쉬 테이블에서 사용되며,
그렇지 않다면 기존 테이블의 것들이 유지됩니다.
라인~\lnref{chk_nomem} 이 메모리 할당 실패를 감지하면 라인~\lnref{rel_nomem} 이
\co{->ht_lock} 을 해제하고 라인~\lnref{ret_nomem} 에서 실패 알림을 리턴합니다.
...
라인~\lnref{get_curidx} 는 현재 테이블의 인덱스를 가져오고
라인~\lnref{put_curidx} 는 그것의 해쉬 테이블로의 역을 저장하여서 이 두개의
해쉬 테이블이 서로의 링크드 리스트를 덮어쓰는 것을 막습니다.
이어서 라인~\lnref{set_newtbl} 은 새 테이블로의 레퍼런스를 기존 테이블의
\co{->ht_new} 필드에 저장함으로써 버킷 분산 프로세스를 시작합니다.
라인~\lnref{sync_rcu} 는 새 테이블을 모르는 모든 읽기 쓰레드가 크기 재조정
오퍼레이션이 이어지기 전에 완료될 것을 보장합니다.

\Clnrefrange{loop:b}{loop:e} 의 반복문의 각 패스는 기존 해쉬 테이블의 버킷들 중
하나의 내용물들을 새로운 해쉬 테이블로 분산시킵니다.
라인~\lnref{get_oldcur} 는 기존 테이블의 현재 버킷으로의 레퍼런스를 가져오고
라인~\lnref{acq_oldcur} 는 그 버킷의 스핀락을 획득합니다.
\end{fcvref}
...
\QuickQuiz{
	\begin{fcvref}[ln:datastruct:hash_resize:resize]
	Listing~\ref{lst:datastruct:Resizable Hash-Table Resizing}
	의 \co{hashtab_resize()} 함수에서, 라인~\lnref{set_newtbl} 에서의
	\co{->ht_new} 로의 업데이트가 라인~\lnref{update_resize} 에서의
	\co{->ht_resize_cur} 업데이트보다 전에 일어난 것으로 \co{hashtab_add()}
	와 \co{hashtab_del()} 의 관점에 보일 것을 무엇이 보장합니까?
	달리 말하자면, \co{hashtab_add()} 와 \co{hashtab_del()} 이
	\co{->ht_new} 에서 얻어진 \co{NULL} 포인터를 역참조 하는 것을 무엇이
	방지합니까?
	\end{fcvref}
	...
	\begin{fcvref}[ln:datastruct:hash_resize:resize]
	Listing~\ref{lst:datastruct:Resizable Hash-Table Resizing}
	의 라인~\lnref{sync_rcu} 에서의 \co{synchronize_rcu()} 가 모든
	앞서서부터 존재해온 RCU 읽기 쓰레드들은 우리가 새 해쉬 테이블
	레퍼런스를 라인~\lnref{set_newtbl} 에 설치한 시점과 우리가
	라인~\lnref{update_resize} 에서 \co{->ht_resize_cur} 를 업데이트 하는
	시점 사이에 완료될 것을 보장합니다.
	이는 \co{->ht_resize_cur} 의 음수가 아닌 값을 보는 모든 읽기 쓰레드는
	\co{->ht_new} 할당 전에 시작했을 수 없음을, 따라서 새 해쉬 테이블로의
	레퍼런스를 볼 수 있을 것임을 의미합니다.

	그리고 이는 업데이트 쪽 \co{hashtab_add()} 와 \co{hashtab_del()} 함수가
	RCU read-side 크리티컬 섹션에 있어야만 하는지에 대한 이유로,
	Listing~\ref{lst:datastruct:Resizable Hash-Table Update-Side Concurrency Control}
	의 \co{hashtab_lock_mod()} 와 \co{hashtab_unlock_mod()} 덕입니다.
	\end{fcvref}
...
\begin{fcvref}[ln:datastruct:hash_resize:resize]
\Clnrefrange{loop_list:b}{loop_list:e} 의 반복문의 각 패스는 현재의 기존 테이블
버킷으로부터 데이터 원소 하나를 연관된 새 테이블의 버킷으로 더하며, 이 더하기
오퍼레이션 동안 새 테이블의 버킷의 락을 잡고 있습니다.
라인~\lnref{update_resize} 는 이 버킷이 분산되었음을 알리기 위해
\co{->ht_resize_cur} 를 업데이트 합니다.
마지막으로, 라인~\lnref{rel_oldcur} 는 기존 테이블 버킷 락을 해제합니다.

모든 기존 테이블 버킷이 새 테이블로 분산되고 나면 수행은
라인~\lnref{rcu_assign} 에 도달합니다.
라인~\lnref{sync_rcu_2} 는 (기존 테이블을 아직 참조하고 있을수도 있는) 모든
읽기 쓰레드들이 완료되기를 기다립니다.
라인~\lnref{rel_master} 는 크기 재조정 직렬화 락을 해제하고, 라인~\lnref{free}
는 기존 해쉬 테이블을 메모리 해제하며, 마지막으로 라인~\lnref{ret_success} 를
성공을 리턴합니다.
\end{fcvref}
...
\QuickQuiz{
	\begin{fcvref}[ln:datastruct:hash_resize:resize]
	Listing~\ref{lst:datastruct:Resizable Hash-Table Resizing}
	의 라인~\lnref{update_resize} 에는 왜 \co{WRITE_ONCE()} 가 있나요?
	\end{fcvref}
	...
	Listing~\ref{lst:datastruct:Resizable Hash-Table Update-Side Concurrency Control} 의
	line~\lnref{l:ifresized} in \co{hashtab_lock_mod()}
	에서의 \co{READ_ONCE()} 와 함께 그것이 컴파일러에게
	\co{->ht_resize_cur} 로의 액세스는 \co{->ht_resize_cur} 로부터의 읽기가
	실제로 쓰기와 레이스를 벌일 수 있으므로 ``if'' 조건을 바꾸지않음으로써
	남아있을 수 있게 해달라고 이야기 합니다.
	\end{fcvref}
	...
	of Listing~\ref{lst:datastruct:Resizable Hash-Table Update-Side Concurrency Control},
...
\QuickQuiz{
	Figure~\ref{fig:datastruct:Overhead of Resizing Hash Tables Between 262,144 and 524,288 Buckets vs. Total Number of Elements}
	에 보인 크고 작은 해쉬 테이블들 사이의 성능 차이 중 얼만큼이 긴 해쉬
	체인 때문이었고 얼만큼은 메모리 시스템 병목 때문이었나요?
	...
	이 질문에 답하는 한가지 쉬운 방법은 2,097,152 원소를 가지고 수행하지만
	이번엔 2,097,152 버킷을 가져서 버킷당 평균 원소 갯수가 하나로 되도록
	되돌리는 것입니다.
...
이 데이터에서의 핵심은 RCU 로 보호되는 크기 재조정 가능한 해쉬 테이블이 그
...
또다른 핵심은 \co{hashtab} 구조체가 파티셔닝 불가하기는 하나 읽기가
...
물론, \co{ht_elem} 구조체의 포인터 쌍은 약간의 메모리 오버헤드를 일으키는데,
다음 섹션에서 이야기 됩니다.
...
최초의 상태가 이 그림의 꼭대기에 보여져 있고, 초기 상태~(a) 로부터 시작해서
시간은 위에서 아래로 흐릅니다.
...
이 시점에서, 기존의 작은 해쉬 버킷들은 메모리 해제될 수 있습니다만 많은
...
다음 섹션들은 특수화, 메모리 보호, 그리고 하드웨어 고려를 다룹니다.
...
\co{ht} 구조체로부터 \co{->ht_cmp()}, \co{->ht_gethash()}, 그리고
...
또한, 이 크기 재조정 가능한 해쉬 테이블은 동시성 제어로부터 버킷 선택을
분리하는 API 에 맞게 설계되었습니다.
이게 단일한 고문 테스트가 이 챕터의 모든 해쉬 테이블 구현을 테스트 할 수 있게
하지만, 이는 또한 많은 오퍼레이션들이 해쉬를 계산하고 가능한 크기 재조정
오퍼레이션들과 한번이 아니라 두번 상호작용해야만 하게 함을 의미합니다.
성능이 중요한 환경에서 \co{hashtab_lock_mod()} 함수는 또한 선택된 버킷으로의
참조를 반환해서 뒤따르는 \co{ht_get_bucket()} 호출을 제거할 겁니다.
...
\QuickQuizSeries{%
\QuickQuizB{
	\path{hashtorture.h} 코드는 \co{ht_get_bucket()} 기능을 포함하는 버전의
	\co{hashtab_lock_mod()} 를 갖도록 수정될 수 없나요?
	...
	아마 그럴 수 있을 거고, 그렇게 하는건 이 챕터에 보인 버킷별 락킹 기반
	해쉬 테이블에 이득을 줄 겁니다.
	이 수정을 가하는 것은 독자 여러분의 연습문제로 남겨둡니다.
...
\QuickQuizE{
	...
	첫번째 질문에 대한 대답은 독자 여러분의 연습문제로 남겨둡니다.
	이 크기 재조정 가능한 해쉬 테이블을 특수화 시키고 얼만큼의 성능 개선이
	일어나는지 보십시오.
	두번째 질문은 일반적으로 답변될 수 없으며, 특정 사용 예에 맞춰
	답변되어야만 합니다.
	어떤 사용처는 다른것들에 비해 성능과 확장성에 무척 예민할 수 있습니다.
...
그렇다고는 하나, 제가 1970년대 초에 처음 프로그램을 배우기 시작했을 때 쓸 수
있었던 것들에 비교해 현대 하드웨어의 큰 이득 중 하나는 훨씬 적은 특수화가
필요하다는 겁니다.
이는 4 킬로바이트 주소 공간의 시대에 가능했던 것에 비해 훨씬 큰 생산성을
가능하게 합니다.
...
page~\pageref{lst:datastruct:Resizable Hash-Table Data Structures} 의
Listing~\ref{lst:datastruct:Resizable Hash-Table Data Structures} 에 보인
\co{ht} 구조체의 \co{->ht_idx} 필드는 항상 1 또는 0 의 값만 갖는데도 메모리의
32 비트를 사용합니다.
이는 예를 들어 \co{->ht_resize_key} 필드의 비트를 하나 훔치는 것으로 제거될 수
있습니다.
\co{->ht_resize_key} 필드는 메모리의 모든 바이트 주소를 담을 수 있을만큼 크며
\co{ht_bucket} 구조체는 1 바이트보다 커서 \co{->ht_resize_key} 필드는 여분의
...
리눅스 커널의 \co{page} 구조체가 한 예입니다.
그러나, 이 크기 재조정 가능한 해쉬 테이블의 \co{ht} 구조체는 그렇게 많이
복사되지는 않습니다.
그보다는 \co{ht_bucket} 구조체에 집중해야 합니다.
\co{ht_bucket} 구조체의 크기를 줄이기 위한 두개의 주요 기회가 있습니다:
(1)~\co{->htb_lock} 필드를 \co{->htb_head} 포인터들 중 하나의 아래쪽 비트에
위치시키기와 (2)~필요한 포인터의 갯수 줄이기.

첫번째 기회를 위해선 리눅스 커널의 \path{include/linux/bit_spinlock.h} 헤더
...
이런 단점에도 불구하고 bit 스핀락은 메모리가 중요할 때 매우 유용합니다.
...
오늘날의 컴퓨터들은 대부분 데이터를 CPU 와 메인 메모리 사이에서 32 바이트에서
...
카운터와 같은 캐쉬라인에 \co{ht_elem} 구조체를 두었다고 생각해 봅시다.
...
있습니다.
여기서 \GCC 의 \co{aligned} 지시어가 \co{->counter} 와 \co{ht_elem} 구조체를
다른 캐쉬 라인에 위치하게 강제합니다.
이는 CPU 들이 잦은 카운터 값 증가에도 불구하고 해쉬 버킷 리스트를 완전한 속도로
순회할 수 있게 합니다.

물론, 이는 ``우린 캐쉬 라인이 64 바이트 크기란 걸 어떻게 하나?'' 라는 질문을
떠올리게 합니다.
리눅스 시스템에서, 이 정보는 \path{/sys/devices/systems/cpu/cpu*/cache/}
...
이 챕터는 완전한 파티셔닝이 불가능한 크기 재조정한 해쉬 테이블을 포함해서 해쉬
테이블을 주로 보았습니다.
Section~\ref{sec:datastruct:Other Data Structures} 은 해쉬 테이블 외의 데이터
구조에 대해 짧게 들여다 보았습니다.
그러나, 이 해쉬 테이블 분석은 고성능 확장형 데이터 액세스를 둘러싼 많은
문제들에 대한 훌륭한 소개가 될텐데, 그런 문제들로는 다음과 같은 것들이
포함됩니다:
...
\begin{enumerate}
\item	완전히 파티셔닝된 데이터 구조는 예를 들면 단일 소켓 시스템과 같은 작은
	시스템에서 잘 동작합니다.
\item	더 큰 시스템은 완전한 파티셔닝에 더해 참조의 지역성을 필요로 합니다.
\item	해저드 포인터와 RCU 같은 읽기가 대부분인 경우를 위한 기법들은 읽기가
	대부분인 워크로드에 대해 좋은 참조 지역성을 제공하며 따라서 큰
	시스템에서도 훌륭한 성능과 확장성을 제공합니다.
\item	읽기가 대부분인 경우를 위한 기법들은 또한 크기 재조정 가능한 해쉬
	테이블과 같은 완전한 파티셔닝이 불가능한 일부 데이터 구조에서도 잘
	동작합니다.
\item	커다란 데이터 구조들은 CPU 캐쉬를 넘치게 해서 성능과 확장성을 떨어뜨릴
	수 있습니다.
\item	데이터 구조를 특정 워크로드에 특수화 시킴으로써 추가적인 성능과
	확장성을 얻을 수 있는데, 예를 들어 범용 키를 32-bit 정수로 만드는
	겁니다.
\item	이식성과 극단의 성능을 위한 요구사항들은 종종 서로 부딪히지만 이 두
	요구사항 집합 사이에서 좋은 균형을 맞출 수 있는 일부 데이터 구조 배치
	기법들이 존재합니다.
...
그렇다고는 하나, 성능과 확장성은 안정성 없이는 쓸모가 없으므로, 다음 챕터는
검증을 다룹니다.
