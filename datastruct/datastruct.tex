...
\QuickQuiz{}
	Listing~\ref{lst:datastruct:Resizable Hash-Table Bucket Selection}
	의 \co{ht_get_bucket()} 과
	Listing~\ref{lst:datastruct:Resizable Hash-Table Resizing}
	의 \co{hashtab_resize()} 의 메모리 배리어는 왜 존재하는 건가요?
	\iffalse

	Why the memory barriers in \co{ht_get_bucket()} in
	Listing~\ref{lst:datastruct:Resizable Hash-Table Bucket Selection}
	and in \co{hashtab_resize()} in
	Listing~\ref{lst:datastruct:Resizable Hash-Table Resizing}?
	\fi
\QuickQuizAnswer{
	\co{ht_get_bucket()} 은 업데이트 쪽 락을 잡지 않은 채로 호출될 수
	있어서, 상호 배제가 없을 것이기 때문입니다.
	이는 \co{ht_get_bucket()} 이 새로운 해시 테이블을 지금 접근하고자 하는
	데이터 원소가 아직 그쪽 테이블에 나타나지지 않았을 때 문제가 될 수
	있고, 따라서 이 두개의 메모리 배리어가 그런 일의 발생을 막습니다.
	즉, \co{hashtab_resize()} 는 기존 bucket 의 데이터 원소를
	\co{->ht_rezie_cur} 의 업데이트 전에 완전히 옮겼고,
	\co{ht_get_bucket()} 은 \co{->ht_resize_cur} 로부터 값이 업데이트 된
	것을 보기 전까지는 새 해시 테이블에 접근하지 않을 겁니다.
	\iffalse

	Because \co{ht_get_bucket()} can be called without holding
	the update-side locks, there is no mutual exclusion.
	It would be bad for \co{ht_get_bucket()} to refer to the
	new hash table before the data element of interest appeared
	there, and the two memory barriers prevent this from happening.
	That is, \co{hashtab_resize()} will have fully moved the
	data elements in the old bucket before updating
	\co{->ht_resize_cur}, and \co{ht_get_bucket()} won't access
	the new hash table until after it sees the updated value
	from \co{->ht_resize_cur}.
	\fi
} \QuickQuizEnd
