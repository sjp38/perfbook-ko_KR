...
(\path{hash_resize.c}) 의 line~12-25 에 보이는 \co{ht} 구조체입니다.
...
Line~\lnref{hls:b}-\lnref{hls:e} 에 보인 \co{ht_lock_state} 는 락 상태를 새로운
\co{hashtab_lock_mod()} 에서 \co{hashtab_add()}, \co{hashtab_del()}, 그리고
\co{hashtab_unlock_mod()} 로 전달하는 데 사용됩니다.
이 상태는 이 알고리즘이 동시적인 크기 재조정 작업 중에 잘못된 bucket 을
찾아가는 걸 방지합니다.
...
분산되었다면, 새로운 테이블은 물론 과거 테이블에서도 bucket 이 선택되어야
합니다.
...
Bucket 선택 코드는
\co{ht_get_bucket()} 을 line~\lnref{single:b}-\lnref{single:e} 에서,
\co{ht_search_bucket()} 을 line~\lnref{hsb:b}-\lnref{hsb:e} 에서 보이는
Listing~\ref{lst:datastruct:Resizable Hash-Table Bucket Selection} 에 있습니다.
\co{ht_get_bucket()} 함수는 특정 해시 테이블의 특정 키에 연관된 bucket 으로의
레퍼런스를 리턴하는데, 이때에는 크기 재조정을 고려하지 않습니다.
...
\co{ht_search_bucket()} 함수는 특정 해시 테이블 버전 내에서 특정 키를
탐색합니다.
Line~\lnref{hsb:get_curbkt} 은 해당 키에 연관된 bucket 으로의 레퍼런스를
얻어옵니다.
Line~\lnref{hsb:loop:b}-\lnref{hsb:loop:e} 의 루프는 이 bucket 을 탐색해서,
만약 line~\lnref{hsb:match} 가 조건이 맞음을 발견하면,
line~\lnref{hsb:ret_match} 에서 둘러싼 데이터 원소로의 포인터를 리턴합니다.
그렇지 않고 조건이 맞는 경우가 없다면 line~\lnref{hsb:ret_NULL} 이 실패를
알리는 \co{NULL} 을 리턴합니다.
\iffalse
...
이와 같은 \co{ht_get_bucket()} 과 \co{ht_search_bucket()} 의 구현은 크기 재조정
작업과 동시에 탐색과 수정이 가능하게 할 겁니다.
\iffalse
...
이 일람표의 line~\lnref{b}-\lnref{e} 에 \co{hashtab_lock_mod()} 가 있습니다.
Line~\lnref{rcu_lock} 는 데이터 구조가 횡단을 하는 도중에 해제되는 일을 막기
위해 RCU read-side 크리티컬 섹션에 들어가고, line~\lnref{refhashtbl} 에서 현재
해시 테이블로의 레퍼런스를 얻어온 후, line~\lnref{refbucket} 에서 이 해시
테이블에서 현재 키에 연관되는 bucket 으로의 레퍼런스를 얻어옵니다.
Line~\lnref{acq_bucket} 는 이 bucket 의 락을 획득하는데, 이 락은 이 bucket 이
이미 분산되었다면 아무 영향이 없을 것임에도 불구하고 그렇습니다.
Line~\lnref{lsp0b}-\lnref{lsp0e} 는 bucket 포인터와 포인터 집합 인덱스를
\co{ht_lock_state} 구조체의 해당하는 필드에 저장하는데, 이것들은 이 정보를
\co{hashtab_add()}, \co{hashtab_del()}, 그리고 \co{hashtab_unlock_mod()} 에
전달합니다.
Line~\lnref{ifresized} 는 이어서 동시의 크기 재조정 작업이 이미 이 bucket
을 새로운 해시 테이블로 분산시켰는지 보고, 그렇지 않다면 line~\lnref{lsp1_1} 이
이미 크기 재조정된 해시 bucket 이 조재하지 않는다는 걸 알리고
line~\lnref{fastret1} 은 선택된 해시 bucket 의 락을 잡은 채 (따라서 이 bucket
을 분산시키는 작업과 동시의 크기 재조정 작업을 방지합니다), 그리고 여전히 RCU
read-size 크리티컬 섹션 안에 있는 채 리턴합니다.
\iffalse
...
것이므로, line~\lnref{new_hashtbl} 이 새로운 해시 테이블로 진행을 이어가서,
line~\lnref{get_newbkt} 이 키에 연관된 bucket 을 고르고,
line~\lnref{acq_newbkt} 이 이 bucket 의 락을 획득합니다.
Line~\lnref{lsp1b}-\lnref{lsp1e} 는 이 bucket 의 포인터와 포인터 집합 인덱스
를 \co{ht_lock_state} 구조체의 연관된 필드에 저장하는데, 이것들은 앞에서와 같이
이 정보를 \co{hashtab_add()}, \co{hashtab_del()}, 그리고
\co{hashtab_unlock_mod()} 로 전달합니다.
이 bucket 은 이미 크기가 재조정되었고 \co{hashtab_add()} 와 \co{hashtab_del()}
이 과거의, 그리고 새로운 \co{ht_bucket} 구조체에 영향을 끼치는데, 두 bucket
각각에서 하나씩 두개의 락을 잡은 채로입니다.
또한, \co{ht_lock_state} 구조체의 각 배열의 원소 모두 사용되는데, \co{[0]}
원소는 과거의 \co{ht_bucket} 구조체에 관련되고 \co{[1]} 원소는 새 구조체에
관련됩니다.
다시 한번, \co{hashtab_lock_mod()} 는 RCU read-side 크리티컬 섹션 내에서
종료됩니다.
\iffalse
...
\co{hashtab_unlock_mod()} 함수는 \co{hashtab_lock_mod()} 로 잡아 두었던
락(들)을 놓아줍니다.
Line~\lnref{relbkt0} 는 기존 \co{ht_bucket} 구조체의 락을 놓습니다.
Line~\lnref{ifbkt1} 이 크기 재조정 작업이 진행중이라 판단하는 흔치 않은 경우,
line~\lnref{relbkt1} 은 새로운 \co{ht_bucket} 구조체의 락을 놓습니다.
어느 쪽이던, line~\lnref{rcu_unlock} 은 RCU read-side 크리티컬 섹션을
빠져나갑니다.
\iffalse
...
\QuickQuiz{}
	크기 재조정 작업이 이루어지는 사이에 한 쓰레드가 해시 테이블에
	원소를 넣는다고 생각해 봅시다.
...
\QuickQuizAnswer{
	...
	장소로 움직일 수 없을텐데, 삽입 작업이 해시 테이블의 하나 또는 두개의
	해시 bucket 의 락(들)을 잡고 있기 때문입니다.
...
이 그림의 line~\lnref{b}-\lnref{e} 의 \co{hashtab_lookup()} 함수는 탐색을
합니다.
Line~\lnref{get_curtbl} 에서는 현재 해시 테이블을 가져오고
line~\lnref{get_curbkt} 에서 특정 키에 연관된 bucket 을 탐색합니다.
Line~\lnref{ret} 은 그 원소로의 포인터를, 또는 탐색이 실패했다면 \co{NULL} 을
리턴합니다.
\iffalse
...
\QuickQuiz{}
	Listing~\ref{lst:datastruct:Resizable Hash-Table Access Functions}
	의 \co{hashtab_lookup()} 함수는 동시의 크기 재조정 작업을 신경쓰지
	않네요.
	이는 읽기 쓰레드가 크기 재조정 작업 중에 추가된 원소를 놓칠 수도 있다는
	걸 의미하지 않나요?
	\iffalse
...
\QuickQuizAnswer{
	아닙니다.
	곧 보게 되겠지만, \co{hashtab_add()} 와 \co{hashtab_del()} 함수는 크기
	재조정 작업이 진행중인 동안 과거의 해시 테이블을 최신상태로 유지합니다.
	\iffalse
...
이 코드의 line~\lnref{b}-\lnref{e} 의 \co{hashtab_add()} 함수는 새로운 데이터
원소를 이 해시 테이블에 넣습니다.
Line~\lnref{htbp} 는 새로운 원소가 추가될 현재의 \co{ht_bucket} 구조체를
가져오고, line~\lnref{i} 는 포인터 쌍의 인덱스를 가져옵니다.
Line~\lnref{add} 는 새로운 원소를 현재 해시 bucket 에 추가합니다.
Line~\lnref{ifnew} 에서 이 bucket 이 새로운 버전의 해시 테이블로 분산되었다고
판단하면, line~\lnref{addnew} 는 새로운 원소를 연관된 새로운 bucket 에도
추가합니다.
이 함수의 호출자는 동시성을 제어할 것이 요구되는데, 예를 들면,
\co{hashtab_add()} 전에 \co{hashtab_lock_mod()} 를, 그 후에는
\co{hashtab_unlock_mod()} 를 호출해야 합니다.
\iffalse
...
코드의 line~\lnref{b}-\lnref{e} 의 \co{hashtab_del()} 함수는 해시 테이블에서
존재하는 원소를 하나 제거합니다.
Line~\lnref{i} 는 포인터 쌍의 인덱스를 고르고 line~\lnref{del} 은 현재
테이블에서 해당 원소를 지웁니다.
Line~\lnref{ifnew} 에서 이 bucket 이 새로운 해시 테이블로 분산되었다고
판단하면, line~\lnref{delnew} 에서 해당 원소를 새로운 테이블의 bucket 에서도
지웁니다.
...
\QuickQuiz{}
	Listing~\ref{lst:datastruct:Resizable Hash-Table Access Functions}
	의 \co{hashtab_add()} 와 \co{hashtab_del()} 함수는 크기 재조정 작업이
	진행중인 동안 해시 bucket 들을 업데이트 할 수 있습니다.
	이 빈번한 크기 재조정 작업이 무시할만하지 않다면 이는 성능을 떨어뜨릴
	겁니다.
	그런 경우에 업데이트 비용을 줄일 수 있지 않나요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	그래요, \co{hashtab_lookup()} 에의 약간의 비용 증가가 괜찮다고 하면
	말이죠.
	그러기 위한 한가지 방법이
	Listing~\ref{lst:datastruct:Resizable Hash-Table Access Functions (Fewer Updates)}
	과~\ref{lst:datastruct:Resizable Hash-Table Update-Side Locking Function (Fewer Updates)}
	(\path{hash_resize_s.c}) 에 보여 있습니다.
	\iffalse
...
\end{listing}
	...
	이 버전의 \co{hashtab_add()} 는 아직 크기 재조정 작업이 이루어지지
	않았다면 과거 bucket 에, 크기 재조정이 되었다면 새 bucket 에 저장하고,
	\co{hashtab_del()} 은 해당 원소를 넣어졌던 bucket 에서 제거합니다.
	\co{hashtab_lookup()} 함수는 과거 bucket 의 탐색이 실패하면 새 bucket
	을 탐색하는데, 이는 룩업 빠른 수행 경로에 오버헤드를 추가하는 단점을
	갖습니다.
	여기서의 대안적인 \co{hashtab_lock_mod()} 는 크기 재조정이 진행 중이면
	아마도 더 자연스러운 선택일 \co{->hbp[1]} 과 \co{->hls_idx[1]} 대신
	\co{->hbp[0]} 와 \co{->hls_idx[0]} 에서 새로운 bucket 의 락킹 상태를
	리턴합니다.
	하지만, 이 덜 자연적인 선택은 \co{hashtab_add()} 를 단순화 시키는
	장점을 갖습니다.

	이 코드를 더 분석하는건 독자 여러분의 몫으로 두겠습니다.
	\iffalse
...
Line~\lnref{get_curidx} 는 현재 테이블의 인덱스를 가져오고
line~\lnref{put_curidx} 는 새로운 테이블로의 인덱스를 저장해서, 이 두 해시
테이블이 서로의 링크드 리스트를 덮어쓰는 것을 막습니다.
Line~\lnref{set_newtbl} 은 이어서 새로운 테이블로의 레퍼런스를 기존 테이블의
\co{->ht_new} 필드에 저장함으로써 bucket 분산 작업을 시작합니다.
Line~\lnref{sync_rcu} 는 새로운 테이블을 알지 못하는 모든 읽기 쓰레드가 크기
재조정 작업이 이어지기 전에 완료될 것을 보장합니다.
\iffalse
...
\QuickQuiz{}
	...
	\co{hashtab_resize()} 함수에서, line~\lnref{set_newtbl} 에서의
	\co{->ht_new} 로의 업데이트가 line~\lnref{update_resize} 에서의
	\co{->ht_resize_cur} 로의 업데이트 전에 일어난 것으로
	\co{hashtab_add()} 와 \co{hashtab_del()} 의 관점에 보일 것을 무엇이
	보장하죠?
...
bucket 의 락을 잡고 있습니다.
Line~\lnref{update_resize} 는 이 bucket 이 분산되었다는 것을 알리기 위해
...
\QuickQuiz{}
	\begin{lineref}[ln:datastruct:hash_resize:resize]
	Listing~\ref{lst:datastruct:Resizable Hash-Table Resizing} 의
	line~\lnref{update_resize} 에 \co{WRITE_ONCE()} 는 왜 있는 건가요?
	\iffalse
...
\QuickQuizAnswer{
	...
	Listing~\ref{lst:datastruct:Resizable Hash-Table Update-Side Concurrency Control}
	의 line~\lnref{l:ifresized} in \co{hashtab_lock_mod()} 과 함께, 이
	코드는 컴파일러에게 \co{->ht_resize_cur} 로의 초기화가 아닌 접근은
	\co{->ht_resize_cur} 로부터의 읽기가 ``if'' 조건에만 쓰이는게 아니라 이
	쓰기와 경주할 수 있기 때문에 유지되어야 한다는 걸 알립니다.
	\iffalse
...
%
\epigraph{There's only one thing more painful than than learning from 
