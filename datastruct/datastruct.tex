...
이와 같은 \co{ht_get_bucket()} 의 구현은 크기 재조정 작업과 동시에 탐색과
수정이 가능하게 할 겁니다.
\iffalse
...
line~\lnref{fastret1} 은 선택된 해시 bucket 의 락을 잡은 채 (그리고 여전히 RCU
...
를 \co{ht_lock_state} 구조체의 연관된 필드에 저장하는데, 이것들은 이 정보를
\co{hashtab_add()}, \co{hashtab_del()}, 그리고 \co{hashtab_unlock_mod()} 로
전달합니다.
이 경우, 두 락이 잡히는데, 하나는 기존의 \co{ht_bucket} 구조체의 것으로 이
bucket 을 (이미 크기 재조정되지 않았다면) 크기 재조정하는 걸 막기 위한 것이고,
하나는 새로운 \co{ht_bucket} 구조체의 것으로 동시의 \co{hashtab_add()} 와
\co{hashtab_del()} 호출과 동기화 하기 위한 것입니다.
\iffalse
...
락(들)을 놓아줍니다.
Line~\lnref{relbkt0} 는 기존 \co{ht_bucket} 구조체의 락을 놓는데, 크기 재조정
작업이 진행중이지 않은 경우에 사용되는 구조체입니다.
Line~\lnref{ifbkt1} 이 크기 재조정 작업이 진행되었다고 판단되면,
...
Line~\lnref{ret} 은 탐색되었다면 그 원소로의 포인터를, 탐색이 실패했다면
\co{NULL} 을 리턴합니다.
\iffalse
...
테이블에서 해당 원소를 지웁니다.
Line~\lnref{ifnew} 는 이 bucket 이 새로운 해시 테이블로 분산되었는지 확인하고,
만약 그렇다면 line~\lnref{delnew} 에서 해당 원소를 새로운 테이블의 bucket 에서
지웁니다.
