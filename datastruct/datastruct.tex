...
Line~\lnref{lsp0b}-\lnref{lsp0e} 는 bucket 포인터, 포인터 집합 인덱스, 그리고
해시 값을 \co{ht_lock_state} 구조체의 해당하는 필드에 저장하는데, 이것들은 이
정보를 \co{hashtab_add()}, \co{hashtab_del()}, 그리고 \co{hashtab_unlock_mod()}
에 전달합니다.
...
Line~\lnref{lsp1b}-\lnref{lsp1e} 는 이 bucket 의 포인터, 포인터 집합 인덱스,
그리고 해시 값을 \co{ht_lock_state} 구조체의 연관된 필드에 저장하는데, 이것들은
...
만약 line~\lnref{entchk} 가 이 탐색이 성공했음을 판단하면
line~\lnref{ret_match} 에서 해당 원소로의 포인터를 리턴합니다.
그렇지 않다면, line~\lnref{get_nxttbl} 이 다음 버전으로의 포인터를 가져오고,
만약 line~\lnref{htpchk} 가 진행중인 크기 재조정이 없음으로 판단하면,
line~\lnref{noresize} 가 \co{NULL} 을 리턴합니다.
크기 재조정이 진행 중이라면, 수행은 line~\lnref{ret_nxtbkt} 에 이르는데, 여기선
새 버전에서 \co{key} 에 연관된 bucket 에 대한 탐색의 결과를 리턴합니다.
\iffalse
...
하나 제거합니다.
Line~\lnref{i} 는 포인터 쌍의 인덱스를 고르고 line~\lnref{del} 은 해당 원소를
지웁니다.
