...
line~\lnref{get_curbkt} 에서 특정 키에 연관된 bucket 으로의 레퍼런스를
얻어옵니다.
Line~\lnref{loop:b}-\lnref{loop:e} 의 루프는 bucket 내부를 탐색하는데,
line~\lnref{match} 에서는 매치되는지를 판단하고, line~18 에서 데이터 원소로의
포인터를 리턴하는데, 매치되지 않는다면 line~20 에서 실패를 알리는 \co{NULL} 을
리턴합니다.
\iffalse
