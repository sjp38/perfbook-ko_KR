...
이 간단한 형태의 병렬성은 속임수이거나 책임을 회피하는게 아니라, 여러분의
코드를 빠르게 수행하기 위한 간단하고 우아한 방법이란 이야기를 반복하겠습니다.
이 방법은 빠르고, 잘 확장되고, 프로그램하기 쉽고, 유지보수하기 쉬우며, 일을
해냅니다.
또한, 이 방법을 (사용할 수 있는 곳에) 취하는 것은 개발자가 \co{compute_it} 의
싱글스레드 기반 최적화를 적용하는 것 또는 이 방법이 적용되기 어려운 코드 부분에
괜찮은 병렬 프로그래밍 패턴을 적용하는것 등의 다른 부분에 신경쓸 시간을
벌어줍니다.
이걸 싫어할 이유가 있나요?
...
부분적 사유화 역시 가능한데, 약간 동기화 요구가 존재하지만, 완전히 공유된
경우에 비하면 적습니다.
부분적 사유화 가능성 중 일부는
Section~\ref{sec:toolsoftrade:Avoiding Data Races} 에서 알아봅니다.
\iffalse
