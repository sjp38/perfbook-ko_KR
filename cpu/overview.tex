...
또하나의 멀티 쓰레딩에서의 CPU 성능에의 장애물은 ``캐쉬 미스'' 입니다.
앞서 말했듯, 근래의 CPU 들은 높은 메모리 반응속도로 발생할 수 있는 성능 하락을
줄이기 위해 큰 캐쉬를 장착하고 있습니다.
하지만, 이런 캐쉬들은 실은 CPU 간에 자주 공유되는 변수들에 대해서는 생산적이지
못합니다.
하나의 CPU 가 한 변수를 수정하려 할 때, 다른 CPU 가 그 값을 최근에 바꾼 경우가
있을 가능성이 크기 때문이죠.
이런 경우, 해당 변수는 지금 수정하려는 CPU 의 캐쉬가 아니라 최근에 값을 수정한
CPU 의 캐쉬에 있을 것이고, 이로 인해 비싼 캐쉬 미스(더 자세한 내용을 위해선
Section~\ref{sec:app:whymb:Cache Structure} 를 보세요) 를 일으킬 것입니다.
이런 캐쉬 미스들은 Figure~\ref{fig:cpu:CPU Meets a Cache Miss} 에서 보여진
...
\QuickQuiz{}
	그래서, CPU 설계자들은 캐쉬 미스 오버헤드 역시 많이 개선 했나요?
	\iffalse
...
\QuickQuizAnswer{
	...
	원자성의 자연 법칙이 큰 시스템에서 캐쉬 미스 오버헤드를 줄일 수 있는
	방법을 제한하고 있습니다.
...
캐쉬 미스는 곧 CPU 와 CPU 사이의 I/O 오퍼레이션으로 볼 수 있고, 이것은 가능한
...
공유메모리 병렬 프로그램은 일반적으로 캐쉬 미스보다 더한 장애는 겪지
...
볼테지만, 병렬 소프트웨어 설계의 중요한 목표는 커뮤니케이션 캐쉬 미스와 같은
