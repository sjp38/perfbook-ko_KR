...
어떤 경우엔 캐시 응답시간을 감출 수 있는 많은 하드웨어 최적화가 존재하긴
하지만, 성능에 끼치는 영향은 너무 많은 경우
...
멀티 쓰레딩에서의 CPU 성능에의 또하나의 장애물은 ``캐시 미스'' 입니다.
앞서 말했듯, 근래의 CPU 들은 큰 메모리 응답시간으로 인해 발생할 수 있는 성능
하락을 줄이기 위해 큰 캐시를 장착하고 있습니다.
하지만, 이런 캐시들은 CPU 간에 자주 공유되는 변수들에 대해서는 생산적이지
못합니다.
하나의 CPU 가 어떤 변수를 수정하려 할 때, 다른 CPU 가 그 값을 최근에 바꾼
경우가 있을 가능성이 크기 때문이죠.
