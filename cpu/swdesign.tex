...
공유하는 것이 있다면, 이 공유는 읽기가 대부분이어야 하는데, 이는 CPU 의 캐시가
읽기가 대부분인 이 데이터들의 복사본을 만들어 사용해서, CPU 들이 빨리 액세스 할
수 있도록 해 줄 것이기 때문입니다.
...
If there is sharing, that sharing should be read-mostly, which allows the
CPUs' caches to replicate the read-mostly data, in turn allowing all
CPUs fast access.
