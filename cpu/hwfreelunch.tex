...
상황이 더 나빠지는게, 실리콘에서의 전자파는 진공에서의 빛에 비해 세배에서
...
\QuickQuizAnswer{
	...
	개별 전자들은 꽤 무작위적으로 튀어다녀서 그들의 순간 속도는 매우
	빠르지만 긴 시간으로 보게 되면 그렇게 멀리 이동하지는 않습니다.
	여기서, 전자들은 대부분의 시간을 고속으로 이동하는데 소모하지만 긴
	시간으로 보면 어디에도 가지 않는 통근자와도 같습니다.
	이런 통근자들의 속도는 시속 70 마일(시속 113 킬로미터) 정도지만, 지구의
	...
	못합니다.
	그러나, 컨덕터에서 측정된 전자파의 속도는 빛의 속도의 발끝은 따라가고
	있는데, 이 때문에 여전히 미스테리는 풀리지 않습니다.
	\iffalse
	...
	또다른 트릭은 전자는 그 음극의 성질로 인해 다른 전자와 상당히(원자적
	관점에서요) 상호작용 한다는 것입니다.
...
그러나, 이를 개선할 기술이 (하드웨어에도 소프트웨어에도) 약간 있습니다:
\iffalse
