...
같은---하지만 \co{inc_count()}를 통해서만 값을 증가시키는---훌륭한 성능과
선형적 확장성을 보이는 통계적 카운터들의 구현을 설명했습니다.
불행히도, \co{read_count()} 를 통해 값을 읽어와야 하는 쓰레드는 글로벌 락을
...
우리가 원하는건 \co{inc_count()} 만이 아니라 \co{read_count()} 에서도 훌륭한
성능과 확장성을 얻기 위해 \co{read_count()} 의 쓰레드 횡단을 보호하는 데에
\co{final_mutex} 대신에 RCU 를 사용하는 것입니다.
하지만, 계산된 합계의 정확성을 포기하지도 않고 싶습니다.
자세히 말하자면, 특정 쓰레드가 종료될 때에, 우린 종료되는 쓰레드의 카운트를
잃어버려서도, 그걸 두번씩 세서도 안됩니다.
그런 에러는 결과의 전체 정확성과 동일한 정도의 비정확성을 초래할 수 있는데,
달리 말하자면 그런 에러는 결과값을 완전히 쓸모없게 만들 수 있습니다.
...
\QuickQuiz{}
	그런데, \co{read_count()} 의 정확성은 대체 뭔가요?
	\iffalse
...
\QuickQuizAnswer{
	...
	\co{read_count()} 는 분명한 결과를 내놓을 것이 분명합니다.
	...
	배타적으로 수행되므로, \co{counterp} 안의 포인터들은 상수로 유지될
	것입니다.
	\iffalse
	...
	봅시다.
	그렇다면 \co{read_count()} 는 이 두 스냅샷 사이의 어떤 시점에서의 각
	쓰레드의 카운터에 접근을 할 것이고, 따라서 이 두개의 스냅샷에 의해
	값이 포괄적으로 한정지어지는 결과를 얻어오게 될 것입니다.
	따라서, 전체 합계는 이 두개의 스냅샷으로부터 각각 얻어와질 수 있는
	...
	$T_\mathrm{r}$ 은 \co{read_count()} 가 실행되는데 걸리는 시간이고,
	$R_\mathrm{i}$ 는 단위 시간당 \co{inc_count()} 호출 횟수의 비율입니다.
...
\QuickQuizAnswer{
	...
	새로운 쓰레드는 그 값이 0인 \co{counter} 값과 함께 시작되므로, 이
...
\QuickQuizAnswer{
	...
	버전이 더 간단하고, 따라서 더 나을 수도 있습니다, 대부분의 차이는
...
\co{__thread} 변수 사용을 마칠 때까지 기다릴 수 있도록 해줍니다.
이는 \co{read_count()} 함수가 락킹을 필요없게 해서 \co{inc_count()} 와
\co{read_count()} 함수 둘 다에 훌륭한 성능과 확장성을 제공합니다.
하지만, 이 성능과 확장성은 약간의 코드 복잡도 증가를 비용으로 지불합니다.
컴파일러와 라이브러리를 작성하는 사람들이 안전한 쓰레드간 \co{__thread}
변수로의 접근을 제공하기 위해 유저 레벨 RCU~\cite{MathieuDesnoyers2009URCU} 를
제공해서 \co{__thread} 변수들의 사용자간에 보여지게 되는 복잡도를 줄일 수
있다면 좋을 겁니다.
\iffalse
...
는 제거 가능한 디바이스로의 I/O 액세스 카운팅을 다루는 한쌍의 코드 조각을
보였습니다.
...
RCU read-side 기능은 최소한의 오버헤드만을 가지고 있으므로, 원했던 대로 빠른
수행 경로의 속도를 높입니다.
...
Listing~\ref{lst:together:RCU-Protected Variable-Length Array} 에 보인 것과
같이 RCU 로 보호되는 가변길이의 배열을 가지고 있다고 생각해 봅시다.
배열 \co{->a[]} 의 길이는 동적으로 언제든 변할 수 있고, 그 길이는 \co{->length}
를 통해 제공됩니다.
물론, 이는 다음과 같은 레이스 컨디션 (race condition) 을 만들어냅니다:
\iffalse
...
한가지 방법은
Chapter~\ref{chp:Advanced Synchronization: Memory Ordering} 에서 소개된 메모리
배리어를 조심스럽게 사용하는 것입니다.
이는 제대로 동작합니다만, 읽는 쪽의 오버헤드를 초래하고, 더 나쁠 수 있는건,
명시적인 메모리 배리어의 사용을 필요로 합니다.
\iffalse
...
연관된 측정치의 집합입니다.
읽기를 하는 쓰레드는 이 세개의 값을 하나의 측정치 업데이트로부터 본다는 점이
특히 중요합니다: 만약 한 읽기 쓰레드가 \co{meas_1} 의 예전 값을 읽지만
\co{meas_2} 와 \co{meas_3} 의 새로운 값을 읽게 되다면, 그 읽기 쓰레드는 완전히
혼란에 빠질 겁니다.
읽기 쓰레드가 이 세개의 값의 통합된 집합을 볼 수 있도록 보장하려면 어떻게 해야
할까요?
\iffalse
...
필드를 업데이트 한 후, 기존의 구조체로의 포인터가 새로운 구조체를 가리키도록
포인터를 업데이트하는 방법으로 교체하는 것입니다.
이는 모든 읽기 쓰레드가 측정 값의 통합된 집합을 볼 수 있을 것을 보장합니다만,
이는 \co{->photo[]} 필드 때문에 커다란 구조체 복사를 필요로 하게 됩니다.
...
Listing~\ref{lst:together:Correlated Measurement Fields} 으로 보여진 것과
같습니다.
