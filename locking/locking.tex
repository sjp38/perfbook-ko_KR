...
그 책임의 수행을 도와주는 도구들도 있기는 하지만 (예를 위해선
Section~\ref{sec:formal:Axiomatic Approaches and Locking} 을 참고하세요),
결국은 그 코드의 개발자에게 있습니다.
\iffalse
...
\QuickQuizAnswer{
	...
	Figure~\ref{fig:count:Atomic Increment Scalability on Kaby Lake} 를
...
이 구현들은 다차원적 설계 트레이드오프에서 서로 다른 요점을
보입니다~\cite{Guerraoui:2019:LPA:3319851.3301501,HugoGuirouxPhD,McKenney96a}.
...
보아, 더 특수한 목적의 락킹 도구들이 계속해서 나타날 것을 예상해 볼 수 있는데,
예를 들어 Guerraoui 등의 것이
있습니다~\cite{Guerraoui:2019:LPA:3319851.3301501,HugoGuirouxPhD}.
...
쉽게 접할 수 있는 저가의 멀티코어 시스템들이 늘어남에 따라, 싱글 쓰레드
기반만을 상정하고 설계된 기존의 라이브러리들을 병렬화 하는건 흔한 작업이
되었습니다.
...
이런 결점들과 락킹에 대한 중요성들을 다음의 섹션들에서 이야기 해보겠습니다.
\iffalse
...
해시 테이블의 모든 아이템의 정확한 갯수를 쉽고 빠르게 유지할 수 있을 것이고,
또한 이 정확한 갯수를 매 원소 추가와 삭제 작업때마다 알려주는 것도 쉽고 빠르게
할 수 있을 것입니다.
그런데 왜 병렬화가 어려울까요?
...
사용해서 대략적인 수치를 주는 겁니다.
또다른 방법은 원소 갯수 카운트 기능을 없애버리는 겁니다.
...
\item	예를 들면 해시 테이블에 집어넣고 빼내는 항목이 없어진 경우의 확인 등을
	위한 검사의 목적을 위해서.
...
병렬 라이브러리 API 자체에 존재하는 성능과 확장성에 대한 일부 제약에 대한 강한
이론적 토대가 존재함이
...
락킹을 동시성에 친화적이지 않은 API 때문에 문제라고 비난하는 것은 너무 쉬운
일이지만 그렇게 하는게 현실적으로 큰 도움이 되지 않습니다.
...
시간은 흐르게 마련이고, 코드도 그에 따라 변화해 갑니다.
그렇다고는 하나, 유명한 라이브러리라면 수많은 사용자가 존재할 수 있고, 그런
경우에 호환성을 잃는 변경을 API 에 가하는 것은 매우 어리석은 짓일 겁니다.
이미 존재하고 매우 많이 사용되고 있는 순차적으로만 동작하는 API 를 보완하기
위한 병렬성 친화적 API 를 추가하는 것이 이런 상황에서는 최선의 선택일 겁니다.

그렇다고 하나, 인간사가 그렇듯이, 우리의 불운한 개발자는 자신의 (이해할 수
있기는 하지만) 잘못된 API 설계 선택보다는 락킹 자체를 비난할 가능성이 더 클
것을 예상할 수 있습니다.
\iffalse
...
에서 아무런 고려 없이 사용되는 콜백이 어떻게 락킹의 비극을 초래할 수 있는지를
알아보았습니다.
...
따라서, 콜백을 많이 사용하는 기존의 싱글 쓰레드 기반 라이브러리를 병렬화 하려는
사람은 락킹의 악행과 마주칠 가능성이 큽니다.
\iffalse
...
이야기 했듯 기존 사용자들이 그들의 코드를 점진적으로 바꿔갈 수 있도록
라이브러리에 병렬성 친화적 API 를 추가하는 것이 현명할 것입니다.
어떤 사람들은 이런 경우에 대안적으로 트랜잭셔널 메모리의 사용을 추천합니다.
...
객체지향 프로그래밍은 1980 년대 또는 1990 년대의 언젠가에 주류가 되었고, 그
결과로 상용 제품에는 많은 양의 객체지향 코드가 존재하게 되었는데, 대부분이 싱글
쓰레드 기반입니다.
객체지향성은 가치 있는 소프트웨어 테크닉이 될 수 있지만, 생각 없이 사용된
객체들은 객체지향적 스파게티 코드를 만들어 낼 수 있습니다.
객체지향 스파게티 코드에서, 실행 흐름 제어는 필수적으로 무작위한 형태로 객체와
...
따라서 만들어져야만 합니다.
어쨌거나, 동기화 메커니즘의 오버헤드가 보호되는 오퍼레이션들의 오버헤드보다
...
락킹은 아마도 가장 많이 사용되고 있고 일반적으로 가장 유용한 동기화 도구입니다.
하지만, 락킹은 어플리케이션이나 라이브러리에 작성 초기부터 고려되어서
설계되었을 때 가장 잘 동작합니다.
기존에 존재하던 싱글 쓰레드 기반의 거대한 코드를 하루만에 병렬적으로 동작하도록
만들어야 한다면, 락킹은 병렬 프로그래밍 도구상자의 유일한 도구여서는 안될
겁니다.
다음의 챕터들에서는 다른 도구들을 알아보고, 그것들이 락킹과 또다른 것들과 함께
어떻게 사용되는게 최선인지에 대해서 알아봅니다.
\iffalse
