...
\QuickQuiz{}
	...
	당신이 송수신된 네트워크 패킷의 갯수 에 대한 통계를 구해야 한다고
	생각해 봅시다.
	패킷들은 시스템의 어떤 CPU 를 통해서든 송신 / 수신될 수 있을 겁니다.
	더 나아가서 이 시스템이 CPU 당 초당 백만개의 패킷을 다룰 수 있고, 그
...
\QuickQuiz{}
	...
	한다고 가정해 봅시다.
	사용자는 디바이스를 제거하고 싶을 때 그 의사를 알려주며, 시스템은
	사용자에게 언제 디바이스를 제거해도 안전한지 알려주는 일반적 디바이스
	제거 절차를 따릅니다.
	\iffalse
...
\QuickQuizAnswer{
	...
	합니다.
	Section~\ref{sec:count:Applying Exact Limit Counters} 를 참고하세요.
	\iffalse
...
\QuickQuizAnswer{
	...
	더 나아가서, \co{READ_ONCE()} 와 \co{WRITE_ONCE()} 안의 volatile
	캐스팅을 알아두시기 바라는데, 이는 해당 위치가 MMIO 장치 레지스터로도
	사용될 수 있음을 컴파일러에게 이야기 합니다.
	MMIO 레지스터는 캐시되지 않기 때문에, 증가 연산자가 실제로 어토믹할
	거라 컴파일러가 생각하는건 현명하지 못한 일일 겁니다.
	\iffalse
...
라인~\lnref{define} 은 \co{long} 타입의 쓰레드별 카운터들을 담는, \co{counter}
라는 이름의 배열을 정의합니다.

라인~\lnref{inc:b}-\lnref{inc:e} 은 이 카운터를 증가시키는 함수로,
\co{__get_thread_var()} 함수를 사용해 \co{counter} 배열에서 현재 수행중인
쓰레드의 원소를 얻어옵니다.  이 원소는 연관된 쓰레드에 의해서만 수정되므로,
어토믹하지 않은 방식으로도 충분합니다.
하지만, 이 코드는 위험한 컴파일러 최적화를 막기 위해 \co{WRITE_ONCE()} 를
사용하고 있습니다.
하나만 예를 들자면, 컴파일러는 쓰기가 될 값을 담는 곳을 임시적 저장소로 대신할
수 있어서, 모든 의도와 목적을 담는 쓰기는 실제 쓰기가 되기 전에 쓰레기가 되어
버릴 겁니다.
물론 이는 이 카운트를 읽으려 하는 모든 시도를 혼란스럽게 만들 겁니다.
\co{WRITE_ONCE()} 는 이런 부류의 최적화를 방지합니다.
\iffalse
...
\QuickQuiz{}
	이거 말고도 \GCC\ 가 할 수 있는 이상한 최적화는 어떤게 있나요?
	\iffalse

	What other nasty optimizations could \GCC\ apply?
	\fi
\QuickQuizAnswer{
	C 표준에 따르면, 다른 쓰레드에 의해 동시에 읽혀질 수 있는 변수에
	평범하게 쓰기를 하는 행동이 일으킬 효과는 정의되어 있지 않습니다.
	C 는 어토믹하게 \co{long} 을 읽어들일 수가 없는 (예를 들어) 8비트
	아키텍쳐를 지원해야만 했기에, C 표준은 그럴 수밖에 없었습니다.
	C 표준의 최신 버전은 이 간극을 채우기 위한 어토믹 기능들을
	도입했습니다.
	하지만 이걸 리눅스 커널에 적용하려던 시도는 엇갈린 결과를
	가져왔는데~\cite{JonathanCorbet2016C11atomics}, 어토믹 기능을 위해
	컴파일러가 만들어내는 코드의 품질이 나빴던 게 한 이유였습니다.
	또한, 리눅스 커널은 C11 어토믹 기능이 제공하는 것보다 더 많은 기능을
	자신의 메모리 모델에 바라고 있는데, 컨트롤 의존성, 완전한 배리어/펜스,
	낮은 비용의 주소/데이터 의존성~\cite{PaulEMcKenney2016P0124R5-LKMM} 이
	여기에 포함됩니다.
	\iffalse
	...
	수 있는 경우라면 \co{READ_ONCE()} 와 \co{WRITE_ONCE()} 로 제공되는
	volatile 접근이\footnote{
		\co{READ_ONCE()} 와 \co{WRITE_ONCE()} 에 대한 간단한 정의가
...
라인~\lnref{read:b}-\lnref{read:e}
는 현재 수행중인 쓰레드의 리스트를 순회하기 위해 \co{for_each_thread()} 기능을
사용하고 특정 쓰레드의 카운터를 읽어오기 위해 \co{per_thread()} 기능을 사용해
카운터의 합산값을 구하는 함수를 보이고 있습니다.
이 코드는 또한 컴파일러가 이 읽기를 잊혀지도록 만드는 최적화를 하지 않도록
보장하기 위해 \co{READ_ONCE()} 를 사용하고 있습니다.
하나만 예를 들자면, \co{read_count()} 를 연속해서 호출하는 코드는 inline 코드로
변환될 수 있고, 용기 있는 최적화 장치는 같은 위치의 값들의 합이 연속해 구해짐을
알아채서 그 값들의 합을 한번만 구하고 재사용하려 할 수 있습니다.
이런 종류의 최적화는 나중의 \co{read_count()} 호출이 더 큰 값을 리턴할 거라
기대하는 사람들을 당황하게 만들 겁니다.
\co{READ_ONCE()} 의 사용은 이 최적화와 비슷한 것들을 방지할 겁니다.
\iffalse
...
라인~\lnref{per_thr_cnt}-\lnref{glb_cnt} 는 카운터의 값을 갖는 쓰레드별 변수와
글로벌 변수를 보여주며, 라인~\lnref{stopflag} 는 프로그램을 정확한 카운터 값과
함께 종료하고자 할 경우를 위해 종료 조건을 알리는 \co{stopflag} 를 보입니다.
라인~\lnref{inc:b}-\lnref{inc:e} 의 \co{inc_count()} 함수는
Listing~\ref{lst:count:Array-Based Per-Thread Statistical Counters} 의 것과
비슷합니다.
라인~\lnref{read:b}-\lnref{inc:e} 의 \co{read_count()} 함수는 단순히
\co{global_count} 변수의 값을 리턴합니다.
\iffalse
...
하지만, 라인~\lnref{init:b}-\lnref{init:e} 의 \co{count_init()} 함수는
라인~\lnref{eventual:b}-\lnref{eventual:e} 의 \co{eventual()} 쓰레드를
생성하는데, 이 쓰레드는 모든 쓰레드를 순회하며 쓰레드별 로컬 \co{counter} 를
더해서 \co{global_count} 변수에 저장하는 일을 반복합니다.
\co{eventual()} 쓰레드는 일의 반복 사이에 임의로 선택된 1 밀리세컨드 동안
기다립니다.
라인~\lnref{cleanup:b}-\lnref{cleanup:e} 의 \co{count_cleanup()} 함수는
프로그램 종료를 관장합니다.
...
라인~\lnref{var:b}-\lnref{var:e} 는 필요한 변수들을 정의합니다: \co{counter} 는
쓰레드별 카운터 변수이고, \co{counterp[]} 배열은 쓰레드들이 다른 쓰레드의
카운터들을 볼 수 있게 하며, \co{finalcount} 는 개별 쓰레드가 끝날 때마다 전체
카운트를 계산해 가지고 있으며, \co{final_mutex} 는 쓰레드들 사이에서 전체
...
업데이트 쓰레드들에 의해 사용되는 \co{inc_count()} 함수는
라인~\lnref{b}-\lnref{e} 에서 볼 수 있듯이 상당히 간단합니다.
\iffalse
...
라인~\lnref{acquire} 에서 종료되는 쓰레드들을 배제하기 위해 락을 획득하고,
라인~\lnref{release} 에서 이 락을 풉니다.
라인~\lnref{sum:init} 에서는 이미 종료한 쓰레드들에 의해 계산된 카운트 값으로
카운트의 합을 초기화 하고, 라인~\lnref{loop:b}-\lnref{loop:e} 에서 현재 동작
중인 쓰레드들에서 얻어온 카운트 값을 더합니다.
마지막으로, 라인~\lnref{return} 에서 그 합을 리턴합니다.
\iffalse
...
\QuickQuiz{}
	\begin{lineref}[ln:count:count_end:whole:read]
	Listing~\ref{lst:count:Per-Thread Statistical Counters} 의
	라인~\lnref{check} 에서의 \co{NULL} 체크는 브랜치 예측 실패를 가져오지
	않나요?
...
라인~\lnref{b}-\lnref{e} 는 \co{count_register_thread()} 함수를 보여주는데, 각
...
라인~\lnref{b}-\lnref{e} 는 \co{count_unregister_thread()} 함수를 보이는데, 각
쓰레드는 종료되기 직전에 반드시 이 함수를 호출해야 합니다.
라인~\lnref{acquire} 에서는 락을 잡고, 라인~\lnref{release} 에서 푸는데, 이로써
\co{read_count()} 호출과 다른 \co{count_unregister_thread()} 호출을
배제시킵니다.
라인~\lnref{add} 에서는 이 쓰레드의 \co{counter} 를 글로벌 \co{finalcount} 에
더하고, 그 후 라인~\lnref{NULL} 에서 자신의 \co{counterp[]} 배열에서의 원소를
\co{NULL} 로 만듭니다.
...
%
\epigraph{Errors using inadequate data are much less than those using no
	  data at all.}
	 {\emph{Charles Babbage}}
...
라인~\lnref{globalcountmax} 의 \co{globalcountmax} 변수는 합계 카운터의
최대값을 가지며, 라인~\lnref{globalcount} 의 \co{globalcount} 변수는 글로벌
카운터 입니다.
\co{globalcount} 와 각 쓰레드의 \co{counter} 의 합은 전체 카운터의 합산 값이
됩니다.
라인~\lnref{globalreserve} 의 \co{globalreserve} 변수는 모든 쓰레드별
\co{countermax} 변수의 값의 합입니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 은 \co{add_count()} 를 보여주는데, 이 함수는 특정값
\co{delta} 를 카운터에 더합니다.
라인~\lnref{checklocal} 에서는 \co{delta} 를 위한 공간이 이 쓰레드의
\co{counter} 에 남아 있는지 확인하고, 만약 그렇다면 라인~\lnref{add} 에서 그
값을 더하고 라인~\lnref{return:ls}에서 성공했음을 리턴합니다.
...
\QuickQuiz{}
	Listing~\ref{lst:count:Simple Limit Counter Add, Subtract, and Read}
	line~\ref{ln:count:count_lim:add_sub_read:add:checklocal}
	의 저 이상한 조건문은 뭔가요?
...
\begin{lineref}[ln:count:count_lim:add_sub_read:add]
라인~\lnref{checklocal} 에서의 테스트가 실패한다면, 글로벌 변수들에 접근해야
하므로, 라인~\lnref{acquire} 의 \co{gblcnt_mutex} 를 반드시 잡아야 하고, 이
락은 실패하는 경우 라인~\lnref{release:s} 에서, 성공하는 경우는
라인~\lnref{release:f} 에서 놓아집니다.
라인~\lnref{globalize} 에서는
...
라인~\lnref{checkglb:b} 와~\lnref{checkglb:e} 에서는 \co{delta} 를 더하는 것이
합법적인지 Listing~\ref{fig:count:Simple Limit Counter Variable Relationships}
에서 가장 왼쪽 두 빨간 막대의 높이 차이로 나타난 크기 규칙이 지켜지는지로
체크합니다.
\co{delta} 의 합이 이루어져선 안된다면, 라인~\lnref{release:f} 은 (앞서
이야기했듯) \co{gblcnt_mutex} 를 풀고 라인~\lnref{return:gf} 에서 실패를 알리는
리턴을 합니다.
\iffalse
...
라인~\lnref{addglb} 는 \co{delta} 를 \co{globalcount} 에 더하고,
라인~\lnref{balance} 에서 글로벌 변수들과 쓰레드별 변수들을 모두 업데이트 하기
위해
...
그러고나서 라인~\lnref{release:s}16 에서 \co{gblcnt_mutex} 를 (역시 앞에서
언급했듯) 놓아주고, 마지막으로 라인~\lnref{return:gs} 에서 성공을 의미하는 값을
리턴 합니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 은 \co{sub_count()} 함수를 보여주는데, 이 함수는
\co{delta} 만큼을 카운터에서 제거합니다.
라인~\lnref{checklocal} 는 쓰레드별 카운터를 체크해서 이 빼기를 해도 되는지
보고, 만약 그렇다면 라인~\lnref{sub} 에서 그 빼기를 수행한 후 라인~24 에서
성공을 리턴합니다.
...
라인~\lnref{acquire}-\lnref{return:gs} 의 느린 실행경로로 이어집니다.
느린 실행 경로는 글로벌 상태에 접근해야 하기 때문에, 라인~\lnref{acquire} 에서
\co{gblcnt_mutex} 를 잡고, 이 락은 (실패의 경우) 라인~\lnref{release:f} 에서,
또는 (성공의 경우) 라인~\lnref{release:s} 에서 해제됩니다.
라인~\lnref{globalize} 은
Listing~\ref{lst:count:Simple Limit Counter Utility Functions} 의
\co{globalize_count()} 를 실행하는데, 앞서 설명했듯 쓰레드 로컬 변수의 값을
지우고, 글로벌 변수들을 적당한 값으로 설정합니다.
라인~\lnref{checkglb} 은 카운터가 \co{delta} 의 빼기를 해도 좋을지 보고, 좋지
않다면 라인~\lnref{release:f} 에서 \co{gblcnt_mutex} 를 (앞서 설명했듯)
해제하고 라인~\lnref{return:gf} 에서 실패를 리턴합니다.
\iffalse
...
\begin{lineref}[ln:count:count_lim:add_sub_read:sub]
한편, 만약 라인~\lnref{checkglb} 에서 카운터가 \co{delta} 빼기를 해도
\emph{적합}하다고 판단되면, 느린 수행경로를 걷게 됩니다.
라인~\lnref{subglb} 에서 실제 빼기를 행하게 되고, 이후 라인~\lnref{balance}
에서 (Listing~\ref{lst:count:Simple Limit Counter Utility Functions} 에 나온)
\co{balance_count()} 를 호출해서 글로벌 변수들과 쓰레드별 변수들을 모두
(가능하면 다음엔 빠른 수행경로로 돌 수 있도록) 업데이트 합니다.
라인~\lnref{release:s} 에서는 \co{gblcnt_mutex} 를 해제하고,
라인~\lnref{return:gs} 에서 성공을 리턴합니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 은 \co{read_count()} 를 보이는데, 이 함수는 카운터의
합산된 값을 리턴합니다.
먼저 라인~\lnref{release} 에서 해제하는 \co{gblcnt_mutex} 를
라인~\lnref{acquire} 에서 잡는데, 이 락으로 \co{add_count()} 와
\co{sub_count()} 의 글로벌한 동작들을 배제시키고, 또한 쓰레드 생성과 종료 역시
배제시킵니다.
라인~\lnref{initsum} 는 로컬 변수 \co{sum} 을 \co{globalcount} 의 값으로 초기화
하고, 라인~\lnref{loop:b}-\lnref{loop:e} 의 루프는 쓰레드별 \co{counter} 변수의
합을 구합니다.
그러고나서 라인~\lnref{return} 에서 합을 리턴합니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 은 \co{globalize_count()} 함수인데, 이 함수는 현
쓰레드의 쓰레드별 카운터를 0으로 만들고, 글로벌 변수들을 적당한 값으로
만듭니다.
이 함수는 카운터의 합산값을 바꾸진 않지만 어떻게 카운터의 현재 값이 보여야
하는지를 바꾼다는 점이 중요합니다.
라인~\lnref{add} 은 쓰레드의 \co{counter} 변수를 \co{globalcount} 에 더하고
라인~\lnref{zero} 는 \co{counter} 를 0으로 만듭니다.
비슷하게, 라인~\lnref{sub} 는 쓰레드별 \co{countermax} 를 \co{globalreserve}
에서 빼고, 라인~\lnref{zeromax} 은 \co{countermax} 를 0으로 만듭니다.
...
라인~\lnref{b}-\lnref{e} 는 \co{balance_count()} 함수를 보이는데, 이 함수는
간단히 말해서 \co{globalize_count()} 의 반대입니다.
...
라인~\lnref{share:b}-\lnref{share:e} 은 \co{globalcount} 나 \co{globalreserve}
로 다뤄지지 않은 \co{globalcountmax} 에서 이 쓰레드의 지분을 계산하고, 계산된
값을 이 쓰레드의 \co{countermax} 에 대입합니다.
라인~\lnref{adjreserve} 는 \co{globalreserve} 를 올바르게 조정합니다.
라인~\lnref{middle} 는 이 쓰레드의 \co{counter} 를 \co{countermax} 절반으로
만듭니다.
라인~\lnref{check} 은 이 \co{counter} 값이 규칙을 깨지 않는 선에서 가능한
값인지 \co{globalcount} 를 보고, 그렇지 않다면 라인~\lnref{adjcounter} 에서
\co{counter} 를 적절하게 감소시킵니다.
앞의 조건과 관계없이 마지막으로, 라인~\lnref{adjglobal} 에서는 \co{globalcount}
를 적절하게 조정합니다.
\iffalse
...
\QuickQuiz{}
	...
	에서는 왜 \co{counter} 를 \co{countermax / 2} 로 만들죠?
...
라인~\lnref{b}-\lnref{e} 은 \co{count_register_thread()} 함수인데, 이 함수는
새로 생성된 쓰레드를 위한 상태를 만듭니다.
...
마지막으로, 라인~\lnref{b}-\lnref{e} 은 \co{count_unregister_thread()} 함수를
보여주는데, 이 함수는 곧 종료될 쓰레드를 위해 상태를 정리합니다.
라인~\lnref{acquire} 에서는 라인~\lnref{release} 에서 해제될 \co{gblcnt_mutex}
를 잡습니다.
라인~\lnref{globalize} 에서는 \co{globalize_count()} 를 호출해서 이 쓰레드의
카운터 상태를 정리하고, 라인~\lnref{clear} 에서 이 쓰레드의 \co{counterp[]}
...
라인~\lnref{enforce:b} 과~\lnref{enforce:e} 에서 쓰레드별 \co{countermax}
변수에 \co{MAX_COUNTERMAX} 한계를 적용하는 내용이 추가된 걸 제외하고는
Listing~\ref{lst:count:Simple Limit Counter Utility Functions} 의
...
라인~\lnref{candmax} 에 있는 하나의 변수 \co{ctrandmax} 의 앞쪽 절반에
\co{counter} 가, 뒷쪽 절반에는 \co{countermax} 가 위치하도록 합쳐졌습니다.
이 변수는 결국 \co{int} 인 \co{atomic_t} 타입입니다.

라인~\lnref{def:b}-\lnref{def:e} 은 \co{globalcountmax}, \co{globalcount},
\co{globalreserve}, \co{counterp}, 그리고 \co{gblcnt_mutex} 의 정의를 보이는데,
이들은 Listing~\ref{lst:count:Approximate Limit Counter Variables} 에 있는
것들과 비슷한 역할을 합니다.
라인~\lnref{CM_BITS} 은 \co{ctrandmax} 의 각 절반에 사용되는 비트의 수를
나타내는 \co{CM_BITS}를 정의하고, 라인~\lnref{MAX_CMAX} 에서는 \co{ctrandmax}
의 각 절반이 가질 수 있는 최대값을 나타내는 \co{MAX_COUNTERMAX} 를 정의합니다.
\iffalse
...
\QuickQuiz{}
	Listing~\ref{lst:count:Atomic Limit Counter Variables and Access Functions}
        의 line~\ref{ln:count:count_lim_atomic:var_access:var:CM_BITS}
	에서는 C 표준을 어기는거 아닌가요?
	\iffalse
...
라인~\lnref{b}-\lnref{e} 는 \co{split_ctrandmax_int()} 함수를 보여주는데, 이
함수는 \co{atomic_t ctrandmax} 변수로부터 얻어온 \co{int} 값을 받아서,
\co{counter} (\co{c}) 와 \co{countermax} (\co{cm}) 으로 쪼갭니다.
라인~\lnref{msh} 은 이 \co{int} 로부터 앞쪽 절반을 떼어내어 인자로 받은 \co{c}
에 넣고, 라인~\lnref{lsh} 에서는 뒷쪽 절반을 떼어내어 인자로 받은 \co{cm} 에
넣습니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 는 \co{split_ctrandmax()} 함수를 보여주는데, 이 함수는
라인~\lnref{int} 에서 주어진 변수로부터 \co{int} 값을 읽어내고,
라인~\lnref{old} 에서 그 값을 인자로 받은 \co{old} 에 저장하고,
라인~\lnref{split_int} 에서 \co{split_ctrandmax_int()} 를 호출해 쪼개는 작업을
합니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 은 \co{merge_ctrandmax()} 함수인데, 이 함수는
\co{split_ctrandmax()} 함수의 반대의 일을 하는 것으로 볼 수 있습니다.
라인~\lnref{merge} 에서 \co{c} 와 \co{cm} 으로 받은 \co{counter} 와
\co{countermax} 값을 합쳐서 그 결과값을 리턴합니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 는 \co{add_count()} 함수를 보이는데, 이 함수는
라인~\lnref{fast:b}-\lnref{fast:e} 에 빠른 수행 경로가, 나머지에는 느린
수행경로가 있습니다.
빠른 수행경로의 라인~\lnref{fast:b}-\lnref{loop:e} 는
라인~\lnref{atmcmpex}-\lnref{loop:e} 에서 실제 CAS 를 \co{atomic_cmpxchg()}
함수로 수행하는 compare-and-swap (CAS) 루프를 구성합니다.
라인~\lnref{split} 는 현재 쓰레드의 \co{ctrandmax} 변수를 \co{counter} (\co{c})
와 \co{countermax} (\co{cm}) 로 쪼개고, 기존의 \co{ctrandmax} 가 갖는 \co{int}
값을 \co{old} 에 넣어 둡니다.
라인~\lnref{check} 에서는 \co{delta} 값이 로컬하게 처리 가능할지 (인티저
오버플로우를 막기 위한 처리를 하면서) 체크하고, 만약 그렇지 않다면
라인~\lnref{goto} 의 느린 수행경로를 밟게 됩니다.
로컬하게 처리 가능하다면, 라인~\lnref{merge} 에서 업데이트된 \co{counter} 값을
원본 \co{countermax} 값과 조합해서 \co{new} 를 만듭니다.
라인~\lnref{atmcmpex}-\lnref{loop:e} 에서는 \co{atomic_cmpxchg()} 함수를 이용해
이 쓰레드의 \co{ctrandmax} 변수를 \co{old} 와 어토믹하게 비교하고, 만약 비교
결과 같다면 그 값을 \co{new} 로 어토믹하게 업데이트 합니다.
만약 비교 결과가 같다면 라인~\lnref{return:fs} 에서 성공을 리턴하고, 그렇지
않았다면 라인~\lnref{fast:b} 의 루프에서 실행을 계속합니다.
\iffalse
...
\QuickQuiz{}
	...
	\fi
\QuickQuizAnswer{
	해당 \co{goto} 를 \co{break} 로 대체하려면
	라인~\ref{ln:count:count_lim_atomic:add_sub:add:return:fs} 에서
	리턴해야 할지 말아야 할지를 결정하기 위한 플래그를 하나 더 만들어야
	할텐데, 이건 빠른 수행 경로에서 하고자 하는 일은 아닐 겁니다.
...
\QuickQuiz{}
	...
	라인~\lnref{atmcpmex}-\lnref{loop:e} 의 \co{atomic_cmpxchg()} 함수는
	어떻게 실패할 수 있죠?
	우린 이전 값을 라인~\lnref{split} 에서 가져오고 나서 바꾼 적 없잖아요!
	\iffalse
...
\QuickQuizAnswer{
	...
	라인~\lnref{fast:b}-\lnref{loop:e} 의 빠른 수행 경로 실행과 동시에
	수정할 수 있는지 알아볼 겁니다.
	\iffalse
...
\begin{lineref}[ln:count:count_lim_atomic:add_sub:add]
Listing~\ref{lst:count:Atomic Limit Counter Add and Subtract} 의
라인~\lnref{slow:b}-\lnref{return:ss} 은
\co{add_count()} 의 느린 수행경로를 보이는데, 라인~\lnref{acquire} 에서
얻어오고 라인~\lnref{release:f} 와~\lnref{release:s} 에서 해제하는
\co{gblcnt_mutex} 로 보호됩니다.
라인~\lnref{globalize} 에서는 \co{globalize_count()} 를 호출하는데, 이 함수는
이 쓰레드의 상태를 글로벌 카운터로 옮겨줍니다.
라인~\lnref{checkglb:b}-\lnref{checkglb:e} 은 \co{delta} 값이 현재의 글로벌
상태에 사용 가능한지 체크하고, 불가능하다면 라인~\lnref{flush} 에서
\co{flush_local_count()} 를 호출해서 모든 쓰레드의 로컬 스테이트를 글로벌
카운터로 몰아넣고, 라인~\lnref{checkglb:nb}-\lnref{checkglb:ne} 에서 \co{delta}
가 이제는 사용 가능한지 다시 체크합니다.
이후에도 \co{delta} 더하기가 불가능하다면, 라인~\lnref{release:f} 에서
\co{gblcnt_mutex} 를 해제하고 라인~\lnref{return:sf} 에서 실패를 리턴합니다.
\iffalse
...
그렇지 않다면, 라인~\lnref{addglb} 에서 \co{delta} 를 글로벌 카운터에 더하고,
라인~\lnref{balance} 에서 만약 적절하다면 카운트를 로컬 스테이트에 분산시킨 후,
라인~\lnref{release:s} 에서 \co{gblcnt_mutex} 를 해제하고, 마지막으로
라인~\lnref{return:ss} 에서 성공을 리턴합니다.
\iffalse
...
구성되어 있어서, 라인~\lnref{fast:b}-\lnref{fast:e} 에 빠른 수행경로를, 그리고
라인~\lnref{slow:b}-\lnref{slow:e} 에 느린 수행경로를 갖습니다.
...
보입니다.
라인~\lnref{acquire} 에서 \co{gblcnt_mutex} 를 잡고 라인~\lnref{release} 에서
해제합니다.
라인~\lnref{initsum} 에서는 로컬 변수 \co{sum} 을 \co{globalcount} 의 값으로
초기화 하고, 라인~\lnref{loop:b}-\lnref{loop:e} 의 루프에서는 쓰레드별 카운터를
이 합에 더하고 라인~\lnref{split} 에서 각 쓰레드별 카운터를
\co{split_ctrandmax} 를 사용해서 고립시킵니다.
마지막으로, 라인~\lnref{return} 에서 아까 만든 합을 리턴합니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 에 있는데 앞의 알고리즘과 거의 같지만
라인~\lnref{split}의, \co{counter} 와 \co{countermax} 를 \co{ctrandmax} 에서
분할해 내기 위한 코드의 추가가 차이점입니다.
\iffalse
...
\co{flush_local_count()} 의 코드는 라인~\lnref{b}-\lnref{e} 에 있습니다.
라인~\lnref{checkrsv} 에서는 \co{globalreserve} 의 값이 모든 쓰레드별 카운트를
허용하는지 체크하고 아니라면 라인~\lnref{return:n} 에서 리턴합니다.
그렇지 않다면, 라인~\lnref{initzero} 에서 로컬 변수 \co{zero} 를 \co{counter}
와 \co{countermax} 조합된 0으로 만듭니다.
라인~\lnref{loop:b}-\lnref{loop:e} 의 루프에서는 각 쓰레드를 순회합니다.
라인~\lnref{checkp} 에서 현재 쓰레드가 카운터 상태를 가지고 있는지 보고, 만약
그렇다면 라인~\lnref{atmxchg}-\lnref{glbrsv} 에서 그 상태를 글로벌 카운터로
옮깁니다.
라인~\lnref{atmxchg} 에서는 현재 쓰레드의 상태를 어토믹하게 0 으로 바꿈과
동시에 얻어옵니다.
라인~\lnref{split} 에서는 이 상태를 자신의 \co{counter} (로컬 변수 \co{c} 로)
와 \co{countermax} (로컬 변수 \co{cm} 으로) 로 분할합니다.
라인~\lnref{glbcnt} 에서 이 쓰레드의 \co{counter} 를 \co{globalcount} 에 더하고
라인~\lnref{glbrsv} 에서 이 쓰레드의 \co{countermax} 를 \co{globalreserve} 에서
뺍니다.
\iffalse
...
\QuickQuiz{}
	...
	\co{flush_local_count()} 가 \co{ctrandmax} 변수를 0 으로 만든 후 그냥
...
\QuickQuiz{}
	Listing~\ref{lst:count:Atomic Limit Counter Utility Functions 1} 의
	라인~\ref{ln:count:count_lim_atomic:utility1:flush:atmxchg} 에서
	\co{flush_local_count()} 가 \co{ctrandmax} 변수를 비우는 동안
	\co{add_count()} 나 \co{sub_count()} 의 빠른 수행경로가 \co{ctrandmax}
...
라인~\lnref{balance:b}-\lnref{balance:e} 는 호출한 쓰레드의 로컬 \co{ctrandmax}
변수를 재설정 하는 \co{balance_count()} 함수의 코드를 보여줍니다.
이 함수는 앞의 알고리즘과 상당히 유사합니다만, \co{ctrandmax} 변수의 병합
작업을 처리하는 부분이 추가되었습니다.
해당 코드의 자세한 분석은 라인~\lnref{register:b} 에 시작하는
\co{count_register_thread()} 함수와 라인~\lnref{unregister:b} 에서 시작하는
\co{count_unregister_thread()} 함수와 함께 독자 여러분의 연습문제로
남겨두겠습니다.
\iffalse
...
\QuickQuiz{}
	...
	어떻게
	Listing~\ref{lst:count:Atomic Limit Counter Utility Functions 2} 의
	라인~\ref{ln:count:count_lim_atomic:utility2:balance:atmcset} 에서의
	\co{balance_count()} 는 \co{flush_local_count()} 의 해당 변수에 동시에
	가해지는 업데이트에도 불구하고 올바르게 동작할 수 있는 거죠?
	\iffalse
...
라인~\lnref{value:b}-\lnref{value:e} 에서는 앞의 섹션에서 설명한 쓰레드별 theft
스테이트 머신을 위한 상태와 값들을 정의합니다.
라인~\lnref{var:b}-\lnref{var:e} 은 앞의 구현과 비슷합니다만, 라인~\lnref{maxp}
와~\lnref{theftp} 에 쓰레드의 \co{countermax} 와 \co{theft} 변수에의 원격에서의
...
라인~\lnref{b}-\lnref{e} 은 \co{globalize_count()} 함수를 보이는데, 이 함수는
앞의 구현과 동일합니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 는 \co{flush_local_count_sig()} 함수로, 훔치기
과정에서 사용되는 시그널 핸들러입니다.
라인~\lnref{check:REQ} 과~\lnref{return:n} 에서는 \co{theft} 상태가 REQ 인지
확인하고, 아니라면 변경 없이 리턴합니다.
라인~\lnref{mb:1} 에서는 theft 변수의 샘플링이 해당 변수에의 변경 이전에
이루어졌음을 분명히 하기 위해 메모리 배리어를 칩니다.
라인~\lnref{set:ACK} 는 \co{theft} 상태를 ACK 로 놓고, 라인~\lnref{check:fast}
에서는 이 쓰레드의 빠른 수행 경로가 수행중인지 확인 후, 라인~\lnref{set:READY}
에서 \co{theft} 상태를 READY 로 놓습니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 는 \co{flush_local_count()} 를 보이는데, 이 함수는
느린 수행 경로에서 모든 쓰레드의 로컬 카운트를 비우기 위해 호출됩니다.
라인~\lnref{loop:b}-\lnref{loop:e} 의 루프에서는 로컬 카운트가 있는 모든
쓰레드의 \co{theft} 상태를 바꾸고, 해당 쓰레드들에 시그널을 날립니다.
라인~\lnref{skip} 에서는 존재하지 않는 쓰레드들을 스킵합니다.
그렇지 않다면, 라인~\lnref{checkmax} 에서 현재 쓰레드가 로컬 카운트를 가지고
있는지 보고, 가지고 있지 않다면 라인~\lnref{READY} 에서 해당 쓰레드의
\co{theft} 상태를 READY 로 바꾸고 라인~\lnref{next} 에서 다음 쓰레드로
넘어갑니다.
그렇지 않고 현재 쓰레드가 로컬 카운트를 가지고 있다면, 라인~\lnref{REQ} 에서
해당 쓰레드의 \co{theft} 상태를 REQ 로 바꾸고, 라인~\lnref{signal} 에서
시그널을 날립니다.
\iffalse
...
\QuickQuiz{}
	Listing~\ref{lst:count:Signal-Theft Limit Counter Value-Migration Functions}
	에서, 왜 라인~\ref{ln:count:count_lim_sig:migration:flush:signal} 은
	현재 쓰레드가 자기 자신에게 시그널을 보내는지 체크하지 않나요?
	\iffalse
...
\QuickQuizAnswer{
	...
	에서의 체크가 성공해서 뒤의 \co{pthread_kill()} 은 스킵될 겁니다.
	\iffalse
...
\begin{lineref}[ln:count:count_lim_sig:migration:flush]
라인~\lnref{loop2:b}-\lnref{loop2:e} 의 루프는 각 쓰레드가 READY 상태에
도달하길 기다리고, 이후 해당 쓰레드의 카운트를 훔칩니다.
라인~\lnref{skip:nonexist}-\lnref{next2} 은 존재하지 않는 쓰레드들을 스킵하고,
라인~\lnref{loop3:b}-\lnref{loop3:e} 의 루프에서 현재 쓰레드의 \co{theft}
상태가 READY 가 되길 기다립니다.
라인~\lnref{block} 에서는 우선순위 역전(priority-inversion) 문제를 막기 위해 1
밀리세컨드 동안 블락되고, 라인~\lnref{check:REQ} 에서 해당 쓰레드의 시그널이
아직 도착하지 않은 것으로 판단되면 라인~\lnref{signal2} 에서 시그널을 다시
날립니다.
현재 살펴보고 있는 쓰레드의 상태가 마침내 READY 가 되면 실행 흐름은
라인~\lnref{thiev:b} 에 도달해 라인~\lnref{thiev:b}-\lnref{thiev:e} 에서
훔치기를 시전합니다.
라인~\lnref{IDLE} 은 해당 쓰레드의 \co{theft} 상태를 다시 IDLE 로 되돌립니다.
\iffalse
...
라인~\lnref{b}-\lnref{e} 에서는 \co{balance_count()} 함수를 보이는데, 앞의
예제와 비슷합니다.
\iffalse
...
라인~\lnref{fast:b}-\lnref{return:fs} 에는 빠른 수행 경로가,
라인~\lnref{acquire}-\lnref{return:ss} 에는 느린 수행 경로가 있습니다.
라인~\lnref{fast:b}에서는 쓰레드별 \co{counting} 변수를 1로 만들어서 이후에 이
쓰레드를 인터럽트하는 어떤 시그널 핸들러도 \co{theft} 상태를 READY 가 아니라
ACK 로 설정하게 해둠으로써, 이 빠른 수행 경로가 올바르게 완료되도록 합니다.
라인~\lnref{barrier:1} 은 컴파일러가 빠른 수행 경로의 본체가 \co{counting}
설정보다 앞에 위치하도록 재배치 하는 것을 막아줍니다.
라인~\lnref{check:b} 과~\lnref{check:e} 은 쓰레드별 데이터가 \co{add_count()}
를 처리할 수 있는지 확인하고 현재 수행중인 훔치기 작업이 없다면
라인~\lnref{add:f} 에서 빠른 수행 경로 더하기 연산을 수행하고
라인~\lnref{fasttaken} 에서 빠른 수행 경로가 진행됨을 알립니다.
\iffalse
...
어떤 경우든, 라인~\lnref{barrier:2} 에서 컴파일러가 빠른 수행 경로 본체가
라인~\lnref{clearcnt} 뒤에 오도록 재배치해 이후의 시그널 핸들러가 훔치기 작업을
하는 사태를 만들지 못하게 합니다.
라인~\lnref{barrier:3} 에서는 다시 컴파일러 재배치를 막고,
라인~\lnref{check:ACK} 에서 시그널 핸들러가 \co{theft} 상태를 READY 로 바꿨는지
보고, 만약 그렇다면 라인~\lnref{mb} 에서 라인~\lnref{READY} 에서 READY 로
설정한 상태를 본 CPU 는 라인~\lnref{add:f} 의 결과도 보도록 해줍니다.
라인~\lnref{add:f} 에서의 빠른 수행 경로 더하기가 실행되었다면,
라인~\lnref{return:fs} 에서 성공을 리턴합니다.
\iffalse
...
\begin{lineref}[ln:count:count_lim_sig:add]
그렇지 않다면, 라인~\lnref{acquire} 부터 시작되는 느린 수행 경로로 떨어집니다.
...
의 라인~\lnref{b}-\lnref{e} 는 \co{count_init()} 를 보여주는데, 이 함수에선
...
이 섹션에서 소개한 정확한 리미트 카운터 구현은 매우 유용할 수 있지만, 예를 들어
I/O 디바이스로의 액세스 수를 세는 경우와 같이 카운터의 값이 항상 0에 가깝게
유지된다면 별로 유용하지 않을 겁니다.
...
\QuickQuizAnswer{
	...
	\item	폴링하는 루프는 실제 시스템에서는 CPU 시간과 전력을 낭비하는
		문제가 있을 수 있습니다.
		많은 경우, 이벤트 기반 설계가 훨씬 나은데, 예를들면 마지막으로
		I/O 를 완료하는 쪽에서 디바이스 제거 쓰레드를 깨우는 식입니다.
...
\QuickQuizAnswer{
	...
	Figure~\ref{fig:count:Atomic Increment Scalability on Kaby Lake} 에서 볼
...
\QuickQuizAnswer{
	...
	\item	어플리케이션의 성능에 치명적인 부분만이 분할되어야 하고, 그런
		...
		부분입니다.
	\item	캐시 미스는 레지스터에서 레지스터로 값을 옮기는 인스트럭션들에
		비하면 매우 느리지만, TCP/IP 네트워킹과 같은 것들보다 빠른
		프로세스간 통신 (inter-process-communication) 기능들에 비해서도
		상당히 빠릅니다.
	\item	공유 메모리 멀티프로세서들은 이미 시장에 나와 있고 상당히
		저렴하므로, 1990년대와는 정반대로, 공유 메모리 병렬성을
