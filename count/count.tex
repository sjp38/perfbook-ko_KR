...
\QuickQuizAnswer{
	...
	없었습니다.
	최신 버전의 C 표준은 이 간극을 매우기 위한 의도로 atomic 기능을
	추가했습니다.
	하지만, 이것들을 리눅스 커널에 적용하려는 시도는 뭐가 뭔지 모를 결과를
	내놓았는데~\cite{JonathanCorbet2016C11atomics}, 이는 부분적으로는
	atomic 기능이 만들어내는 코드의 낮은 품질 때문이기도 합니다.
	또한, 리눅스 커널은 스스로의 메모리 모델에서 C11 어토믹 기능이 제공하는
	것보다 훨씬 다양한 기능을 기대하는데, 컨트롤 의존성, full
	barrier/fence, 그리고 낮은 오버헤드의 어드레스와 데이터 종속성이 여기
	포함됩니다~\cite{PaulEMcKenney2016P0124R5-LKMM}.
	\iffalse
...
업데이트 쓰레드들에 의해 사용되는 \co{inc_count()} 함수는 라인~6-10 에서 볼 수
있듯이 상당히 간단합니다.

읽기 쓰레드에 의해 사용되는 \co{read_count()} 함수는 약간 복잡합니다.
라인~17 에서 종료되는 쓰레드들을 배제하기 위해 락을 획득하고, 라인~22 에서 이
락을 풉니다.
라인~18 에서는 이미 종료한 쓰레드들에 의해 계산된 카운트 값으로 카운트의 합을
초기화 하고, 라인~19-21 에서 현재 동작 중인 쓰레드들에서 얻어온 카운트 값을
더합니다.
마지막으로, 라인~23 에서 그 합을 리턴합니다.
\iffalse
...
\QuickQuiz{}
	Listing~\ref{lst:count:Per-Thread Statistical Counters} 의 라인~20
...
라인~26-33 는 \co{count_register_thread()} 함수를 보여주는데, 각 쓰레드는
...
라인~35-43 는 \co{count_unregister_thread()} 함수를 보이는데, 각 쓰레드는
종료되기 직전에 반드시 이 함수를 호출해야 합니다.
라인~39 에서는 락을 잡고, 라인~42 에서 푸는데, 이로써 \co{read_count()} 호출과
다른 \co{count_unregister_thread()} 호출을 배제시킵니다.
라인~40 에서는 이 쓰레드의 \co{counter} 를 글로벌 \co{finalcount} 에 더하고, 그
후 라인~41 에서 자신의 \co{counterp[]} 배열에서의 원소를 \co{NULL} 로 만듭니다.
