...
쓰레드별 \co{counter} 와 \co{countermax} 변수는 각각 연관된 쓰레드의 로컬
카운터와 해당 카운터의 상한선입니다.
라인~\lnref{globalcountmax} 의 \co{globalcountmax} 는 합해진 카운터의 상한선을
담고 있으며, 라인~\lnref{globalcount} 의 \co{globalcount} 변수는 전역
카운터입니다.
\co{globalcount} 의 합과 각 쓰레드의 \co{counter} 는 전체 카운터의 합산값을
제공합니다.
라인~\lnref{globalreserve} 의 \co{globalreserve} 변수는 최소한 모든 쓰레드별
\co{countermax} 변수의 합산값입니다.
\end{fcvref}
이 변수들 사이의 관계가
Figure~\ref{fig:count:Simple Limit Counter Variable Relationships} 에 그려져
있습니다:
\begin{enumerate}
\item	\co{globalcount} 와 \co{globalreserve} 의 합은 \co{globalcountmax}
	이하여야만 합니다.
\item	모든 쓰레드의 \co{countermax} 값의 합은 \co{globalreserve} 이하여야만
	합니다.
\item	각 쓰레드의 \co{counter} 는 해당 쓰레드의 \co{countermax} 이하여야만
	합니다.
\end{enumerate}
...
\co{counterp[]} 배열의 각 원소는 연관된 쓰레드의 \co{counter} 변수를 참조하며,
마지막으로, \co{gblcnt_mutex} 스핀락은 모든 전역 변수를 보호하는데, 달리
말하자면, 어떤 쓰레드도 \co{gblcnt_mutex} 를 얻지 않고서는 어떤 전역 변수도
액세스하거나 수정할 수 없습니다.
...
는 \co{add_count()}, \co{sub_count()}, 그리고 \co{read_count()} 함수들을
(\path{count_lim.c}) 보입니다.
...
\QuickQuiz{
	\Cref{lst:count:Simple Limit Counter Add; Subtract; and Read}
	는 왜
	Section~\ref{sec:count:Statistical Counters} 에서 보인
	\co{inc_count()} 와 \co{dec_count()} 인터페이스 대신 \co{add_count()}
	와 \co{sub_count()} 를 제공하는 거죠?
	...
	구조체는 각자 다른 크기를 가지기 때문입니다.
	물론, 특정 크기의 구조체에만 연관된 한계선 카운터는 \co{inc_count()} 와
	\co{dec_count()} 를 사용할 수도 있을 겁니다.
...
\Clnrefrange{b}{e} 는 특정 값 \co{delta} 를 이 카운터에 더하는 \co{add_count()}
를 보입니다.
라인~\lnref{checklocal} 는 이 쓰레드의 \co{counter} 에 \co{delta} 를 더할
공간이 있는지 검사하고, 그렇다면 라인~\lnref{add} 에서 그걸 더하고
라인~\lnref{return:ls} 에서 성공했음을 리턴합니다.
이게 \co{add_counter()} 의 fastpath 로, 여기선 어토믹 오퍼레이션을 사용하지
않고, 쓰레드별 변수들만을 참조하며, 어떤 캐쉬 미스도 일으키지 않을 겁니다.
\end{fcvref}
...
\QuickQuiz{
	\Cref{lst:count:Simple Limit Counter Add; Subtract; and Read}
	의 라인~\ref{ln:count:count_lim:add_sub_read:add:checklocal} 에서의
	이상한 형태의 조건은 뭐죠?
	\Cref{lst:count:Intuitive Fastpath} 에서 보인 더 직관적인 형태의
	fastpath 는 어떤가요?
	...
	``정수형 오버플로우.''
	 ...
	 \Cref{lst:count:Intuitive Fastpath} 의 공식에 \co{counter} 가 10이고
	 \co{delta} 가 \co{ULONG_MAX} 라고 대입해 보세요.
	 그리고 나서
	 \Cref{lst:count:Simple Limit Counter Add; Subtract; and Read} 에 보인
	 코드를 다시 실행해 보세요.

	 이 예의 나머지 부분을 위해선 정수형 오버플로우에 대한 좋은 이해가
	 필요할 것이며, 따라서 여러분이 정수형 오버플로우를 이전에 다뤄본
	 경험이 없다면, 그걸 위해 여러 예제를 시도해 보시기 바랍니다.
	 정수형 오버플로우는 가끔 병렬 알고리즘보다도 옳게 처리하기가
	 어렵습니다!
...
\begin{fcvref}[ln:count:count_lim:add_sub_read:add]
라인~\lnref{checklocal} 에서의 테스트가 실패하면, 우린 전역 변수들에 액세스
해야만 하며, 따라서 라인~\lnref{acquire} 에서 \co{gblcnt_mutex} 를 잡아야 하며,
이 락은 실패의 경우 라인~\lnref{release:f} 에서, 성공의 경우
라인~\lnref{release:s} 에서 해제합니다.
라인~\lnref{globalize} 는
Listing~\ref{lst:count:Simple Limit Counter Utility Functions}
에 보인 \co{globalize_count()} 를 호출하는데, 여기선 쓰레드 지역 변수들을
정리하고, 전역 변수들을 필요한대로 조정 하여서 전체 과정을 단순화 시킵니다.
(하지만 \emph{제} 말만 듣고 그렇겠거니 하지 말고 스스로 코딩해 보세요!)
라인~\lnref{checkglb:b} 와~\lnref{checkglb:e} 는 \co{delta} 의 합이 가능할지
체크해 보는데, 여기선
Figure~\ref{fig:count:Simple Limit Counter Variable Relationships}
의 작음 부호를 앞서는 표현이 (가장 왼쪽의) 두개의 빨간 막대의 높이 차를
의미합니다.
\co{delta} 를 합하는 게 가능하지 않다면, 라인~\lnref{release:f} 는 (앞서
언급되었듯) \co{gblcnt_mutex} 를 해제하고 라인~\lnref{return:gf} 는 실패를
의미하는 값을 리턴합니다.
...
그렇지 않다면, 우린 slowpath 를 취합니다.
라인~\lnref{addglb} 에서는 \co{globalcount} 에 \co{delta} 를 더하고, 이어서
라인~\lnref{balance} 에서는 이 전역 변수와 쓰레드별 변수를 모두 업데이트 하기
위해
(Listing~\ref{lst:count:Simple Limit Counter Utility Functions} 에 보인)
\co{balance_count()} 를 호출합니다.
이 \co{balance_count()} 호출은 일반적으로 fastpath 를 다시 가능하게 하기 위해
이 쓰레드의 \co{countermax} 값을 설정할 겁니다.
라인~\lnref{release:s} 는 (다시, 앞서 이야기 되었듯) \co{gblcnt_mutex} 를
해제하고, 마지막으로 라인~\lnref{return:gs} 는 성공을 의미하는 값을 리턴합니다.
\end{fcvref}
...
\QuickQuiz{
	\Cref{lst:count:Simple Limit Counter Add; Subtract; and Read} 에서
	\co{globalize_count()} 는 왜 나중에 \co{balance_count()} 를 호출해
	그것을 다시 채우기 위한 이유만으로 쓰레드별 변수를 0으로 초기화
	시키나요?
	왜 그 쓰레드별 변수를 그냥 0이 아닌 채로 두지 않는 거죠?
	...
	실제로 이 코드의 이전 버전이 그렇게 했습니다.
	하지만 더하기와 빼기는 무척 비용이 낮고, 도사리고 있는 모든 특수 경우를
	처리하는 건 상당히 복잡합니다.
	다시 말하지만, 스스로 해보세요, 하지만 정수형 오버플로우를 조심하세요!
...
\Clnrefrange{b}{e} 는 이 카운터에서 \co{delta} 를 빼는 \co{sub_count()} 를
보입니다.
라인~\lnref{checklocal} 은 이 쓰레드별 카운터가 이 빼기를 할 수 있는지 보고,
그렇다면 라인~\lnref{sub} 는 이 빼기를 행하고 라인~\lnref{return:ls} 에서
성공을 의미하는 값을 반환합니다.
이 라인들이 \co{sub_count()} 의 fastpath 를 형성하며, \co{add_count()} 에서와
마찬가지로, 이 fastpath 는 어떤 비용이 높은 오퍼레이션도 수행하지 않습니다.

이 fastpath 가 \co{delta} 빼기를 하지 못한다면, 수행은
\clnrefrange{acquire}{return:gs} 의 slowpath 로 이어집니다.
이 slowpath 는 전역 상태에 접근해야만 하므로, 라인~\lnref{acquire} 는
\co{gblcnt_mutex} 를 획득하는데, 이는 라인~\lnref{release:f} 에서 (실패의 경우)
또는 라인~\lnref{release:s} 에서 (성공의 경우) 해제됩니다.
라인~\lnref{globalize} 는
Listing~\ref{lst:count:Simple Limit Counter Utility Functions}
에 보인 \co{globalize_count()} 를 호출하는데, 이는 다시 쓰레드 지역 변수들을
정리하고 전역 변수들을 필요한대로 조정합니다.
라인~\lnref{checkglb} 는 이 카운터가 \co{delta} 빼기를 할 수 있는지 체크하고,
그렇지 않다면 라인~\lnref{release:f} 에서 \co{gblcnt_mutex} 를 (앞서 이야기
했듯) 해제하고 라인~\lnref{return:gf} 에서 실패를 의미하는 값을 리턴합니다.
\end{fcvref}
...
\QuickQuizSeries{%
\QuickQuizB{
	\co{add_count()} 에서는 \co{globalreserve} 가 우리를 위해 세어졌는데,
	\cref{lst:count:Simple Limit Counter Add; Subtract; and Read}
	의 \co{sub_count()} 에서는 왜 그렇지 않나요?
	...
	따라갑니다.
	이 쓰레드들의 \co{counter} 변수의 합은 0과 \co{globalreserve} 사이
	어디엔가 있을 수 있습니다.
	따라서 우리는 모든 쓰레드의 \co{counter} 변수가 \co{add_count()}
	시점에서 꽉 차있다고 가정하고 \co{sub_count()} 시점에서 모두 비어
	있다고 가정하는 보수적 전략을 취합니다.
...
\QuickQuizE{
	\Cref{lst:count:Simple Limit Counter Add; Subtract; and Read}
	에 보인 \co{add_count()} 를 한 쓰레드가 호출하고, 다른 쓰레드가
	\co{sub_count()} 를 호출한다고 해봅시다.
	카운터의 값은 0이 아님에도 불구하고 \co{sub_count()} 는 실패를 리턴하지
	않을까요?
	...
	실제로 그럴 겁니다!
	많은 경우, 이는
	Section~\ref{sec:count:Simple Limit Counter Discussion} 에서 이야기한
	대로 문제가 될 것이며, 그런 경우엔
	Section~\ref{sec:count:Exact Limit Counters}
	에서 이야기한 알고리즘이 도움이 될 겁니다.
...
\begin{fcvref}[ln:count:count_lim:add_sub_read:sub]
반면, 라인~\lnref{checkglb} 에서 이 카운터가 \co{delta} 빼기를 할 수
\emph{있다고} 확인되면, 우린 이 slowpath 를 마무리 합니다.
라인~\lnref{subglb} 는 이 빼기를 하고 이어서 라인~\lnref{balance} 에서는 전역
변수와 쓰레드별 변수를 모두 업데이트 하기 위해 (fastpath 가 다시 가능해 지길
바라며) \co{balance_count()} (
Listing~\ref{lst:count:Simple Limit Counter Utility Functions} 에 보여져
있습니다) 를 호출합니다.
라인~\lnref{release:s} 는 \co{gblcnt_mutex} 를 해제하고, 라인~\lnref{return:gs}
는 성공을 의미하는 값을 리턴합니다.
\end{fcvref}
...
\QuickQuiz{
	\Cref{lst:count:Simple Limit Counter Add; Subtract; and Read}
	에는 왜 \co{add_count()} 와 \co{sub_count()} 가 모두 있죠?
	왜 단순히 \co{add_count()} 에 음수를 넘기지 않는 건가요?
	...
	\co{add_count()} 가 그 인자로 \co{unsigned} \co{long} 을 받음을 생각해
	보면, 음수를 넘기기는 좀 어려울 겁니다.
	그리고 여러분이 반물질 메모리를 가지고 있는게 아니라면, 사용 중인
	구조체의 수를 세는데 음수를 허용하는 건 큰 의미가 없을 수 있습니다.

	이 장난은 뒤로 하고 이야기 하자면, \co{add_count()} 와 \co{sub_count()}
	를 합치는 것도 가능할 겁니다만, 이 합쳐진 함수의 \co{if} 조건은 현재의
	함수들 한쌍보다 더 복잡할 것이엇, 결국 이 fast path 의 더 느린 수행을
	의미하게 될 겁니다.
...
\Clnrefrange{b}{e} 는 이 카운터의 합산값을 반환하는 \co{read_count()} 를
보입니다.
이 함수는 라인~\lnref{acquire} 에서 \co{gblcnt_mutex} 를 획득하고
라인~\lnref{release} 에서 이를 해제하여, \co{add_count()} 와 \co{sub_count()}
에서의 전역적 오퍼레이션들을 배제시키며, 우리가 곧 보겠지만, 쓰레드 생성과 종료
역시 배제시킵니다.
라인~\lnref{initsum} 은 지역 변수 \co{sum} 을 \co{globalcount} 의 값으로 초기화
시키고, 이어서 \clnrefrange{loop:b}{loop:e} 의 루프는 쓰레드별 \co{counter}
변수들의 값을 합합니다.
이어서 라인~\lnref{return} 은 이 합을 반환합니다.
\end{fcvref}
...
은
\cref{lst:count:Simple Limit Counter Add; Subtract; and Read}
에서 보인 \co{add_count()}, \co{sub_count()}, 그리고 \co{read_count()}
기능들에서 사용된 유틸리티 합수들을 보입니다.

\begin{fcvref}[ln:count:count_lim:utility:globalize]
\Clnrefrange{b}{e} 는 \co{globalize_count()} 를 보이는데, 이 함수는 현재
쓰레드의 쓰레드별 카운터를 9으로 만들고 전역 변수들을 적절히 조정합니다.
이 함수가 이 카운터의 합산값을 바꾸지 않고, 그 대신 이 카운터의 현재 값이
어떻게 표현되는지를 바꿈을 알아두는 게 중요합니다.
라인~\lnref{add} 는 이 쓰레드의 \co{counter} 변수의 값을 \co{globalcount} 에
더하고, 라인~\lnref{zero} 는 \co{counter} 의 값을 0으로 만듭니다.
비슷하게, 라인~\lnref{sub} 는 쓰레드별 \co{countermax} 를 \co{globalreserve}
에서 빼고, 라인~\lnref{zeromax} 는 \co{countermax} 를 0으로 만듭니다.
이 함수와 \co{balance_count()} 를 읽을 때는
Figure~\ref{fig:count:Simple Limit Counter Variable Relationships}
...
\Clnrefrange{b}{e} 는 대략적으로 말해서 \co{globalize_count()} 의 반대 역할인
\co{balance_count()} 를 보입니다.
이 함수의 일은 현재 쓰레드의 \co{countermax} 변수를 이 카운터가
\co{globalcountmax} 한계를 넘어버리는 위험을 방지하는 가장 큰 값으로 설정하는
것입니다.
현재 쓰레드의 \co{countermax} 변수를 바꾸는 것은 물론
Figure~\ref{fig:count:Simple Limit Counter Variable Relationships}
를 다시 참고하면 볼 수 있듯이 연관된 \co{counter}, \co{globalcount} 그리고
\co{globalreserve} 를 조정할 것이 필요합니다.
이걸 함으로써, \co{balance_count()} 는 \co{add_count()} 와 \co{sub_count()} 의
오버헤드 낮은 fastpath 의 사용을 극대화 시킵니다.
\co{globalize_count()} 에서와 마찬가지로, \co{balance_count()} 는 이 카운터의
합산값을 바꾸는 것이 허용되지 않습니다.
...
\Clnrefrange{share:b}{share:e} 는 \co{globalcount} 나 \co{globalreserve} 에
의해서 커버되지 않는 \co{globalcountmax} 의 부분에 대한 이 쓰레드의 지분을
계산하고, 이 값을 이 쓰레드의 \co{countermax} 에 할당합니다.
라인~\lnref{adjreserve} 는 \co{globalreserve} 로의 이 연관된 조정을 행합니다.
라인~\lnref{middle} 은 이 쓰레드의 \co{counter} 를 0 과 \co{countermax} 사이
중간 값으로 설정합니다.
라인~\lnref{check} 는 \co{globalcount} 가 실제로 \co{counter} 의 이 값을 수용할
수 있을지 검사하고, 그렇지 않다면 라인~\lnref{adjcounter} 에서 \co{counter} 를
적절히 감소시킵니다.
마지막으로, 양쪽 경우 모두, 라인~\lnref{adjglobal} 에서 연관된 조정을
\co{globalcount} 에 행합니다.
\end{fcvref}
...
\QuickQuiz{
	\begin{fcvref}[ln:count:count_lim:utility:balance]
	Listing~\ref{lst:count:Simple Limit Counter Utility Functions}
	의 \clnref{middle} 에서 왜 \co{counter} 를 \co{countermax / 2} 로
	설정하는 거죠?
	그냥 \co{countermax} 카운트를 취하는 게 더 간단하지 않을까요?
	\end{fcvref}
	...
	\begin{fcvref}[ln:count:count_lim:utility:balance]
	첫째로, 그건 실제로는 \co{countermax} 카운트를 예약하는 것입니다만
	(\clnref{adjreserve} 를 참고하세요), 이 부분은 이것들의 절반만이 실제로
	이 순간에 이 쓰레드에 의해 사용되도록 조정합니다.
	이는 이 쓰레드가 \co{globalcount} 를 다시 참조하기 전에 최소한
	\co{countermax / 2} 만큼의 값 증가나 감소를 행할 수 있게 합니다.

	\co{globalcount} 에서의 계산은 \clnref{adjglobal} 에서의 조정 덕분에
	정확하게 유지됨을 알아 두시기 바랍니다.
	\end{fcvref}
...
에 보인 것처럼 카운터들의 관계가 첫번째 \co{globalize_count()} 와 뒤이은
\co{balance_count()} 의 수행에 의해 어떻게 바뀌는지 보는 게 도움이 될 겁니다.
시간은 왼쪽에서 오른쪽으로 흐르며, 가장 왼쪽의 구성은 대략적으로
Figure~\ref{fig:count:Simple Limit Counter Variable Relationships}
의 그것입니다.
가운데의 구성은 쓰레드~0 에 의해 \co{globalize_count()} 가 수행된 후의 이 같은
카운터들의 관계를 보입니다.
이 그림에서 볼 수 있듯이, 쓰레드~0 의 \co{counter} (그림 내의 ``c~0'')a 는
\co{globalcount} 에 더해졌으며, 그 사이 \co{globalreserve} 의 값은 똑같은
양만큼 줄었습니다.
쓰레드~0 의 \co{counter} 도 그것의 \co{countermax} (그림 내의 ``cm~0'') 도 0이
되었습니다.
다른 세개의 쓰레드의 카운터들은 변하지 않았습니다.
이 변화는 이 카운터의 전체 값에는 영향을 주지 않았는데, 가장 바닥쪽의, 가장
왼쪽과 중간 구성을 잇는 점선을 통해 보여져 있음을 알아 두시기 바랍니다.
달리 말하면, \co{globalcount} 와 네 쓰레드의 \co{counter} 변수들의 값의 합은 두
구성 모두에서 같습니다.
비슷하게, 이 변화는 \co{globalcount} 와 \co{globalreserve} 의 합에 영향을
...
오른쪽 구성은 다시 쓰레드~0 에 의해 \co{balance_count()} 가 호출된 후의 이
카운터들의 관계를 보입니다.
각 세개의 구성으로부터 위쪽으로 확장되는 세로 선에 의해 보이는 남아 있는
카운트의 4분의 1이 쓰레드~0 의 \co{countermax} 에 더해지고 그 절반이 쓰레드~0
의 \co{counter} 에 더해졌습니다.
쓰레드~0 의 \co{counter} 에 더해진 양만큼이 또한 이 카운터의 전체 값 (다시
말하지만 이는 \co{globalcount} 와 세 쓰레드의 \co{counter} 변수의 값의
합입니다) 을 바꾸는 걸 막기 위해 \co{globalcount} 에서 빼졌으며, 이 역시 다시
말하지만 가장 아래쪽의, 가운데와 오른쪽 구성을 잇는 두개의 점선으로 표시되어
있습니다.
\co{globalreserve} 변수 역시 이 변수가 네 쓰레드의 \co{countermax} 변수의 값의
합으로 남아있게끔 조정되었습니다.
쓰레드~0 의 \co{counter} 는 그것의 \co{countermax} 보다 작으므로, 쓰레드~0 은
...
\QuickQuiz{
	...
	비록 남아 있는 카운트의 최대 한계까지의 4분의 1이 쓰레드~0 에 할당되어
	있다고는 해도, 가운데와 오른쪽 구성을 잇는 위쪽의 점선이 보이듯
	남아있는 카운트의 8분의 1만이 소모됩니다.
	왜 그런거죠?
	...
	이런 일이 일어나는 이유는 쓰레드~0 의 \co{counter} 가 그것의
	\co{countermax} 의 절반으로 설정되었기 때문입니다.
	따라서, 쓰레드~0 에 할당된 4분의 1 가운데, 절반은 (8분의 1)
	\co{globalcount} 에서 오고, 나머지 절반 (다시 말하지만, 8분의 1) 은
	남아있는 카운트로부터 옵니다.

	이 방법을 취하는 두가지 목적이 있습니다:
	(1)~쓰레드~0 이 증가는 물론 감소에서도 fastpath 를 이용할 수 있게 하는
	것, 그리고
	(2)~모든 쓰레드가 그 한계까지 단조롭게 값을 증가시키기만 할 때의
	부정확도를 줄이는 것.
	이 마지막 부분을 더 자세히 알아보려면, 이 알고리즘을 좀 더 자세히
	들여다 보시기 바랍니다.
...
\Clnrefrange{b}{e} 는 \co{count_register_thread()} 를 보이는데, 이 함수는 새로
생성된 쓰레드의 상태를 설정합니다.
이 함수는 단순히 새로 생성된 쓰레드의 \co{counter} 변수로의 포인터를 연관된
\co{counterp[]} 배열 내 원소에 \co{gblcnt_mutex} 의 보호 아래 넣습니다.
\end{fcvref}

\begin{fcvref}[ln:count:count_lim:utility:unregister]
마지막으로, \clnrefrange{b}{e} 는 \co{count_unregister_thread()} 를 보이는데,
이 함수는 곧 종료될 쓰레드의 상태를 정리합니다.
라인~\lnref{acquire} 는 \co{gblcnt_mutex} 를 획득하고 라인~\lnref{release} 에서
이를 해제합니다.
라인~\lnref{globalize} 에서는 이 쓰레드의 카운터 상태를 지우기 위해
\co{globalize_count()} 를 호출하고, 라인~\lnref{clear} 는 이 쓰레드의
\co{counterp[]} 배열 내 원소를 지웁니다.
\end{fcvref}
...
이런 종류의 카운터는 \co{add_count()} 와 \co{sub_count()} 의 fastpath 내의
비교와 브랜치들로 인한 오버헤드도 있지만 합산값이 0에 가까울 때 상당히
빠릅니다.
하지만, 쓰레드별 \co{countermax} 예약값의 사용은 이 카운터의 합산값이
\co{globalcountmax} 에 전혀 가깝지 않을 때조차도 \co{add_count()} 가 실패할 수
있음을 의미합니다.
비슷하게, \co{sub_count()} 는 이 카운터의 합산값이 0 근처도 아닐 때 조차도
실패할 수 있습니다.

많은 경우, 이는 받아들여질 수 없습니다.
비록 \co{globalcountmax} 가 대략적 한계를 의도했다 하더라도, 정확히 얼만큼
대략적인지가 조절되는 한계도 종종 있습니다.
대략적인 정도를 제한하는 한가지 방법은 쓰레드별 \co{countermax} 인스턴스의 값의
상한값을 정하는 것입니다.
이 작업이 다음 섹션에서 다뤄집니다.
...
이 구현 (\path{count_lim_app.c}) 은 앞의 섹션의 것들 (
(\cref{lst:count:Simple Limit Counter Variables,%
lst:count:Simple Limit Counter Add; Subtract; and Read,%
lst:count:Simple Limit Counter Utility Functions}) 과 상당히 유사하므로, 여기엔
변경 사항만 보입니다.
\Cref{lst:count:Approximate Limit Counter Variables}
는
\cref{lst:count:Simple Limit Counter Variables}
와 동일하지만, 쓰레드별 \co{countermax} 변수의 가능한 최대값을 설정하는
\co{MAX_COUNTERMAX} 를 추가합니다.
...
은
Listing~\ref{lst:count:Simple Limit Counter Utility Functions}
의 \co{balance_count()} 함수와 똑같지만, 쓰레드별 \co{countermax} 변수의
\co{MAX_COUNTERMAX} 한계를 강제하는 라인~\lnref{enforce:b} 와~\lnref{enforce:e}
를 추가합니다.
\end{fcvref}
...
나타냅니다: \co{MAX_COUNTERMAX} 가 어떤 값이든 워크로드에 의존적인 일부 분량의
...
쓰레드는 해당 쓰레드의 \co{counter} 와 \co{countermax} 변수들을 원자적으로
조정해야 합니다.
이걸 위한 일반적인 방법은 이 두 변수를 하나의 변수로 결합시키는 것으로, 예를
들어 32비트 변수가 있다면, 앞쪽 16비트는 \co{counter} 를 나타내게 하고 뒤쪽
16비트는 \co{countermax} 를 나타내게 하는 겁니다.
...
\QuickQuiz{
	왜 이 쓰레드의 \co{counter} 와 \co{countermax} 변수들을 하나의 단위로
	원자적 조정해야 하죠?
	그것들 각자를 원자적으로 조정하는 것으로 충분하지 않을까요?
	...
	이것도 어쩌면 가능할지도 모르지만, 상당한 주의가 필요합니다.
	먼저 \co{countermax} 를 0으로 만들지 않고 \co{counter} 를 제거하는 것은
	연관된 쓰레드가 \co{counter} 를 0이 된 직후에 증가시켜서 이 카운터를
	0으로 만드는 효과를 완전히 무효화 하는 결과를 초래할 수 있습니다.

	반대의 순서, 즉 \co{countermax} 를 0으로 만들고 나서 \co{counter} 를
	제거하는 것은 역시 \co{counter} 가 0이 아니게 할 수 있습니다.
	이걸 알아보기 위해, 다음 순서의 이벤트들을 생각해 봅시다:

	\begin{enumerate}
	\item	쓰레드~A 가 자신의 \co{countermax} 를 읽어오고, 그게 0이 아님을
		확인합니다.
	\item	쓰레드~B 가 쓰레드~A 의 \co{countermax} 를 0으로 만듭니다.
	\item	쓰레드~B 가 쓰레드~A 의 \co{counter} 를 제거합니다.
	\item	자신의 \co{countermax} 가 0이 아님을 확인한 쓰레드~A 는 자신의
		\co{counter} 에 값을 더하여서, 0이 아닌 값을 갖는 \co{counter}
		변수를 초래합니다.
	\end{enumerate}
	...
	다시 말하지만, \co{countermax} 와 \co{counter} 를 별도의 변수들로 두고
...
(\path{count_lim_atomic.c}) 에 보여져 있습니다.
앞의 알고리즘에서의 \co{counter} 와 \co{countermax} 변수들은
라인~\lnref{candmax} 에 보인 단일 변수 \co{counterandmax} 에 앞부분 절반은
\co{counter} 로, 뒷부분 절반은 \co{countermax} 로 쓰이게끔 합쳐졌습니다.
이 변수는 실제 표현은 \co{int} 인 \co{atomic_t} 타입입니다.

\Clnrefrange{def:b}{def:e} 는 
Listing~\ref{lst:count:Approximate Limit Counter Variables}
의 것들과 비슷한 역할을 하는\co{globalcountmax}, \co{globalcount},
\co{globalreserve}, \co{counterp}, 그리고 \co{gblcnt_mutex} 의 정의를 보입니다.
라인~\lnref{CM_BITS} 는 \co{counterandmax} 의 각 절반의 비트 수를 제공하는
\co{CM_BITS} 를 정의하며, 라인~\lnref{MAX_CMAX} 는 \co{counterandmax} 의 각
절반에 들어갈 수 있는 최대값인 \co{MAX_COUNTERMAX} 를 정의합니다.
\end{fcvref}
...
\QuickQuiz{
	Listing~\ref{lst:count:Atomic Limit Counter Variables and Access Functions}
	의 라인~\ref{ln:count:count_lim_atomic:var_access:var:CM_BITS} 는
	어떻게 C 표준을 위반하나요?
	...
	이것은 바이트당 여덟 비트가 사용된다고 가정합니다.
	이 가정은 쉽게 공유 메모리 멀티프로세서로 조립될 수 있는 현재의 일반
	상용 마이크로프로세서에 성립합니다만, C 코드를 수행해 본 적 있는 모든
	컴퓨터 시스템에 대해 성립하지는 않습니다.
	(C 표준에 맞추기 위해선 대신 어떤 일을 할 수 있을까요?  그 단점은
	무엇일까요?)
...
\Clnrefrange{b}{e} 는 \co{atomic_t counterandmax} 변수 아래의 \co{int} 를
받아서 \co{counter} (\co{c}) 와 \co{countermax} (\co{cm}) 부분으로 나눠주는
\co{split_counterandmax_int()} 함수를 보입니다.
라인~\lnref{msh} 는 이 \co{int} 의 앞쪽 절반을 떼어내서 인자 \co{c} 로 명시된
대로 결과를 위치시키며, 라인~\lnref{lsh} 는 이 \co{int} 의 뒤쪽 절반을 떼어내서
인자 \co{cm} 으로 명시된 대로 결과를 위치시킵니다.
\end{fcvref}

\begin{fcvref}[ln:count:count_lim_atomic:var_access:split]
\Clnrefrange{b}{e} 는 라인~\lnref{int} 에서 명시된 변수로부터 아래의 \co{int}
를 가져와서 라인~\lnref{old} 의 \co{old} 인자로 명시된 대로 저장하고
라인~\lnref{split_int} 에서 그것을 쪼개기 위해 \co{split_counterandmax_int()}
를 호출하는 \co{split_counterandmax()} 함수를 보입니다.
\end{fcvref}
...
\QuickQuiz{
	단 하나의 \co{counterandmax} 변수만 있는데, 왜
	Listing~\ref{lst:count:Atomic Limit Counter Variables and Access Functions}
	의
        line~\ref{ln:count:count_lim_atomic:var_access:split:func}
	에서는 포인터를 넘기는 거죠?
	...
	\emph{쓰레드당} 단 하나의 \co{counterandmax} 변수가 있습니다.
	나중에 우리는 다른 쓰레드의 \co{counterandmax} 변수를
	\co{split_counterandmax()} 로 넘기는 코드를 보게 될 겁니다.
...
\Clnrefrange{b}{e} 는 \co{split_counterandmax()} 의 반대라고 생각될 수 있는
\co{merge_counterandmax()} 함수를 보입니다.
라인~\lnref{merge} 는 \co{c} 와 \co{cm} 으로 각각 전달된 \co{counter} 와
\co{countermax}  값을 병합하고 그 결과를 리턴합니다.
\end{fcvref}
...
\QuickQuiz{
	Listing~\ref{lst:count:Atomic Limit Counter Variables and Access Functions}
	의 \co{merge_counterandmax()} 는 직접 \co{atomic_t} 에 저장을 하는 대신
	\co{int} 를 리턴하나요?
	...
	뒤에서 우린 \co{atomic_cmpxchg()} 기능에 넘기기 위한 \co{int} 를
	반환받을 필요가 있음을 알게 될 겁니다.
...
은 \co{add_count()} 와 \co{sub_count()} 함수들을 보입니다.

\begin{fcvref}[ln:count:count_lim_atomic:add_sub:add]
\Clnrefrange{b}{e} 는 \clnrefrange{fast:b}{return:fs} 에 fastpath 가 있고
나머지 부분은 이 함수의 slowpath 인 \co{add_count()} 를 보입니다.
Fastpath 의 \Clnrefrange{fast:b}{loop:e} 은 \clnrefrange{atmcmpex}{loop:e} 에서
실제 CAS 를 행하는 \co{atomic_cmpxchg()} 기능을 가지고 구현된 compare-and-swap
(CAS) 루프를 형성합니다.
라인~\lnref{split} 은 현재 쓰레드의 \co{counterandmax} 변수를 자신의
\co{counter} (\co{c}) 와 \co{countermax} (\co{cm}) 컴포넌트로 쪼개고, 아래의
\co{int} 는 \co{old} 에 위치시킵니다.
라인~\lnref{check} 는 \co{delta} 의 양이 지역적으로 처리될 수 있는지 검사하고
(정수형 오버플로우를 막기 위해 신경쓰면서), 그렇지 않다면 라인~\lnref{goto} 가
slowpath 로 전환합니다.
그렇지 않다면, 라인~\lnref{merge} 는 업데이트 된 \co{counter} 값을 원래의
\co{countermax} 값과 함께 \co{new} 로 결합시킵니다.
\Clnrefrange{atmcmpex}{loop:e} 의 \co{atomic_cmpxchg()} 기능은 어토믹하게 이
쓰레드의 \co{counterandmax} 변수를 \co{old} 와 비교하고, 이 비교가 성공했다면
자신의 값을 \co{new} 에 업데이트 합니다.
이 비교가 성공했다면, 라인~\lnref{return:fs} 는 성공을 의미하는 값을 리턴하고,
그렇지 않다면 수행은 라인~\lnref{fast:b} 의 루프로 이어집니다.
\end{fcvref}
...
\QuickQuizB{
	...
	Listing~\ref{lst:count:Atomic Limit Counter Add and Subtract} 의
        라인~\ref{ln:count:count_lim_atomic:add_sub:add:goto}
	의 추한 \co{goto} 는 왠말이죠?
	\co{break} 문 모르세요???
	...
	이 \co{goto} 를 \co{break} 로 교체하려면
        라인~\ref{ln:count:count_lim_atomic:add_sub:add:return:fs}
	이 리턴해야 하는지를 알기 위한 플래그를 유지할 것을 필요로 할텐데, 이건
	fastpath 에서 하고 싶은 일이 아닐 겁니다.
	여러분이 정말로 \co{goto} 를 그렇게나 싫어한다면, 여러분이 할 일은 이
	fastpath 를 성공 또는 실패를 리턴해서 ``실패'' 는 slowpath 의 필요를
	의미하는 별개의 함수로 만드는 것일 겁니다.
	이건 goto 를 싫어하는 독자 여러분의 연습문제로 남겨두겠습니다.
...
\QuickQuizE{
        \begin{fcvref}[ln:count:count_lim_atomic:add_sub:add]
	Listing~\ref{lst:count:Atomic Limit Counter Add and Subtract}
	의 \clnrefrange{atmcmpex}{loop:e} 의 \co{atomic_cmpxchg()} 기능은 왜
	실패할 수 있죠?
	어쨌건, 우린 기존 값을 라인~\lnref{split} 에서 가져온 후 바꾸지
	않았잖아요!
	\end{fcvref}
	...
	의 \co{flush_local_count()} 함수가 어떻게 이 쓰레드의
	\co{counterandmax} 변수를
	Listing~\ref{lst:count:Atomic Limit Counter Add and Subtract}
	의 \clnrefrange{fast:b}{loop:e} 의 fastpath 수행과 동시에 업데이트
	할수도 있는지 볼 겁니다.
	\end{fcvref}
...
\begin{fcvref}[ln:count:count_lim_atomic:add_sub:add]
Listing~\ref{lst:count:Atomic Limit Counter Add and Subtract}
의 \clnrefrange{slow:b}{return:ss} 는 라인~\lnref{acquire} 에서 획득되고
라인~\lnref{release:f} 와~\lnref{release:s} 에서 해제되는 \co{gblcnt_mutex} 로
보호되는 \co{add_count()} 의 slowpath 를 보입니다.
라인~\lnref{globalize} 는 이 쓰레드의 상태를 전역 카운터로 옮기는
\co{globalize_count()} 를 호출합니다.
\Clnrefrange{checkglb:b}{checkglb:e} 는 \co{delta} 값이 현재 전역 상태에 의해
처리될 수 있는지 검사하고, 그렇지 않다면 라인~\lnref{flush} 에서 모든 쓰레드의
지역 상태를 전역 카운터로 쓸어넘기는 \co{flush_local_count()} 를 호출하고,
이어서 \clnrefrange{checkglb:nb}{checkglb:ne} 에서 \co{delta} 가 처리될 수
있는지 다시 검사합니다.
만약 이 모든 것 후에도 \co{delta} 더하기가 될 수 없다면, 라인~\lnref{release:f}
는 \co{gblcnt_mutex} 를 (앞에서 이야기 된 것처럼) 해제하고
라인~\lnref{return:sf} 에서 실패를 의미하는 값을 리턴합니다.

그렇지 않다면, 라인~\lnref{addglb} 는 \co{delta} 를 전역 카운터에 더하고,
라인~\lnref{balance} 는 카운트를 적절하다면 각 지역 상태로 흩뿌리며,
라인~\lnref{release:s} 에서 (다시 말하지만, 앞에서도 이야기했듯)
\co{gblcnt_mutex} 를 해제하고, 마지막으로 라인~\lnref{return:ss} 에서 성공을
의미하는 값을 리턴합니다.
\end{fcvref}
...
Listing~\ref{lst:count:Atomic Limit Counter Add and Subtract}
의 \clnrefrange{b}{e} 는 \co{add_count()} 와 비슷한 구조를 가져서
\clnrefrange{fast:b}{fast:e} 에 fast path 를, \clnrefrange{slow:b}{slow:e} 에
slowpath 를 갖는 \co{sub_count()} 함수를 보입니다.
이 함수의 라인별 분석은 독자 여러분의 연습을 위한 것으로 남겨두겠습니다.
\end{fcvref}
...
Listing~\ref{lst:count:Atomic Limit Counter Read} 은 \co{read_count()} 를
보입니다.
라인~\lnref{acquire} 는 \co{gblcnt_mutex} 를 획득하고 라인~\lnref{release} 는
이를 해제합니다.
라인~\lnref{initsum} 은 지역 변수 \co{sum} 을 \co{globalcount} 의 값으로 초기화
하고, \clnrefrange{loop:b}{loop:e} 의 루프는 쓰레드별 카운터를 이 합에
더하는데, 라인~\lnref{split} 에서는 \co{split_counterandmax} 를 사용해 각
쓰레드별 카운터를 격리시킵니다.
마지막으로, 라인~\lnref{return} 은 이 합을 반환합니다.
\end{fcvref}
...
\co{globalize_count()} 를 위한 코드는
\cref{lst:count:Atomic Limit Counter Utility Functions 1} 의 \clnrefrange{b}{e}
에 보이고 있으며, 앞의 알고리즘과 비슷합니다만, 이제 \co{counter} 와
\co{countermax} 를 \co{counterandmax} 로부터 쪼갤 필요가 있는
라인~\lnref{split} 이 추가되었습니다.
\end{fcvref}
...
모든 쓰레드의 지역 카운터 상태를 전역 카운터로 옮기는 \co{flush_local_count()}
의 코드가 \clnrefrange{b}{e} 에 보여져 있습니다.
라인~\lnref{checkrsv} 는 \co{globalreserve} 의 값이 모든 쓰레드별 카운트를
허용하는지 검사하고, 그렇지 않다면 라인~\lnref{return:n} 에서 리턴합니다.
그렇지 않다면, 라인~\lnref{initzero} 는 지역 변수 \co{zero} 를 0 이 된
\co{counter} 와 \co{countermax} 의 조합으로 초기화 시킵니다.
\Clnrefrange{loop:b}{loop:e} 의 루프는 각 쓰레드를 돌아갑니다.
라인~\lnref{checkp} 는 현재 쓰레드가 캉누터 상태를 가지고 있는지 검사하고,
그렇다면 \clnrefrange{atmxchg}{glbrsv} 가 이 상태를 전역 카운터로 옮깁니다.
라인~\lnref{atmxchg} 는 어토믹하게 현재 쓰레드의 상태를 가져오면서 그 값을
0으로 만듭니다.
라인~\lnref{split} 은 이 상태를 \co{counter} (지역변수 \co{c}) 와
\co{countermax} (지역변수 \co{cm}) 부분으로 쪼갭니다.
라인~\lnref{glbcnt} 는 이 쓰레드의 \co{counter} 와 \co{globalcount} 를 더하며,
그동안 라인~\lnref{glbrsv} 는 이 쓰레드의 \co{countermax} 를 \co{globalreserve}
로부터 뺍니다.
\end{fcvref}
...
\QuickQuizSeries{%
\QuickQuizB{
	쓰레드가 간단하게 \co{counterandmax} 변수를
	Listing~\ref{lst:count:Atomic Limit Counter Utility Functions 1} 의
        라인~\ref{ln:count:count_lim_atomic:utility1:flush:b}
	에서의 \co{flush_local_count()} 가 비운 후에 곧바로 다시 채우는 건 왜
	안되나요?
	...
	이 다른 쓰레드는 \co{flush_local_count()} 호출자가 \co{gblcnt_mutex} 를
	해제하기 전에는 자신의 \co{counterandmax} 를 도로 채울 수 없습니다.
	그 때에는, \co{flush_local_count()} 의 호출자는 이 카운트의 사용을
	끝냈을 것이며, 따라서 이 다른 쓰레드가 값을 도로 채우는데 문제는 없을
	겁니다---\co{globalcount} 의 값이 재충전을 허용할 만큼 충분히 크다는
	가정 하에서요.
...
\QuickQuizE{
	Listing~\ref{lst:count:Atomic Limit Counter Utility Functions 1} 의
        line~\ref{ln:count:count_lim_atomic:utility1:flush:atmxchg} 에서
	\co{flush_local_count()} 가 \co{counterandmax} 변수를 액세스 하는 동안
	\co{add_count()} 와 \co{sub_count()} 의 동시에 수행되는 fastpath 들이
	해당 변수를 간섭하는 것은 무엇이 막고 있습니까?
	...
	다음 세가지 경우를 고려해 보세요:
	\begin{enumerate}
	\item	만약 \co{flush_local_count()} 의 \co{atomic_xchg()} 각 이
		fastpath 들의 \co{split_counterandmax()} 전에 수행되었다면, 이
		fastpath 는 0 값의 \co{counter} 와 \co{countermax} 를 보게 될
		것이며, 따라서 slowpath 로 이동할 겁니다 (물론 \co{delta} 가
		0이 아니라면요).
	\item	\co{flush_local_count()} 의 \co{atomic_xchg()} 가 각 fastpath
		의 \co{split_counterandmax()} 후에, 그러나 이 fastpath 의
		\co{atomic_cmpxchg()} 전에 수행되었다면, \co{atomic_cmpxchg()}
		는 실패하여, fastpath 가 재시작하게 하고, 따라서 앞의 case~1 을
		줄입니다.
	\item	\co{flush_local_count()} 의 \co{atomic_xchg()} 가 각tpath 의
		\co{atomic_cmpxchg()} 후에 수행된다면, 이 fastpath 는 (대부분의
		경우) \co{flush_local_count()} 가 이 쓰레드의
		\co{counterandmax} 변수를 0으로 만들기 전에 성공적으로 완료될
		겁니다.
	\end{enumerate}
...
의 \clnrefrange{balance:b}{balance:e} 는 호출하는 쓰레드의 지역
\co{counterandmax} 변수를 재충전하는 \co{balance_count()} 의 코드를
보입니다.
이 함수는 앞의 알고리즘과 상당히 유사한데 합쳐진 \co{counterandmax} 변수를
처리하는 것이 요구된다는 변경을 갖습니다.
라인~\lnref{register:b} 에서 시작하는 \co{count_register_thread()} 함수와
라인~\lnref{unregister:b} 에서 시작하는 \co{count_unregister_thread()} 함수와
함께 이 코드의 자세한 분석은 독자 여러분의 몫으로 남겨둡니다,
\end{fcvref}
...
\QuickQuiz{
	\co{atomic_set()} 기능은 명시된 \co{atomic_t} 에 간단한 값 할당을 할
	뿐인데,
	Listing~\ref{lst:count:Atomic Limit Counter Utility Functions 2}
	의 \co{balance()} 의
        라인~\ref{ln:count:count_lim_atomic:utility2:balance:atmcset}
	은 어떻게 이 변수를 업데이트 할 수 있죠?
	...
	\co{gblcnt_mutex} 를 쥐며, 따라서 한번에 하나만 수행될 수 있습니다.
...
이것은 이 카운터가 그 양쪽 한계까지 닿는 것을 실제로 허용하는 첫번째
구현입니다만, fastpath 에 어토믹 오퍼레이션을 추가하는 비용 아래 그렇게 하며,
이는 일부 시스템에서는 이 fastpath 를 상당히 느려지게 만듭니다.
일부 워크로드는 이 속도 저하를 감당할 수 있겠으나, 더 나은 읽기 쪽 성능을 위한
알고리즘을 찾아볼 가치가 있습니다.
그런 알고리즘들 중 하나는 다른 쓰레드로부터 카운트를 가져오기 위해 시그널
핸들러를 사용합니다.
시그널 핸들러는 시그널을 받은 쓰레드의 컨텍스트에서 수행되므로, 어토믹
오퍼레이션은 필요하지 않게 되는데, 다음 섹션에서 이를 보겠습니다.
...
\QuickQuiz{
	하지만 시그널 핸들러는 수행되는 동안 다른 CPU 로 옮겨질 수 있습니다.
	이 가능성은 한 쓰레드와 이 쓰레드를 언터럽트 한 시그널 핸들러
	사이에서의 안정적인 통신을 위해 어토믹 인스트럭션과 메모리 배리어를
	필요로 하게 하지 않나요?
	...
	아뇨.
	이 시그널 핸들러가 다른 CPU 로 옮겨진다면, 이 인터럽트 당한 쓰레드 역시
	함께 이동합니다.
...
이제 쓰레드별 상태는 이 연관된 쓰레드에 의해서만 조정되지만, 이 시그널
...
이 상태 머신은 IDLE 상태에서 시작되며, \co{add_count()} 나 \co{sub_count()} 가
이 쓰레드의 지역 카운트와 전역 카운트의 조합이 현재 요청을 처리할 수 없다는
것을 발견하면, 이 연관된 slowpath 는 각 쓰레드의 \co{theft} 상태를 REQ 로
설정합니다 (이 쓰레드가 카운트가 없어서 곧바로 READY 로 가는 경우가
아니라면요).
\co{gblcnt_mutex} 락을 잡는 slowpath 만이 초록색으로 표시되었듯 IDLE 상태로부터
...
연관된 쓰레드의 \co{theft} 와 \co{counting} 변수를 체크합니다.
이 \co{theft} 상태가 REQ 라면, 이 시근러 핸들러는 이 상태를 바꾸는 것이
허용되지 않으며, 따라서 그냥 리턴합니다.
그렇지 않고 \co{counting} 변수가 셋 되어 있어서 현재 쓰레드의 fastpath 가 진행
중임을 알리고 있다면, 이 시그널 핸들러는 \co{theft} 상태를 ACK 로, 그렇지
않다면 READY 로 바꿉니다.
...
만약 이 \co{theft} 상태가 ACK 이라면, 파란 색으로 표시되었듯 fastpath 만이 이
\co{theft} 상태를 바꾸는 게 허용됩니다.
이 fastpath 가 완료되었을 때, \co{theft} 상태를 READY 로 바꿉니다.

일단 이 slowpath 가 이 쓰레드의 \co{theft} 상태가 READY 임을 보면, 이 slowpath
는 이 쓰레드의 카운트를 가져갈 수 있습니다.
그러면 이 slowpath 는 이 쓰레드의 \co{theft} 상태를 IDLE 로 설정합니다.
...
\QuickQuizB{
	...
	Fastpath 만이 \co{theft} 상태를 바꿀 수 있음을, 그리고 이 쓰레드가 이
	상태에 너무 오래 머무른다면 이 slowpath 를 수행하고 있는 쓰레드는 POSIX
	시그널을 다시 보낼 것임을 알리기 위해서입니다.
...
%
\QuickQuizE{
	Figure~\ref{fig:count:Signal-Theft State Machine} 에서, 분리된 REQ 와
	ACK \co{theft} 상태를 갖는 것의 요지가 무엇인가요?
	왜 이것들을 하나의 REQACK 상태로 만들어서 이 상태 머신을 더 간단하게
	만들지 않죠?
	그러면 시그널 핸들러든 fastpath 든 먼저 그 상태에 도달한 사람이 상태를
	READY 로 만들면 될텐데요.
	...
	REQ 와 ACK 상태를 하나로 만드는 것이 나쁜 생각인 이유는 여러가지가
	있는데 다음의 것들이 포함됩니다:
	\begin{enumerate}
	\item	이 slowpath 는 시그널이 어디서 다시 전송되어야 하는지
		파악하는데 REQ 와 ACK 상태를 사용합니다.
		만약 이 상태가 합쳐져 있다면, slowpath 는 시그널들을 무작정
		계속해서 보내는 것밖에 선택의 여지가 없을텐데, 이는 불필요하게
		fastpath 를 느리게 만드는 도움 안되는 효과만 낼 겁니다.
	\item	다음의 레이스가 초래될 겁니다:
		\begin{enumerate}
		\item	이 slowpath 가 특정 쓰레드의 상태를 REQACK 으로
			설정합니다.
		\item	해당 쓰레드가 fastpath 를 끝내고 REQACK 상태를
			발견합니다.
		\item	이 쓰레드가 시그널을 받고, 이 역시 REQACK 상태를
			발견하고, 현재 진행중인 fastpath 가 없으므로, 이 상태를
			READY 로 만듭니다.
		\item	이 slowpath 는 이 READY 상태를 발견하고, 카운트를
			가져오고, 상태를 IDLE 로 만들고, 완료됩니다.
		\item	이 fastpath 는 상태를 READY 로 만들어서, 이 쓰레드의
			다음 fastpath 수행을 막습니다.
		\end{enumerate}
		여기서의 기본적인 문제는 합쳐진 REQACK 상태가 시그널 핸들러에
		의해서도 fastpath 에 의해서도 보여질 수 있다는 겁니다.
		네개의 상태에 의해 관리되는 깔끔한 분리가 제대로 순서지어진
		상태 전환을 보장합니다.
	\end{enumerate}
	하지만, 여러분이 세개 상태의 셋업을 올바르게 만드는 것도 가능할 수도
	있습니다.
	만약 성공한다면, 그것을 네개 상태 셋업과 주의 깊게 비교해 보시기
	바랍니다.
	그 세개의 상태 해법은 정말로 선호될 만 한가요, 그리고 그렇다면 왜,
	안그렇다면 왜 그런가요?
...
\Clnrefrange{value:b}{value:e} 는 앞의 섹션에서 설명된 쓰레드별 상태 머신을
위한 상태들과 값들을 정의합니다.
\Clnrefrange{var:b}{var:e} 는 앞의 구현들과 비슷하지만, 쓰레드의
\co{countermax} 와 \co{theft} 변수들에 원격 접속을 허용하기 위해
라인~\lnref{maxp} 와~\lnref{theftp} 를 각각 추가했습니다.
\end{fcvref}
...
보입니다.
\Clnrefrange{b}{e} 는 앞의 구현과 동일한 \co{globalize_count()} 를 보입니다.
\end{fcvref}
\begin{fcvref}[ln:count:count_lim_sig:migration:flush_sig]
\Clnrefrange{b}{e} 는 카운트 가져오기 프로세스에서 사용되는 시그널 핸들러인
\co{flush_local_count_sig()} 를 보입니다.
라인~\lnref{check:REQ} 와~\lnref{return:n} 은 \co{theft} 상태가 REQ 인지
체크하고, 그렇지 않다면 변경 없이 리턴합니다.
라인~\lnref{mb:1} 은 값 가져오기 위한 변수의 샘플링이 해당 변수로의 어떤
변경보다도 이전에 이루어졌음을 보장하기 위한 메모리 배리어를 수행합니다.
라인~\lnref{set:ACK} 은 \co{theft} 상태를 ACK 으로 설정하고,
라인~\lnref{check:fast} 가 이 쓰레드의 fastpath 가 수행 중이지 않음을 보게
된다면, 라인~\lnref{set:READY} 가 \co{theft} 상태를 READY 로 설정합니다.
\end{fcvref}
...
\QuickQuiz{
	...
	의 \co{flush_local_count_sig()} 함수에서, 쓰레드별 변수인 \co{theft} 의
	사용은 왜 \co{READ_ONCE()} 와 \co{WRITE_ONCE()} 로 싸여있나요?
	...
	\begin{fcvref}[ln:count:count_lim_sig:migration:flush_sig]
	첫번째 것 (라인~\lnref{check:REQ}) 은 불필요하지 않은가 논쟁이 될 수
	있습니다.
	마지막의 두개 (라인~\lnref{set:ACK} 과~\lnref{set:READY}) 는
	중요합니다.
	이것들이 없어지면, 컴파일러는 \clnrefrange{set:ACK}{set:READY} 를
	다음과 같이 바꿀 수 있습니다:
	\end{fcvref}
...
\end{VerbatimN}
	...
	이는 치명적인 일이 될텐데, slowpath 는 \co{THEFT_READY} 의 변경 중인
	값을 보게 될수도 있고, 따라서 연관된 쓰레드가 준비되기 전에 값
	가져오기를 시작할 수 있기 때문입니다.
...
\Clnrefrange{b}{e} 는 모든 쓰레드의 지역 카운트를 날리기 위해 slowpath 에서
호출되는 \co{flush_local_count()} 를 보입니다.
\Clnrefrange{loop:b}{loop:e} 의 루프는 지역 카운트를 가진 각 쓰레드의
\co{theft} 상태를 진행시키며, 또한 해당 쓰레드에 시그널을 보냅니다.
라인~\lnref{skip} 은 존재하지 않는 쓰레드들을 건너 뛰게 합니다.
그렇지 않다면, 라인~\lnref{checkmax} 는 현재 쓰레드가 어떤 지역 카운트를 가지고
있는지 보고, 그렇지 않다면 라인~\lnref{READY} 은 이 쓰레드의 \co{theft} 상태를
READY 로 만들고 라인~\lnref{next} 에서 다음 쓰레드로 넘어갑니다.
그렇지 않다면, 라인~\lnref{REQ} 는 이 쓰레드의 \co{theft} 상태를 REQ 로 만들고
라인~\lnref{signal} 은 이 쓰레드에 시그널을 보냅니다.
\end{fcvref}
...
\QuickQuizB{
	...
	이 다른 쓰레드의 \co{countermax} 변수에 직접 접근하는게 안전하죠?
	...
	그 다른 쓰레드는 \co{gblcnt_mutex} 락을 잡지 않고는 자신의
	\co{countermax} 변수의 값을 바꾸는 게 허락되지 않았기 때문입니다.
	하지만 그 호출자가 이 락을 쥐었고, 따라서 다른 쓰레드가 이 락을 잡는게
	불가능하며, 그러니 그 다른 쓰레드는 \co{countermax} 변수를 바꿀 수
	없습니다.
	그러므로 우리는 안전하게 이걸 접근할 수 있습니다---하지만 바꿀 순
	없습니다.
...
\QuickQuizM{
	...
	은 왜 현재 쓰레드가 자신에게 시그널을 보내는지 체크하지 않는 거죠?
	...
	추가적인 검사가 필요치 않습니다.
	\co{flush_local_count()} 는 이미 \co{globalize_count()} 를 호출했으며,
	따라서
	라인~\ref{ln:count:count_lim_sig:migration:flush:checkmax}
	의 체크는 성공하고 뒤따르는 \co{pthread_kill()} 을 건너뛰었을 겁니다.
...
\QuickQuizE{
	...
	에 보인 코드는 \GCC\ 와 POSIX 에서 동작합니다.
	이게 ISO C 표준도 따르게 하기 위해선 뭐가 필요할까요?
	...
	\co{theft} 변수는 시그널 핸들러와 그 시그널에 의해 인터럽트된 코드
	사이에서 안전하게 공유됨을 보장하기 위해 \co{sig_atomic_t} 타입이어야만
	합니다.
...
\begin{fcvref}[ln:count:count_lim_sig:migration:flush]
\Clnrefrange{loop2:b}{loop2:e} 의 루프는 각 쓰레드가 READY 상태에 도달하길
기다렸다가 해당 쓰레드의 카운트를 가져옵니다.
\Clnrefrange{skip:nonexist}{next2} 는 모든 존재하지 않는 쓰레드를 건너 뛰고,
\clnrefrange{loop3:b}{loop3:e} 의 루프는 현재 쓰레드의 \co{theft} 상태가 READY
가 될 때까지 기다립니다.
라인~\lnref{block} 은 우선순위 역전 문제를 방지하기 위해 1 밀리세컨드 동안
블록되고, 라인~\lnref{check:REQ} 가 이 쓰레드의 시그널이 아직 도착하지 않았음을
판단하면, 라인~\lnref{signal2} 는 이 시그널을 다시 보냅니다.
수행은 이 쓰레드의 \co{theft} 상태가 READY 가 되었을 때 라인~\lnref{thiev:b} 에
도달하며, 따라서 \clnrefrange{thiev:b}{thiev:e} 는 값 가져오기를 합니다.
라인~\lnref{IDLE} 은 이제 이 쓰레드의 \co{theft} 상태를 IDLE 로 되돌립니다.
\end{fcvref}
...
\QuickQuiz{
	Listing~\ref{lst:count:Signal-Theft Limit Counter Value-Migration Functions}
	에서, 라인~\ref{ln:count:count_lim_sig:migration:flush:signal2}
	은 왜 시그널을 다시 보낼까요?
	...
	많은 운영체제가 수십년에 걸쳐 간혹 시그널을 잃어버리는 성격을
	가져버렸기 때문입니다.
	이게 기능이냐 버그냐에 대해서는 논쟁이 가능하겠습니다만 관계 없습니다.
	사용자의 시점에서 분명한 증상은 커널 버그가 아니라 사용자
	어플리케이션이 멈춰있는 것일 겁니다.

	\emph{여러분의} 사용자 어플리케이션이 멈춰있다구요!
...
\Clnrefrange{b}{e} 는 앞의 예와 비슷한 \co{balance_count()} 를 보입니다.
\end{fcvref}
...
은 \co{add_count()} 함수를 보입니다.
Fastpath 는 \clnrefrange{fast:b}{return:fs} 에 위치하며, slowpath 는
\clnrefrange{acquire}{return:ss} 에 있습니다.
라인~\lnref{fast:b} 는 쓰레드별 \co{counting} 변수를 1로 설정해서 이 쓰레드를
인터럽트하는 모든 뒤따르는 시그널 핸들러들은 \co{theft} 상태를 READY 가 아닌
ACK 으로 설정해서 이 fastpath 가 올바르게 완료되도록 합니다.
라인~\lnref{barrier:1} 은 컴파일러가 fastpath 몸체 중 어떤 것도 \co{counting}
설정을 앞서도록 재배치 하는 것을 방지합니다.
라인~\lnref{check:b} 와~\lnref{check:e} 는 이 쓰레드별 데이터가
\co{add_count()} 를 처리할 수 있는지, 그리고 진행 중인 값 가져오기가 없는지
검사하고, 그렇다면 라인~\lnref{add:f} 에서 이 fastpath 더하기를 수행하고
라인~\lnref{fasttaken} 에서 이 fastpath 가 취해졌음을 알립니다.

어떤 경우든, 라인~\lnref{barrier:2} 는 이 컴파일러가 fastpath 몸체가
라인~\lnref{clearcnt} 를 뒤따르도록 재배치 하는 것, 즉 모든 뒤따르는 시그널
핸들러가 값 가져오기를 제대로 못하게 할 수 있는 행위를 못하게 막습니다.
라인~\lnref{barrier:3} 는 또한번 컴파일러의 재배치를 불가하게 하고, 이어서
라인~\lnref{check:ACK} 은 이 시그널 핸들러가 \co{theft} 의 READY 로의 상태
변경을 뒤로 미뤘는지 검사하고, 그렇다면 라인~\lnref{mb} 에서 라인~\lnref{READY}
가 상태를 READY 로 설정한 것을 보는 모든 CPU 는 라인~\lnref{add:f} 의 효과도 볼
수 있을 것을 확실히 합니다.
라인~\lnref{add:f} 에서의 fastpath 더하기가 수행되었다면,
라인~\lnref{return:fs} 는 성공을 리턴합니다.
\end{fcvref}
...
\begin{fcvref}[ln:count:count_lim_sig:add]
그렇지 않다면, 우린 라인~\lnref{acquire} 에서 시작하는 slowpath 로 넘어갑니다.
이 slowpath 의 구조는 앞의 예들과 비슷하므로, 그 분석은 독자 여러분의
연습문제로 남겨둡니다.
\end{fcvref}
비슷하게,
Listing~\ref{lst:count:Signal-Theft Limit Counter Subtract Function}
의 \co{sub_count()} 의 구조는 \co{add_count()} 의 것과 동일하므로,
\co{sub_count()} 의 분석 역시 독자 여러분들의 연습문제로 남겨두며,
Listing~\ref{lst:count:Signal-Theft Limit Counter Read Function}
의 \co{read_count()} 역시 마찬가지입니다.
...
의 \clnrefrange{b}{e} 는 \co{flush_local_count_sig()} 를 \co{SIGUSR1} 의
핸들러로 설정하고, \co{flush_local_count()} 에서의 \co{pthread_kill()} 호출이
\co{flush_local_count_sig()} 를 실행시키게 하는 \co{count_init()} 를 보입니다.
쓰레드 등록과 등록 해제를 위한 코드는 앞의 예와 비슷하므로, 그에 대한 분석은
...
제 코어 여섯개짜리 x86 랩톱에서 이 시그널 기반 구현은 어토믹 구현에 비해 8배나
빨리 돌아갑니다.
이게 항상 선호될 만 할까요?

이 시그널 기반 구현은 펜티엄 4 시스템에서는 어토믹 인스트럭션이 느리므로 상당히
선호될 수 있겠습니다만, 오래된 80386 기반의 Sequent Symmetry 시스템은 어토믹
구현의 더 짧은 코드 길이를 훨씬 잘 수행해낼 수 있을 겁니다.
그러나, 이 증가된 업데이트 쪽 성능은 높아진 읽기 쪽 오버헤드의 비용과 함께
옵니다: 이 POSIX 시그널은 공짜가 아닙니다.
만약 궁극의 성능이 핵심이라면, 여러분은 여러분의 어플리케이션이 실제 배포되는
시스템에서도 그것들을 모두 측정해 봐야 할 겁니다.
...
\QuickQuiz{
	POSIX 시그널은 느리기만 한 게 아니고, 시그널을 각 쓰레드에 보내는 것은
	확장이 안됩니다.
	만약 여러분이 (예를 들어) 10,000 쓰레드를 가지고 있고 이 읽기 쪽이
	빠르게 만들어야 한다면 어떻게 하겠습니까?
	...
	한가지 방법은
	Section~\ref{sec:count:Eventually Consistent Implementation}
	에 보인 기법으로, 전체 카운터 값에 대한 근사치를 단일 변수에 요약하는
	겁니다.
	또다른 방법은 여러 쓰레드가 읽기를 수행하게끔 해서, 각 쓰레드가
	업데이트를 하는 쓰레드 중 일부 집합들과만 상호작용하게 하는 겁니다.
...
이건 하지만 왜 고품질 API 가 그렇게 중요한가에 대한 한가지 이유에 불과합니다:
그것은 계속해서 바뀌는 하드웨어 성능 특성에 의해 필요시 되는대로 구현이 바뀔 수
있게 해줍니다.
...
\QuickQuiz{
	만약 여러분이 원하는 건 이 정확한 리미트 카운터가 아래쪽 한계에는
	정확하지만 위쪽 한계에는 정확하지 않아도 되는 것이라면 어떻게
	하시겠습니까?
	...
	한가지 간단한 해결책은 위쪽 한계를 원하는 만큼 실제보다 크게 잡는
	겁니다.
	그런 부풀리기의 제약은 이 위쪽 한계가 이 카운터가 표현할 수 있는 가장
	큰 값이 되는 경우입니다.
...
이 섹션에서 소개된 정확한 한계 카운터 구현들이 매우 유용할 수 있긴 하지만, 이
카운터의 값이 항상 0 근처라면 별로 도움이 되지 못할 텐데, I/O 기기로의 액세스
수를 세는 게 그럴 수도 있을 겁니다.
그런 0 근처에서의 카운팅의 높은 오버헤드는 우리가 거기 얼마나 많은 참조가
있는지에 대해선 보통 관심이 없다는 점에서 특히 고통스럽습니다.
\QuickQuizRef{\QcountQIOcnt} 에 나온 제거 가능한 I/O 기기 액세스 카운트
문제에서 이야기 되었듯이, 액세스 횟수는 누군가가 정말로 이 기기를 제거하려고 할
때 같은 드문 경우를 제외하고는 상관없습니다.

이 문제에 대한 한가지 간단한 해결책은 이 카운터가 효율적으로 동작할 수 있을
만큼 그 값이 0으로부터 멀다는 것을 보장하기 위해 큰 ``편향치'' (예를 들어, 10억
정도) 를 더하는 것입니다.
누군가가 이 기기를 제거하고자 한다면, 이 편향치는 이 카운터 값으로부터
빼집니다.
마지막 몇 액세스를 카운팅 하는건 상당히 비효율적이게 되겠지만, 그 전의 많은
액세스는 온전한 속도로 카운트 되었을 거라는 게 핵심입니다.
...
\QuickQuiz{
	편향된 카운터를 사용할 때 상황을 더 좋게 하기 위해 여러분이 해보았을
	법한 더 나은 방법은 무엇이 있을까요?
	...
	이 위쪽 한계를 이 편향치, 예상되는 최대 액세스 횟수, 그리고
	``엎질러짐'' 을 충분히 받아들일 수 있을 만큼 크게 설정해서 액세스
	횟수가 그 최대치일 때에도 일이 효율적으로 진행되게 하는 것이 좋을
	겁니다.
...
편향된 카운터가 상당히 도움되고 유용할 수 있지만, 이는
page~\pageref{chp:Counting} 에서 나온, 제거 가능한 I/O 기기 액세스 카운트
문제의 부분적 해결책일 뿐입니다.
기기를 제거하고자 할 때, 우린 현재 I/O 액세스의 정확한 횟수만 알아야 하는 것이
아니라, 시작부터의 미래 액세스도 예측해야 합니다.
이를 달성하는 한가지 방법은 이 카운터를 업데이트 할 때 reader-writer 락을 읽기
모드로 획득하고, 이 카운터를 체크할 때에는 쓰기 모드로 획득하는 것입니다.
I/O 를 하기 위한 코드는 다음과 같을 수 있을 겁니다:
...
라인~\lnref{acq} 는 이 락을 읽기 모드로 획득하고, 라인~\lnref{rel1}
또는~\lnref{rel2} 에서 이를 해제합니다.
라인~\lnref{check} 는 이 기기가 제거되었는지 검사하고, 그렇다면
라인~\lnref{rel1} 에서 이 락을 해제하고 라인~\lnref{cancel} 에서 이 I/O 를
취소하거나 이 기기가 제거될 것임을 생각할 때 적절할 무슨 행동이든 취합니다.
그렇지 않다면, 라인~\lnref{inc} 는 이 액세스 카운트를 증가시키고,
라인~\lnref{rel2} 는 이 락을 해제하며, 라인~\lnref{do} 가 I/O 를 행하고,
라인~\lnref{dec} 는 이 액세스 카운트를 감소시킵니다.
\end{fcvref}
...
\QuickQuiz{
	웃기네요!
	이 카운터를 \emph{업데이트} 하기 위해 reader-writer 락을 \emph{읽기}
	모드로 획득한다구요?
	뭐하는 짓이예요???
	...
	아마도, 이상하겠죠, 하지만 정말입니다!
	이건 ``Reader-writer lock'' 이란 이름은 생각 없이 붙여진 것이라
	여러분이 생각하게 하기 충분할 거예요, 그렇지 않나요?
...
이 기기를 제거하기 위한 코드는 다음과 같을 수 있겠습니다:
...
라인~\lnref{acq} 는 이 락을 쓰기 모드로 획득하고 라인~\lnref{rel} 은 이를
해제합니다.
라인~\lnref{note} 는 이 기기가 제거되는 중임을 알리고,
\clnrefrange{loop:b}{loop:e} 의 루프는 모든 I/O 오퍼레이션이 완료되기를
기다립니다.
마지막으로, 라인~\lnref{remove} 는 기기 제거를 준비하기 위해 필요한 모든 추가적
처리를 행합니다.
\end{fcvref}
...
\QuickQuiz{
	...
	\begin{enumerate}
	\item	기기가 얼마든지 있을 수 있으며, 따라서 전역 변수들은 적절치
		않을 것이고, \co{do_io()} 같은 함수로의 인자의 부재도
		그렇습니다.
	\item	루프에서의 폴링은 실제 시스템에서는 CPU 시간과 전력을
		낭비하므로 문제가 될 수 있습니다.
	\item	I/O 는 실패할 수도 있으며, 따라서 \co{do_io()} 는 리턴 값을
		가져야 할 겁니다.
	\item	만약 이 기기가 고장나면, 마지막 I/O 는 결코 완료되지 않을
		겁니다.
		그런 경우라면, 에러로부터의 회복을 위해 시간 제한 같은 게
		필요할 수도 있습니다.
	\item	\co{add_count()} 도 \co{sub_count()} 도 실패할 수 있습니다만,
		이것들의 리턴값이 체크되지 않고 있습니다.
	\item	Reader-writer 락은 잘 확장되지 못합니다.
		Reader-writer 락의 이 높은 읽기 모드 획득 쪽 비용을 제거하기
		위한 한가지 방법이
		\cref{chp:Locking,chp:Deferred Processing} 에 소개됩니다.
	\end{enumerate}
...
이 쓰레드별 변수 구현 (\path{count_end.c}) 은 배열 기반 구현
(\path{count_stat.c}) 보다 업데이트에 있어 무척 빠르지만, 많은 수의 코어
위에서는 읽기가 더 느리며, 많은 병렬 읽기 쓰레드가 존재할 때에는 상당한 락
경쟁으로 힘들어합니다.
이 경쟁은 Chapter~\ref{chp:Deferred Processing} 에서 소개하는 뒤로 미뤄
처리하기 기법으로
\cref{tab:count:Statistical/Limit Counter Performance on x86}
의 \path{count_end_rcu.c} 열에서 보였듯 해결될 수 있습니다.
미뤄 처리하기는 또한 결과적 일관성 덕분에 \path{count_stat_eventual.c} 열에서도
빛을 발합니다.
...
\QuickQuizB{
	\Cref{tab:count:Statistical/Limit Counter Performance on x86}
	의 \path{count_stat.c} 열에서, 우린 읽기쪽의 쓰레드 수에 따른 선형적
	확장성을 볼 수 있습니다.
	더 많은 쓰레드가 존재할수록 더 많은 쓰레드별 카운터가 합산되어야 하는데
	이게 어떻게 가능하죠?
	...
	읽기쪽 코드는 쓰레드 수에 관계 없이 고정된 크기의 배열 전체를 스캔해야
	하며, 따라서 성능에 차이가 없습니다.
	대조적으로, 마지막 두개의 알고리즘에서, 읽기 쓰레드는 더 많은 쓰레드가
	있을 때 더 많은 일을 해야 합니다.
	또한, 마지막 두개의 알고리즘은 쓰레드 ID 라는 정수로부터 연관된
	\co{__thread} 변수로의 매핑을 가지므로 추가적인 간접 단계를 갖습니다.
...
\QuickQuizE{
	\Cref{tab:count:Statistical/Limit Counter Performance on x86}
	의 네번째 열에서조차도, 이 통계적 카운터 구현의 읽기쪽 성능은 상당히
	무섭네요.
	그런데도 왜 이걸 신경쓰죠?
	...
	Figure~\ref{fig:count:Atomic Increment Scalability on x86}
	에서 볼 수 있듯이, 병렬 업데이트의 많은 사용이 필요한 일에 단일 변수
	어토믹 값 증가는 사용될 필요 없습니다.
	반면, 
	\cref{tab:count:Statistical/Limit Counter Performance on x86}
	의 위쪽 절반에 보여진 알고리즘들은 업데이트가 많은 상황의 일을 훌륭하게
	해냅니다.
	물론, 읽기가 대부분인 상황이라면, 여러분은 다른 무언가, 예를 들면
	Section~\ref{sec:count:Eventually Consistent Implementation}
	에서 사용된 방법과 비슷한, 단일 로드를 이용해 읽어질 수 있는 단일
	어토믹하게 값 증가되는 변수를 사용한 결과적 일관성 설계를 사용해야
	합니다.
...
\Cref{tab:count:Statistical/Limit Counter Performance on x86}
의 아래쪽 절반은 병렬 한계 카운팅 알고리즘의 성능을 보입니다.
한계의 정확한 제한은 업데이트 쪽 성능 페널티에 큰 영향을 끼칩니다, 이 x86
시스템에서 그 페널티는 어토믹 오퍼레이션을 시그널로 대체함으로써 크게
줄어들지만요.
이 모든 구현이 동시의 읽기 쓰레드들이 있을 때에는 읽기 쪽 락 경쟁으로
고통받습니다.
...
\QuickQuizB{
	\Cref{tab:count:Statistical/Limit Counter Performance on x86}
	의 아래쪽 절반에 보여진 성능 데이터를 놓고 보면, 우린 항상 어토믹
	오퍼레이션보다 시그널을 선호해야 하겠군요, 그렇죠?
	...
	그건 워크로드에 달려 있습니다.
	64 코어 시스템에서라면, 단 하나의 시그널을 (거의 5
	\emph{마이크로세컨드} 성능 저하를 일으키는) 위해서도 100개가 넘는
	어토믹이 아닌 오퍼레이션이 필요함을 (대략 40 나노세컨드 성능 이득을
	위해) 알아 두시기 바랍니다.
	훨씬 더 읽기 집약도가 높은 워크로드가 많이 있긴 하지만, 여러분은
	여러분의 워크로드를 고려해야 할 겁니다.

	또한, 역사적으로 메모리 배리어는 평범한 인스트럭션들보다 비쌌지만,
	여러분이 사용할 실제 하드웨어 위에서 이걸 체크해 보셔야 합니다.
	컴퓨터 하드웨어의 특성은 시간에 따라 변하며, 알고리즘은 그에 따라
	변해야만 합니다.
...
\QuickQuizE{
	\Cref{tab:count:Statistical/Limit Counter Performance on x86}
	의 아래쪽 절반에 보여진 읽기 쓰레드의 락 경쟁을 해결하기 위해 진보된
	기법들이 사용될 수 있을까요?
	...
	한가지 방법은 scalable non-zero indicators
	(SNZI)~\cite{FaithEllen:2007:SNZI} 에서처럼 업데이트 쪽 성능을 포기하는
	것입니다.
	이에 대해선 해볼만한 여러 방법들이 있으며, 이것들은 독자 여러분의
	연습문제로 남겨 두겠습니다.
	빈번한 전역 락 획득을 연관된 아래 계층의 지역 락 획득으로 대체하는 모든
	방법들 역시 상당히 잘 동작할 겁니다.
...
요약하자면, 이 챕터는 여러 특수한 경우에 성능도 확장성도 굉장히 좋은 카운팅
알고리즘 여럿을 보였습니다.
하지만 우리의 병렬 카운팅은 특수 경우에만 국한되어야만 할까요?
모든 경우에 효율적으로 동작하는 범용 알고리즘을 가지면 더 좋지 않을까요?
다음 섹션인 이 질문들을 돌아봅니다.
...
이 알고리즘들이 각각의 특수한 경우에만 잘 동작한다는 사실은 일반적인 병렬
프로그래밍의 주요 문제로 여겨질수도 있습니다.
어쨌건, C 언어의 \co{++} 오퍼레이터는 싱글 쓰레드 코드에서는 잘 동작하는데,
특수한 경우가 아니라 일반적인 경우에 그렇습니다, 그렇죠?

이 논리는 약간의 사실을 담고 있지만, 핵심은 방향을 잘못 잡고 있습니다.
문제는 병렬성이 아니라 확장성입니다.
이를 이해하기 위해, 먼저 C 언어의 \co{++} 오퍼레이터에 대해 생각해 봅시다.
사실은 그것이 일반적으로 잘 동작하는게 \emph{아니고}, 제한된 범위의 숫자에
대해서만 그렇다는 것입니다.
여러분이 1,000 자리 십진수를 처리해야 한다면, C 언어 \co{++} 오퍼레이터는
여러분을 위해 잘 동작하지 않을 것입니다.
...
\QuickQuiz{
	...
	C++ 언어에서는, 1,000 자리 숫자를 위해서도 여러분이 그 숫자를 구현하는
	클래스로의 접근 권한이 있다는 가정 하에 \co{++} 를 사용할 수도 있을
	겁니다.
	하지만 2021년 기준으로, C 언어는 오퍼레이터 오버로딩을 허용하지
	않습니다.
...
이 문제는 숫자에만 국한된 게 아닙니다.
여러분이 데이터를 저장하고 검색해야 한다고 생각해 봅시다.
ASCII 파일을 사용해야 할까요?
XML?
관계형 데이터베이스?
링크드 리스트?
...
B-tree?
Radix tree?
또는 데이터가 저장되고 검색되는 것을 허용하는 다른 수많은 자료 구조와 환경 중
하나?
이는 여러분이 무엇을 해야 하는지, 얼마나 빨리 해야 하는지, 그리고 여러분의
데이터 셋이 얼마나 큰지에 달려있습니다---순차적 시스템에서조차도요.

비슷하게, 여러분이 카운팅을 해야 한다면, 여러분의 해결책은 얼마나 큰 수를
다뤄야 하는지, 얼마나 많은 CPU 가 해당 숫자를 동시에 처리해야 하는지, 그 숫자가
어떻게 사용되는지, 그리고 어떤 수준의 성능과 확장성을 여러분이 필요로 하는지에
의존될 것입니다.
...
이 문제는 또한 소프트웨어만의 것도 아닙니다.
사람들이 작은 개울을 건널 수 있게 하기 위한 다리를 설계하는 것은 하나의 나무
널빤지를 만드는 것만큼이나 간단할 수도 있습니다.
하지만 콜럼비아 강의 수킬로미더에 달하는 거리를 위해서라면 널빤지 하나만
사용하지는 않을 것이고, 콘크리트 트럭을 옮겨야 하는 다리를 위해서는 앞의 설계는
조언이 될 수도 없을 겁니다.
짧게 말해서, 다리 설계가 증가하는 길이와 부하에 따라 달라져야 하는 만큼,
소프트웨어 설계 역시 증가하는 CPU 갯수에 따라 달라져야 합니다.
그렇다고는 하나, 이 과정을 자동화 해서, 소프트웨어가 하드웨어 구성과 워크로드의
변화에 적응할 수 있게 하는게 좋을 겁니다.
실제로 이런 종류의 자동화에 대한 연구가 있었으며~\cite{Appavoo03a,Soules03a},
리눅스 커널은 제한된 정도의 바이너리 재작성을 포함한 부팅 시점의 재설정을
가지고 있습니다.
이런 종류의 적응은 주류 시스템의 CPU 수가 계속 증가하는 만큼 점점 더 중요해 질
겁니다.
...
요약하자면,
Chapter~\ref{chp:Hardware and its Habits}
에서 이야기 된 것처럼, 물리 법칙은 그것이 다리와 같은 기계 장치들에 제약을
가하는 것과 같이 병렬 소프트웨어에도 제약을 가합니다.
이런 제약들은 특수화를 강요합니다, 소프트웨어의 경우에는 실제 하드웨어와
워크로드에 맞춰 특수화 선택을 자동화 할 수 있을 수도 있지만 말입니다.

물론, 범용화 된 카운팅조차 상당히 특수화 되어 있습니다.
우린 컴퓨터를 가지고 다른 많은 일도 해야만 합니다.
다음 섹션은 우리가 카운터에서 얻은 교훈들을 이 책의 뒷부분에서 다룰 주제들과
연결지어 봅니다.
...
이 챕터를 시작하는 문단은 우리의 카운팅에 대한 연구가 병렬 프로그래밍으로의
훌륭한 소개를 제공할 것이라고 약속했습니다.
이 섹션은 이 챕터에서의 교훈과 뒤의 여러 챕터에서 소개될 것들 사이의 명확한
연결을 만듭니다.

이 챕터의 예제들은 중요한 확장성과 성능을 위한 도구가 \emph{분할하기} 임을
보였습니다.
카운터는 \cref{sec:count:Statistical Counters} 에서 논한 통계적 카운터에서처럼 완전히 분할 될 수도 있고, 
\cref{sec:count:Approximate Limit Counters,%
sec:count:Exact Limit Counters} 에서 이야기 된 한계 카운터에서처럼 부분적으로
분할될 수도 있습니다.
분할하기는
\cref{cha:Partitioning and Synchronization Design} 에서 훨씬 더 깊게 다뤄질
것이고, 부분적 분할하기는
\cref{sec:SMPdesign:Parallel Fastpath} 에서 \emph{parallel fastpath} 라는
이름으로 특별히 다뤄질 것입니다.
...
\QuickQuiz{
	하지만 우리가 모든 것을 분할해야 한다면, 왜 공유 메모리 멀티쓰레딩을
	고려하죠?
	문제를 완전하게 분할하고 각각 각자의 주소공간을 가지는 여러 프로세스로
	돌리는 게 어떤가요?
	...
	실제로, 별도의 주소 공간을 갖는 여러 프로세스는 병렬성을 노출시키는
	훌륭한 방법이 될 수 있습니다, fork-join 방법론과 Erlang 언어의 제안자도
	곧바로 여러분에게 그렇게 말할 겁니다.
	하지만, 공유메모리 병렬성에는 몇가지 장점도 있습니다:
	\begin{enumerate}
	\item	어플리케이션의 가장 성능에 중요한 부분만이 분할되어야 하며,
		그런 부분은 보통 어플리케이션의 작은 부분입니다.
	\item	캐쉬 미스는 개별 레지스터간 인스트럭션에 비교하면 상당히
		느리지만, TCP/IP 네트워킹 같은 것보다는 무척 빠른 프로세스간
		통신 기능들보다도 일반적으로 상당히 빠릅니다.
	\item	공유메모리 멀티프로세서는 이미 사용 가능하고 무척 저렴합니다,
		따라서 1990년대와는 다르게, 공유메모리 병렬성의 사용에 대한
		비용 페널티가 무척 작습니다.
	\end{enumerate}
...
이 부분적으로 분할된 카운팅 알고리즘은 전역 데이터를 보호하기 위해 락킹을
사용했고, 락킹은
Chapter~\ref{chp:Locking} 의 주제입니다.
대조적으로, 분할된 데이터는 연관된 쓰레드의 완전한 제어 하에 있어서 어떤
동기화도 필요치 않게 되는 경향이 있었습니다.
이 \emph{데이터 소유권} 은
Section~\ref{sec:SMPdesign:Data Ownership}
에서 소개되고
...
정수 더하기와 빼기는 일반적인 동기화 오퍼레이션에 비하면 무척이나 비용이
저렴해서, 합리적인 확장성을 이루기 위해선 동기화 오퍼레이션을 검소하게 사용할
것이 요구됩니다.
이를 해내는 한가지 방법은 이 더하기와 빼기 오퍼레이션을 한번에 몰아서 해서, 이
비용이 저렴한 오퍼레이션이 여러개가 하나의 동기화 오퍼레이션으로 처리되게 하는
것입니다.
한번에 몰아서 처리하기 최적화의 또다른 종류의 하나는
\cref{tab:count:Statistical/Limit Counter Performance on x86}
에 리스트 된 카운팅 알고리즘들 각각에 의해 사용되었습니다.
...
마지막으로,
Section~\ref{sec:count:Eventually Consistent Implementation}
에서 이야기 된 결과적으로 일관적인 통계적 카운터는 일을 뒤로 미루는 것 (이
경우, 전역 카운터를 업데이트 하는 것) 이 상당한 성능과 확장성 이득을 제공함을
보였습니다.
이 방법은 일반적인 경우를 위한 코드가 그렇지 않다면 사용할 수 있을 법한
것들보다 훨씬 비용이 저렴한 동기화 오퍼레이션을 사용하는 것이 가능하게
했습니다.
Chapter~\ref{chp:Deferred Processing} 는 미뤄서 처리하기가 성능, 확장성, 심지어
real-time 반응까지 향상시킬 수 있는 몇가지 방법을 더 알아봅니다.
...
\begin{enumerate}
\item	분할하기는 성능과 확장성을 향상시킵니다.
\item	부분적 분할하기, 즉 일반적 코드 패쓰에만 분할하기를 적용하는 것은 거의
	항상 잘 동작한다.
\item	부분적 분할하기는 코드에 적용될 수 있지만 (
	Section~\ref{sec:count:Statistical Counters} 의 통계적 카운터의 분할된
	업데이트와 분할되지 않은 읽기처럼), 시간에 대해서도 적용될 수 있습니다
	(Section~\ref{sec:count:Approximate Limit Counters} 과
	Section~\ref{sec:count:Exact Limit Counters} 의 한계로부터 멀때는 훨씬
	빠르게 동작하고, 한계에 가까울 때에는 느리게 동작하는 한계
	카운터들처럼).
\item	시간을 통한 분할하기는 비싼 전역 오퍼레이션의 횟수를 줄이고, 그럼으로써
	동기화 오버헤드를 줄이고, 결국 성능과 확장성을 향상시키게끔 종종
	업데이트를 지역적으로 몰아서 합니다.
	\cref{tab:count:Statistical/Limit Counter Performance on x86}
	는 몰아서 하기를 상당히 많이 사용합니다.
...
\item	읽기만 하는 코드 패쓰는 읽기만 하도록 남아있어야 합니다:  공유
	메모리로의 가짜 동기화 쓰기는 
	\cref{tab:count:Statistical/Limit Counter Performance on x86}.
	의 \path{count_end.c} 열에서 보여진 것처럼 성능과 확장성을 죽일 수
	있습니다.
\item	지연의 현명한 사용은
	Section~\ref{sec:count:Eventually Consistent Implementation}
	에서 보인 것처럼 성능과 확장성을 향상시킵니다.
\item	병렬 성능과 확장성은 보통 균형잡기입니다: 어떤 지점을 넘어서면, 일부
	코드 패쓰를 최적화 하는 것은 다른 부분의 성능을 하락시킵니다.
	\Cref{tab:count:Statistical/Limit Counter Performance on x86}
	의 \path{count_stat.c} 와 \path{count_end_rcu.c} 열이 이 점을 잘
	보입니다.
\item	다른 수준의 성능과 확장성은 다른 많은 요소들처럼 알고리즘과 자료구조
	설계에 영향을 끼칩니다.
	\Cref{fig:count:Atomic Increment Scalability on x86}
	가 이 점을 잘 보이고 있습니다:  어토믹 값 증가는 두개 CPU 를 갖는
	시스템에서는 완전히 허용 가능하지만 여덟개 CPU 를 갖는 시스템에서는
	말도 안됩니다.
...
계속해서 요약을 해보자면, 우린 ``큰 세개의'' 성능과 확장성 증가 방법이 있는데,
각각
(1) CPU 와 쓰레드들에 걸쳐 \emph{분할하기},
(2) 더 많은 일이 각각의 비용 높은 동기화 오퍼레이션을 통해 처리되도록
\emph{몰아서 처리하기}, 그리고
(3) 가능한 곳에서는 동기화 오퍼레이션을 \emph{약화시키기}.
대략적인 법칙으로, 여러분은 이 방법들을
page~\pageref{fig:intro:Ordering of Parallel-Programming Tasks}
의
Figure~\ref{fig:intro:Ordering of Parallel-Programming Tasks}
의 토론에서 앞서 이야기 되었듯 이 순서대로 적용해야 합니다.
Figure~\ref{fig:count:Optimization and the Four Parallel-Programming Tasks}
에 보여진 대로, 분할하기 최적화는 ``Resource Partitioning and Replication'' 에,
몰아서 처리하기 최적화는 ``Work Partitioning'' 으로, 그리고 약화시키기 최적화는
``Parallel Access Control'' 에 적용 되어야 합니다.
물론, 디지털 신호 처리기 (DSP), field-programmable gate arrays (FPGAs), 또는
범용 그래픽 처리기 (GPGPU) 를 가지고 있다면, 설계 과정 중에 ``Interacting With
Hardware'' 부분에 대해서도 많은 관심을 기울여야 합니다.
예를 들어, GPGPU 의 하드웨어 쓰레드와 메모리 연결성 구조는 매우 주의 깊은
분할하기와 몰아서 처리하기 설계 결정에 큰 영향을 끼칠 겁니다.
...
짧게 말해서, 이 챕터의 시작점에서 이야기 되었듯, 카운팅의 단순성은 복잡한
동기화 기능들이나 복잡한 데이터 구조로부터 방해받지 않고서 많은 기본적 동시성
문제를 탐험할 수 있게 해 주었습니다.
그런 동기화 도구들과 데이터 구조들은 뒤의 챕터에서 다루어집니다.
