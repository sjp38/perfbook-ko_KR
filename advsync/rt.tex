...
커널에 3.10 버전에 받아들여진
\co{NO_HZ_FULL}~\cite{JonCorbet2013NO-HZ-FULL,FredericWeisbecker2013nohz}
...
인터럽트는 OS jitter 의 많은 부분의 훌륭한 원천입니다.
불행히도, 인터럽트는 시스템이 바깥 세상과 소통하기 위해 절대적으로 필요합니다.
이 OS jitter 와 바깥 세상과의 연락을 유지하는 것 사이의 충돌을 해결하는 한가지
방법은 작은 수의 최소 운영 CPU 를 예약하고 모든 인터럽트를 이 CPU 들로 향하도록
강제하는 겁니다.
리눅스 소스 트리의 \path{Documentation/IRQ-affinity.txt} 파일은 어떻게 기기
인터럽트를 특정 CPU 로 향하게 할 수 있는지 설명하는데, 2021년 초 기준으로는
다음과 같습니다:
...
이 커맨드는 인터럽트 \#44 를 CPU~0--3 에 가둡니다.
스케쥴링 클락 인터럽트는 특수한 처리를 필요로 하며, 이 섹션의 뒷부분에서 다룸을
알아두십시오.

OS jitter 의 두번째 원천은 커널 쓰레드와 daemon 입니다.
RCU 의 grace-period kthread (\co{rcu_bh}, \co{rcu_preempt}, 그리고
\co{rcu_sched}) 와 같은 개별 커널 쓰레드는 \co{taskset} 커맨드,
\co{sched_setaffinity()} 시스템콜, 또는 \co{cgroups} 를 통해 어떤 CPU 에게든
할당될 수 있습니다.
...
않게 해야 합니다.
특정 per-kthread 를 위한 조언을 리눅스 커널 소스 \path{Documentation}
디렉토리의 \path{admin-guide/kernel-per-CPU-kthreads.rst} 에서 찾을 수 있을
겁니다.
...
로부터 격리시키는 것입니다.
또한, \path{Documentation/scheduler} 디렉토리의 것들을 주의깊게 읽어야 합니다.
\path{sched-rt-group.rst} 파일의 내용은 특히 중요한데, 여러분이
\co{CONFIG_RT_GROUP_SCHED} Kconfig 패러미터를 통해 켜지는 \co{cgroups} 리얼타임
기능을 사용한다면 더욱 그렇습니다.
...
OS jitter 의 다섯번째 원천은 리얼타임용으로 개발되지 않은 디바이스 드라이버에
의해 제공됩니다.
...
제공됩니다.
이것들은 여러분의 커널을 \co{NO_HZ_FULL} Kconfig 파러미터를 켠 채로 빌드하고
\co{nohz_full=} 패러미터가 리얼타임 쓰레드를 수행하게 될 작업용 CPU 들의
리스트를 명시하게 함으로써 막아질 수 있습니다.
예를 들어, \co{nohz_full=2-7} 은 CPU~2, 3, 4, 5, 6, 그리고~7 을 작업용 CPU 로
...
때문에 그 CPU 는 \emph{adaptive ticks mode} 또는 \co{nohz_full} 모드에 있다고
이야기 됩니다.
...
\co{mlock()} 과 \co{mlockall()} 시스템콜을 사용해서 여러분의 어플리케이션의
...
물론, \clnref{cat} 의 \co{N} 은 문제의 CPU 의 숫자로 바뀌어야 하며,
\clnref{echo1} 의 \co{1} 은 커널 내에서의 함수 호출 단계를 추가적으로 보기 위해
높아질 수 있습니다.
...
항상 그렇듯, 공짜 점심은 없고 \co{NO_HZ_FULL} 도 예외가 아닙니다.
앞서 언급되었듯, \co{NO_HZ_FULL} 은 커널/유저 전환 비용을 더 높게 하는데 델타
프로세스 처리와 RCU 같은 커널 서브시스템에게 그 전환을 알려야 하는 필요
때문입니다.
거친 경험적 법칙으로, \co{NO_HZ_FULL} 은 많은 종류의 리얼타임과 컴퓨팅이 주인
워크로드에 도움이 됩니다만, 높은 비율로 시스템콜과 I/O 를 사용하는 워크로드를
해칩니다~\cite{AbdullahAljuhni2018nohzfull}.
추가적인 제한, 트레이드오프, 그리고 구성상의 조언이 리눅스 소스 트리의
\path{Documentation/timers/no_hz.rst} 에서 얻어질 수 있습니다.
...
리얼타임 소프트웨어 컴포넌트를 위해 봐야할 자연스런 장소는 wait-free
...
\QuickQuiz{
	하지만 fail-stop 버그에도 불구하고 올바른 운영을 하는게 가치 있는
	fault-tolerance 속성 아닌가요?
	...
	Non-blocking 알고리즘이 fail-stop 버그에 대해 실패 내성을 제공한다는
	점에서 맞습니다만, 이는 실용적인 실패 내성이 되기엔 상당히 부족하다는
	점에서 아닙니다.
	예를 들어, 여러분의 하나의 wait-free queue 를 가지고 있으며 하나의
	쓰레드가 원소 하나를 dequeue 했다고 해봅시다.
	그 쓰레드가 이제 어떤 fail-stop 버그를 당하면, 지금 dequeue 된 그
	원소는 실질적으로 잃어버린게 됩니다.
	진정한 실패 내성은 non-blocking 속성 이상의 것을 필요로 하며, 이 책의
	범위 밖입니다.
...
실제 세계에서, 락킹은 이론적 염려에도 불구하고 리얼타임 프로그램에서 종종
사용됩니다.
그러나, 보다 엄격한 제한 하에서, 락 기반의 알고리즘 또한 제한된 응답시간을
...
\QuickQuiz{
	...
	실제로, 그리고 많이 있습니다.
	그러나, 그것들은 특정 상황에만 존재한느 경향을 보이며 많은 것들은 앞에
	나열된 제약들의 수정본으로 여겨질 수 있습니다.
	예를 들어, 데이터 구조의 선택에서의 많은 제약은 ``특정 크리티컬
	섹션에서 보내는 시간의 제한'' 제약을 지키는 걸 도울겁니다.
...
이 결과는 리얼타임 소프트웨어에서 사용하기 위한 풍부한 알고리즘과 데이터
구조를, 그리고 오랫동안 이어진 리얼타임 규칙을 검증할 기회를 가능하게 합니다.

물론, 주의 깊고 간단한 어플리케이션 설계도 굉장히 중요합니다.
세계 최고의 리얼타임 컴포넌트는 제대로 생각되지 않은 설계에서 만들어질 수
없습니다.
병렬 리얼타임 어플리케이션을 위해서, 동기화 오버헤드는 분명히 설계의 핵심
컴포넌트여야 합니다.
...
접근할 수 있는 방법은 리눅스 커널의 \co{NO_HZ_FULL} 기능을 사용하는 것으로,
...
한가지 문제는 \co{CLOCK_REALTIME} 으로, 이상하게 들리겠지만 이는 리얼타임에
사용되라고 만들어지지 않았습니다.
대신, 이것은 프로세스나 쓰레드에 의해 소요된 CPU 시간량에 반대되는 의미의
``리얼타임'' 을 의미합니다.
리얼타임 컴퓨팅에 사용되기 위해선 \co{CLOCK_MONOTONIC} 이 사용되어야 합니다.
...
또다른 문제는 이 쓰레드가 \co{sched_setscheduler()} 시스템콜을 사용해 리얼타임
우선순위가 되어야만 한다는 겁니다.
그러나 우린 여전히 페이지폴트를 보게 되므로 이 변화만으로는 불충분합니다.
우린 또한 페이지폴트를 막기 위해 \co{mlockall()} 시스템 콜을 사용해 이
어플리케이션의 메모리를 고정시켜야 합니다.
이 모든 변화를 가지고서야 결과는 마침내 받아들여질 수도 있습니다.

다른 상황에서는 추가적인 변경이 필요할 수도 있ㅅ브니다.
...
이 프로그램은 상상컨대 \co{cur_cal} 이라 이름 지어진, 정적으로 할당되고 초기화
되었으며 \co{a}, \co{b}, 그리고 \co{c} 라 이름지어진 필드에 기본 조정값을 담고
있는 \co{default_cal} 을 참조하는 전역 포인터를 사용할 수 있을 겁니다.
그렇지 않다면, \co{cur_cal} 은 현재 조정값을 제공하는 동적으로 할당된 구조체를
가리킵니다.
...
는 RCU 가 이 문제를 해결하기 위해 어떻게 사용될 수 있는지 보입니다.
탐색은 \clnrefrange{calc:b}{calc:e} 의 \co{calc_control()} 에 보인 것과 같이
소요시간이 결정되어 있으며 리얼타임 요구사항과 일관되어 있습니다.
업데이트는 더 복잡한데, \clnrefrange{upd:b}{upd:e} 의 \co{update_cal()} 에 보인
것과 같습니다.
\end{fcvref}
...
\QuickQuizSeries{%
\QuickQuizB{
	리얼타임 시스템은 종종 안전에 치명적인 어플리케이션을 위해 사용된다는
	점, 그리고 수행시간 메모리 할당은 많은 안전에 치명적인 상황에서
	금지되었다는 점을 놓고 볼 때, \co{malloc()} 은 뭘 위한거죠???
	...
	2016년 초, 수행시간 메모리 할당을 금지한 프로젝트들은 멀티쓰레드
	컴퓨팅에 관심조차 없었습니다.
	따라서 수행시간 메모리 할당은 안전에 치명적인 상황을 위한 추가적인
	장애가 아닙니다.

	그러나, 2020년에 이르러서는 멀티코어 리얼타임 시스템에서의 수행시간
	메모리 할당이 약간 매력을 얻고 있습니다.
...
%
\QuickQuizE{
	\co{update_cal()} 을 보호하기 위한 어떤 동기화가 필요하지 않나요?
	...
	실제로 그렇고, 여러분은 이 책의 앞에서 이야기된 어떤 기법이든 사용할 수
	있습니다.
	그런 기법 중 하나는 단일 업데이트 쓰레드의 사용으로,
	\cref{lst:advsync:Real-Time Calibration Using RCU} 의 \co{update_cal()}
	에 보인 코드와 정확히 같은 결과를 낼 겁니다.
...
이 예제는 어떻게 RCU 가 리얼타임 프로그램에 소요 시간이 결정된 읽기쪽 데이터
...
리얼타임과 정말 빠른 컴퓨팅 사이의 선택은 어려울 수 있습니다.
리얼타임 시스템은 비 리얼타임 컴퓨팅에 비해 처리량에 페널티를 종종 추가하므로,
필요하지 않은 곳에 리얼타임을 사용하는 것은
\cref{fig:advsync:The Dark Side of Real-Time Computing} 에 보인 것처럼 현명치
못한 선택일 수 있습니다.
...
다른 한편, 리얼타임 시스템이 \emph{필요한데} 사용하지 못하는 것 역시 문제를
일으킬 수 있는데,
\cref{fig:advsync:The Dark Side of Real-Fast Computing} 에 보인 것과 같습니다.
이는 여러분이 여러분의 상사에게 미안함을 느끼게 하기 거의 충분합니다!
...
\begin{enumerate}
\item	장기간 평균 처리량이 유일한 목표인가?
\item	큰 부하가 응답시간을 악화시키는게 허용되는가?
\item	메모리가 부족해서 \co{mlockall()} 시스템콜의 사용이 불허되는가?
\item	어플리케이션의 기본 처리 항목이 완료되는데 100 밀리세컨드 이상을
	소요하는가?
\end{enumerate}

이 질문들 중 하나라도 그에 대한 답이 ``그렇다'' 라면, 여러분은 리얼타임이
아니라 정말 빠른 시스템을 선택해야 하며, 그렇지 않다면 리얼타임이 여러분에게
맞을 수도 있습니다.

현명하게 선택하시고, 여러분이 리얼타임을 선택했다면, 여러분의 하드웨어, 펌웨어,
그리고 운영체제가 여러분의 일을 위해 잘 선택되었음을 분명히 하세요!
