...
``Hard real-time 시스템은 \emph{항상} 데드라인을 지킵니다!'' 와 같은 문장은
...
\QuickQuizAnswer{
	...
	그런 queueing-theory 의 결론은 리눅스 커널이라면 무한한 수의 task 로
	대응되는, 무한한 ``사용자'' 를 가정하고 있습니다.
	2016 년 중반의 시점에서, 어떤 실제 시스템도 무한한 수의 task 를
	지원하지 않으며, 따라서 무한한 사용자를 가정한 결과는 무한보다는 적은
	응용성을 갖게 될 것이라 기대될 겁니다.
	\iffalse
	...
	또다른 queueing-theory 의 결론은 \emph{유한한} 사용자를 갖는데, 이
	경우는 깔끔하게 그 한계가 제한되어 있는 응답 시간을
	갖습니다~\cite{Hillier86}.
	이런 결론은 실제 시스템을 더 잘 모델링 하고, 이런 모델링은 리소스
	활용율의 감소에 따른 평균 응답시간과 최악의 경우의 응답시간의 감소를
	모두 예측합니다.
	이런 결론은 락킹과 같은 동기화 메커니즘을 사용하는 동시성 시스템을
	모델링하는데에도 확장될 수
	있습니다~\cite{BjoernBrandenburgPhD,DipankarSarma2004OLSscalability}.

	짧게 요약해서, 정확하게 실제 세계의 real-time 시스템을 묘사하는
	queueing-theory 의 결론은 줄어드는 리소스 활용율에 따라 최악의 경우의
	응답시간도 줄어듦을 보입니다.
	\iffalse
...
이런 단계에 따른 방법의 장점은 응답시간 예산이 나누어질 수 있어서,
...
\QuickQuiz{}
	Formal 검증은 수십년간 집중된 연구로 인해 이미 상당히 실용 가능한
	수준입니다.
	추가적인 진보가 \emph{정말로} 필요한 건가요, 아니면 이건 그저 실무자의
	게으르기를 계속하고 formal 검증의 대단한 위력을 무시하려는 변명일
	뿐인가요?
	\iffalse
...
\QuickQuizAnswer{
	...
	\begin{enumerate}
	\item	Formal 검증은 더 커다란 소프트웨어 작품들을 처리할 수 있어야만
		합니다.
...
\QuickQuiz{}
	...
	\fi
\QuickQuizAnswer{
	이 구분법은 엄격한 이론적 관점에서는 분명 만족스럽지 못합니다.
...
사용됩니다.
이 네가지 영역은 ``생산의 탐색'', ``삶의 탐색'', ``죽음의 탐색'', 그리고 ``돈의
탐색'' 으로 성격지어질 수 있을 겁니다.
\iffalse
...
또다른 간단한 방법은 하나의 하드웨어 쓰레드만을 켜고 나머지는 모두 꺼서 타협된
...
RTOS 들을 리눅스 커널과 짝짓는 것은 리눅스 커널이 최소한의 real-time 기능만을
...
보여진 non-preemptible 커널은 \co{CONFIG_PREEMPT=n} 과 함께 빌드되어져서, 이
...
Event-driven real-time 어플리케이션을 위해 필요한 운영 체제의 지원은 비용이
...
\paragraph{Priority inheritance} 는 preemption 당할 수 있는 인터럽트 핸들러에
...
\QuickQuizAnswer{
	...
	하지만, 이 방법은 read-side 의 확장성을 분명하게 그리고 상당히
	제한합니다.
	...
	리눅스 커널의 reader-writer 락이 RCU 로 변경되었습니다.
...
\co{__rcu_read_lock()} 함수가 line~1-5 에, 그리고 \co{__rcu_read_unlock()}
...
크리티컬 섹션의 뒤따르는 코드가 앞의 \co{rcu_read_lock()} 보다 앞으로
...
\QuickQuiz{}
	...
	못하게 되어서, 자기 자신을 현재 grace period 를 막고 있는 태스크들의
...
이 preemptible RCU 구현은 읽기가 대부분인 데이터 구조에 많은 수의 읽기
...
버전의 리눅스 커널에 받아들여진 \co{NO_HZ_FULL}
...
인터럽트는 OS jitter 의 많은 부분의 큰 원인입니다.
...
그러나, atomic test and set, atomic exchange, atomic
...
\QuickQuizAnswer{
	...
	Non-blocking 알고리즘이 fail-stop 버그의 존재 하에서도 fault tolerance
	를 제공할 수 있다는 점에서 그러합니다만, 이는 실질적인 fault tolerance
	에는 심하게 충분치 못하다는 점에서 그렇지 않습니다.
...
\QuickQuizAnswer{
	...
	하지만, 그것들은 특정 상황에 종속적인 경향을 갖고, 그중 많은 것들은
	앞서 언급된 제약들의 세련된 형태로 생각될 수 있습니다.
...
탐색은 line~9-15 의 \co{calc_control()} 로 보여진 것처럼 결정적이며, real-time
요구사항을 일관적으로 지킵니다.
업데이트는 line~17-35 의 \co{update_cal()} 에 보여진 것처럼 좀 더 복잡합니다.
\iffalse
...
\QuickQuiz{}
	Real-time 시스템은 많은 경우 safety-critical 어플리케이션들에
	사용된다는 점을 생각해 보면, 그리고 수행시간 메모리 할당은 많은
...
이 예는 RCU 가 어떻게 real-time 프로그램에 결정론적 read-side 데이터 구조를
제공할 수 있는지를 보입니다.
\iffalse
