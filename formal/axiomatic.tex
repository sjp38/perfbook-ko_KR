...
있는데, Listing~\ref{lst:formal:Locking Example} (\path{C-Lock1.litmus}) 에
...
languages and also to hgher-level synchronization primitives, as
...
\QuickQuiz{}
	...
	테스트를 \co{herd} 로 돌려 보려면 뭘 해야 하나요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	v4.17 (또는 그보다 나중의) 리눅스 커널 소스 코드를 얻고,
	\path{tools/memory-model/README} 의 설명을 따라서 필요한 도구들을
	설치하세요.
	이어서 이 도구들을 사용해 여러분이 고른 리트머스 테스트를 수행하기 위해
	다음 설명을 따르세요.
	\iffalse
...
물론, Listing~\ref{lst:formal:Broken Locking Example} 에 보인 것처럼 \co{P0()}
와 \co{P1()} 이 다른 락을 사용하면 완전 망합니다.
이 경우에, \co{herd} 의 출력은 \co{Sometimes} 라는 문자열을 보여서, 다른 락을
사용하는건 \co{P1()} 이 \co{x} 의 값이 1인 경우를 볼 수 있게 됨을 올바르게
이야기 합니다.
\iffalse
...
\QuickQuiz{}
	왜 귀찮게 락킹을 직접 모델링 하려 하는거죠?
	그냥 어토믹 오퍼레이션을 이용해 락킹을 에뮬레이션 하는게 어때요?
	\iffalse
...
\end{table}
	...
	에 잘 보여져 있습니다.
	첫번째 열은 herd 가 모델링한 프로세스의 수를 의미합니다.
	두번째 열은 cat 언어로 직접 \co{spin_lock()} 과 \co{spin_unlock()} 을
	모델링 했을 때의 herd 의 수행시간을 보입니다.
	세번째 열은 \co{cmpxchg_acquire()} 로 \co{spin_lock()} 을,
	\co{smp_store_release()} 로 \co{spin_unlock()} 을 에뮬레이션 했을 때의
	herd 수행시간을 보이는데, 이 때 락을 획득하는데 실패하는 경우를 막기
	위해 herd \co{filter} 절을 사용했습니다.
	네번째 열은 세번째와 같지만, \co{cmpxchg_acquire()} 대신
	\co{xchg_acquire()} 를 사용했습니다.
	다섯번째와 여섯번째 열은 세번째와 네번째 열과 같지만, 락을 획득하는데
	실패하는 경우를 제거하기 위해 \co{exists} 절을 사용했습니다.
	\iffalse
	...
	\co{filter} 절의 사용이 \co{exists} 절의 사용보다 두배 가량 빠르다는 걸
	알아두시기 바랍니다.
	\co{filter} 절은 락이 하나 이상의 프로세스에 의해 동시에 잡혀 있는
	...
	더 중요한게, \co{spin_lock()} 과 \co{spin_unlock()} 을 직접 모델링
...
얻을 수 있습니다.
Line~\lnref{head} 는 리스트 헤드인 \co{x} 를 보이는데, 이 값은 초기에 \co{y} 를
...
Line~\lnref{P0start}--\lnref{P0end} 의 \co{P0()} 는 리스트에서 \co{y} 를 \co{z}
로 바꿈으로써 (line~\lnref{assignnewtail}) 리스트에서 제거하고, 하나의 그레이스
피리어드가 지나가길 기다리고 (line~\lnref{sync}), 마지막으로 \co{y} 의 값을
0으로 만들어 \co{free()} 의 행동을 따라합니다 (line~\lnref{free}).
Line~\lnref{P1start}--\lnref{P0end} 의 \co{P1()} 은 RCU read-side 크리티컬 섹션
내에서 (line~\lnref{rl}--\lnref{rul}), 리스트 헤드를 가져오고
(line~\lnref{rderef} 다음 원소를 가져옵니다 (line~\lnref{read}).
이 다음 원소는 아직 메모리 해제되지 않은, 0이 아닌 값이어야
합니다(line~\lnref{exists}).
일부 다른 변수들은 디버깅 목적의 출력입니다 (line~\lnref{locations_}).
\iffalse
...
\co{herd} 툴에서 이 리트머스 테스트를 돌리면 그 결과는 \co{Never} 로, \co{P0()}
가 예상대로 결코 메모리 해제된 원소에 접근하지 않음을 알립니다.
역시 예상대로, line~\lnref{sync} 를 지우면 \co{P0()} 가 메모리 해제된 원소를
접근할 수 있어서, \co{herd} 결과에 \co{Sometimes} 로 보입니다.
\iffalse
...
이 예제에서, line~\lnref{P0start}--\lnref{P0end} 의 읽기 쓰레드들 (\co{P0()} 로
모델됨) 은 리스트의 마지막 원소로의 포인터를 변수 \co{c} 로 ``흘림'' 으로써
링크드 리스트를 라운드 로빈 형태로 접근합니다.
업데이트 쓰레드들 (line~\lnref{P1start}--\lnref{P1end} 의 \co{P1()} 으로
...
Line~\lnref{listtail}--\lnref{listhead} 는 초기의 링크드 리스트를, tail 먼저
정의합니다.
...
다시, line~\lnref{P0start}--\lnref{P0end} 는 읽기 쓰레드를 모델링 합니다.
이 프로세스는 리스트를 라운드 로빈으로 순회하는 한상의 연속적인 읽기 쓰레드들을
모델링 하는데, 첫번째 읽기 쓰레드가 line~\lnref{rl1}--\lnref{rul1} 에, 두번째
읽기 쓰레드는 line~\lnref{rl2}--\lnref{rul2} 에 있습니다.
Line~\lnref{rdcache} 는 \co{c} 에 캐시된 포인터를 가져오고,
line~\lnref{rdckcache} 가 이 포인터가 \co{NULL} 인 걸 보게 되면,
line~\lnref{rdinitcache} 에서 리스트의 처음부터 재시작합니다.
어느 경우든, line~\lnref{rdnext} 는 다음 리스트 원소로 진행하고,
line~\lnref{rdupdcache} 는 이 원소로의 포인터를 변수 \co{c} 에 다시 저장합니다.
line~\lnref{rl2}--\lnref{rul2} 는 이 과정을 반복합니다만, 레지스터 \co{r1} 과
