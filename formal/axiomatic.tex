...
그러나, PPCMEM 은 IRIW 를 푸는데 14 CPU 시간을 소요하는데 반해 herd 는 17
밀리세컨드만에 이를 완료하여, 백만배 이상의 속도향상을 보입니다.
그렇다고는 하나, 이 문제는 근본적으로 지수적이어서 우린 herd 가 큰 문제에
...
마다 네개의 쓰기를 더하면, herd 는 50,000 배 이상 느려져서 15 \emph{분} 이 넘는
CPU 시간을 필요로 합니다.
...
Howeved, where PPCMEM requires 14 CPU hours to solve IRIW, herd does so
...
그 폭발적 본성에도 불구하고, PPCMEM 과 herd 는 모두 x86 시스템에서의
queued-lock handoff 를 포함한 핵심 병렬 알고리즘을 검사하는데 상당히 유용함이
증명되었습니다.
herd 의 약점은 Section~\ref{sec:formal:PPCMEM Discussion} 에서 보인 PPCMEM 의
그것과 비슷합니다.
PPCMEM 과 herd 가 다른 결과를 내는 눈에 잘 안띄는 (그러나 분명 존재하는)
