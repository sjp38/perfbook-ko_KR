...
여러 statement 들의 그룹이 \co{EXECUTE_MAINLINE()} 으로 전달될 때에는
line~12-15 와 같이 해당 그룹 내의 모든 statement 들이 어토믹하게 수행됨을
알아둘 필요가 있습니다.
\iffalse
...
\QuickQuiz{}
	하지만 \co{EXECUTE_MAINLINE()} 내의 statement 들이 어토믹 하지않게
	...
	\fi
\QuickQuizAnswer{
	그렇기 위한 가장 쉬운 방법은 각각의 statement 를 각자의
	\co{EXECUTE_MAINLINE()} 안에 넣는 것입니다.
	\iffalse
...
\QuickQuiz{}
	...
	\fi
\QuickQuizAnswer{
	뒤의 섹션에서 보게 되겠지만, 한가지 방법은 명시적인 라벨과 ``goto''
	문을 사용하는 겁니다.
	예를 들면, 아래와 같은 것은:
	\iffalse
...
\QuickQuizAnswer{
	...
	이것들을 굳이 어토믹하게 모델링 하게 되는 동기는 상태 공간의 크기를
	줄이려는 것입니다.
	\iffalse
...
Line~11-25 는 \co{rcu_irq_enter()} 를 모델링하고, line~26 과 27 dms
