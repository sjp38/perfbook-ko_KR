...
이 \co{rcu_riq_enter()} 와 \co{rcu_irq_exit()} 함수는 dynticks-idle CPU 들이
...
\QuickQuizAnswer{
	...
	\item	비록 코드의 정확성을 증명할 수는 없지만 숨겨진 버그들을 찾아줄
		수 있는, 신중히 생각한 고문 테스트를 고안하세요.
	\item	작은 컴퓨터들로 이루어진 클러스터를 사용해서 모델 체크를 하는
		도구를 만들고 사용하려는 움직임이 일부 있습니다.
		하지만, Paul 은 사용할 수 있는 커다란 컴퓨터가 여럿 있어서 그런
		도구를 직접 사용해 본 적은 없음을 알아두시기 바랍니다.
	\item	당신의 문제를 적용하기에 맞는 크기의 메모리 크기를 가진, 구매할
		만한 가격대의 시스템이 나오길 기다리세요.
...
이 함수는 한번 더 \co{dynticks_progress_counter} 의 값을 증가시킵니다만, 어떤
다른 CPU 가 뒤의 RCU read-side 크리티컬 섹션의 결과를 보게 된다면 그 CPU 는
\co{dynticks_progress_counter} 의 증가된 값 역시 볼 수 있을 것을 보장하기 위해
값 증가에 이어 메모리 배리어를 실행시킵니다.
마지막으로, \co{rcu_exit_nohz()} 는 값 증가의 결과가 홀수임을 확인합니다.

\co{rcu_enter_nohz()} 와 \co{rcu_exit_nohz()} 함수는 어떤 CPU 가 dynticks-idle
...
\co{rcu_irq_enter()} 와 \co{rcu_irq_exit()} 함수는 인터럽트/NMI 진입과 종료를
각각 처리합니다.
물론, 중첩된 인터럽트들 또한 적절하게 처리되어야만 합니다.
중첩 인터럽트의 가능성은 인터럽트나 NMI 핸들러 진입 시에 (\co{rcu_irq_enter()}
에서) 값이 증가되고 종료 시에(\co{rcu_irq_exit()} 에서) 값이 감소되는 두번째
per-CPU 변수, \co{rcu_update_flag} 에 의해 처리됩니다.
추가적으로, 이전부터 존재해온 \co{in_interrupt()} 기능은 인터럽트/NMI 가 가장
...
만약 그 경우라면, line~10 에서 \co{dynticks_progress_counter} 의 값을
증가시키고, line~11 에서 메모리 배리어를 실행한 후, line~12 에서
\co{rcu_update_flag} 의 값을 증가시킵니다.
...
\QuickQuizAnswer{
	...
	이는 RCU grace-period 에게 이 CPU 에서 NMI 핸들러가 실행되었음을 알 수
	있는 증거를 남기지 않을 것이고, 따라서 이 NMI 핸들러 내에서의 모든 RCU
	red-side 크리티컬 섹션들은 RCU 보호를 깨트릴 겁니다.

	그 정의에 의해 마스킹 될 수 없는 NMI 핸들러의 존재 가능성은 정말로 이
	코드를 복잡하게 만듭니다.
	\iffalse
...
이 두 섹션은 \co{dynticks_progress_counter} 변수가 태스크들과 인터럽트, NMI
...
가능한 RCU grace-period 상태 가운데, \co{rcu_try_flip_waitack_state()} 와
\co{rcu_try_flip_waitmb_state()} 상태만이 다른 CPU 들이 응답하길 기다려야
합니다.
...
\co{rcu_try_flip_waitack_state()} 상태는 아래에 보여진 것과 같은
...
Line~7 과~8 은 \co{dynticks_progress_counter} 의 현재 버전과 스냅샷 버전을 각각
가져옵니다.
...
\co{dynticks_progress_counter} 값 증가를 모델링 하며, line~12 는 \co{WARN_ON()}
을 모델링 합니다.
...
\QuickQuizAnswer{
	...
	모델링할 필요가 없습니다.
	사실, 그대신
	page~\pageref{lst:analysis:QRCU Unordered Summation} 의
	Listing~\ref{lst:analysis:QRCU Unordered Summation}
	처럼 메모리 배리어의 부재를 명시적으로 모델링 해야 합니다.
	\iffalse
...
Line~23 은 또다시 CPU 의 \co{dynticks_progress_counter} 변수의 스냅샷을
얻어오는 \co{rcu_try_flip_waitzero()} 의 코드 중 한줄과 그것의
\co{dyntick_save_progress_counter()} 함수 호출을 모델링 합니다.

마지막으로, line~24-36 은 \co{rcu_try_flip_waitack()} 과 이 함수의
...
\QuickQuizAnswer{
	...
	만약 그 grace-period 코드가 특정 CPU 의 특정 값을 가지고 뭔가 특별한
	일을 하려 했다면, 우린 정말로 여러개의 CPU 들을 모델링 해야 할겁니다.
	하지만 다행히도, 우린 하나의 CPU 는 grace-period 처리를 수행하고 있고
	다른 하나는 dynticks-idle 모드를 들어가고 빠져나오는 두개의 CPU 에만
	우리를 안전히 국한시킬 수 있습니다.
	\iffalse
...
스크립트를 통해 수행될 때, 964 개의 상태들을 생성하고 안심스럽게도 에러없이
돌아갑니다.
...
어쨌건, \co{dyntick_nohz()} 가 완료되었다면, 더이상 우리를 해당 루프의 바깥으로
...
하지만, 라인 수는 전체 모델 (\path{dyntickRCU-base-sl-busted.spin} 과는
\emph{맞아떨어질} 겁니다.
\iffalse
...
따라서 우리는 짝수가 되어야 할 값으로 \co{snap} 이 아니라 \co{curr} 를 테스트
해야합니다.
...
첫번째 단계는 \co{dyntick_nohz()} 를 다음과 같이 \co{EXECUTE_MAINLINE()} 의
형태로 바꾸는 겁니다:
\iffalse
...
여러 statement 의 그룹이 \co{EXECUTE_MAINLINE()} 으로 전달될 때에는 line~12-15
와 같이 해당 그룹 내의 모든 statement 가 어토믹하게 수행됨을 알아둘 필요가
있습니다.
\iffalse
...
\QuickQuiz{}
	하지만 \co{EXECUTE_MAINLINE()} 내의 statement 들이 어토믹 하지 않게
	수행되어야 하는 경우라면 어떻게 하겠습니까?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	가장 쉬운 방법은 각각의 statement 를 각자의 \co{EXECUTE_MAINLINE()}
	안에 넣는 것입니다.
	\iffalse
...
\QuickQuiz{}
	하지만 \co{dynticks_nohz()} 프로세스가 ``if'' 나 ``do'' 문을 가지고
	있는데 그 안의 조건절들이 그 분기 본체들은 어토믹하지 않게 수행되는
	형태라면 어떻게 되죠?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	뒤의 섹션에서 보게 되겠지만, 명시적인 라벨과 ``goto'' 문을 사용하는게
	한가지 방법입니다.
	예를 들면, 아래와 같은 것은:
	\iffalse
	...
	하지만, 이 매크로가 ``if'' 문의 경우에 있어서도 도움이 될지는 명확하지
	않은데, 따라서 이런 종류의 상황들에 대해서는 뒤의 섹션들에서
	정리하겠습니다.
	\iffalse
...
다음 단계는 인터럽트 핸들러를 모델링 하기 위해 \co{dyntick_irq()} 프로세스를
작성하는 것입니다:
\iffalse
...
Line~7-48 의 루프는 \co{MAX_DYNTICK_LOOP_IRQ} 까지의 인터럽트들을 모델링하는데,
line~8 과~9 는 루프 조건을 구성하고 line~46 은 제어 변수를 증가시킵니다.
Line~10 은 \co{dyntick_nohz()} 에게 인터럽트 핸들러가 돌아가는 중이라고
이야기하고, line~45 는 \co{dyntick_nohz()} 에게 이 핸들러가 완료되었다고
이야기합니다.
Line~49 는 \co{dyntick_nohz()} 의 관련된 라인과 같이 liveness 검증을 위해
사용됩니다.
\iffalse
...
\QuickQuiz{}
	Line~45 와~46 (\co{in_dyntick_irq = 0;} 와 \co{i++;}) 은 왜 어토믹하게
	수행되는 건가요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	이 코드 라인들은 모델을 제어하는 부분에 속하지 모델링되는 코드에 속하지
	않으므로, 이것들을 어토믹하지 않게 수행시킬 이유가 없습니다.
	이것들을 굳이 어토믹하게 모델링 하는건 상태 공간의 크기를 줄이기
	위해서입니다.
	\iffalse
...
Line~11-25 는 \co{rcu_irq_enter()} 를 모델링하고, line~26 과~27 은
\co{__irq_enter()} 의 관련된 부분을 모델링 합니다.
Line~28 과~29 는 \co{dynticks_nohz()} 의 관련된 부분과 거의 같은 방법으로
안전성을 검증합니다.
Line~30 과~31 은 \co{__irq_exit()} 의 관련 부분을 모델링 하고, line~32-43 에서
\co{rcu_irq_exit()} 을 모델링 합니다.
\iffalse
...
\QuickQuiz{}
	이 \co{dynticks_irq()} 프로세스가 모델링할 수 없는 인터럽트들의 특성은
	무엇이 있을까요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	그런 특성 가운데 하나는 뒤의 섹션에서 다루게 될, 중첩된
	인터럽트들입니다.
	\iffalse
...
이제 \co{grace_period()} 프로세스는 다음과 같이 됩니다:
\iffalse
...
체크를 위한 새로운 \co{dyntick_irq_done} 변수를 추가하는 line~19 와~39 의
변경, 그리고 line~22 와~42 에서의 최적화 뿐입니다.

이 모델 (\path{dyntickRCU-irqnn-ssl.spin})
은 대략 50만개의 상태를 에러 없이 성공적으로 검증합니다.
하지만, 이 버전의 모델링은 중첩된 인터럽트를 처리하지 않습니다.
...
중첩된 인터럽트 핸들러는 \co{dyntick_irq()} 의 루프의 몸체를 다음과 같이
...
이는 \co{dynticks_irq()} 프로세스와 유사합니다.
여기선 line~5 에서 두번째 카운터 변수 \co{j} 를 추가해서 \co{i} 는 인터럽트
핸들러들로의 진입을 셈하고 \co{j} 는 인터럽트 핸들러로부터 빠져나가는 횟수를
세게 합니다.
Line~7 에서의 \co{outermost} 변수는 안전성 검사를 위해 언제 \co{gp_state}
변수가 샘플링 되어야 하는지를 결정합니다.
Line~10 과~11 에서의 루프 종료 체크는 명시된 수의 인터럽트 핸들러들이 진입된
만큼 빠져나와지기도 했음을 요청하기 위해 업데이트 되었고, \co{i} 의 값 증가는
인터럽트 진입 모델의 마지막인 line~41 로 옮겨졌습니다.
Line~13-16 은 이게 중첩된 인터럽트들의 집합의 가낭 바깥인지 여부를 알리기 위해
\co{outermost} 변수를 설정하고 \co{dyntick_nohz()} 프로세스에 의해 사용되는
\co{in_dyntick_irq} 변수를 설정합니다.
Line~34-40 에서는 \co{gp_state} 변수의 상태를 읽어오는데, 가장 바깥의 인터럽트
핸들러 내일 때에만 그렇습니다.
\iffalse
...
빠져나갈 때에만 그렇습니다.
마지막으로, line~65-68 은 인터럽트 종료 카운트 \co{j} 의 값을 증가시키고, 이게
가장 바깥의 인터럽트 단계라면, \co{in_dyntick_irq} 의 값을 지웁니다.

이 모델 (\path{dyntickRCU-irq-ssl.spin})
은 약 50만개가 조금 넘는 상태들에 대한 정확성 검사를 만들어내며, 모두 에러 없이
통과합니다.
...
NMI 는 중첩되지 않는다는 점을 생각하면서 우리는 NMI 를 위해 인터럽트에 했던
것과 동일한 일반적 방법을 사용합니다.
이는 다음과 같은 \co{dyntick_nmi()} 프로세스를 만들어 내게 됩니다:
\iffalse
...
\co{dyntick_nmi_done} 변수의 체크를 통해 NMI 핸들러 (\co{in_dyntick_nmi}) 는
물론 인터럽트 핸들러 (\co{in_dyntick_irq}) 에도 주의를 기울여야 합니다:
\iffalse
...
우리는 또한, \co{dyntick_irq()} 가 \co{dyntick_nmi()} 를 배척하도록 하기 위해
\co{in_dyntick_nmi} 를 체크하는 \co{EXECUTE_IRQ()} 매크로를 만들어야 합니다:
\iffalse
...
우리가 ``if'' 문을 open-code 했음을 알아 두시기 바랍니다 (예를 들면,
line~17-29).
또한, (line~58 과 같은) 엄격한 로컬 상태를 처리하는 문장들은 \co{dyntick_nmi()}
를 배제시킬 필요가 없음을 알아두시기 바랍니다.

마지막으로, \co{grace_period()} 는 몇가지 변경만 있으면 됩니다:
\iffalse
...
우리는 line~11 에 \co{MAX_DYNTICK_LOOP_NMI} 패러미터를 위한 새로운
\co{printf()} 를 추가했고 \co{dyntick_nmi_done} 을 line~22 와~44 의
\co{shouldexit} 할당에 추가했습니다.

이 모델 (\path{dyntickRCU-irq-nmi-ssl.spin})
...
\QuickQuiz{}
	Paul 은 \emph{항상} 그의 코드를 이렇게 고통스럽도록 점진적인 방법으로
	작성하나요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	항상 그렇지는 않지만, 매우 매우 자주 그럽니다.
	이 경우, Paul 은 인터럽트 핸들러를 포함한 가장 작은 코드 조각으로
	시작했는데, 인터럽트들을 Promel 에서 어떻게 모델링 하는게 최선일지에
	대한 확신이 없었기 때문입니다.
	일단 동작하게 하고 나서는, 그는 다른 기능들을 추가해 갔습니다.
	(하지만 그가 이걸 다시 하게 된다면, 그는 ``장난감'' 핸들러를 가지고
	시작할 겁니다.
	예를 들어, 한 변수의 값을 두번 증가시키는 핸들러를 가지고 그 값이 항상
	짝수임을 검증하는 메인 코드를 만들 겁니다.)

	왜 점진적 방법을 사용하는가?
	Brian W. Kerninghan 에 의한 다음의 것들을 고려해 보세요:
	\iffalse
	...
	\begin{quote}
		디버깅은 코드를 처음 작성하는 것보다 두배만큼 어렵습니다.
		따라서, 당신이 코드를 할 수 있는한 한 현명하게 작성한다면, 그
		정의에 따라, 당신은 그걸 디버깅 하기에 충분히 똑똑하지
		않습니다.
		\iffalse
	...
	이는 코드 작성과정을 최적화하려는 노력은 그것의 최소한 66\,\% 의 중점은
	디버깅 과정을 최적화 하는데에 쏟아져야 하는데, 설령 코딩 자체에 쏟는
	시간과 노력을 증가시키더라도 그렇습니다.
	점진적 코딩과 테스트는 코딩 자체에 드는 노력을 증가시키는 대신에 디버깅
	과정을 최적화하는 한가지 방법입니다.
	Paul 은 코딩과 디버깅을 하는데에 (매주는 말할 것도 없고) 모든 날들을
	전념할 호사를 부릴 수 있는 기회는 거의 없기 때문에 이 방법을
	사용합니다.
	\iffalse
...
\item	{\bf 코드를 문서화 하는 것은 버그를 찾는데 도움을 줄 수 있습니다}.
	...
	것처럼 \co{rcu_enter_nohz()} 와 \co{rcu_exit_nohz()} 에 있던 잘못
	배치된 메모리 배리어들을 찾아냈습니다.
\item	{\bf 코드를 빨리, 자주, 그리고 없어지기 전까지 검증하십시오.}
	이런 노력은
	Listing~\ref{lst:formal:Variable-Name-Typo Fix Patch} 의 패치에서 보여진
	것과 같이, \co{rcu_try_flip_waitack_needed()} 안의 테스트하거나
	디버깅 하기에 상당히 어려울 수 있는 버그를 찾아냈습니다.
\iffalse
...
\item	{\bf 항상 검증 코드를 검증하십시오.}
	...
	숙면을 취하는 게 효과적인 디버깅 테크닉이 될 수 있습니다.
	그렇게 되면 분명한 검증을 검증하는 테크닉은 고의적으로 검증되는 코드에
	버그를 집어넣는 것이란 걸 알게 될겁니다.
	만약 검증 코드가 그것들을 찾는데 실패한다면, 그 검증은 분명히 버그로
	가득차 있는 것입니다.
\item	{\bf 어토믹 인스트럭션의 사용이 검증을 단순화 시킬 수 있습니다.}
	안타깝게도, \co{cmpxchg} 어토믹 명령어의 사용은 중요한 \IRQ\ 빠른 수행
...
Preemption 가능한 RCU 의 dynticks 인터페이스의 복잡도는 주로 \IRQ 도 NMI 도
같은 코드 수행 경로와 같은 상태 변수들을 사용한다는 사실에서 기인합니다.
이는
Manfred Spraul~\cite{ManfredSpraul2008StateMachineRCU} 에 의해 간접적으로
제안된대로 계층적 RCU~\cite{PaulEMcKenney2008HierarchicalRCU} 의 경우처럼
별도의 코드 수행 경로와 변수를 제공하는 방법으로 결론지어집니다.
\iffalse
...
은 새로운 per-CPU 상태 변수들을 보입니다.
이 변수들은 복수의 개별적인 RCU 구현들 (ex: \co{rcu} 와 \co{rcu_bh}) 이 편하고
...
코드 수행 경로들을 위한 것이고, \co{dynticks_nmi} 와 \co{dynticks_nmi_snap}
변수는 NMI 코드 수행 경로를 위한 것입니다만, NMI 코드 수행 경로는
\co{dynticks_nesting} 변수 역시 레퍼런스할 (하지만 수정하지는 않을) 겁니다.
이 변수들은 다음과 같이 사용됩니다:
\iffalse
...
\item	[\tco{dynticks_nesting}]
	이 변수는 연관된 CPU 가 RCU read-side 크리티컬 섹션들을 위해 모니터
...
\item	[\tco{dynticks_nmi}]
	...
	해당 NMI 가 이 CPU 가 현재 수행중인 \IRQ\ 핸들러 없이 dyntick-idle
	모드에 빠져 있을 때에만 그렇습니다.
	그렇지 않다면, 해당 카운터의 값은 짝수가 됩니다.
\item	[\tco{dynticks_snap}]
	이것은 \co{dynticks} 카운터의 스냅샷이 됩니다만, 현재 RCU grace period
	가 너무 긴 기간 동안 연장되었을 때에만 그렇습니다.
\item	[\tco{dynticks_nmi_snap}]
	이것은 \co{dynticks_nmi} 카운터의 스냅샷이 됩니다만, 역시 현재 RCU
...
주어진 기간 동안 \co{dynticks} 와 \co{dynticks_nmi} 가 모두 짝수를 가지고
있었다면, 연관된 CPU 는 해당 기간 동안 quiescent 상태를 지나온 것입니다.
\iffalse
...
\QuickQuiz{}
	하지만, 만약 NMI 핸들러가 \IRQ\ 핸들러가 완료되기 전에 동작하기
	시작하면, 그리고 그 NMI 핸들러가 두번째 \IRQ\ 핸들러가 시작될 때까지
	돌아가면 어떤 일이 발생할까요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	이는 하나의 CPU 에 몰아넣은 상황에선 일어날 수 없는 일입니다.
	첫번째 \IRQ\ 핸들러는 NMI 핸들러가 리턴하기 전까지는 완료될 수
	없습니다.
	따라서, \co{dynticks} 와 \co{dynticks_nmi} 변수들 각각이 주어진 시간
	기간동안 짝수를 가졌다면, 연관된 CPU 는 그 기간 중의 어느 시점에는
	quiescent 상태에 정말로 있었을 겁니다.
	\iffalse
...
\co{rcu_enter_nohz()} 와 \co{rcu_exit_nohz()} 를 보이고 있습니다.
이 두 함수는 프로세스 컨텍스트에서 호출됩니다.

Line~6 는 (RCU read-side 크리티컬 섹션들로부터의 액세스들을 포함할 수 있는)
앞의 모든 메모리 액세스들이 dynticks-idle 모드로의 진입을 표시하는 CPU 들보다
앞서서 다른 CPU 들에 보여질 것을 보장합니다.
Line~7 과~12 는 \IRQ\ 를 각각 비활성화시키고 재활성화 시킵니다.
Line~8 은 현재 CPU 의 \co{rcu_dynticks} 구조체로의 포인터를 얻어오고, line~9 는
현재 CPU 의 \co{dynticks} 카운터의 값을 증가시키는데, 우린 프로세스
컨텍스트에서 dynticks-idle 모드에 진입하고 있으므로 짝수가 되어야 할 겁니다.
마지막으로, line~10 은 이제 0이 되어야 하는 \co{dynticks_nesting} 의 값을
감소시킵니다.

\co{rcu_exit_nohz()} 함수는 이와 상당히 유사합니다만, \co{dynticks_nesting} 을
감소시키는게 아니라 증가시키며 \co{dynticks} 의 반대 특성을 체크합니다.
\iffalse
...
\co{rcu_nmi_enter()} 와 \co{rcu_nmi_exit()} 함수를 보입니다.
하지만, 해당 NMI 가 \IRQ\ 핸들러 수행 중에 들어오게 되면, RCU 는 이 CPU 의 RCU
read-side 크리티컬 섹션들을 탐색 하고 있을 것이므로, \co{rcu_nmi_enter()} 의
line~6 과~7 과 \co{rcu_nmi_exit()} 의 line~18 과~19 는 \co{dynticks} 가
홀수라면 아무것도 하지않고 리턴합니다.
그렇지 않다면, 두 함수는 \co{dynticks_nmi} 의 값을 증가시켜서
\co{rcu_nmi_enter()} 는 그 값을 홀수로, \co{rcu_nmi_exit()} 는 그 값을 짝수로
만들어둡니다.
...
\co{rcu_irq_enter()} 와 \co{rcu_irq_exit()} 함수들을 보입니다.
\co{rcu_irq_enter()} 의 Line~6 는 \co{dynticks_nesting} 의 값을 증가시키고, 이
값이 0이 아니었다면, line~7 에서 조용히 리턴합니다.
그렇지 않다면, line~8 에서 \co{dynticks} 의 값을 증가시켜서 그 값을 홀수로
만들어서 이 CPU 가 이제 RCU read-side 크리티컬 섹션을 수행할 수 있다는 사실과
일관적이게 만듭니다.
따라서 line~10 은 \co{dynticks} 의 값 증가가 뒤따르는 irq 핸들러가 실행할 수도
...
\co{rcu_irq_exit()} 의 line~18 에서는 \co{dynticks_nesting} 의 값을 감소시키고,
그 결과가 0이 아니라면, line~19 에서 조용히 리턴합니다.
그렇지 않다면, line~21 에서의 \co{dynticks} 의 값 증가가 앞의 \IRQ\ 핸들러가
실행했을 수도 있는 모든 RCU read-side 크리티컬 섹션들 뒤에 보여질 것을 보장하기
위해 메모리 배리어를 수행합니다.
Line~22 는 \co{dynticks} 가 짝수임을 검증해서 어떤 RCU read-side 크리티컬
...
는 명시된 CPU 의 \co{dynticks} 와 \co{dyntiks_nmi} 카운터들의 스냅샷을 가져오는
\co{dyntick_save_progress_counter()} 를 보입니다.
Line~8 과~9 는 이 두개의 변수들의 스냅샷을 떠오고, line~10 은
Listing~\ref{lst:formal:Entering and Exiting Dynticks-Idle Mode},
...
Line~11 과~12 는 나중의 \co{rcu_implicit_dynticks_qs} 호출을 위한 스냅샷을
...
는 CPU 가 \co{dynticks_save_progress_counter()} 호출에 이어서 dyntick-idle
모드에 들어갔는지 여부를 체크하는 \co{rcu_implicit_dynticks_qs()} 함수를
보입니다.
Line~9 와~11 은 연관된 CPU 의 \co{dynticks} 와 \co{dynticks_nmi} 변수들의
새로운 스냅샷을 가져오고, line~10 과~12 는 기존에
\co{dynticks_save_progress_counter()} 에 의해 저장된 스냅샷을 가져옵니다.
Line~13 은 이어서
Listing~\ref{lst:formal:Entering and Exiting Dynticks-Idle Mode},
\ref{lst:formal:NMIs From Dynticks-Idle Mode}, 그리고
\ref{lst:formal:Interrupts From Dynticks-Idle Mode} 의 함수들의 메모리
배리어들과 짝을 맞추기 위해 메모리 배리어를 실행합니다.
Line~14-16 은 이어서 해당 CPU 가 지금 (\co{curr} 와 \co{curr_nmi} 가 짝수를
가져서) quiescent state 에 있거나 (\co{dynticks} 와 \co{dynticks_nmi} 의 값들이
이미 변해서) 마지막의 \co{dynticks_save_progress_counter()} 호출 후에 quiescent
...
\QuickQuiz{}
	이건 여전히 상당히 복잡하네요.
	왜 그냥 dyntick-idle 모드에 있는 각 CPU 들에 대해 bit 의 값이 설정되어
	있는 \co{cpumask_t} 를 갖고 \IRQ\ 나 NMI 핸들러에 들어갈 때에는 그
	비트의 값을 지우고, 빠져나올 때에는 그 값을 설정하는 식으로 하지
	않는거죠?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	이 방법이 기능적으로는 올바르겠지만, 이는 커다란 머신에서는 지나치게
	커다란 \IRQ\ 들어가고 빠져나올 때의 오버헤드를 초래할 겁니다.
	반면에, 이 섹션에서 이야기한 방법은 각각의 CPU 가 \IRQ\ 와 NMI 에
	들어가고 나올 때에 per-CPU 데이터만을 만지게 하므로 훨씬 적은 \IRQ\ 에
	들어가고 빠져나올 때의 오버헤드를 초래할 것이고, 특히 커다란 기계에선
	더욱 그러할 겁니다.
	\iffalse
