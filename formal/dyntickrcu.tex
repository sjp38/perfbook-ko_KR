...
\co{rcu_irq_endter()} 와 \co{rcu_irq_exit()} 함수들을 보입니다.
...
Line~11 과 12 는 나중의 \co{rcu_implicit_dynticks_qs} 호출을 위한 스냅샷들을
...
\co{dynticks_save_progress_counter()} 에 ㅡ이해 저장된 스냅샷을 가져옵니다.
Line~13 은 이어서
Listing~\ref{lst:formal:Entering and Exiting Dynticks-Idle Mode},
\ref{lst:formal:NMIs From Dynticks-Idle Mode}, 그리고
\ref{lst:formal:Interrupts From Dynticks-Idle Mode} 의 함수들의 메모리
...
\QuickQuiz{}
	...
	있는 \co{cpumask_t} 를 갖고 \IRQ\ 나 NMI 핸들러에 들어갈 때에는 그 비트의
	값을 지우고, 빠져나올 때에는 그 값을 설정하는 식으로 하지 않는거죠?
	\iffalse
...
\QuickQuizAnswer{
	...
	들어가고 나올 때에 per-CPU 데이터만을 만지게 하므로 훨씬 적은 \IRQ\
