...
는 ``nohz'' 모드라고도 알려져 있는 dynticks-idle 모드를 지입하고 빠져나오는
\co{rcu_enter_nohz()} 와 \co{rcu_exit_nohz()} 를 보입니다.
...
\Clnref{mb} 는 모든 앞선 메모리 액세스가 (RCU read-side 크리티컬 섹션으로부터의
액세스를 포함할 수 있습니다) dynticks-idle 모드로의 진입을 표시하는 것들보다
앞서서 다른 CPU 들에게 보일 것을 보장합니다.
\Clnref{irq_sv,irq_rs} 는 \IRQ 를 불능화하고 다시 활성화 합니다.
\Clnref{get_ptr} 은 현재 CPU 의 \co{rcu_dynticks} 구조체로의 포인터를 얻어오고,
\clnref{inc_cnt} 는 현재 CPUdml \co{dynticks} 카운터를 증가시키는데 우리는
프로세스 컨텍스트에서 dynticks-idle 모드에 진입하고 있으므로 이 카운터는 짝수가
될 겁니다.
마지막으로 \clnref{dec_nst} 는 \co{dynticks_nesting} 의 값을 감소시키는데, 이제
0이 될겁니다.
\end{fcvref}

\co{rcu_exit_nohz()} 함수는 상당히 비슷하지만 \co{dynticks_nesting} 을
감소시키는게 아니라 증가시키고 반대의 \co{dynticks} 극성을 검사합니다.
...
\co{rcu_nmi_enter()} 와 \co{rcu_nmi_exit()} 함수들을 보입니다.
그러나, NMI 가 \IRQ\ 핸들러의 중간에 도착한다면 RCU 는 이미 이 CPU 에서의 RCU
read-side 크리티컬 섹션들을 찾는 중이었을 것이어서 \co{rcu_nmi_enter()} 의
\clnref{chk_ext1,ret1} 와 \co{rcu_nmi_exit()} 의 \clnref{chk_ext2,ret2} 는
\co{dynticsk} 가 홀수였다면 조용히 리턴합니다.
그렇지 않다면, 이 두 함수는 \co{dynticks_nmi} 를 증가시키는데
\co{rcu_nmi_enter()} 는 이를 홀수로 만들고 \co{rcu_nmi_exit()} 는 이를 짝수로
만듭니다.
두 함수 모두 이 값 증가와 가능한 RCU read-side 크리티컬 섹션들 사이에
\clnref{mb1,mb2} 에서 메모리 배리어를 수행합니다.
\end{fcvref}
...
는 \IRQ\ 컨텍스트로의 진입과 종료를 RCU 에게 알리는 \co{rcu_irq_enter()} 와
\co{rcu_irq_exit()} 함수들을 보입니다.
\co{rcu_irq_enter()} 의 \clnref{inc_nst} 는 \co{dynticks_nesting} 의 값을
증가시키고, 이 변수가 이미 0이 아니었다면 \clnref{ret1} 에서 조용히 리턴합니다.
그렇지 않았다면, \clnref{inc_dynt1} 은 \co{dynticks} 의 값을 증가시키는데
그러면 그 값은 홀수가 되어서 이 CPU 는 이제 RCU read-side 크리티컬 섹션을
수행할 수 있는 사실과 일관성을 갖게 됩니다.
\Clnref{mb1} 은 따라서 \co{dynticks} 의 값 증가가 뒤따르는 \IRQ\ 핸들러가
수행할 수도 있는 모든 RCU read-side 크리티컬 섹션들에게 보일  것을 보장하기
위해 메모리 배리어를 수행합니다.
...
\co{rcu_irq_exit()} 의 \clnref{dec_nst} 은 \co{dynticks_nesting} 의 값을
감소시키고, 그 결과가 0이 아니라면 \clnref{ret2} 에서 조용히 리턴합니다.
그렇지 않다면, \clnref{mb2} 는 \clnref{inc_dynt2} 에서의 \co{dynticks} 값
증가가 앞의 \IRQ\ 핸들러가 수행했을 수도 있는 모든 RCU read-side 크리티컬 섹션
뒤에 보일 것을 보장하기 위해 메모리 배리어를 수행합니다.
\Clnref{chk_even} 은 \co{dynticks} 가 이제 짝수여서 어떤 RCU read-side 크리티컬
섹션들도 dynticks-idle 모드에서는 나타날 수 없다는 사실과 일관될 것을
검사합니다.
\Clnrefrange{chk_cb:b}{chk_cb:e} 는 앞의 \IRQ\ 핸들러가 어떤 RCU 콜백을 등록해
두었는지 검사하고, 그렇다면 reschedule API 를 사용해 이 CPU 가 dynticks-idle
모드에서 나가도록 강제합니다.
\end{fcvref}
...
는 특정 CPU 의 \co{dynticks} 와 \co{dynticks_nmi} 카운터의 스냅샷을 가져오는
\co{dyntick_save_progress_counter()} 를 보입니다.
\Clnref{snap,snapn} 은 이 두 변수의 값을 지역변수에 스냅샷 뜨고, \clnref{mb} 는
\cref{lst:formal:Entering and Exiting Dynticks-Idle Mode,%
lst:formal:NMIs From Dynticks-Idle Mode,%
lst:formal:Interrupts From Dynticks-Idle Mode} 의 함수들 내의 메모리 배리어들과
짝을 맞추기 위해 메모리 배리어를 수행합니다.
\Clnref{rec_snap,rec_snapn} 은 \co{rcu_implicit_dynticks_qs()} 의 다음 호출을
위한 스냅샷을 기록해 두고, \clnref{chk_prgs} 는 그 CPU 가 진행중인 \IRQ 나 NMI
없이 dynticks-idle 모드에 있어서 (달리 말하자면 두 스냅샷이 모두 짝수여서)
연장된 quiescent state 에 있는지 검사합니다.
그렇다면, \clnref{cnt:b,cnt:e} 는 이 이벤트를 세고, \clnref{ret} 은 이 CPU 가
quiescent state 에 있었다면 true 를 리턴합니다.
\end{fcvref}
...
는 어떤 CPU 가 \co{dynticks_save_progress_counter()} 호출에 뒤이어 dyntick-idle
모드에 진입했는지 검사하기 위해 호출되는 \co{rcu_implicit_dynticks_qs()} 를
보입니다.
\Clnref{curr,currn} 은 연관된 CPU 의 \co{dynticks} 와 \co{dynticks_nmi}
변수들의 새 스냅샷을 취하며 \clnref{snap,snapn} 은
\co{dynticks_save_progress_counter()} 에 의해 앞서 저장된 스냅샷들을
얻어옵니다.
\Clnref{mb} 는 이어서
\cref{lst:formal:Entering and Exiting Dynticks-Idle Mode,%
lst:formal:NMIs From Dynticks-Idle Mode,%
...
짝을 맞추기 위해 메모리 배리어를 수행합니다.
\Clnrefrange{chk_q:b}{chk_q:e} 는 이어서 이 CPU 가 현재 quiescent state 에
있는지 (\co{curr} 와 \co{curr_nmi} 가 짝수) 아니면 마지막
\co{dynticks_save_progress_counter()} 호출 후 quiescent state 를 하나 지났는지
(\co{dynticks} 와 \co{dynticks_nmi} 가 변경되었는지) 검사합니다.
이 검사들이 이 CPU 는 dyntick-idle quiescent state 를 지났음을 확인한다면
\clnref{cnt} 는 그 사실을 세고 \clnref{ret_1} 은 이 사실의 알림을 리턴합니다.
어느 경우든, \clnref{chk_race} 는 RCU 이 오프라인된 CPU 를 기다리는 결과를 만들
수 있는 레이스 컨디션을 검사합니다.
\end{fcvref}
...
\QuickQuiz{
	이는 여전히 무척 복잡합니다.
	간단하게 CPU 별 비트를 갖는 \co{cpumask_t} 를 가지고 그 비트를 \IRQ\ 나
	NMI 핸들러에 진입할 때 지우고 종료할 때 값 설정하는건 어떨까요?
	...
	이 방법이 기능적으로는 올바르지만 큰 기계에서는 지나친 \IRQ\ 진입/종료
	오버헤드를 초래할 수 있습니다.
	반면에, 이 섹션에서 보인 방법은 각 CPU 가 \IRQ\ 와 NMI 진입/종료 시에
	CPU 별 데이터만 만져서 특히나 큰 기계에서는 \IRQ\ 진입/종료 오버헤드를
	훨씬 낮게 만듭니다.
...
리눅스 커널 RCU 의 dyntick-idle 코드는 이 이후로도 Andy Lutomirski 의
제안에~\cite{PaulMcKenney2015dyntickAndyNMI} 의해 한번 더 재작성 되었습니다만
이젠 정리하고 다음 주제로 넘어갈 시간입니다.
...
관점을 약간 비트는 것이 RCU 를 위한 dynticks 인터페이스의 상당한 단순화를
이루었습니다.
이 단순화를 이끈 핵심 변경사항은 \IRQ\ 와 NMI 컨텍스트 사이에 공유되는 것을
최소화 하는 것이었습니다.
이 단순화된 인터페이스에서의 유일한 공유 정보는 NMI 컨텍스트에서 \IRQ\
변수들로의 (\co{dynticks} 변수) 참조들입니다.
이런 종류의 공유는 괜찮은데, NMI 함수는 이 변수를 결코 업데이트 하지 않아서 그
값은 NMI 핸들러의 생명주기동안 변하지 않기 때문입니다.
이 공유의 제한은 개별 함수들이 한번에 하나씩 이해될 수 있게 해서 NMI 가 \IRQ\
함수의 수행 도중 언제든 공유된 상태를 변경할 수도 있었던
\cref{sec:formal:Promela Parable: dynticks and Preemptible RCU}
에 설명된 상황과는 행복한 대비를 이룹니다.
