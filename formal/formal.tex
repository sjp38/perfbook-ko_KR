...
병렬 알고리즘은 작성하기 어렵고, 디버깅 하기는 그보다도 더 어렵습니다.
테스트가 필수라 하더라도 race condition 은 극단적으로 낮은 발현 확률을 가질 수
...
Section~\ref{sec:formal:State-Space Search}
...
이 챕터에서 설명한 정형적 검증 기술은 작은 병렬 알고리즘들을 검증하는데에는
매우 강력한 도구입니다만, 여러분의 도구상자에 그것만 있어선 안됩니다.
지난 수십년간의 정형적 검증에 대한 노력에도 불구하고, 테스트는 커다란 병렬
...
이를 확실히 하기 위해, 2017년 기준으로 200억개가 넘는 리눅스 커널의 인스턴스가
존재한다고 추정된다는 점을 생각해 보세요.
...
첫번째 버그는 코드 검사나 컴파일러의 진단 기능을 통해 발견되어집니다.
최신의 컴파일러에 의해 제공되며, 지속적으로 세련되어지고 있는 진단 기능이
가벼운 형태의 정형적 검증으로 여겨질 수도 있긴 하지만, 그것을 그런 용어로
부르는건 흔하지 않습니다.
이는 부분적으로는 ``내가 그걸 사용하고 있다면, 그것은 정형적 검증일 수 없다''
...
관계없는 버그들은 black-hat 공격 덕에 최소한의 알맞은 사이즈에 있어서는 갑자기
관계있는 것이 되어버리는 안좋은 습관을 가지고 있습니다.
전체 소프트웨어 가운데 그 분포도를 지속적으로 높여가는 보안에 관련된
소프트웨어에 있어서는 마지막 버그를 찾아내고 고쳐내려는 강한 동기가 존재할 수
있습니다.
테스트는 마지막 버그를 찾아내는 것은 명백히 불가능하므로 정형적 검증에게만
가능한 역할이 존재합니다.
정형적 검증이 그 안으로 적용될 수 있다고만 하면 그런 역할이 있다는
이야기입니다.
이 챕터에서 보였듯이, 현재의 정형적 검증 시스템들은 상당히 제한적입니다.
\iffalse
...
\QuickQuizAnswer{
	...
	있다고 생각했는데, Row Hammer 의 발전은 그렇지 않음을 보였습니다.
	무엇보다도, 그 블랙햇들이 시스템의 DRAM 을 건들 수 있다면, 저수준
	소프트웨어의 모든 부분을 건들 수 있는데, 여기에 RCU 조차도 포함됩니다.
	\iffalse
...
또다른 방법은 정형적 검증이 많은 경우 테스트보다 적용하기가 어렵다는 점을
고려하는 겁니다.
물론 이는 문화적인 측면에서의 이야기일 수 있고, 정형적 검증이 더 많은
사람들에게 친숙해질수록 더 쉽게 전파될 것이라는 희망을 가져볼 수도 있습니다.
그러나, 매우 간단한 테스트 사용은 임의의 거대한 소프트웨어 시스템에서 심각한
버그들을 찾아낼 수 있습니다.
반면에, 정형적 검증을 적용하기 위해 필요한 노력은 시스템의 크기가 증가할수록
극적으로 증가합니다.

전 20년이 넘도록 정형적 검증을 정형적 검증이 효력을 발휘하는 곳에서 필요할
...
\QuickQuiz{}
	L4 마이크로커널의 전체 검증을 생각해 보면, 이런 정형적 검증에 대한
...
\QuickQuizAnswer{
	...
	L4 마이크로커널의 첫번째 전체 검증은 많은 수의 Ph.D.~학생들이 각각
	...
	MongoDB, 그 외의 커다란 많은 것들과 같은 많은 수의 프로젝트들에
	비교하면 매우 작은 것입니다.
...
\item[정의:]	버그가 없는 프로그램은 간단한 프로그램이다.
...
겁니다: (1)~간단한 프로그램의 증가 또는 (2)~신뢰할 수 있는 프로그램의 감소.
물론, 인류의 멀티코어 시스템과 소프트웨어에 대한 의존도의 증가는 간단한
...
하지만, 만약 여러분의 코드가 너무 복잡해서 여러분이 정형적 검증 도구에 너무
과하게 의존하고 있다는 점을 알게 된다면, 여러분은 여러분의 설계를 다시 세심하게
생각해 봐야 하는데, 당신의 정형적 검증 도구들이 당신의 코드가 특정 목적 언어로
...
그리고 정형적 검증 테크닉들과 시스템들에 대한 열려있는 도전은 이 요약 내용이
틀리다고 증명하는 것입니다!
