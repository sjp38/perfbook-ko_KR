...
\QuickQuiz{}
	특정 상황에서 어떤 메모리 배리어가 충분히 강력한지 어떻게 이야기 할 수
	있나요?
	\iffalse
...
Listing~\ref{lst:memorder:Memory Ordering: Store-Buffering Litmus Test}
은 이 점에서의 한 예입니다.
Line~10 과~20 에서의 \co{smp_mb()} 는 배리어의 역할을, line~9 에서의 \co{x0}
...
비록
Figure~\ref{fig:memorder:Memory Barriers Provide Conditional If-Then Ordering}
가 구체적으로 메모리 배리어를 언급하지만, 리눅스 커널의 나머지 순서 보장
오퍼레이션들에도 같은 규칙이 적용됩니다.
\iffalse
...
이 리트머스 테스트들은 또한
Table~\ref{tab:memorder:Linux-Kernel Memory-Ordering Cheat Sheet}
로 보여진 리눅스 커널 메모리 순서규칙 커닝 페이퍼로 나타내어지는 개념들을
명확하게 할 것이고, 올바른 도구들을
...
함정들로부터 배운 더 정교한 경험상의 법칙들과 함께 이 커닝 페이퍼를 다시 한번
들여다 볼 겁니다.
\iffalse
...
고려해 봅시다.
이 코드 조각은 여러 CPU 에서 병렬적으로 수행됩니다.
...
\QuickQuizAnswer{
	...
	마지막으로 동의된 값을 보게 될 것이라 가정합니다.
	실제 하드웨어에서, 어떤 CPU 는 이 마지막 값을 보게 되기 전에 중간의
	결과를 일부 보게 될수도 있습니다.
	따라서 이 섹션의 뒤에서 이야기될 그림의 데이터를 만드는데 사용된 실제
	코드는 좀 더 복잡합니다.
	\iffalse
...
순서보장도 없습니다.
다시 한번 말하지만, 상대적으로 강력한 순서 규칙의 아키텍쳐는 순서를
지켜주지만, 완화된 순서 규칙의 아키텍쳐는 그러지 않기
...
경우입니다.
이 상황에서 스토어들이 비순차적으로 완료될 수 있게 하면 수행이 진행될 수
있을 겁니다.
따라서, 이식성 있는 코드는
...
\QuickQuiz{}
	강력한 순서규칙의 시스템은 불필요한 \co{smp_rmb()} 와 \co{smp_wmb()}
	호출로 인한 성능 비용을 왜 지불해야 하죠?
	완화된 순서규칙의 시스템은 자신의 순서를 지켜주지 않는 선택으로
...
경우에는 필요했던 \co{lockless_dereference()} 없이도 line~25 의 \co{exists}
절이 발동될 수 없음을 의미합니다.
\iffalse
...
\QuickQuizAnswer{
	...
	배리어를 가지고 있거나 (v4.15 이후 리눅스 커널에서의 DEC Alpha),
...
나온다면, \co{P3()} 가 \co{P1()} 의 스토어가 먼저 행해졌다고 믿는 결과는 없어야
합니다.
...
\QuickQuiz{}
	...
	\co{r1} 과 \co{r2} 가 값 1 과 2 를 각각 얻게 되는것 역시 만만치 않게
	나쁘지 않을까요?
	\iffalse
...
하나의 메모리 영역에 대한 다른 크기의 겹치는 로드와 스토어는 (C-언어의
\co{union} 키워드를 통해 이뤄질 수 있을 겁니다) 비슷한 순서 보장을 제공할거라
생각하고 싶을 겁니다.
하지만, Flur 등은 이런 보장이 실제 하드웨어 위에서 지켜지지 않을 수 있다는 것을
...
Cumulative 메모리 순서 강제 오퍼레이션은 그 앞의 특정 액세스들만이 아니라 같은
...
\QuickQuiz{}
	하지만 이 리트머스 테스트에 세개의 쓰레드가 존재하지 않는다면 전파에
	대해서는 걱정할 필요가 없을 거예요, 그렇죠?
	\iffalse
...
하지만, \co{P0()} 의 \co{smp_store_release()} (line~\lnref{P0:rel}) 는
cumulativity 를 가지므로, \co{P3()} 의 \co{READ_ONCE()} 가 0 을 리턴한다면, 이
cumulativity 는 이 \co{READ_ONCE()} 가 \co{P0()} 의 \co{smp_store_release()}
앞으로 순서지어지게 강제할 겁니다.
또한, 이 release-acquire 체인 (line~\lnref{P0:rel}, \lnref{P1:acq},
\lnref{P1:rel}, \lnref{P2:acq}, \lnref{P2:rel}, 그리고~\lnref{P3:acq}) 은
...
되어있고, 의지도 있고, 그럴 능력을 가지고 있음을 의미합니다.
...
\item	평범한 액세스는 찢어질 (tear) 수 있는데, 예를 들어 컴파일러는 8 바이트
	...
	이 찢기는 \co{READ_ONCE()} 와 \co{WRITE_ONCE()} 를 사용해 방지될 수
	있습니다.
...
\item	Plain accesses can be reordered in surprising ways by modern
	...
	called out above.
\fi
\item	평범한 로드는 창조어질 수 있는데, 예를 들어 레지스터 부족은 컴파일러가
...
\QuickQuizAnswer{
	...
	하지만 메모리 순서 규칙에 더 관계있게 말하면, 컴파일러는 이 비교를
...
\fi
\item	충분한 종속성을 가지고 있는 두 포인터간의 비교.
	예를 들어, 두개의 포인터를 가지고 있는데 각 포인터는 종속성을 가지고
	있으며, 포인터가 가리키는 데이터 원소의 주소 순서대로 락을 잡음으로써
	데드락을 회피하는 경우.
...
이 예제는 두 쓰레드, line~9-22 에 보인 \co{updater()} 와 line~24-39 의
\co{reader()} 를 사용합니다.
\iffalse
...
있기 때문에 특히나 교묘합니다.
이 섹션의 규칙과 예시들로 여러분이 컴파일러가 코드를 깨버리는 행위를 막는 걸
돕고자 합니다.
...
CPU 들이 \co{y} 로부터의 로드를 \co{x} 로부터의 로드보다 먼저 일어난 걸로 보게
...
\qco{if} 문의 양갈래 경우의 동일한 스토어에 다음과 같이 순서를 강제하고 싶을 수
있을 겁니다:
\iffalse
...
여기서 \co{MAX} 가 1로 정의되어 있다면, 컴파일러는 \co{(q\%MAX)} 가 0과 같다는
...
두 스토어가 동일하다면, 앞서 언급한 것처럼, 컴파일러는 이 스토어를 \qco{if}
문 바깥으로 끌어올릴 수 있습니다.
...
마지막으로, control dependency 는 cumulativity 를 제공하지
\emph{않습니다}.\footnote{
...
의 두개 쓰레드 예제의 \co{exists} 절은 결코 발동되지 않습니다.
만약 control dependency 가 (보장하지 않는) cumulativity 를 보장한다면,
Listing~\ref{lst:memorder:WWC Litmus Test With Control Dependency (Cumulativity?)}
(\path{C-WWC+o-cgt-o+o-cgt-o+o.litmus})
에서와 같이 이 예제에 쓰레드를 추가하는게 관련된 \co{exists} 절이 발동하지 않게
해줄 수 있을 겁니다.
\iffalse
...
하지만 control dependency 는 cumulativity 를 제공하지 \emph{않기} 때문에, 세개
쓰레드 리트머스 테스트의 \co{exists} 절은 발동될 수 있습니다.
이 세개 쓰레드 예제가 순서를 지키게 해야 한다면, 여러분은 \co{P0()} 의 로드와
스토어 사이에, 즉 \qco{if} 절 바로 뒤에 \co{smp_mb()} 를 추가해야 합니다.
더 나아가서, 원래의 두개 쓰레드 예제는 매우 깨지기 쉬우므로 사용되지 않아야
합니다.
\iffalse
...
다음 리스트의 규칙들은 이 섹션에서의 교훈들을 요약합니다:
\iffalse
...
의 quick quiz 중 하나에 대한 대답은 더 높은 단계의 추상화로 모델링된
프로그램의 검증으로 인한 폭발적 속도 증가를 보였습니다.
이 섹션은 더 높은 단계의 추상화가 어떻게 동기화 기능 자체에 대한 깊은 이해도
제공하는지 알아봅니다.
...
여기서 핵심 요구사항은 특정 메모리 블락이 해제되기 전에 해당 블락에 수행된 모든
...
대한 이유이기도 합니다.
어쨌건, CPU 로컬의 경우, 할당자는 어떤 순서도 제공하지 않아도 됩니다.
\iffalse
...
하지만, 이 정의는 완전하지 않은데, 아래와 같은 질문들\footnote{
	이 중 몇개는 LKMM 초기 작업 중에 Jade Alglave 에 의해 Paul 에게
...
\item	\co{synchronize_rcu()} 와 \co{synchronize_rcu_expedited()} 는 RCU
	read-side 크리티컬 섹션과 관계 없이는 어떤 순서를 제공하는가?
\item	특정 RCU read-side 크리티컬 섹션의 전체 부분이 특정 RCU grace period 의
	끝을 앞선다면, 이 크리티컬 섹션을 앞서는 액세스들은 어떤가?
\item	특정 RCU read-side 크리티컬 섹션의 전체 부분이 특정 RCU grace period 의 
	시작 뒤에 있다면, 이 크리티컬 섹션을 뒤따르는 액세스들은 어떤가?
\iffalse
...
\item	하나 이상의 RCU read-side 크리티컬 섹션과/또는 하나 이상의 RCU grace
	period 가 연관된 상황에서는 어떤 일이 벌어지는가?
\item	RCU 가 다른 메모리 순서 메커니즘들과 엮이면 무슨 일이 벌어지는가?
\iffalse
...
RCU 의 읽기 쪽 기능인 \co{rcu_read_lock()} 과 \co{rcu_read_unlock()} 은 그들
스스로는 어떤 순서 규칙도 제공하지 않습니다.
특히, 그 이름에도 불구하고,
Listing~\ref{lst:memorder:RCU Readers Provide No Lock-Like Ordering}
(\path{C-LB+rl-o-o-rul+rl-o-o-rul.litmus})
에서 보이듯, 이것들은 락처럭 동작하지도 않습니다.
이 리트머스 테스트에 \co{herd} 를 돌리면 볼 수 있듯, 이 리트머스 테스트의
사이클은 허용되어 있습니다: \co{rc1} 레지스터의 두 인스턴스는 마지막 값 1을
가질 수 있습니다.
\iffalse
...
에서 보이는 것처럼 grace period 와 섞이거나 하지 않은 한은 배리어 같은 순서
속성을 갖지 않습니다.
이 리트머스 테스트의 사이클은 허용되어 있는데, \co{herd} 로 이 리트머스
테스트를 돌리면 검증할 수 있습니다.

물론, 이 두개의 리트머스 테스트에서의 순서규칙 부재는 RCU 의 QSBR 구현에서
\co{rcu_read_lock()} 과 \co{rcu_read_unlock()} 이 no-op 이란 걸 고려하면 전혀
놀라운 일이 아닙니다.
\iffalse
...
RCU 읽기 쓰레드와 대조적으로, RCU 업데이트쪽 함수들인 \co{synchronize_rcu()} 와
\co{synchronize_rcu_expedited()} 는 \co{smp_mb()} 만큼이나 강력한 메모리 순서를
제공하는데,
Listing~\ref{lst:memorder:RCU Updaters Provide Full Ordering}
의 리트머스 테스트를 \co{herd} 에서 돌려보면 볼 수 있습니다.
이 테스트의 사이클은 \co{smp_mb()} 의 경우처럼 금지되어 있습니다.
Table~\ref{tab:memorder:Linux-Kernel Memory-Ordering Cheat Sheet}
이 제공한 정보를 생각하면 이는 놀라운 일이 아닙니다.
\iffalse
...
이 섹션을 읽기 전에, 사용 가능한 보장들과 유지 가능한 소프트웨어가 의존해야
하는 보장들 사이의 차이를 생각해 봅시다.
이를 기억해 두고, 이 섹션은 보다 이국적인 RCU 보장사항들을 몇가지 보입니다.
\iffalse
...
은
Listing~\ref{lst:memorder:RCU Fundamental Property}
의 것과 유사한 리트머스 테스트를 보입니다만, RCU 읽기 쓰레드의 첫 읽기는 RCU
read-side 크리티컬 섹션을 앞서는데, 이는 읽기를 크리티컬 섹션 안에 두는
관례적인 (그리고 유지가능한!) 방법과는 다른 것입니다.
아마도 놀랍겠지만, 이 리트머스 테스트를 \co{herd} 에서 돌리면
Listing~\ref{lst:memorder:RCU Fundamental Property}
에서와 같은 결과가 나옵니다:
사이클이 나타나지 않습니다.
...
\co{P1()} 의 액세스는 둘 다 volatile 이기 때문에,
Section~\ref{sec:toolsoftrade:A Volatile Solution}
에서 이야기 된 바와 같이, 컴파일러는 이것들을 재배치 하지 못합니다.
이는 \co{P1()} 의 \co{WRITE_ONCE()} 가 만들어낸 코드는 \co{P1()} 의
\co{READ_ONCE()} 를 앞섭니다.
따라서, 메모리 배리어 명령어를 \co{rcu_read_lock()} 과 \co{rcu_read_unlock()}
안에 위치시키는 RCU 구현은 \co{P1()} 의 두 액세스의 순서를 하드웨어 레벨까지
지켜줄 겁니다.
한편, 인터럽트 기반의 상태 머신에 의존하는 RCU 구현 역시 이 순서를 \emps{grace
period 에 상대적으로} 지켜줄 텐데, 인터럽트는 인터럽트된 코드의 수행 사이에
정확히 자리하기 때문입니다.
\iffalse
...
이는 결국 \co{WRITE_ONCE()} 가 특정 RCU grace period 의 종료 뒤에 온다면, 이
RCU read-side critical section 안에서의 액세스는 같은 grace period 의 시작을
뒤따라야만 합니다.
사실, 일부 RCU 구현에서는, 이 크리티컬 섹션의 액세스들은 (이 구현에서의
사고로!) 또한 이 grace period 의 처음만이 아니라 종료를 뒤따를 겁니다.
비슷하게, 이 critical section 안의, 그리고 \emph{앞서는} 모든 것이 같은 grace
period 의 종료를 앞서야만 합니다.
\iffalse
...
은 유사하지만, 이 RCU read-side critical section 뒤에 액세스들을 봅니다.
이 테스트의 사이클 역시 금지되어 있는데, \co{herd} 툴로 확인할 수 있습니다.
그 이유는
Listing~\ref{lst:memorder:What Happens Before RCU Readers?}
에서의 것과 유사하며 독자 여러분의 몫으로 두겠습니다.
\iffalse
...
은 이걸 한발짝 더 나아가서, \co{P1()} 의 \co{WRITE_ONCE()} 를 RCU read-side
크리티컬 섹션을 앞서게 옮기고 \co{P1()} 의 \co{READ_ONCE()} 가 이를 앞서게
해서, 텅 빈 RCU read-side 크리티컬 섹션을 만듭니다.
\iffalse
...
아마도 놀랍겠지만, 이 텅 빈 크리티컬 섹션에도 불구하고, RCU 는 여전히 사이클을
막습니다.
이는 역시 \co{herd} 를 통해 체크될 수 있습니다.
더 나아가서, 그 이유는
Listing~\ref{lst:memorder:What Happens Before RCU Readers?} 과 같은데, 다시
말하자면, \co{P1()} 의 \co{WRITE_ONCE()} 가 주어진 grace period 의 끝을
뒤따른다면 \co{P1()} 의 RCU read-side 크리티컬 섹션은---그리고 그것을 뒤따르는
모든 것들은---이 grace period 의 시작을 뒤따라야 합니다.
유사하게, \co{P1()} 의 \co{READ_ONCE()} 가 특정 grace period 의 시작을
앞선다면, \co{P1()} 의 RCU read-side 크리티컬 섹션은---그리고 그걸 앞서는
모든것들은---이 grace period 의 끝을 앞서야 합니다.
두 경우 모두, 크리티컬 섹션의 텅비어있음은 관계 없습니다.
\iffalse
...
\QuickQuiz{}
	잠깐만요!
	RCU 의 QSBR 구현에서는 \co{rcu_read_lock()} \co{rcu_read_unlock()} 모두
	어떤 코드도 만들어내지 않습니다.
	이 말은
	Listing~\ref{lst:memorder:What Happens With Empty RCU Readers?}
	의 RCU read-side 크리티컬 섹션이 단순히 비어있을 뿐 아니라 완전
	존재하지 않는단 뜻이예요!!!
	그런데 어떻게 존재하지도 않는 것이 순서에 어떤 영향을 줄 수 있는거죠???
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	QSBR 내에서, RCU read-side 크리티컬 섹션은 실제로 사라지지 않기
	때문입니다.
	대신, 그것들은 quiescent state 를 만날 때까지 양방향으로 확장됩니다.
	예를 들어, 리눅스 커널에서, 이 크리티컬 섹션은 \co{schedule()} 호출
	한쌍까지 확장되는데, 하나는 이 크리티컬 섹션을 앞서고 하나는 뒤따르는
	데까지이죠.\footnote{
		물론, QSBR 구현이 아닌 곳에서 \co{rcu_read_lock()} 과
		\co{rcu_read_unlock()} 은 진짜로 코드를 만들어낼 수 있는데, 이
		때엔 분명한 순서를 제공합니다.}
	\ifflase
	...
	따라서, 이상하게 보이겠지만, 텅 빈 RCU read-side 크리티컬 섹션은 정말로
	일부 정도의 순서 규칙을 제공합니다.
	\iffalse
...
에 보인 것과 같이 \co{rcu_read_lock()} 과 \co{rcu_read_unlock()} 이 완전히
사라지면 무슨 일이 일어나는지에 대한 질문을 일으킵니다.
\co{herd} 로 체크되겠지만, 이 리트머스 테스트의 사이클은 허용되어 있는데, 즉,
\co{r2} 의 인스턴스 두개가 최종값 0을 가질 수 있습니다.
\iffalse
...
이는 텅빈 RCU read-side 크리티컬 섹션이 순서를 제공할 수 있다는 사실을 놓고 볼
때 이상하게 보일 수 있습니다.
그리고 RCU 의 QSBR 구현은 실제로 이 결과를 막을 것이라는 것도 사실입니다.
하지만, RCU 는 또한 QSBR 이 아닌 구현도 가지고 있으며, 이런 구현들은 이 경우에
순서를 강제할 방법이 없습니다.
따라서, 다시 말하지만, 이 리트머스 테스트의 사이클은 허용됩니다.
\iffalse
...
리트머스 테스트가 하나의 RCU grace period 와 두 RCU 읽기 쓰레드를 갖는다면 어떤
일이 벌어질까요?
...
있습니다.
이 세번째 읽기 쓰레드는 \co{P0()} 의 grace period 의 종료 전에 끝날 수 있고,
...
우리가 지금까지 알아본 일반적 규칙은 사이클을 방지하는데 하나의 메커니즘만을
사용한 경우에 대한 겁니다.
\iffalse
...
요약해서, RCU 의 semantic 이 한때는 순수히 실용적이었지만, 이제는 완전히 정형화
...
각각의 CPU 는 메모리 순서 규칙을 위한 각자의 특수한 방법을 가지고 있는데, 이는
...
Table~\ref{tab:memorder:Summary of Memory Ordering} 로 돌아가서, 첫번째 무리의
행들은 메모리 순서 보장 속성들을 보이고 두번째 행들은 인스트럭션 속성들을
보입니다.
...
마지막 세줄은 인스트럭션 수준에서의 선택과 문제들을 다룹니다.
첫번째 줄은 각 CPU 가 어떻게 load-acquire 와 store-release 를 구현했는지
보이고, 두번째 줄은 CPU 를 atomic-instruction 타입으로 구분하고, 세번째와
...
\item	[\tco{smp_mb__after_atomic()}] 앞의 RMW atomic 오퍼레이션을 앞지르는
	액세스들을 \co{smp_mb__after_atomic()} 을 뒤따르는 오퍼레이션에 대해
	순서지음.
...
재배치해서, 기기가 이상하게 동작하거나, 심지어 여러분의 커널에, 심지어는
...
따라서 이런 도구들을 올바르게 사용하는 코드를 가지고 일하고 있다면, 리눅스의
...
각 CPU 의 메모리 일관성 모델에 대한 이해를 원하는 분들을 위해, 다음 섹션들은
...
수명이 다한지 오래된 CPU 에 대해 이야기하는게 좀 이상하게 보일수도
...
Alpha 는 실제로
Listing~\ref{lst:memorder:Insert and Lock-Free Search} line~20 의 코드가
...
line~6-8 에 의해 일어난 (\co{p->next}, \co{p->key}, \co{p->data}) 캐시 무효화가
...
순서대로 읽게 강제하는 소프트웨어 메커니즘을 구현할 수도 있을 겁니다.
...
은 다른 메모리 배리어 명령어 집합을 사용합니다~\cite{ARMv7A:2010}:
\iffalse
...
\item	[\tco{DMB}] (data memory barrier) 는 특정 타입의 오퍼레이션들이 같은
	...
	합니다.
	오퍼레이션의 ``타입'' 은 모든 오퍼레이션 또는 쓰기만으로 제한될 수
...
\item	[\tco{ISB}] (instruction synchronization barrier) 는 CPU 파이프라인을
	...
	만들었다면, 해당 코드를 만들고 수행하기 전에 \co{ISB} 를 수행해야
	합니다.
\iffalse
...
\co{STLLRH}, 그리고 \co{STLLR}) 인스트럭션을 갖습니다.
...
중 하나인데, 이는 리눅스 커널의 상대적으로 완화된 락 획득 구현 때문입니다.
...
이런 반쪽짜리 memory fence 는 크리티컬 섹션에 유용한데, 오퍼레이션을 크리티컬
섹션 안으로 들여보내는건 안전하지만 크리티컬 섹션 밖으로 내보내는건 치명적일 수
있기 때문입니다.
...
인스트럭션을 완전한 배리어로 구현했다는 소문이 있습니다.
...
마지막으로, Itanium 은 ``mf'' 인스트럭션을 포함해 ``release'' 오퍼레이션에
글로벌하고 완전한 순서를 제공합니다.
이는 transitivity 를 제공하는 것으로, 특정 코드 조각이 특정 액세스를 이미
일어난 것으로 파악했다면, 뒤따르는 모든 코드 조각 역시 이 액세스가 이미 일어난
것으로 보게 됨을 의미합니다.
다시 말하지만, 모든 관련된 코드 조각이 올바르게 메모리 배리어를 사용했다는 가정
하의 이야기 입니다.
\iffalse
