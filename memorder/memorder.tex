...
은 메모리 순서규칙과 메모리 배리어들에 대한 짧은 소개를 제공합니다.
일단 이 배경지식이 자리잡으면, 다음 단계는 여러분이 여러분의 직관에 문제가
있음을 인정할 차례입니다.
이 고통스러운 작업은
Section~\ref{sec:memorder:Variables Can Have More Than One Value} 를 통해
이루어는데, 여기선 동시에 다양한 값들을 취할 수 있는 scalar 변수들을 포함하는
코드를 일부 보이며,
Section~\ref{sec:memorder:If B Follows A, and C Follows B, Why Doesn't C Follow A?}
에서는 직관적으로는 옳은 것으로 보이지만 실제 하드웨어에서는 비참하게 실패해
버리는 코드를 보입니다.
일단 여러분의 직관이 이 슬픈 과정을 통해 일단 제대로 자리잡히면,
Section~\ref{sec:memorder:What Can You Trust?} 에서는 메모리 배리어들이 따르는
기본적 규칙들을 제공하는데, 이게 우리가 의지해야할 규칙들입니다.
이 규칙들은
Sections~\ref{sec:memorder:Review of Locking Implementations}
부터~\ref{sec:memorder:Where Are Memory Barriers Needed?} 를 통해 더 자세히
설명됩니다.
\iffalse
...
\QuickQuizAnswer{
	...
	생각하신다면,
	Section~\ref{sec:memorder:Variables Can Have More Than One Value} 까지
	잠깐 기다려 주시기 바랍니다.
	\iffalse
...
실제 하드웨어는 훨씬 더 복잡하고 교묘하며 여기서 설명된 것보다 더 미묘하다는 걸
부디 알아두시기 바랍니다.
무엇보다도, 이 논의의 핵심은 여러분을 하드웨어 설계자로 훈련시키려는 게 아니라,
여러분이 올바르게 낮은 단계의 동시성 도구들을 짤 수 있게 돕는 겁니다.
하지만 먼저, 하나의 변수가 한 순간에 얼마나 많은 값들을 가지고 있을 수 있는지
빠르게 보겠습니다.
\iffalse
