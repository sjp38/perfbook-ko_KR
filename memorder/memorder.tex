...
Section~\ref{sec:memorder:Care and Feeding} 에서 우리가 근간으로 삼게 될,
...
이 테스트에서, \co{smp_wmb()} 는 \co{P0()} 의 쓰기들이 순서대로 행해지게
강제하지만, 읽기들에 대해서는 어떤 순서도 강제하지 않습니다.
...
의 line~20 에 보인 \co{smp_rmb()} 를 추가하는 식입니다.
여기서는 \co{exists} 절이 발동될 수 없습니다.
\iffalse
...
흥미롭게도, 실제 하드웨어가 이런 재배치를 하는건 비교적 드문
일입니다~\cite{LucMaranget2017aarch64}.
그렇다 하더라도, 여러분은
Listing~\ref{lst:memorder:Enforcing Ordering of Load-Buffering Litmus Test}
(\path{C-LB+o-r+a-o.litmus})
에 보인 것과 같이 line~22 의 \co{exists} 절이 절대 발동되지 않도록 필요한 순서
강제를 해야 합니다.
\iffalse
...
로의 쓰기들에 대한 명시적 순서가 없고 \co{P1()} 의 읽기들에 대한 순서가
...
보인 것과 같이 쓰기들에도 명시적 순서를 부여해서 \co{exists} 절이 발동되지 않게
해야 합니다.
\iffalse
