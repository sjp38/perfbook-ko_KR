...
\QuickQuizAnswer{
	...
	했습니다.
	그때 후로, C 와 C++ 메모리 모델~\cite{PeteBecker2010N3225} 가 정형화
	되었고 (그리고
	...
	컴퓨터 시스템들을 위한 실행될 수 있는 정식 메모리 모델들이 표준이
...
은 어떻게 하면 이걸 DEC Alpha 에서 돌아가는 오래된 리눅스 커널에서 안정적으로
...
useful when the surrounding code has consumed all of the
machine registers.
...
\QuickQuizAnswer{
	...
	해서는 안될 일을 할수도 있습니다.
	그리고 만약 이 코드가 이미 데이터 레이스를 가지고 있다면, 개발자의
	바램과는 상관없이 컴파일러가 원하는대로 어떤 코드든 만들어낼 수 있는,
	비정의된 행동의 무서운 유령을 이미 불러낸 것입니다.
...
\item	Control dependencies apply only to the ``\co{then}'' and
	...
	end of the ``\co{if}'' statement containing the control dependency.
...
or RCU, and to let those primitives take care of memory ordering for you.
\fi
