...
\item	만약 어떤 공유 변수가 특정 주인 CPU 나 쓰레드에 의해 특정 락을 잡고
	있는 동안만 수정된다면, 모든 스토어는 \co{WRITE_ONCE()} 를 사용해야
	하고, 이 락을 잡고 있지 않은 주인이 아닌 CPU 또는 쓰레드들은 로드를
	위해 \co{READ_ONCE()} 를 사용해야만 합니다.
	다른 CPU 나 쓰레드가 해당 락을 잡게 되므로, 주인 CPU 또는 쓰레드는
	평범한 로드를 써도 됩니다.
\item	공유 변수가 특정 락을 잡고 있는 동안만 수정된다면, 모든 스토어가
	\co{WRITE_ONCE()} 를 사용해야 합니다.
	이 락을 잡지 않고 있는 CPU 또는 쓰레드는 로드를 위해 \co{READ_ONCE()}
	를 사용해야 합니다.
...
\item	공유 변수가 주인 CPU 또는 스레드에 의해서만 수정된다면, 모든 스토어는
	\co{WRITE_ONCE()} 를 사용해야 하고 주인이 아닌 CPU 또는 쓰레드는 로드를
...
대부분의 경우, 공유 변수로의 로드와 스토어는 반드시 \co{READ_ONCE()} 와
\co{WRITE_ONCE()} 또는 이보다 강한 것들을 각각 사용해야 합니다.
...
한가지 중요한 특수 케이스는 특정 변수가 프로세스 수준 코드로 공유되면서
언터럽트나 시그널 핸들러에 의해서도 공유되지만, 다른 CPU 또는 쓰레드에 의해서는
접근되지 않는 경우입니다.
이 경우, 프로세스 수준 코드는 이 변수에 접근하기 위해 \co{READ_ONCE()} 와
\co{WRITE_ONCE()} 또는 그보다 강한 것들을 사용해야 합니다.
\iffalse
...
\QuickQuiz{}
	특정 인터럽트나 시그널 핸들러가 그 스스로 인터럽트 되면 어떻게 되나요?
	\iffalse
