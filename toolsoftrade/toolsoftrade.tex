...
리턴합니다.
\co{fork()} 가 리턴하는 값은
Listing~\ref{lst:toolsoftrade:Using the fork() Primitive}(\co{forkjoin.c}) 에
...
그래서
Listing~\ref{lst:toolsoftrade:Using the wait() Primitive}(\path{api-pthread.h})
...
(\path{forkjoinvar.c}) 에 나타난 프로그램에 보여지는데, 자식 프로세스는 라인~6
...
\QuickQuizAnswer{
	...
	Listing~\ref{lst:toolsoftrade:Processes Created Via fork() Do Not Share
...
\QuickQuiz{}
	Listing~\ref{lst:toolsoftrade:Threads Created Via pthread-create() Share
...
\QuickQuizAnswer{
	...
	더도 아니고 덜도 아니고, 리눅스 커널의 경우에 있어서도, 데이터 경주
	상황은 상당히 위험하고 가능하다면 막아져야
	합니다~\cite{JonCorbet2012ACCESS:ONCE}.
...
Listing~\ref{lst:toolsoftrade:Demonstration of Exclusive Locks} 의 라인~31-49 는
...
\QuickQuiz{}
	``x = 0'' 만이 Listing~\ref{lst:toolsoftrade:Demonstration of Same
...
POSIX API 는 \co{pthread_rwlock_t} 로 사용되는, 리더-라이터 락을 제공합니다.
...
\co{pthread_rwlock_rdlock()} 함수는 특정 \co{pthread_rwlock_t} 의 읽기 권한을
얻어오고, \co{pthread_rwlock_wrlock()} 함수는 쓰기 권한을 얻어오며,
\co{pthread_rwlock_unlock()} 함수는 락을 해제합니다.
언제든 \co{pthread_rwlock_t} 의 쓰기권한은 하나의 쓰레드만이 획득 가능하며,
...
해당 리더-라이터 락을 잡는 시간을 조절하는 \co{holdtime} 인자를 보여줍니다.
라인~3 에서는 리더-라이터 락의 해제와 다음 획득 사이의 시간을 조절하는
\co{thinktime} 인자를 보여주고, 라인~4 에서는 각 리더 쓰레드가 자신이 락을
획득한 횟수를 저장하는 \co{readcounts} 배열을 정의합니다. 그리고 라인~5 에서는
언제 모든 리더 쓰레드들이 수행을 시작했는지 알려주는 \co{nreadersrunning}
변수를 정의합니다.
...
라인~7-10 은 테스트의 시작과 끝을 맞춰주는 \co{goflag} 를 정의합니다.
이 변수는 처음엔 \co{GOFLAG_INIT} 로 설정되어 있고, 이후에 모든 리더 쓰레드들이
시작된 이후에 \co{GOFLAG_RUN} 으로 변경된 후, 마지막으로 테스트를 종료시키기
위해 \co{GOFLAG_STOP} 으로 값을 바꿉니다.
...
(그리고 \co{barrier()} 지시어가 컴파일러가 루프를 없애는 것을 막습니다),
라인~30-33 에서 락을 풀어주고, 라인~34-36 에서 락을 다시 얻어오기 전에 특정
시간동안 기다립니다.
라인~37 은 이 락 획득 횟수를 셉니다.

라인~39 에서는 해당 락 획득 횟수를 \co{readcounts[]} 배열의 해당 쓰레드의
원소에 저장하고, 라인~40 에서는 리턴해서 쓰레드를 종료합니다.
...
\QuickQuiz{}
	Figure~\ref{fig:toolsoftrade:Reader-Writer Lock Scalability} 에서 100M 에서의
...
대기시간이 긴 파일이나 네트워크 I/O 를 하는 경우 등에 매우 유용합니다.
...
라인~18 에서 \co{__sync_fetch_and_add()} 를 통해 어토믹 오퍼레이션 하나를 본 바
있습니다.
...
{\sf gcc} 컴파일러는 이외에도 아래와 같이 추가적인 어토믹 오퍼레이션들을
제공합니다.
먼저
...
고전의 compare-and-swap 오퍼레이션은 \co{__sync_bool_compare_and_swap()} 과
\co{__sync_val_compare_and_swap()} 두개의 함수로 제공됩니다.
두 함수 모두 어토믹하게 변수의 현재 값이 제시한 값과 같을 경우 새로운 값으로
변경해 줍니다.
...
Chapter~\ref{chp:memorder:Memory Ordering} 에서
...
이 마지막 두개의 함수들은 \GCC 에 의해 제공되지는 않습니다만 다음과 같이 구현될
수 있을 겁니다:
...
커널의 ``\co{READ_ONCE()}'' 와 대략적으로 유사합니다.\footnote{
	메모리 순서 규칙은
	Chapter~\ref{chp:memorder:Memory Ordering} 와
...
변수에 값을 지정하기 위해 \co{pthread_setspecific()} 을, 그리고 그 값을
리턴받기 위해 \co{pthread_getspecific()} 을 제공합니다.
\iffalse
...
지정하기 위해 사용될 수 있는 \co{__thread} 지시어를 제공합니다.
...
예를 들어, \GCC \co{__sync_} 류 함수들은 메모리 순서 의미를 제공해서 많은
...
\co{pthrad_t} 로부터 연속적인 정수들 사이의 매핑을 초기화하는 \co{smp_init()}
함수의 호출로 시작합니다.
...
쓰레드가 쓰레드 생성 시에 \co{rcu_register_thread()} 를, 그리고 쓰레드 종료
...
리눅스 커널은 kthread 들을 쫓는데에 \co{struct task_struct} 포인터를, 그것들을
생성하는데 \co{kthread_create()} 를, 그것들이 멈출 것을 외부로 제안하는데에
(POSIX 에는 똑같은 일을 하는 게 없습니다) \co{kthread_should_stop()} 을,
그것들이 멈추길 기다리는데에 \co{kthread_stop()} 을, 그리고 시간이 지정된
대기를 위해서 \co{schedule_timeout_interruptible()} 을 사용합니다.
