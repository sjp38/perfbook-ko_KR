...
C 표준이공유된 변수로의 동시의 read/write 액세스에 대해 어떤 일이 벌어지는지
정의한 건 2011년에 와서입니다만, 동시성 있는 C 코드는 그보다 최소 25년은 전부터
...
\QuickQuizAnswer{
	...
	저는 1990년대초 DYNIX/ptx 커널의 메모리 할당자에 정확히 이 실수를
	했습니다.
	이 버그를 추적하는데 저만이 아니라 제 동료들도 연휴 주말을 보냈습니다.
	요약하자면, 이건 새로운 문제도, 스스로 사라질 문제도 아닙니다.
	\iffalse
...
만들어내기 (invented store), 스토어를 로드로 바꾸기 (store-to-load
transformation), 그리고 안쓰이는 코드 제거하기 (dead-code elimination) 와 같은
것들이 포함되며 싱글 쓰레드 코드에서는 잘 동작합니다.
...
이건 엄격한 이론적 문제도 아닙니다.
예를 들어, 작은 immediate 값을 직접 메모리에 쓸 수 있는 CPU 들이 존재하고, 그런
CPU 에서 컴파일러는 명시적으로 32-bit 상수를 만드는데 드는 오버헤드를 없애기
위해 이를 16비트 스토어들로 쪼갤 거라 예상될 수 있습니다.
더 나아가서, C 표준은 32비트 시스템에서 64비트 정수형을 사용하는 코드 등의
가능성으로 인해, 이 일반적 경우에 대해 선택이 없습니다.
\iffalse
...
For example, there are CPUs that can store small immediate values
...
of explicitly forming the 32-bit constant.\footnote{
	One such CPU is the rare and elusive x86:
	\url{https://gcc.gnu.org/bugzilla/show_bug.cgi?id=55981}.}
Furthermore, the C standard simply has no choice in the general case, given
...
\co{t0()} 와 \co{t1()} 은 동시에 수행되고, \co{do_something()} 과
\co{do_something_else()} 는 인라인 함수입니다.
라인~\lnref{gp} 는 포인터 \co{gp} 를 선언하는데, C 는 이를 기본값 \co{NULL} 로
초기화 시킵니다.
어떤 시점에서, \co{t0()} 의 라인~\lnref{wgp} 에서는 \co{NULL} 이 아닌 값을
\co{gp} 에 저장합니다.
그사이 \co{t1()} 은 라인~\lnref{p1}, \lnref{p2}, 그리고~\lnref{p3} 에서 \co{gp}
로부터의 로드를 세번 행합니다.
라인~\lnref{if} 는 \co{gp} 가 \co{NULL} 이 아니란 것을 확인했으므로, 대부분
라인~\lnref{p3} 에서의 디레퍼런스가 잘못되지 않을 거라 믿을 겁니다.
불행히도, 컴파일러는 라인~\lnref{p1} 과~\lnref{p3} 에서의 읽기를 합칠 수
있는데, 이는 라인~\lnref{p1} 이 \co{NULL} 을 읽어오고 라인~\lnref{p2} 가
\co{&myvar} 를 읽어오고, 라인~\lnref{p3} 는 \co{NULL} 을 읽어올 수 있어서,
문제를 만들 수 있습니다.\footnote{
	Will Deacon 은 이게 리눅스 커널에서도 발생했다고 말했습니다.}
중간의 \co{READ_ONCE()} 는 세개의 로드가 모두 같은 변수에 대해 행해짐에도
...
\QuickQuiz{}
	Listing~\ref{lst:toolsoftrade:C Compilers Can Fuse Non-Adjacent Loads}
	의 \co{do_something()} 과 \co{do_something_else()} 가 인라인 함수인게
	왜 문제되죠?
	\iffalse
...
\QuickQuizAnswer{
	\begin{lineref}[ln:toolsoftrade:C Compilers Can Fuse Non-Adjacent Loads]
	\co{gp} 는 static 변수가 아니므로, \co{do_something()} 이나
	\co{do_something_else()} 가 별도로 컴파일 되었다면, 컴파일러는 이 두
	함수가 \co{gp} 의 값을 바꿀 수 있다고 가정하게 됩니다.
	이 가능성은 컴파일러가 라인~\lnref{p3} 에서 \co{gp} 를 다시 읽어오는 걸
	방지해서, \co{NULL} 포인터 디레퍼런스 문제를 막을 겁니다.
	\iffalse
...
아랫단의 하드웨어가 재배치를 할 수 있는데 컴파일러가 액세스 순서를 바꾸는 걸
방지하는 건 헛수고처럼 보일 수도 있습니다.
하지만, 최신 기계들은 \emph{exact exception} 과 \emph{exact interrupt} 를
가지고 있는데, 이는 모든 인터럽트나 익셉션은 인스트럭션 스트림의 특정 위치에서
일어난 것으로 보일 것을 의미합니다.
이는 이 핸들러가 앞의 모든 인스트럭션들의 효과를 보게 되지만 뒤따르는
인스트럭션들의 효과는 보지 못할 것을 의미합니다.
따라서 \co{READ_ONCE()} 와 \co{WRITE_ONCE()} 는 인터럽트된 코드와 인터럽트
핸들러 사이의 통신을 아랫단의 하드웨어가 제공한 순서 규칙에 무관하게 제어하는데
사용될 수 있습니다.\footnote{
	그렇다고 하나, 다양한 표준 위원회들은 여러분이 \co{READ_ONCE()} 와
	\co{WRITE_ONCE()} 대신 atomic 이나 \co{sig_atomic_t} 를 사용할 것을
	선호할 겁니다.}
\iffalse
...
\QuickQuiz{}
	으악!
	그러니까 컴파일러는 평범한 변수로의 스토어를 얼마든지 원하는 만큼 만들
	수는 없는 건가요?
	\iffalse
...
\QuickQuizAnswer{
	감사하게도, 만들 수 없습니다.
	이는 컴파일러가 데이터 경주로부터 숨겨져 있기 때문입니다.
	평범한 스토어 앞에 스토어를 만들어내는 경우는 매우 특수합니다:  CPU,
	쓰레드, 시그널 핸들러, 인터럽트 핸들러가 아닌 것들은 코드가 이미 이
	만들어진 스토어 없이도 존재하는 데이터 경주를 가지고 있지 않는 이상 이
	만들어진 스토어를 감지할 수 없습니다.
	그리고 코드가 이미 데이터 경주를 가지고 있다면, 이 코드는 이미 정의되지
	않은 동작의 무서운 악령을 이미 호출했을 것인데, 이는 개발자가 희망한
	것과는 무관하게 컴파일러가 뭐가 됐던 원하는 코드를 만들어낼 수 있게
	합니다.
	\iffalse
	...
	하지만 원래의 스토어가 \co{WRITE_ONCE()} 의 경우처럼 volatile 이었다면,
	모든 컴파일러는 다른 쓰레드에 신호를 줄 수 있는, 해당 스토어와 연관된
	사이드 이펙트가 발생할 것을 알 수 있어서 해당 변수로의 데이터 경주가
	없는 액세스를 가능하게 합니다.
	스토어를 만들어냄으로써, 컴파일러는 허용되지 않은 데이터 경주를 만들어
	낼 수도 있는 겁니다.

	\co{volatile} 과 atomic 변수들의 경우에는 컴파일러가 명시적으로 쓰기를
	만들어내지 못하게 되어 있습니다.
	\iffalse
...
{\bf Store-to-load transformation} 은 컴파일러가 평범한 스토어가 메모리에 있는 값을 실제로는 바꾸지 않을 것이란 걸 알 수 있게 되면 발생할 수 있습니다.
\begin{lineref}[ln:toolsoftrade:Inviting a Store-to-Load Conversion]
예를 들어,
Listing~\ref{lst:toolsoftrade:Inviting a Store-to-Load Conversion} 을 봅시다.
Line~\lnref{load:p} 는 \co{p} 를 읽지만, line~\lnref{if} 의 \qco{if} 문은 또한
컴파일러에게 개발자는 \co{p} 가 일반적으로 0의 값일 거라 생각한다고 이야기
합니다.\footnote{
	\co{unlikely()} 함수가 이 힌트를 컴파일러에게 제공하며, 다른
	컴파일러는 \co{unlikely()} 를 구현하는 다른 방법을 제공합니다.}
Line~\lnref{barrier} 의 \co{barrier()} 문은 컴파일러가 \co{p} 의 값을 잊도록
강제합니다만, 어떤 사람은 컴파일러가 이 힌트를 기억하도록 하는 경우---또는
피드백 기반의 최적화를 통해 추가적인 힌트를 얻어내는 경우를 상상해 볼 수 있을
겁니다.
그렇게 하는건 컴파일러가 line~\lnref{null} 은 종종 비싼 no-op 일 뿐이란 걸
깨닫게 할 겁니다.
\iffalse
...
의 line~\lnref{if1} 과~\lnref{null} 에서 보인 것처럼 \co{NULL} 저장을 확인 후
하도록 할 수 있을 겁니다.
이 변환은 종종 바람직하지만, 이 실제 스토어가 순서대로 이뤄지길 바라는 경우엔
문제가 될 수 있습니다.
예를 들어, 쓰기 메모리 배리어 (리눅스 커널의 \co{smp_wmb()}) 는 이 스토어를
순서잡아 주지만, 로드는 그렇지 않습니다.
이 상황은 \co{smp_wmb()} 대신 \co{smp_store_release()} 의 사용을 추천할 겁니다.
\iffalse
...
{\bf Dead-code elimination} 은 컴파일러가 어떤 로드로부터의 값이 결코 사용되지
않거나 어떤 값이 저장은 되지만 결코 로드되지 않는다는 걸 알아챈다면 발생할 수
있습니다.
이는 물론 공유 변수로의 액세스를 제거할 수 있는데, 이는 결국 메모리 순서 강제
기능을 망하게 만들어서 여러분의 동시성 코드가 놀라운 방식으로 동작하게끔 만들
겁니다.
그간의 경험은 상대적으로 적은 놀라움이 존재할 것을 이야기 합니다.
저장되기만 하는 변수의 제거는 해당 코드를 심볼 테이블을 통해 접근하는 외부의
코드가 존재하는 경우 특히나 위험합니다: 컴파일러는 그런 외부 코드 액세스를
무시하게 되고, 따라서 외부 코드가 의존하는 그 변수를 제거할 수 있을 겁니다.
\iffalse
...
안전한 동시성 코드는 컴파일러가 공유 메모리로의 중요한 접근의 숫자, 순서,
그리고 타입을 잘 지키게 해야 할 필요가 분명한데, 이 주제는
Section~\ref{sec:toolsoftrade:A Volatile Solution}
과~\ref{sec:toolsoftrade:Assembling the Rest of a Solution} 에서 다루는데, 이
섹션들이 뒤이어집니다.
\iffalse
...
바가 없습니다.\footnote{
	JF Bastien 은 C++ 에서의 \co{volatile} 키워드의 역사와 사용 예들을
	완벽하게 문서화 했습니다~\cite{JFBastien2018DeprecatingVolatile}.}
...
Listing~\ref{lst:toolsoftrade:Preventing Load Fusing} 에 보인 것처럼,
\co{READ_ONCE()} 는
...
\co{READ_ONCE()} 와 \co{WRITE_ONCE()} 는 또한
...
요약하자면, \co{volatile} 키워드는 로드와 스토어가 머신에서 제공하는 사이즈고
올바르게 정렬되어 있다면 load tearing 과 store tearing 을 막습니다.
이 키워드는 또한 load fusing, store fusing, invented loads, 그리고 invented
stores 를 막습니다.
하지만, 이게 컴파일러가 \co{volatile} 액세스를 다른 그것들과 재배치하는 건
막지만, CPU 가 이 액세스들을 재배치 하는 것은 막지 않습니다.
더 나아가서, 컴파일러나 CPU 가 \co{volatile} 액세스가 아닌 것을 그것들끼리,
도는 \co{volatile} 액세스와 재배치 하는 것은 막지 않습니다.
...
\co{barrier()} 매크로 내에서, \co{__asm__} 은 asm 지시어를 소개하고,
\co{__volatile__} 은 컴파일러가 어셈블리 코드를 최적화해 없애는 걸 막고, 빈
문자열은 실제 인스트럭션이 만들어질 필요는 없음을 의미하며, 마지막의
\co{"memory"} 는 컴파일러에게 이 아무것도 하지 않는 asm 은 메모리를 어떻게든
바꿀 수 있음을 알립니다.
이에 대한 응답으로, 컴파일러는 모든 메모리 접근을 \co{barrier()} 매크로 앞 또는 뒤로 옮기는 것을 막을 겁니다.
이는
Listing~\ref{lst:toolsoftrade:C Compilers Can Fuse Loads}
...
에 보인 것처럼 \co{barrier()} 를 호출함으로써 막아질 수 있습니다.
이 두 라인의 코드는 컴파일러가 \co{need_to_stop} 으로부터의 로드가
\co{do_something_quickly()} 뒤로 또는 앞으로 재배치 되는걸 막아줍니다.
\iffalse
...
라인~\lnref{mb1}, \lnref{mb2}, \lnref{mb3}, \lnref{mb4},
그리고~\lnref{mb5}.
\end{lineref}
에 \co{smp_mb()} 를 추가함으로써 이 남아있는 재배치 가능성을 없애버립니다.
이 \co{smp_mb()} 매크로는
Listing~\ref{lst:toolsoftrade:Compiler Barrier Primitive (for GCC)}
에 보인 \co{barrier()} 와 비슷합니다만, 빈 문자열이 전체 메모리 배리어를 위한
인스트럭션을 담은 문자열로 바뀐게 차이점인데, 예를 들어 x86 에선 \co{"mfence"},
PowerPC 에서는 \co{"sync"} 입니다.
\iffalse
...
\QuickQuiz{}
	하지만 전체 메모리 배리어를 너무 무겁지 않나요?
	Listing~\ref{lst:toolsoftrade:Preventing Reordering} 의 순서 규칙을
	강제하는데에 더 비용이 적은 방법도 있지 않아요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	대부분의 경우 그렇듯이, 답은 ``상황에 종속적이다'' 입니다.
	하지만, 두개의 쓰레드만이 \co{status} 와 \co{other_task_ready} 를
	접근하고 있는 상황이라면,
	Section~\ref{sec:toolsoftrade:Atomic Operations} 의
	\co{smp_store_release()} 와 \co{smp_load_acquire()} 함수들만으로도
	충분할 겁니다.
	\iffalse
...
Section~\ref{sec:toolsoftrade:Atomic Operations} 에 일부가 소개된 일부
read-modify-write 어토믹 오퍼레이션들로도 제공됩니다.
일반적인 경우, 메모리 순서 규칙은
Chapter~\ref{chp:Advanced Synchronization: Memory Ordering}.
상당히 애매할 수 있습니다.
다음 섹션은 메모리 순서 규칙의 대안을 알아보는데, 말하자면 데이터 레이스를
제한하거나 아예 없애는 겁니다.
\iffalse
...
이 의사 선생님의 조언은 별 도움이 되지 않는 것처럼 보일 수도 있겠으나, 공유
변수에 동시적으로 접근하는 걸 막는, 테스트 된 한가지 방법은 그런 변수들을 특정
락을 잡고 있을 때만 접근하는 것으로, Chapter~\ref{chp:Locking} 에서 이야기
됩니다.
또다른 방법은 특정 ``공유'' 변수를 특정 CPU 또는 쓰레드에서만 접근하는 것으로,
Chapter~\ref{chp:Data Ownership} 에서 이야기 합니다.
이 두가지 방법을 조합하는 것도 가능한데, 예를 들어, 특정 변수는 특정 락이 잡혀
있는 동안 하나의 CPU 또는 쓰레드에 의해서만 수정되고, 같은 쓰레드 또는 CPU 에
의해서, 또는 같은 락을 잡고 있는 다른 누군가에 의해서 읽혀질 수도 있습니다.
이런 상황들에서는 이 공유 변수로의 모든 접근은 평범한 C-언어 액세스일 겁니다.
\iffalse
...
여기 특정 변수로의 일부 액세스가, 다른 액세스는 마킹을 (\co{READ_ONCE()} 와
\co{WRITE_ONCE()} 같은) 필요로 하는 동안 평범한 로드와 스토어를 사용해도 되는
경우에 대한 상황들의 목록이 있습니다:
\iffalse
...
\begin{enumerate}
\item	하나의 공유 변수가 특정한 해당 변수를 소유한 CPU 또는 쓰레드에 의해서만
	수정되고 다른 CPU 나 쓰레드에 의해서 읽혀지는 경우.
	모든 스토어는 \co{WRITE_ONCE()} 여야만 합니다.
	이 소유자 CPU 또는 쓰레드는 평범한 로드를 사용해도 됩니다.
	다른 모든 것들은 로드에 \co{READ_ONCE()} 를 사용해야만 합니다.
\item	하나의 공유 변수가 특정 락을 잡은 상태에서만 변경되지만, 해당 락을 잡지
	않은 코드가 읽기를 하는 경우.
	모든 스토어는 \co{WRITE_ONCE()} 를 사용해야만 합니다.
	이 락을 잡고 있는 CPU 나 쓰레드는 평범한 로드를 사용해도 됩니다.
	그 외의 모든 것들은 로드에 \co{READ_ONCE()} 를 사용해야만 합니다.
\iffalse
...
\item	A shared variable is only modified while holding a given
	...
	Everything else must use \co{READ_ONCE()} for loads.
\fi
\item	하나의 공유 변수가 그 변수를 소유한 CPU 나 쓰레드에 의해서 특정 락을
	잡은 채로 수정되지만, 다른 CPU 나 쓰레드 또는 해당 락을 잡지 않은
	코드에 의해 읽혀지는 경우.
	모든 스토어는 \co{WRITE_ONCE()} 를 사용해야만 합니다.
	소유자 CPU 또는 쓰레드는 평범한 로드를 사용해도 되는데, 모든 CPU 또는
	쓰레드가 락을 잡는 경우입니다.
	그 외의 모든 경우는 로드를 위해 \co{READ_ONCE()} 를 사용해야만 합니다.
\item	하나의 공유 변수가 특정 CPU 또는 쓰레드에 의해 그리고 해당 CPU 나
	쓰레드의 컨텍스트에서 수행중인 인터럽트 또는 시그널 핸들러에 의해
	접근되는 경우.
	이 핸들러는 평범한 로드와 스토어를 사용할 수 있는데, 해당 핸들러가
	호출되는 걸 방지한 코드에서라면 그러한데, 즉, 시그널이나 인터럽트를
	블록한 코드에서입니다.
	모든 다른 코드는 \co{READ_ONCE()} 와 \co{WRITE_ONCE()} 를 사용해야만
	합니다.
\iffalse
...
\fi
\item	하나의 공유 변수가 특정 CPU 또는 쓰레드에 의해서, 그리고 해당 CPU 또는
	쓰레드의 컨텍스트에서 수행되는 시그널 또는 인터럽트 핸들러에 의해서만
	접근되고, 이 인터럽트 핸들러가 항상 자신이 쓴 변수의 값을 리턴하기 전에
	복구시키는 경우.
	이 핸들러는 평범한 로드와 스토어를 사용할 수 있고, 이 핸들러가 호출되는
	걸 막은 모든 코드, 즉 시그널 그리고/또는 인터럽트를 블록한 코드도
	마찬가지입니다.
	모든 다른 코드는 평범한 로드를 사용할 수 있습니다만, store tearing,
	store fusing, 그리고 만들어진 스토어를 막기 위해 \co{WRITE_ONCE()} 는
	사용해야만 합니다.
\iffalse
...
\QuickQuiz{}
	만약 어떤 인터럽트나 시그널 핸들러가 스스로 인터럽트 될 수 있다면 어떤
	일이 일어날까요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	그렇다면 그 인터럽트 핸들러는 다른 인터럽트된 코드가 따라야 하는 규칙을
	그대로 따라야 합니다.
	그 스스로는 인터럽트 당할 수 없거나 인터럽트 하는 핸들러와 어떤 변수도
	공유하지 않는 코드만이 평범한 액세스를 사용할 수 있고, 그렇다 하더라도
	다른 CPU 또는 스레드에서 동시에 그 변수들을 접근하지 않아야 합니다.
	\iffalse
...
대부분의 다른 경우, 어떤 공유 변수로의 로드와 스토어는 각각 \co{READ_ONCE()} 와
\co{WRITE_ONCE()} 또는 그보다 강력한 걸 사용해야 합니다.
하지만 \co{READ_ONCE()} 도 \co{WRITE_ONCE()} 도 어떤 순서 보장을 제공하지
않음을 반복해 둘 필요가 있습니다.
그런 보장을 위해선
Section~\ref{sec:toolsoftrade:Assembling the Rest of a Solution} 또는
Chapter~\ref{chp:Advanced Synchronization: Memory Ordering} 를 참고해 주시기
바랍니다.
\iffalse
...
이런 데이터 레이스 제거 패턴의 많은 예제가 Chapter~\ref{chp:Counting} 에
있습니다.
\iffalse
