...
\QuickQuiz{
	...
	가능하게 하는 asm 같은 특수한 GNU 확장 기능을 포함하는, 주의 깊게
...
이 장애는 ``뜨거운'' 캐쉬라인의 변수로부터의 로드가 \co{if} 문 바깥에서 일어날
때 발생할 수 있습니다.
...
예를 들어,
Listing~\ref{lst:toolsoftrade:C Compilers Can Fuse Stores}
에서 \co{work_until_shut_down()} 의 코드를 생성하는 컴파일러는
\co{other_task_ready} 가 \co{do_more_work()} 에 의해 액세스 되지 않으며
라인~\lnref{other:store} 에서 스토어 된다는 것을 알아챌 수도 있습니다.
만약 \co{do_more_work()} 이 복잡한 인라인 함수였다면, 레지스터 비우기를 해야
할수도 있으며, 이 경우 \co{other_task_ready} 는 임시 저장소로 사용하기 매력적인
장소입니다.
어쨌건, 거기 액세스가 없는데, 뭐가 문제겠습니까?

물론, 잘못된 시간에 이 변수로 0이 아닌 값을 저장하는 것은 라인~\lnref{loop:b}
의 \co{while} 로프가 예상보다 빨리 종료되게 하여, \co{finish_shutdown()} 이
\co{do_more_work()} 과 동시에 수행되는 것을 허용합니다.
이 \co{while} 의 요점은 그런 동시 수행을 막기 위한 것임을 생각하면, 이는 좋은
일이 아닙니다.
\end{fcvref}
...
Listing~\ref{lst:toolsoftrade:Inviting an Invented Store}
의 코드를 만들어내는 컴파일러는 \co{a} 의 값이 초기에는 0임을 알수도 있는데,
이는 이 코드를
Listing~\ref{lst:toolsoftrade:Compiler Invents an Invited Store}
의 코드로 변형시킴으로써 브랜치 하나를 최적화해 없애버리고자 하는 강한 욕망을
느끼게 될 겁니다.
\begin{fcvref}[ln:toolsoftrade:Compiler Invents an Invited Store]
여기서, 라인~\lnref{store:uncond} 는 \co{1} 을 \co{a} 에 무조건적으로 저장하고,
이어서 라인\lnref{store:cond} 에서 \co{condition} 의 값이 0이라면 \co{a} 의
값을 0으로 되돌립니다.  이는 이 if-then-else 를 if-then 으로 변형시켜서 하나의
브랜치를 아낍니다.
\end{fcvref}
...
\QuickQuiz{
	이런!
	그러니까 컴파일러는 언제든 원하면 평범한 변수로의 스토어를 만들어낼 수
	없나요?
	...
	감사하게도, 그에 대한 답은 아니오 입니다.
	이는 컴파일러가 데이터 레이스로부터 숨겨져 있기 때문입니다.
	일반적 스토어 직전에 스토어를 만ㄷ르어내는 것은 상당히 특수한
	경우입니다:  이는 일부 다른 경우들, CPU, 쓰레드, 시그널 핸들러, 또는
	인터럽트 핸들러 같은 것들이 만들어진 스토어를 볼 수 있게 되는 것은 해당
	코드에 이미 만들어진 스토어 없이도 데이터 레이스가 존재하지 않는다면
	불가능합니다.
	그리고 해당 코드에 데이터 레이스가 이미 존재한다면, 이는 개발자의
	소망과는 관계 없이 컴파일러가 뭐가 됐든 원하는 코드를 만들어낼 수 있게
	되는 정의되지 않은 행위의 악령을 이미 호출했을 것입니다.

	하지만 원래의 스토어가 volatile 이라면, \apik{WRITE_ONCE()} 에서처럼,
	모든 컴파일러가 여기에는 어떤 다른 쓰레드에게 신호를 주거나 하는 이
	스토어에 연관된 부수 작용이 있을 수도 있음을 알게 되어서 해당 변수로의
	데이터 레이스로부터 자유로운 액세스를 허용합니다.
	스토어를 만들어내는 것을 통해, 컴파일러는 그러지 말아야 하는데 데이터
	레이스를 만들어낼 수도 있습니다.

	\co{volatile} 과 어토믹 변수의 경우에, 컴파일러는 쓰기를 만들어내는
	것을 명시적으로 금지당합니다.
...
마지막으로, C11 이전의 컴파일러는 쓰여진 변수들에 인접한 관계 없는 변수들에
쓰기를 만들어낼 수도
...
을 생각해 봅시다.
라인~\lnref{load:p} 는 \co{p} 를 읽어오지만, 라인~\lnref{if} 의 \qco{if} 문은
또한 컴파일러에게 개발자는 \co{p} 가 일반적으로 0일 것이라 생각한다고 이야기
합니다.\footnote{
	\apik{unlikely()} 함수는 이 힌트를 컴파일러에게 전달하며, 다른
	컴파일러는 \co{unlockely()} 를 구현하는 다른 방법을 제공합니다.}
라인~\lnref{barrier} 의 \apik{barrier()} 문은 컴파일러가 \co{p} 의 값을 잊게
만듭니다만, 원한다면 컴파일러가 이 힌트를 기억하게 할수도---또는 피드백 방향
최적화를 통해 추가적인 힌트를 줄수도 있습니다.
그렇게 하는 것은 컴파일러에게 라인~\lnref{null} 이 종종 비싸기만 한 no-op 일
뿐임을 알게 해줍니다.
\end{fcvref}
...
의 라인~\lnref{if1} 과~\lnref{null} 에서 보인 것처럼 \co{NULL} 저장을 추가적
검사를 통해 보호할 수도 있습니다.
이 변형은 종종 바람직하지만, 순서를 위해 실제 스토어가 필요했다면 문제가 될
수도 있습니다.
예를 들어, 쓰기 메모리 배리어 (리눅스 커널의 \apik{smp_wmb()}) 가 스토어를
순서잡아줄 수는 있으나, 로드에 대해서는 그렇지 않습니다.
이 상황은 \apik{smp_wmb()} 를 넘어서 \apik{smp_store_release()} 를 사용할 것을
제안할 수도 있습니다.
\end{fcvref}
...
{\bf Dead-code elimination} 은 어떤 로드를 통해 얻어진 값이 절대 사용되지
...
\apic{volatile} 키워드는 병렬 프로그래머의 도구상자에서 필수적인 도구였습니다.
이는 \co{volatile} 이 정확히 뭘 의미하는지에 대한 질문을 떠올리게 하는데, 이
질문은 이 표준~\cite{RichardSmith2019N4800}의 더 최신 버전에서조차 충분히
꼼꼼하게 답변되지 않았습니다.\footnote{
	JF Bastien 은 C++ 에서 \co{volatile} 키워드의 역사와 사용 예에 대해
	자세하게 문서화 했습니다~\cite{JFBastien2018DeprecatingVolatile}.}
이 버전은 ``\co{volatile} \co{glvalues} 를 통한 액세스는 abstract machine 의
규칙에 근거해 엄격하게 평가된다'' 는 것을, \co{volatile} 액세스는 부수작용을
...
\begin{quote}
	\apic{volatile} 은 어떤 객체의 값이 구현에 의해서는 탐지될 수 없는
	방법에 의해 변경될 수도 있기 때문에 해당 객체에 영향을 끼치는 적극적인
	최적화를 방지하기 위해 구현에게 주는 힌트입니다.
	더 나아가, 일부 구현을 위해서는 \apic{volatile} 은 해당 객체에 액세스
	하기 위해 특수한 하드웨어 인스트럭션이 필요함을 알릴 수도 있습니다.
	구체적 의미를 위해서 6.8.1 을 참고하십시오.
	일반적으로, \apic{volatile} 의 의미는 그것이 C 에서 그러한 것처럼 C++
	에서도 그러할 것으로 의도됩니다.
\end{quote}
...
그래서 \apic{volatile} 은 앞의 예제들과 어떻게 엮일 수 있을까요?
...
에서 \apik{READ_ONCE()} 를 사용하는 것은 만들어진 로드를 방지하여,
...
Listing~\ref{lst:toolsoftrade:Preventing Load Fusing} 에서 보인 것처럼,
\apik{READ_ONCE()} 는
...
\apik{READ_ONCE()} 와 \apik{WRITE_ONCE()} 는
...
요약하자면, \apic{volatile} 키워드는 로드와 스토어가 기계 워드 크기이고 적절히
정렬되었을 때 load tearing 과 store tearing 을 방지합니다.
이것은 또한 load fusing, store fusing, invented loads, 그리고 invented stores
를 방지합니다.
하지만, 이게 컴파일러가 \apic{volatile} 액세스를 서로간에 재배치 하는 것을
막기는 하나, CPU 가 이 액세스들을 재배치 하는 것을 막는 일은 전혀 하지
못합니다.
더 나아가서, 컴파일러나 CPU 가 \co{volatile} 이 아닌 액세스를 그것들 간에 또는
\co{volatile} 액세스와 재배치 하는 것을 막는 일은 전혀 하지 않습니다.
...
이상하게 들리겠지만, 이 지시어들은
Listing~\ref{lst:toolsoftrade:Compiler Barrier Primitive (for GCC)}
의 \apik{barrier()} 매크로에 의해 예시 되듯이 어셈블리어를 포함하지는 않습니다.
...
\apik{barrier()} 매크로 내부에서 \co{__asm__} 이 asm 지시어를 가져오며,
\co{__volatile__} 이 이 컴파일러가 asm 을 최적화 해서 없애버리는 걸 방지하고,
빈 문자열은 어떤 실제 명령도 생성될 필요가 없음을 명시하고, 마지막으로
\co{"memory"} 는 컴파일러에게 이 아무것도 하지 말라는 asm 은 메모리를 임의로
바꿀수도 있음을 알립니다.
이에 대한 응답으로, 컴파일러는 이 \apik{barrier()} 매크로 앞뒤로 메모리
참조들을 옮기는 것을 막을 겁니다.
이는
Listing~\ref{lst:toolsoftrade:C Compilers Can Fuse Loads}
에 보인 루프 풀어버리기가
...
이 두 줄의 코드는 컴파일러가 \co{need_to_stop} 으로부터의 로드를
\co{do_something_quickly()} 내로 또는 그 뒤로 어떤 방향으로든 변경하는 것을
방지합니다.
...
\apik{smp_mb()} 매크로는
Listing~\ref{lst:toolsoftrade:Compiler Barrier Primitive (for GCC)}
에 보인 \apik{barrier()} 와 비슷하지만 빈 문자열이 예를 들어 x86 이라면
\co{"mfence"}, PowerPC 라면 \co{"sync"} 와 같은 실제 명령을 담은 문자열로
대체되어 있습니다.
...
\QuickQuiz{
	...
	Listing~\ref{lst:toolsoftrade:Preventing Reordering}
	에 필요한 순서만을 강제하는 더 가벼운 방법이 있지 않나요?
	...
	많은 경우 그렇듯, 답은 ``상황에 따라 다르다'' 입니다.
	하지만, 두개의 쓰레드만이 \co{status} 와 \co{other_task_ready} 변수에
	접근한다면,
	Section~\ref{sec:toolsoftrade:Atomic Operations}
	에서 이야기되는
	\apik{smp_store_release()} 와 \apik{smp_load_acquire()}
	함수만으로도 충분할 겁니다.
...
순서 규칙 강제는 일부 read-modify-write 어토믹 오퍼레이션들에 의해서도
제공되는데, 그 중 일부는
Section~\ref{sec:toolsoftrade:Atomic Operations}
에서 보였습니다.
일반적인 경우에 메모리 순서 규칙 강제는
Chapter~\ref{chp:Advanced Synchronization: Memory Ordering}
에서 이야기 되었듯 상당히 미묘할 수 있습니다.
다음 섹션은 메모리 순서 규칙 강제의 대안, 즉 데이터 레이스를 제한하거나 아예
방지하는 방법을 알아보겠습니다.
...
여기 있는데, 해당 변수로의 다른 액세스들은 마킹이 (\apik{READ_ONCE()} 와
\apik{WRITE_ONCE()} 같은) 필요합니다:
...
\item	어떤 공유 변수가 특정 소유권을 가진 CPU 나 쓰레드에 의해서 변경되지만,
	...
	모든 스토어는 \apik{WRITE_ONCE()} 를 사용해야 합니다.
	그외의 모든 다른 것은 로드를 위해 \apik{READ_ONCE()} 를 사용해야만
	합니다.
...
\item	어떤 공유 변수가 특정 CPU 나 쓰레드, 그리고 해당 CPU 나 쓰레드의
	...
	그렇습니다.
	모든 다른 코드는 \apik{READ_ONCE()} 와 \apik{WRITE_ONCE()} 를
	사용해야만 합니다.
...
\QuickQuiz{
	인터럽트나 시그널 핸들러가 그 스스로도 인터럽트 당할 수 있다면 뭘 해야
	하나요?
	...
	그렇다면 해당 인터럽트 핸들러는 다른 인터럽트된 코드가 따르는 규칙을
	그대로 따라야만 합니다.
	스스로 인터럽트 될 수 없거나 인터럽트하는 핸들러와 어떤 변수도 공유하지
	않는 핸들러만이 안전하게 평범한 액세스를 사용할 수 있을 것이며, 그렇다
	해도 해당 변수들은 다른 CPU 나 쓰레드에 의해 동시에 액세스될 수는
	없습니다.
...
대부분의 다른 경우에는 어떤 공유 변수로의 로드와 스토어는 \apik{READ_ONCE()} 와
\apik{WRITE_ONCE()} 또는 그보다 강력한 것을 각각 사용해야만 합니다.
하지만 \apik{READ_ONCE()} 도 \apik{WRITE_ONCE()} 도 컴파일러 외의 것에 대해서는
어떠한 순서 보장도 제공하지 않음을 명심해 두시기 바랍니다.
그런 보장들에 대한 정보를 위해선
Section~\ref{sec:toolsoftrade:Assembling the Rest of a Solution} 또는
Chapter~\ref{chp:Advanced Synchronization: Memory Ordering}
를 읽어보시기 바랍니다.

데이터 레이스 방지 패턴의 많은 예가
Chapter~\ref{chp:Counting} 에 보여집니다.
...
리눅스 커널은 다양한 어토믹 오퍼레이션을 제공합니다만, \apik{atomic_t} 타입에
대해 정의된 것들이 시작하기에 좋을 겁니다.
평범한 찢어지지 않는 (non-tearing) 로드와 스토어는 \apik{atomic_read()} 와
\apik{atomic_set()} 을 통해 각각 제공됩니다.
Acquire load 는 \apik{smp_load_acquire()} 를 통해, release store 는
\apik{smp_store_release()} 를 통해 제공됩니다.

값을 리턴하지 않는 fetch-and-add 오퍼레이션은 \apik{atomic_add()},
\apik{atomic_sub()}, \apik{atomic_inc()}, 그리고 \apik{atomic_dec()} 를 통해
제공됩니다.
오퍼레이션 결과 값이 0이 되면 이를 알리는 어토믹 값 감소 오퍼레이션은
\apik{atomic_dec_and_test()} 와 \apik{atomic_sub_and_test()} 를 통해
제공됩니다.
새로운 값을 리턴하는 어토믹 값 증가 오퍼레이션은 \apik{atomic_add_return()}
을 통해 제공됩니다.
\apik{atomic_add_unless()} 와 \apik{atomic_inc_not_zero()} 는 둘 다 조건적
...
어토믹 값 교환 오퍼레이션은 \apik{atomic_xchg()} 를 통해 제공되고, 그 유명한
compare-and-swap (CAS) 오퍼레이션은 \apik{atomic_cmpxchg()} 를 통해 제공됩니다.
이것들 둘 다 기존 값을 리턴합니다.
많은 추가적 어토믹 RMW 기능들이 리눅스 커널 내에 존재하는데, 리눅스 소스 트리의
\path{Documentation/atomic_t.txt} 파일을 읽어보시기 바랍니다.

이 책의 CodeSamples API 는 리눅스 커널의 것에 가깝습니다.
...
리눅스 커널은 per-CPU 변수를 정의하기 위해 \apik{DEFINE_PER_CPU()} 를, 해당
per-CPU 변수의 이 CPU 의 인스턴스로의 참조를 형성하기 위해 \apik{this_cpu_ptr()}
를, 해당 per-CPU 변수의 특정 CPU 의 인스턴스로의 액세스를 위해 \apik{per_cpu()}
...
이 API 는 엄밀하게 말하면 필요치 않지만\footnote{
	여러분은 이것 대신 \apig{__thread} 나 \apic{_Thread_local} 을 사용할 수
	있습니다.},
...
\QuickQuiz{
	Per-thread-변수 API 가 존재하지 않는 시스템에선 이걸 어떻게 할 수
	있을까요?
	...
	한가지 방법은 \apipf{smp_thread_id()} 로 인덱스 되는 배열을 만드는
	것이고, 또다른 방법은 \apipf{smp_thread_id()} 를 배열 인덱스로 매핑하는
	해쉬 테이블을 만드는 것이겠습니다---사실 이 API 집합이 pthread 환경에서
	사용하는 방법입니다.

	또다른 방법은 부모가 각 per-thread 변수를 위한 필드를 갖는 구조체를
	할당하고, 이를 자식 쓰레드에게 쓰레드 생성 시에 넘기는 것입니다.
	하지만, 이 방법은 큰 시스템에선 거대한 소프트웨어 엔지니어링 비용을
	부과할 수 있습니다.
	이를 이해하기 위해, 어떤 커다란 시스템의 모든 전역 변수가 그것들이 C
	static 변수인가 아닌가에 관계 없이 하나의 파일에 모두 선언되어야 한다고
	생각해 보세요!
...
\apipf{DEFINE_PER_THREAD()} 기능은 per-thread 변수 하나를 정의합니다.
불행히도, 리눅스 커널의 \apik{DEFINE_PER_THREAD()} 기능에서는 가능한 것처럼
초기화를 제공하는 것은 불가능합니다만, 쉬운 런타임 초기화를 가능하게 해주는
\apipf{init_per_thread()} 기능이 있습니다.
...
\apipf{DECLARE_PER_THREAD()} 기능은 C 센스의 정의가 아닌 선언입니다.
따라서, \apipf{DECLARE_PER_THREAD()} 기능은 어떤 다른 파일에서 정의된
...
\apipf{per_thread()} 기능은 특정 쓰레드의 변수에 액세스 합니다.
...
\apipf{__get_thread_var()} 기능은 현재 쓰레드의 변수를 액세스 합니다.
...
\apipf{init_per_thread()} 기능은 특정 변수의 모든 쓰레드의 인스턴스를 특정
값으로 설정합니다.
...
그로 인한 셸 스크립트의 \co{fork()}/\co{exec()} 오버헤드가 (Intel Core Duo
랩톱에서의 가장 작은 C 프로그램이라면 약 480 마이크로세컨드가 소요됩니다)
너무 크다면, C-언어의 \apipx{fork()} 와 \apipx{wait()} 기능을 시도해 보세요.
...
\QuickQuiz{
	셸은 \apipx{fork()} 대신 \apipx{vfork()} 를 사용하지 않을까요?
	...
	그럴수도 있습니다만, 그걸 확인하는 건 독자 여러분의 할일로 남겨져
	있습니다.
	하지만 그동안, 전 우리가 \apipx{vfork()} 는 \apipx{fork()} 의 변종이며,
	따라서 우린 이 둘을 모두 의미하는 범용적 용어로 \apipx{fork()} 를
	사용할 수 있음에 동의했으면 합니다.
...
동시성은 C 언어가 처음으로 동시성 시스템을 만드는데 사용된지 수십년이 지나서 C
...
고전의 \GCC\ \co{__sync} API 를 사용하는 오래된 코드베이스 위에서 일하고
있다면,
Section~\ref{sec:toolsoftrade:Atomic Operations (gcc Classic)}
와 연관된 \GCC\ 문서들을 읽어야 합니다.
리눅스 커널이나 
\apic{volatile} 키워드를 인라인 어셈블리와 결합해 사용하는 비슷한 코드베이스
