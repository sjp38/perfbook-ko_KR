...
Figure~\ref{fig:intro:Reader-Writer Lock Scalability} 가 리더-라이터 락킹의
오버헤드는 크리티컬 섹션이 작을수록 커진다는 것을 보여줬으니, 매우 작은
...
는 대략적인 POSIX 와 gcc 도구들, 그리고 리눅스 커널 사이의 대응표를
보여줍니다.
완벽한 대체물을 항상 찾긴 불가능한데, 예를 들어 리눅스 커널은 다양한 락킹
도구를 갖는 반면, gcc 는 리눅스 커널에서는 곧바로 사용할수는 없는 여러 어토믹
오퍼레이션들을 제공합니다.
물론, 사용자 레벨 코드는 리눅스 커널의 다양한 락킹 도구들이 필요하진 않으며,
gcc 의 어토믹 오퍼레이션도 \co{cmpxchg()} 를 사용해 흉내내어질 수 있습니다.
