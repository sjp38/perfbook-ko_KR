...
중요한 특이 케이스 중 하나는 가치있지만 그걸 구현하는데 필요한 시간을 정당화
할만큼은 가치있지 않은 프로젝트가 되겠습니다.
이 특이 케이스는 상당히 흔한 케이스이고 이런 경우에 초기부터 생기는 문제는
결정권자들이 그 프로젝트를 실제 구현하는데 노력할 의지가 없다는 점입니다.
...
이 정의들로부터, 모든 안정적이고 간단하지 않은 프로그램은 알려지지 않은 버그가
...
\QuickQuizAnswer{
	...
	\item	``user'' 와 ``sys'' 시간의 합이 ``real'' 시간보다 큰 경우를
		위한 테스트 케이스가 있습니까?
		(멀티쓰레드 프로그램에 있어서 이건 물론 완전히 합법적인
		결과입니다.)
...
지나치게 오래 기다리고 있는 것입니다.
당신의 자연적인 수준의 낙관이 당신이 요구사항들을 완전히 이해하기 전에 설계를
시작하도록 하는 일은 없었을까요?
이 질문에 대한 대답은 거의 항상 ``그렇다'' 입니다.
잘못된 요구사항을 막는 한가지 좋은 방법은 당신의 사용자들에 대해서 아는
것입니다.
정말로 그들을 위한 봉사를 잘 하려 한다면, 그들과 함께 살아야만 합니다.
\iffalse
...
\QuickQuizAnswer{
	...
	이 경우, 최선의 방법은 빠르게 간단한 해결책의 여러 프로토타입들을
	만들어 보고, 그걸 사용해 보고, 뭐가 가장 잘 동작하는지 보는 겁니다.

	다른 한편으로, 당신이 이미 존재하는 시스템과 전체적으로 유사한 시스템을
	만들도록 돈을 받았다면, 당신은 당신의 사용자인 고용인에게, 그리고
	일찍부터 자주 검증을 하는 스스로의 미래에 빚을 지고 있는 셈입니다.
	\iffalse
...
세상에 없던 새로운 종류의 프로젝트는 검증에 있어 다른 전략을 필요로 하는데,
...
그리고 누군가가 당신의 패치를 테스트 한다고 하더라도, 그 사람이 당신의 버그를
...
만약 유저 모드 C-언어 어플리케이션을 작업 중이라면 \co{printf()} 를요.
...
을 잡아야만 함''과 같은 이야기를 하는 헤더 주석을 가질 수도 있습니다만, 그런
주석은 누군가가 그걸 정말로 읽지 않는다면 어떤 좋은 일도 하지 않습니다.
...
다양한 코드 리뷰는 정적 분석의 특수한 경우입니다만, 사람이 분석을 한다는 차이가
있습니다.
...
전통적인 walkthrough (가상 리허설) 는 정규적 검사와 비슷하지만, 사람들이 코드를
가지고 특정 테스트 케이스에 대해서 ``컴퓨터인 척 한다''는 점이 다릅니다.
...
\QuickQuiz{}
	...
	어떻게 당신은 소프트웨어의 충분한 부분이 이런 방식으로 작성되었을
	거라고 생각할 수 있는거죠?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	실제로, 코드를 손으로 복사하는 행위를 반복하는 것은 노동집약적이고
	느립니다.
	...
	스크립트를 작성하고 있다면, 다른 방법론이 당신을 가장 잘 도와줄 겁니다.
...
\QuickQuiz{}
	이 모든 종이 위에 펜으로 하는 복사 뒤에 최종 코드를 타이핑하는 과정에서
	버그가 생기면 어떻게 하나요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	종종 그렇듯이, 그 답은 ``경우에 따라 다릅니다''.
	만약 그 버그가 간단한 오타라면, 이 오타를 고치고 계속해서 타이핑 하는게
	최선입니다.
	하지만, 만약 이 버그가 설계상의 결함을 의미한다면, 종이와 펜으로
	돌아가는게 최선입니다.
	\iffalse

	The answer, as is often the case, is ``it depends''.
	If the bug is a simple typo, it is best to fix that typo and
	continue typing.
	However, if the bug indicates a design flaw, it is best to
	go back to the pen and paper.
	\fi
} \QuickQuizEnd

앞의 방법은 새로운 코드에 있어서는 잘 동작합니다만, 이미 작성한 코드에 대해서
...
\begin{enumerate}
\item	당신이 가장 좋아하는 문서화 도구 (\LaTeX{}, HTML, OpenOffice, 또는
...
버그를 더 빨리 노출시킬 것인데, 그런 사용자들이 최초의 개발자들이 고려하지
못했던 일들을 하게 되면 특히 그렇습니다.
다음 섹션은 병렬 소프트웨어를 검증하면 너무도 자주 발생하는 확률적 버그를
다루는 방법에 대해 설명하겠습니다.
\iffalse
...
\QuickQuiz{}
	당신의 쓰레기 봉투에 굉장히 많은 시스템이 있다고 생각해 봅시다.
	예를 들어, 현재의 클라우드 가격 정책대로라면, 당신은 충분히 낮은 가격에
	수많은 CPU 시간을 구입할 수 있습니다.
...
\QuickQuizAnswer{
	...
	\fi
	\begin{enumerate}
	\item	일부 버그는 극단적으로 낮은 발생 가능성을 가지고 있지만, 그래도
		고쳐져야 합니다.
		예를 들어, 리눅스 커널의 RCU 구현이 평균적으로 100년의 CPU
		시간에 딱 한번 발생하는 버그를 가지고 있다고 생각해 봅시다.
		100년의 CPU 시간은 가장 싼 클라우드 플랫폼에서라고 해도 상당히
		비싼 가격입니다만, 2011년 기준으로 세계에 1억개 이상의 리눅스
		인스턴스가 존재하므로, 이 버그는 하루에도 2,000 회 이상의
		문제를 일으킬 수 있을 것으로 예상할 수도 있습니다.
	\item	이 버그는 당신의 테스트 환경에서는 발생 가능성이 없을 수
		있는데, 이는 당신이 테스트를 하는데에 얼마나 많은 시간을
		쏟아붓는가와 관계없이 당신은 그 버그를 볼 수 없을 것이란 것을
		의미합니다.
	\iffalse
	...
	정형적 검증 절차가 도움이 될 수 있을 겁니다.
	하지만 주의하세요: 당신의 코드의 정형적 검증은 당신의 가정, 요구사항에
...
하지만 절대적인 정확성을 포기하고 높은 가능성에 치중하도록 하려 한다고 생각해
봅시다.
...
비록 커널 부팅 테스트를 하는데에 한시간을 소비할 수도 있긴 하지만, 대부분의
경우에 있어서 커널을 부팅하기 위해 시도한 시도의 횟수와 부팅이 성공한 횟수가
테스트에 소비한 시간의 길이보다 더 흥미로운 것이 될 겁니다.
기능 테스트는 개별적이게 되는 경향이 있습니다.
\iffalse
...
한편, 만약 제 패치가 RCU 에 관련되어 있다면, 저는 RCU 를 테스트하는 커널 모듈인
rcutorture 를 돌려볼겁니다.
로그인 프롬프트 시그널이 띄워지는 것으로 성공을 의미하는 커널 부팅과 달리,
rcutorture 는 커널이 크래시 나거나 당신이 멈추라고 하기 전까지는 RCU 를
고문하기를 계속할 겁니다.
따라서 rcutorture 테스트에 걸리는 시간이 (일반적으로) 당신이 그걸 시작하고 멈춘
시간보다 더 중요하게 됩니다.
따라서, rcutorture 는 많은 스트레스 테스트가 포함되는 연속적 테스트 카테고리의
한 예입니다.
\iffalse
...
생각하므로, 설령 여러분은 앞의 단계들이 더 편하다고 생각한다 하더라도, 한번
해봅시다!
...
\QuickQuiz{}
	뭐라구요???
	제가 앞에서 이야기한 각각 10\,\% 실패 확률을 갖는 다섯번의 테스트의
	예를 이 공식에 집어넣어보면 59,050\,\% 를 얻게 되는데 이건 말이
	안되잖아요!!!
	\iffalse
...
\QuickQuizAnswer{
	...
	되는데, 이는 반올림하면 41\,\% 가 되는데, 이렇게 되면 앞의 결과와
	상당히 일관적이지요.
	\iffalse
...
당신이 가한 수정이 정말로 일을 개선시켰다고 99\,\% 확신할 수 있으려면 얼마나
많은 테스트를 수행시켜 봐야 할까요?
\iffalse
...
98.92\,\% 의 최소 한번은 테스트가 실패할 확률을 가져오고, 44번의 수행은
...
우리의 수정사항은 실제로 프로그램의 안정성을 개선했을 가능성이 99\,\% 라고
이야기 하게 됩니다.
\iffalse
...
교리적인 통계학자는 이런 접근을 허용하지 않을겁니다만, 슬픈 사실은 이런 종류의
통계 방법론의 오용으로 인해 발생하는 에러는 당신의 프로그램의 실패 확률의
측정에서 피할 수 없는 에러들보다는 훨씬 작다는 것입니다.
하지만, 다음 섹션에서는 약간 덜 사기적인 방법을 설명합니다.
\iffalse
...
여기서 $F_m$ 은 테스트에서 $m$ 번 실패할 확률이고 $\lambda$ 는 단위시간 동안
예상되는 실패 비율입니다.
더 엄격한 유도는 더 나은 확률에 대한 교재들에서 찾아볼 수 있을텐데, 예를 들어
Feller 의 고전인 ``An Introduction to Probability Theory and Its
Application''~\cite{Feller58} 이 있으며, 더 직관적인 접근법은 이 책의 첫번째
...
예제를 Poisson 분포를 이용해서 다시 한번 다뤄봅시다.
...
우리가 시간당 $0.3$ 회 실패하므로, 요구되는 시간은 $4.6/0.3 = 14.3$ 으로,
...
더 일반적으로 말해서, 단위시간당 $n$ 번 실패한다면, 그리고 어떤 수정사항이 실패
확률을 줄였다고 $P$\,\% 확신하려면 다음의 공식을 사용할 수 있습니다:
\iffalse
...
\QuickQuiz{}
	...
	위해서 테스트는 에러 없이 얼마나 오래 돌아가야 할까요?
	\iffalse
...
두번만 실패했다고 생각해 봅시다.
해당 버그로 만들어진 실패는 무작위적으로 발생한다고 하면, 두번째 테스트
...
\QuickQuizAnswer{
	...
	있는데, 여기서 \co{m} 은 $m$ 을 위한 원하는 값으로 바꾸고 (실제
	테스트에서의 실제 failure 횟수) \co{l} 은 원하는 $\lambda$ 값으로 (실제
	테스트에서 기대된 failure 횟수) 바꿔야 할 겁니다.
	\iffalse
...
\QuickQuiz{}
	하지만 잠시만요!!!
	실패의 \emph{어떤} 횟수가 있어야 한다는 (실패가 없을 수 있다는 가능성을
	포함해서) 점을 놓고 보면,
...
이런 종류의 생각을 해보는게 heisenbug 를 설명하는데에 도움이 됩니다:
추적 기능과 단정문을 추가하는 것은 버그의 발생 가능성을 줄여버릴 수 있습니다.
그리고 이게 극단적으로 가벼운 추적기능과 단정문 메커니즘이 그렇게 중요한
이유입니다.
\iffalse
...
``Heisenbug'' 란 용어는 특정 입자의 위치와 속도를 어떤 특정 시점에 명시하는
...
\begin{enumerate}
\item	레이스 컨디션을 일으키기 쉬운 영역에 딜레이를 추가하기.
\item	워크로드의 강도를 증가시키기.
\item	의심스러운 서브시스템을 격리시켜서 테스트하기.
\item	일반적이지 않은 이벤트를 시뮬레이션 해보기.
...
가깝습니다만, 다음 섹션들은 anti-heisenbug 에 관련된 것들을 만들어내는데 몇가지
팁을 줄겁니다.
이것들은 토론 섹션인
...
누실되는 코드를 생각해 봅시다.
여기에 \co{printf()} 문을 집어넣는 행위는 누실되는 카운트를 엄청나게 줄이거나
심지어 없애버릴 수도 있을 겁니다.
하지만, 읽어오고-더하고-저장하는 시퀀스를 읽어오고-더하고-기다리고-저장하는
시퀀스로 바꾸는 것은 누실되는 카운트의 발생률을 엄청나게 증가시킬 겁니다
(해보세요!).
레이스 컨디션에 관련되어 있는 버그를 일단 발견했다면, 이런 식으로 딜레이를
추가하는 것으로 anti-heisenbug 를 만들어내는게 많은 경우에 가능합니다.
\iffalse
...
물론, 이는 먼저 레이스 컨디션을 어떻게 찾아야 하는지에 대한 의문을 이끌어
냅니다.
이건 약간 어둠의 예술입니다만, 이를 찾기 위해 할 수 있는 것들이 몇가지
있습니다.
\iffalse
...
한가지 방법은, 레이스 컨디션은 많은 경우에 그 레이스에 관련된 데이터를
오염시키는 결과를 이끌어 냄을 인식하는 것입니다.
따라서 모든 오염된 데이터의 동기화를 두번씩 체크하는 것은 좋은 습관입니다.
설령 당장은 그 레이스 컨디션을 인식할 수 없다고 해도, 오염된 데이터로의 액세스
전과 후에 딜레이를 추가하는 것은 실패 확률을 바꿀 것입니다.
...
\QuickQuizAnswer{
	...
	많은 CPU 가 그런 관계없는 포인터를 찾아내는 것을 도울 수 있는 하드웨어
	디버깅 기능을 가지고 있습니다.
	...
	하지만, 이건 어려운 문제인데, 그게 제가 ``약간 어둠의 예술'' 이라는
	말을 사용한 이유입니다.
	\iffalse
...
하지만, 일반적인 디버깅 테크닉을 사용해서 그런 문제를 추적하는 것은 어떤
경우에는 상당히 복잡합니다.
...
특정 테스트 셋이 특정 서브시스템에만 상대적으로 적은 스트레스를 가함으로 인해
타이밍에 대한 작은 변화가 생겨 heisenbug 를 사라지게 하는 경우가 종종 있습니다.
이런 경우를 위한 anti-heisenbug 를 만들어내는 한가지 방법은 워크로드의 강도를
증가시키는 것으로, 이는 버그가 나타날 확률을 올립니다.
만약 확률이 충분히 증가한다면, 버그가 사라지게 하지 않으면서도 추적 기능과 같은
가벼운 디버깅 기능들을 추가할 수도 있을 겁니다.

워크로드의 강도를 어떻게 증가시킬 수 있을까요?
프로그램에 따라 다르긴 합니다만, 여기 몇가지 시도해볼만한 것들이 있습니다:
\iffalse
...
\begin{enumerate}
\item	CPU 를 더 사용합니다.
\item	프로그램이 네트워크를 사용한다면, 더 많은 네트워크 어댑터와 원격의
	시스템을 더 많이 추가하거나 더 빠른 것으로 바꾸세요.
...
\item	예를 들어 병렬 행렬 곱셈을 하고 있다면 행렬의 크기를 바꾸는 식으로
	문제의 크기를 바꾸세요.
	더 큰 문제는 더 많은 복잡도를 가져올 수 있습니다만, 더 작은 문제는 종종
	경쟁의 수준을 증가시킵니다.
...
의심되는 서브시스템에 큰 스트레스를 가하는 것이 어렵거나 불가능한 구조로
프로그램이 짜여져 있는 경우에 유용한 anti-heisenbug 는 서브시스템을 격리시켜서
테스트하는 스트레스 테스트 입니다.
리눅스 커널의 rcutorture 모듈은 RCU 에 대해 정확히 이 방법을 취하고 있습니다:
제품 환경에서 가능한 것보다 많은 스트레스를 RCU 에 가함으로 인해, 제품 사용
상황에서보다 rcutorture 테스트 중에 RCU 버그가 발견될 확률이
높아집니다.\footnote{
	슬프게도 확률이 1까지 되지는 못하긴 하지만 말이죠.}

사실, 병렬 프로그램을 만들 때에는 컴포넌트들을 개별적으로 스트레스 테스트
하는게 현명합니다.
그런 컴포넌트 단계의 스트레스 테스트들을 만들어내는 것은 시간 낭비처럼 보일
수도 있습니다만 약간의 컴포넌트 단계 테스트가 수많은 시스템 단계 디버깅을 줄일
수 있습니다.
\iffalse
...
Heisenbug 는 어떨 때에는 메모리 할당 실패, 조건적 락 획득 실패, CPU-hotplug
오퍼레이션, 타임아웃, 패킷 로스 등의 일반적이지 않은 이벤트로 인해 발생하기도
합니다.
...
리턴할지 \co{NULL} 을 리턴할지 무작위적으로 선택하는 wrapper 함수를 호출하는
겁니다.
가짜 실패를 만들어내는것은 병렬 프로그램은 물론, 순차적 프로그램에서도 견고성을
만들어내기 위한 좋은 방법입니다.
\iffalse
...
\QuickQuizAnswer{
	...
	있음을 나타낸다면, 가짜 실패는 그 일이 결코 완료가 되지 못하게 만들 수
...
버그는 대부분의 경우 그렇거나 아니거나인 것들이어서 버그는 발생하거나 그렇지
않거나일뿐, 중간의 것은 없습니다.
...
동시성이 존재하는 프로그램에서는 타임스탬프를 찍는 일이 가끔은 near miss 를
파악해 내는데 사용될 수 있습니다.
예를 들어, 락킹 도구들은 상당한 딜레이를 일으키는데, 같은 락에 대해서 서로 다른
획득을 통해 보호될 것으로 생각되는 한쌍의 오퍼레이션들 사이에 너무 짧은
...
시간의 실패 없는 테스트 수행이 필요하기 때문에, 그 실패를 찾아내기 위한
\co{git bisect} 과정은 고통스러울 정도로 느릴겁니다---또는 극단적으로 거대한
테스트 환경을 필요로 할겁니다.
...
하지만, 정형적 RCU 의 정의는 RCU read-side 크리티컬 섹션이 하나의 grace
period 를 넘어 확장되는 것을 금지하는데, 이는 ``Near Miss'' 화살표로 나타내어져
있습니다.
...
있습니다.\footnote{
	아무일도 하지 않고 있는 CPU 들은 최근의 수백개의 grace period 에
...
횟수를 세는 것을 더 빈번하게 발생하며 그 실패들과 연관되어 있을 것으로 여겨지는
near miss 의 횟수를 세는 것으로 대체하는 것입니다.
...
여태까지, 우리는 병렬 프로그램의 기능성에서의 버그에 대해서만 집중해 왔습니다.
...
여러분의 코드에 버그가 있을 것이라 믿는다 해도, 그 버그가 무엇인지, 무엇이
...
이 모든-너무-흔한 경우에, 통계는 여러분을 돕지 못합니다.\footnote{
	여러분의 프로그램이 해야 할 일이 무엇인지 (여러분 생각과 달리 그럴
	확률이 적겠지만) 알고 여러분의 프로그램이 충분히 작다면
...
있습니다.
일단 제가 이런 종류의 실수에 취약하다는 사실을 스스로 인정하기로 한 후로, 저
자신이 제 초기화 코드를 두번씩 체크하도록 하는게 더 쉬워졌습니다 (하지만 쉽지
않아요!).
...
궁극의 성능과 선형적 확장성까지는 필요하지 않을 수도 있습니다만, 순차적으로
같은 일을 하는 최적화된 프로그램보다 느리게 동작하는 병렬 프로그램은 많이
사용되지 않을 겁니다.
...
\QuickQuizAnswer{
	...
	더 나아가서 당신이 사람들과 함께 여덟개의 CPU 를 가진 시스템에서 순차적
...
자주 오남용되긴 하지만, 벤치마크는 유용하기도 하고 많이 사용되므로, 벤치마크에
너무 부정적인건 도움이 되지 않습니다.
벤치마크는 임시 변통의 움직임들부터 국제적인 표준까지 그 범위가 다양합니다만,
그것들의 정형성의 수준과는 관계없이, 벤치마크는 네개의 주요 목적을 위해
사용됩니다:
\iffalse
...
어플리케이션 자체를 벤치마크로 사용하지 않고 왜 불완전한 벤치마크들을 만들어낼
생각을 하는 걸까요?

실제 어플리케이션을 수행하는 것은 그게 실용적인 곳에서라면 실제로 그게 최고의
방법입니다.
불행히도, 그건 다음과 같은 이유로 실용적이지 못한 경우가 많습니다:
\iffalse
...
\begin{enumerate}
\item	그 어플리케이션은 독점된 것이고, 당신이 그 어플리케이션을 수행할 권한을
	갖고 있지 못할 수 있습니다.
\item	그 어플리케이션은 당신이 접근할 수 없는 더 많은 하드웨어를 필요로 할
	수도 있습니다.
\item	그 어플리케이션은 예를 들어 개인정보 규제와 같은 이유로 여러분이 접근할
...
소프트웨어의 아주 작은 부분이 성능과 확장성 한계의 주요한 책임을 가지고 있는
경우가 많습니다.
하지만, 개발자들이 실제 병목지점을 손으로 찾아내기는 매우 어렵습니다.
...
그렇지만, 당신의 관심을 가장 필요한 곳에 집중할 수 있도록 도와주는데 있어
일반적으로는 훨씬 나은 일을 해주는 많은 도구들이 있습니다.
...
멀티 쓰레드 프로그램의 성능 디버깅을 위한 도구를 위한 많은 노력들이 있었는데,
...
확장성 문제는 프로그램을 매우 큰 시스템들에서 수행해보기 전까지 분명하지 않을
겁니다.
하지만, 훨씬 작은 시스템에서 프로그램을 수행할 때라고 해도 임박한 확장성 문제를
찾아내는 것이 가능할 때도 있습니다.
...
그러고나선 두 수행의 프로파일 결과를 모으고, 수학적으로 관련된 프로파일
측정결과를 조합해 보는 겁니다.
예를 들어, 주요한 관심이 확장성이라면, 연관된 측정의 비율을 취하고, 그 비율들을
숫자상으로 내림차순 정렬해 볼 수 있을 겁니다.
...
불행히도, 측정을 위한 이 방법은 여러 오류가 끼어들 수 있는데, 다음과 같은
것들이 있습니다:
\iffalse
...
\item	이 측정은 시각 측정의 오버헤드가 포함될 수 있습니다.
	이 에러는 수행 반복 횟수를 증가시킴으로써 매우 작은 크기로 만들 수
	있습니다.
\item	처음 몇번의 테스트 수행 반복은 캐시 미스나 (더 나쁜) 페이지 폴트를
	유발할 수도 있는데, 이는 측정된 값을 부풀릴 수도 있을 겁니다.
	이런 에러 역시 수행 반복의 수를 늘림으로써 줄일 수 있으며, 측정 기간이
	시작되기 전에 몇번의 웜업 수행 반복을 수행시킴으로써 완전히 없앨수 있는
	경우도 많습니다.
	대부분의 시스템은 특정 프로세스가 페이지 폴트를 냈는지 확인할 수 있는
	방법들을 제공하며, 그런 이유로 성능이 저해된 경우의 수행을 제거하기
	위해 여러분은 그 방법들을 사용해야 합니다.
...
\item	Any iteration of the test might be interfered with by other
	...
	iterations.
\fi
\item	발열처리가 확장성을 낮춰지게 할 수 있는데, CPU 활동을 증가시키는 건
	발열을 증가시키고, 적당한 냉각 기능이 없는 시스템 (대부분이 그래요!)
		에서는, 이는 CPU 갯수가 늘어날수록 CPU 프리퀀시를 낮추는 결과를
		초래할 수 있습니다.\footnote{
		적절한 냉각 장치가 있는 시스템은 게이밍 시스템처럼 보이곤
		합니다.}
	물론, 여러분의 어플리케이션이 제품 단계에서 수행될 때의 예상되는 동작을
	평가하고자 한다면, 그런 열처리는 인생에 있어서의 현실입니다.
	그렇지 않고, 여러분이 이론적 확장성에 관심 있다면, 적절한 냉각 기능이
	있는 시스템을 사용하거나 냉각 시스템이 처리할 수 있는 수준으로 CPU
	클락을 낮추시기 바랍니다.
\iffalse
...
간섭 유발 사건들 중 첫번째와 네번째 것은 서로 상충되는 조언을 하고 있는데, 이는
...
\QuickQuizAnswer{
	...
	하지만 다양한 메모리 레이아웃 병목지점이 존재할 수 있음을 알아 두시기
	바랍니다.
	메모리 대역폭에 민감한 벤치마크 (행렬 연산과 같은) 는 메모리 병렬성을
	최대화 시키기 위해 스레드들을 존재하는 코어와 소켓에 흩뿌려야 합니다.
	이 벤치마크는 또한 데이터를 NUMA 노드 사이로, 메모리 컨트롤러 사이로,
	그리고 DRAM 칩들 사이로 가능한한 흩뿌려야 합니다.
	반대로, 메모리 응답시간에 민감한 벤치마크 (가장 확장되지 못하는
	어플리케이션을 포함해) 는 로컬리티를 극대화 하고, 다른 코어와 소켓을
	사용하기 전에 각 코어와 소켓을 먼저 채워야 합니다.
	\iffalse
...
다음의 섹션들은 측정 에러들을 처리하는 방법들을 이야기 하는데,
...
먼저, 다른 프로세스, 쓰레드, 그리고 태스크에 의해 가해질 수 있는 간섭을 봅시다.
POSIX \co{sched_setaffinity()} 시스템 콜은 많은 태스크들을 특정 CPU 들로부터
몰아내고 당신의 테스트들을 같은 그룹의 CPU 들에 가둬두는데 사용될 수 있습니다.
...
사용되는 per-CPU 커널 쓰레드에 대해서는 아무일도 해주지 못합니다.
\iffalse
...
per-CPU 커널 쓰레드로부터의 간섭을 피하는 한가지 방법은, 예를 들어 POSIX
...
심지어 없애버릴 수도 있습니다.
하지만, 쓰레드 인터럽트가 없다 해도 디바이스 인터럽트로부터의 간섭을 막는
효과는 없습니다.
...
얻어질 수도 있습니다.
시스템의 인터럽트 벡터들의 리스트, 각 CPU 에 의해 얼마나 많이 처리되는지,
...
\QuickQuizAnswer{
	...
	그러나, 어떤 이유로 테스트 되는 코드를 그 어플리케이션 내에 두어야만
	한다면,
...
간섭을 예방할 수 없다면, 증상에 기반해서 간섭을 찾아내고 그 간섭으로 영향을
...
Section~\ref{sec:debugging:Detecting Interference Via Statistics} 에서는
...
스위치가 일어났을 겁니다.
리눅스 기반의 시스템에서 컨텍스트 스위치는 \path{/proc/<PID>/sched} 안의
...
\item	테스트 수행 결과 중 합리적 수준의 분량은 좋은 데이터를 내놓을 것이다.
\iffalse
...
있도록 합니다: 만약 간섭의 효과가 이 불확실성에 비해서 크다면, 이는 나쁜
데이터를 버리는 것을 덜어줄 겁니다.
...
Listing~\ref{lst:debugging:Statistical Elimination of Interference} 는 이
방법을 구현한 \co{sh}/\co{awk} 스크립트를 보입니다.
...
\begin{description}
\item	[\lopt{divisor}\nf{:}] 리스트를 나누게 될 세그먼트들의 갯수로,
	예를 들어 4의 값을 갖게 되면 데이터 원소들의 앞쪽 4분의 1이 좋은
		데이터라고 가정됩니다.
	기본값은 3입니다.
\item	[\lopt{relerr}\nf{:}] 상대적 측정 에러.  이 스크립트는 이
	에러보다 적은 차이값을 갖는 값들은 모두 같은 의도로 만들어진 것이고
		같은 목적을 위한 것이라고 가정합니다.
...
\QuickQuizAnswer{
	...
	물론, 절대적인 차이보다 표준편차를 사용해서 비슷한 효과를 얻을 수
	있도록
...
\QuickQuizAnswer{
	...
	평균과 표준편차에 기반하는 많은 방법들이 이런 종류의 데이터 집합에
	대해서 비슷한 문제를 가지고 있음을 알아두시기 바랍니다.
	\iffalse
...
검증은 결코 엄밀한 과학이 될수는 없겠지만, 정리된 방법은 해야하는 일에 걸맞는
검증 도구를 선택하는걸 도와서
Figure~\ref{fig:debugging:Choose Validation Methods Wisely} 에 재밌게 그려진
것과 같은 상황을 피할 수 있게 할 것이기에 정리된 방법을 통해서 많은 것을 얻을
수 있을 겁니다.
\iffalse
...
선택의 핵심은 통계입니다.
이 챕터에서 소개된 방법들은 거의 모든 경우에 잘 동작하지만, 이것들은 한계를
가지고 있습니다.
이 한계들은
...
다행히도 우리에게는 주어진 프로그램이 동작을 정지할 것인지 아닌지만이 아니라
...
생길 수 있는 한가지 문제는 다양성으로, 반복된 테스트 수행 각각은 상당히 다른
결과들을 내놓을 수도 있습니다.
...
다양한 결과가 나타나는 한가지 이유는 혼동스러운 요소들입니다.
...
이끌어내지는 못할 겁니다.
여기서 마땅히 해야할 일은 리스트 길이에 대한 상수를 가지고 있거나 리스트 길이에
따른 CPU 시간의 관계를 측정하는 식으로 리스트의 길이를 제어하는 것일 겁니다.
\iffalse
...
물론, 이 충고는 여러분이 혼동되는 요소들에 대해서 인지하고 있다는 가정을 가지고
...
어떤 경우에는 이런 경우에 좋은 효과를 줄 수 있는 한가지 중요한 도구는 정형적
검증으로, 다음 챕터, 더 이론적으로는
