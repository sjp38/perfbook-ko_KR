...
\QuickQuizB{
	웃기네요!!!
	어쨌건, 남들보다 늦게 올바른 답을 얻는게 올바르지 않은 답을
	얻는것보다는 낫지 않겠습니까???
	...
	이 질문은 아예 어떤 답도 계산하지 않는 선택지를 고려하는 방향까지 가게
	되는데 그렇게 함으로써 답을 계산하는 비용을 고려하는데까지 이르릅니다.
	예를 들어, 정확한 모델이 존재하며 여러분이 그 모델을 날씨보다는 빠르게
	수행하고자 하는 단기적 날씨 예보의 경우를 생각해 봅시다.

	이 경우, 이 모델이 실제 날씨보다 빠르게 수행되는 것을 방해하는 모든
	성능 버그는 모든 예보를 막습니다.
	커다란 슈퍼컴퓨터 클러스터를 구매한 전체 목표는 날씨 예보였음을 놓고
	보면, 여러분이 날씨보다 빠르게 그 모델을 돌릴 수 없다면 여러분은 그
	모델을 아예 돌리지 않는게 나을 겁니다.

	더 가혹한 예들을 안전에 치명적인 리얼타임 컴퓨팅 분야에서 찾아볼 수
	있을 겁니다.
...
%
\QuickQuizE{
	하지만 그 모든 어플리케이션 병렬화의 어려운 일을 하고자 한다면, 왜 그걸
	옳게 하지 않죠?
	왜 최적의 성능과 선형 확장성 미만의 것에 만족합니까?
	...
	그 정신과 열망에 진심으로 경의를 표하는 바입니다만 당신은 프로그램
	완료의 지연에 의한 높은 비용을 잊고 있습니다.
	극단적인 예를 들어, 싱글쓰레드 어플리케이션에서의 40\,\% 성능 하락이
	매일 사람 한명을 죽게 하고 있다고 해봅시다.
	더 나아가 여덟개 CPU 시스템에서 순차적 버전에 비해 50\,\% 빠르게
	동작하지만 더러운 병렬 프로그램을 하루만에 만들 수 있지만 최적의 병렬
	프로그램을 만드는데에는 4개월의 힘든 설계, 코딩, 디버깅, 그리고 튜닝이
	필요하다고 해봅시다.
...
따라서 병렬 프로그램을 검증하는것은 그 성능을 검증하는 것도 포함해야합니다.
하지만 성능을 검증함은 그 프로그램을 평가하기 위해 수행할 워크로드와 성능지표를
필요로 함을 의미합니다.
이 필요성은 종종 \emph{성능 벤치마크} 에 의해 충족되는데, 다음 섹션에서 이야기
됩니다.
...
물론, 완전하게 공평한 유일한 프레임웍은 의도된 어플리케이션 그 자체입니다.
...
그 어플리케이션을 추상화한 벤치마크를 만드는 게 이 장애물을 극복하는데
도움됩니다.
...
여럿 있습니다.
두개의 대중적인 선택은 \co{gprof} 와 \co{perf} 입니다.
단일 프로세스 프로그램에서 \co{perf} 를 사용하기 위해선 여러분의 커멘드 앞에
\co{perf record} 를 붙이고 그 커맨드가 완료된 후 \co{perf report} 라고
입력하세요.
...
\co{perf} 같은 어떤 도구들은 내장된 차이점 프로파일링 지원이 있습니다.
...
\QuickQuiz{
	예를 들면 캐쉬와 메모리 배치 사이의 상호작용 같은 다른 오류의 원천들은
	어떡하죠?
	...
	메모리 배치의 변화는 실제로 비현실적인 수행시간 감소를 초래할 수
	있습니다.
	예를 들어, 특정 마이크로벤치마크가 거의 항상 L0 캐쉬의 용량을 넘치게
	하지만 올바른 메모리 배치에서는 항상 딱 맞는다고 해봅시다.
	이게 정말 문제라면, 메모리 배치를 완전히 제어하기 위해 여러분의
	마이크로벤치마크를 huge page 를 이용해서 (또는 커널에서, 또는 bare
	metal 에서) 수행하는 걸 고려해 보세요.

	하지만 다양한 메모리 배치 병목이 존재함을 알아두시기 바랍니다.
	메모리 대역폭에 민감한 벤치마크는 (행렬 계산과 관계되는 것 같은) 메모리
	병렬성을 극대화 하기 위해 수행되는 쓰레드를 사용 가능한 코어와 소켓들
	사이로 흩뿌려야 합니다.
	그것들은 또한 데이터를 NUMA 노드, 메모리 컨트롤러, 그리고 DRAM 칩
	사이에 가능한 만큼 분배해야 합니다.
	대조적으로, 메모리 응답시간에 민감한 벤치마크는 (가장 잘 확장되지
	못하는 어플리케이션을 포함해) 그대신 지역성을 최대화 해야 해서, 다른
	코어와 소켓을 추가하기 전에 개별 코어와 소켓을 최대한 사용해야 합니다.
...
다음 섹션은 이 측정 오류를 다루는 방법들을 이야기 하는데,
...
POSIX \co{sched_setaffinity()} 시스템콜은 대부분의 태스크를 특정 CPU
집합으로부터 떨어뜨리고 여러분의 테스트를 같은 그룹에 묶이게 하기 위해 사용될
수 있습니다.
리눅스에 있는 사용자 단계의 \co{taskset} 커맨드가 같은 목적으로 사용될 수
있습니다만, \co{sched_setaffinity()} 와 \co{taskset} 둘 다 높은 권한을 필요로
합니다.
...
리얼타임 우선순위로 돌리는 것으로, 예를 들면 POSIX \co{sched_setscheduler()}
시스템콜을 사용할 수 있습니다.
...
\path{/proc/irq} 디렉토리를 통한 쓰레드 기반 인터럽트의 일부 조정을 허락합니다.
각 숫자 이름 디렉토리는 \co{smp_affinity} 와 \co{smp_affinity_list} 를
갖습니다.
...
``\co{cat /proc/interrupts}'' 를 사용할 수 있습니다.
...
드러났습니다.\footnote{
	...
	불능화 되게 하는 \co{NO_HZ_FULL} adaptive-ticks 프로젝트를 Frederic
...
\QuickQuiz{
	테스트 되는 코드를 격리하기 위해 추천된 기법들 또한 그 코드의 성능에
	영향을 끼치지 않을까요?  특히나 그게 더 큰 어플리케이션 내에서
	돌아간다면요.
	...
	실제로 그럴수도 있습니다만, 대부분의 마이크로벤치마크에서는 여러분은
	테스트 되는 코드를 그걸 감싸는 어플리케이션으로부터 꺼낼 겁니다.
	그러나, 어떤 이유로 여러분이 그 테스트 되는 코드를 어플리케이션 내에
	두어야 한다면,
	Section~\ref{sec:debugging:Detecting Interference} 에서 이야기되는
	기법을 사용해야 할 겁니다.
...
물론, 흥미로운 행동을 만드는게 정말 그 간섭이라면, 여러분은 오히려 간섭이 잘
일어나게 해야 할텐데, 이 경우엔 그걸 막을 수 없는게 문제가 되지 않습니다.
하지만 정말 간섭이 없는 측정을 필요로 한다면, 간섭을 막는 대신 다음 섹션에
설명되는 것처럼 간섭을 파악해야 할수도 있습니다.
...
시스템에서는 \path{/proc/<PID>/sched} 의 \co{nr_switches} 를 통해 알 수
있습니다.
비슷하게, 인터럽트 기반의 간섭은 \path{/proc/interrupt} 파일을 통해 알 수
있습니다.
...
에 보인 것처럼 \co{getrusage()} 시스템콜을 통해 특정 쓰레드의 컨텍스트 스위치
횟수를 알 수 있습니다.
이 시스템콜은 또한 minor page fault 와 (\co{ru_minflt}) major page fault
(\co{ru_majflt}) 횟수를 파악하는 데에도 사용될 수 있습니다.
...
은 이 방법을 구현하는 간단한 \co{sh}/\co{awk} 스크립트를 보입니다.
...
Listing~\ref{lst:debugging:Statistical Elimination of Interference} 의
\clnrefrange{param:b}{param:e} 는 이 패러미터들의 기본값을 설정하고,
\clnrefrange{parse:b}{parse:e} 는 이 패러미터에 대한 커맨드 라인의 덮어쓰기를
처리합니다.
\end{fcvref}
\begin{fcvref}[ln:debugging:datablows:whole:awk]
라인~\lnref{invoke} 에서의 \co{awk} 수행은
\co{divisor}, \co{relerr}, 그리고 \co{trendbreak} 변수를 \co{sh} 의 것들과
맞춥니다.
일반적인 \co{awk} 방식에서,
\clnrefrange{copy:b}{end} 는 각 입력 줄별로 수행됩니다.
라인~\lnref{copy:b} 와~\lnref{copy:e} 사이의 반복문은 입력 y 값을 \co{d} 배열에
복사하는데, 라인~\lnref{asort} 가 증가 순서로 정렬을 하는 배열입니다.
라인~\lnref{comp_i} 는 \co{divisor} 를 적용하고 반올림 함으로써 믿을 수 있는 y
값을 계산합니다.
...
\Clnrefrange{delta}{comp_max:e} 는 y 값의 최대값의 아래쪽 한계인 \co{maxdelta}
를 계산합니다.
여기서, 라인~\lnref{maxdelta} 는 데이터의 믿어지는 영역을 넘어서는 값의 차이를
\co{divisor} 로 곱하는데, 이는 믿어지는 영역의 값들의 차이를 전체 y 값 집합으로
외삽합니다.
그러나, 이 값은 상대적 오류보다는 훨씬 작을 수도 있으므로,
라인~\lnref{maxdelta1} 은 절대적 오류값을 (\co{d[i] * relerr}) 구하고 이를
데이터의 믿어지는 영역의 차이인 \co{delta} 에 더합니다.
라인~\lnref{comp_max:b} 와~\lnref{comp_max:e} 는 이 두 값 중 큰 값을 구합니다.

\clnrefrange{add:b}{add:e} 의 반복문의 각 수행은 좋은 데이터 집합에 다른 데이터
값을 더하려 시도합니다.
\Clnrefrange{chk_engh}{break} 는 추세 깨뜨림 차이를 계산하는데,
라인~\lnref{chk_engh} 는 우리가 아직 추세를 계산하기 충분한 값을 갖고 있지
않다면 이 한계를 불능화 시키며, 라인~\lnref{mul_avr} 는 \co{trendbreak} 를 좋은
데이터 집합의 데이터 값들의 쌍들 사이 차이값의 평균으로 곱합니다.
라인~\lnref{chk_max} 가 후보 데이터 값이 최대 값의 하한선을 (\co{maxdelta})
넘어서\emph{고} 이 후보 데이터 값과 그 앞의 것 사이의 차이가 추세 깨뜨림 차이
(\co{maxdiff}) 를 넘어섬을 파악하면, 라인~\lnref{break} 가 이 반복문을
나갑니다: 우린 모든 좋은 데이터 집합을 얻었습니다.
...
\Clnrefrange{comp_stat:b}{comp_stat:e} 는 이어서 통계를 계산하고 출력합니다.
\end{fcvref}
...
\QuickQuizB{
	이 방법은 그저 이상해요!
	왜 통계 수업에서 배운대로 중간값과 표준편차를 이용하지 않죠?
	...
	중간값과 표준편차는 이 일을 하기 위해 설계되지 않았기 때문입니다.
	이를 보기 위해, 측정에 1\,\% 상대 오류를 갖는다고 가정하는 다음 데이터
	집합에 중간값과 표준편차를 적용해 보시기 바랍니다:
	...
	문제는 중간값과 표준편차가 어떤 종류의 측정 오류 가정을 갖지 않으며,
	따라서 그것은 49,500 근처의 값과 49,900 근처의 값들 사이의 차이를
	실제로는 추정된 측정 오류 한계 내에 있음에도 통계적으로 심각하다고
	본다는 겁니다.

	물론, 비슷한 효과를 위해 절대적 차이값 대신 표준편차를 사용하는
	Listing~\ref{lst:debugging:Statistical Elimination of Interference}
	와 비슷한 스크립트를 만들 수 있겠는데 이는 관심있는 독자 여러분의
	연습문제로 남겨두겠습니다.
	동일한 데이터 값들의 문자열에 의해 일어날 수 있는 divide-by-zero 오류를
	막는데 주의하세요!
...
%
\QuickQuizE{
	하지만 믿어지는 데이터 그룹의 모든 y 값이 0이면 어떡하죠?
	그건 이 스크립트가 모든 0이 아닌 값을 제거하게 하지 않을까요?
	...
	실제로 그럴겁니다!
	하지만 여러분의 성능 측정이 정확히 0 값을 만들어낸다면, 여러분의 성능
	측정 코드를 깊이 들여다봐야 할수도 있습니다.

	중간값과 표준편차에 기반한 많은 접근법들이 이런 종류의 데이터에 비슷한
	문제를 가질 것임을 알아두시기 바랍니다.
...
통계적 간섭 탐지가 유용할 수 있지만, 마지막 수단으로만 사용되어야 합니다.
간섭을 차단하는게 최선이고 (Section~\ref{sec:debugging:Isolation}), 그게
불가능하다면 측정을 통해 간섭을 탐지하십시오
(Section~\ref{sec:debugging:Detecting Interference Via Measurement}).
...
검증은 결코 정확한 과학이 되지 못하겠지만, 조직화된 접근은 여러분이 여러분의
