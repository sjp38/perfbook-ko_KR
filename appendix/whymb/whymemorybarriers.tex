...
짧게 말하자면, 메모리 참조를 재배치 하는 것은
\cref{sec:cpu:Overheads} 에서, 그리고 특히
\QuickQuizRef{\QspeedOfLightAtoms} 에서 암시된 하드웨어 성능 질문에서 설명되었
듯 유한한 빛의 속도와 0이 아닌 원자의 크기 덕분에 훨씬 나은 성능을 가능하게
하기 때문입니다.
따라서 메모리 배리어는 올바른 오퍼레이션이 순서 맞춰진 메모리 참조에 의존적인
동기화 기능들 같은 것을 위해 순서를 강제하기 위해 필요합니다.
