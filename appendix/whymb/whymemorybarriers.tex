...
거짓말 같다면 온라인 문서~\cite{Compaq01,WilliamPugh2000Gharachorloo} 를
참고하세요.
...
집합을 제공하는데, 다음과 같습니다:
\begin{description}
\item	\co{smp_mb()}: 로드 오퍼레이션들과 스토어 오퍼레이션들 모두 순서
	맞추는 ``메모리 배리어''.
	이는 이 메모리 배리어보다 앞의 로드들과 스토어들은 이 메모리 배리어
	뒤의 로드들과 스토어들보다 먼저 메모리에 처리될 것이란 뜻입니다.
\item	\co{smp_rmb()}: 로드들만 순서 맞추는 ``읽기 메모리 배리어''.
\item	\co{smp_wmb()}: 스토어들만 순서 맞추는 ``쓰기 메모리 배리어''.
\item	\co{smp_read_barrier_depends()} 는 앞의 오퍼레이션들에 의존적인
...
\end{description}
\fi
The \co{smp_mb()}, \co{smp_rmb()}, and \co{smp_wmb()} 는 또한 컴파일러가
배리어 전후로 메모리 오퍼레이션 순서를 재배치 하는 효과를 줄 수 있는 최적화를
하지 않도록 강제합니다.
\co{smp_read_barrier_depends()} 기능 역시 비슷한 효과를 갖지만, Alpha CPU
들에서만 그렇습니다.
\IfInBook{이런 기능들에 대해 더 자세한 정보를 위해선
Chapter~\ref{chp:memorder:Memory Ordering} 를 참고하세요.}
\iffalse
...
\co{read_barrier_depends()})을 각각 갖습니다.
대부분의 경우에는 \co{smp_} 버전들이 사용되어야 합니다.
...
있습니다.
이 그림의 line~9 의 \co{smp_wmb()} 는 line 6-8 의 원소 초기화가 line~10 에서
...
\co{head.next} 의 로드는 cache bank~0 에 액세스 할 수 있을 것이며, line~20
에서의 \co{p->key} 의 로드와 line~23 에서의 \co{p->next} 의 로드는 cache bank~1
에 접근할 수 있을 겁니다.
Alpha 에서, Figure~\ref{fig:app:whymb:Insert and Lock-Free Search} line~6-8 에
의해 야기된 (\co{p->next}, \co{p->key}, 그리고 \co{p->data} 를 위한) 캐시
무효화는 \co{smp_wmb()} 에 의해 line~10 이전에 (\co{head.next} 에 대해)
...
하고 있지 않을 수 있습니다.
이런 경우 새 원소 (\co{p->next}, \co{p->key}, 그리고 \co{p->data}) 에 대한 캐시
무효화 결과의 도착이 지연되어 읽는 CPU 는 \co{head.next} 를 위한 새 값은
받지만, 캐시되어 있던, \co{p->key} 와 \co{p->next} 의 예전 값을 읽는 결과가
나올 수 있습니다.
여전히 제가 가상의 이야기를 하고 있다고 생각된다면, 더 자세한 정보를 위해 앞서
이야기한 문서~\cite{Compaq01,WilliamPugh2000Gharachorloo} 를 보세요.\footnote{
...
포인터를 가져오는 것과 포인터를 디레퍼런스 사이에 \co{smp_rmb()} 를 넣어
문제를 해결할 수 있을 겁니다.
하지만, 이는 읽는 쪽의 데이터 의존성을 고려하는 (i386, IA64, PPC, 그리고 SPARC
같은) 시스템들에는 불필요한 오버헤드를 가져올 수 있습니다.
이런 시스템에서의 오버헤드를 없애기 위해 \co{smp_read_barrier_depends()}
...
배리어를 \co{smp_wmb()} 자리에 구현할 수도 있을 겁니다.
...
물론, 데이터 의존성을 지키는 CPU 들은 그런 배리어를 단순히 \co{smp_wmb()} 로
정의할 것입니다.
...
때문에, \co{smp_mb()} 는 {\tt mb}, \co{smp_rmb()} 는 {\tt rmb}, 그리고
\co{smp_wmb()} 는 {\tt wmb} 입니다.
Alpha 는 \co{smp_read_barrier_depends()} 가 no-op 이 아니라 \co{smp_mb()}
인 유일한 CPU 입니다.
\iffalse
...
리눅스 \co{smp_mb()} 기능의 AMD64 구현은 {\tt mfence} 이고, \co{smp_rmb()}
는 {\tt lfence}, 그리고 \co{smp_wmb()} 는 {\tt msfence} 입니다.
...
IA64 {\tt mf} 인스트럭션이 리눅스 커널의 \co{smp_rmb()}, \co{smp_mb()},
그리고 \co{smp_wmb()} 기능을 위해 사용됩니다.
...
하지만, 리눅스의 \co{smp_wmb()} 인스트럭션은 (어차피 드라이버가 MMIO 의
...
\co{smp_rmb()} 와 {\tt rmb()} 는 가벼운 {\tt lwsync} 인스트럭션으로 정의되어
있습니다.
\iffalse
...
리눅스 \co{smp_mb()} 기능은 앞의 네개 인스트럭션들을
\co{membar #LoadLoad | #LoadStore | #StoreStore | #StoreLoad} 처럼
함께 사용해서 메모리 오퍼레이션들이 완전히 순서맞춰지도록 구현합니다.

그런데, 왜 \co{membar #MemIssue} 가 필요할까요?
\co{membar #StoreLoad} 는 뒤의 로드가 값을 스토어 버퍼에서 가져오도록 허용할
수 있는데, 이는 해당 스토어가 읽힐 값에 사이드 이펙트를 주는 MMIO 레지스터로의
쓰기였다면 문제가 될 수 있기 때문입니다.
반면, \co{membar #MemIssue} 는 해당 로드들을 실행하기 전에 해당 스토어
버퍼들이 비워질 때까지 기다리도록 할 것이므로, 해당 로드는 실제로 그 값을 MMIO
레지스터에서 읽어오도록 보장할 것입니다.
드라이버들은 대신 \co{membar #Sync} 를 사용할 수 있습니다만, 상대적으로 비싼
\co{membar #Sync} 의 기능이 모두 필요하지 않은 경우라면 더 가벼운
\co{membar #MemIssue} 가 선호될 것입니다.
\iffalse
...
\co{membar #Lookaside} 는 \co{membar #MemIssue} 의 경량화 버전으로, MMIO
레지스터에 쓰기를 하는데 그 쓰기가 뒤에 그 레지스터로부터 읽힐 값에 영향을 주는
경우 융요합니다.
하지만, 해당 MMIO 레지스터에의 쓰기가 뒤의 {\em 다른} MMIO 레지스터로부터의
읽기에 영향을 끼친다면 더 무거운 \co{membar #MemIssue} 가 사용되어야만
합니다.

현재의 정의는 일부 드라이버에서 버그에 취약할 수 있어 보임에도 왜 SPARC 가 {\tt
wmb()} 를 \co{membar #MemIssue} 로, 그리고 \co{smp_wmb()} 가
\co{membar #StoreStore} 로 정의하지 않았는지는 불명확합니다.
...
쓰기 순서에 동의하므로 해당 CPU~\cite{IntelXeonV3-96a} 에서 \co{smp_wmb()}
기능은 no-op 으로 동작합니다.
하지만, 컴파일러가 \co{smp_wmb()} 를 가로지르는 재배치의 결과를 만드는
최적화를 가하는 것을 막기 위해 컴파일러 지시어가 필요합니다.

반면, x86 CPU 들은 전통적으로 로드들에 대해선 순서에 대한 보장을 제공하지
않았으므로, \co{smp_mb()} 와 \co{smp_rmb()} 기능들은 {\tt lock;addl} 로
확장됩니다.
...
SSE 를 갖는 CPU 들은 \co{smp_mb()} 를 위해 {\tt mfence} 를,
\co{smp_rmb()} 를 위해 {\tt lfence} 를, 그리고 \co{smp_wmb()} 를 위해
...
있고, 이런 CPU 들에서는 \co{smp_wmb()} 가 {\tt lock;addl} 로 정의되어야만
합니다.
...
{\tt bcr 15,0} 인스트럭션은 리눅스 \co{smp_mb()}, \co{smp_rmb()}, 그리고
\co{smp_wmb()} 기능으로 사용됩니다.
이들은 또한 Table~\ref{tab:app:whymb:Summary of Memory Ordering} 에 나온 것과
같이 비교적 강력한 메모리 순서 규칙을 가져서, \co{smp_wmb()} 가 {\tt nop} 이
