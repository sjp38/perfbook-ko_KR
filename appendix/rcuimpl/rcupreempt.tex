...
\co{rcu_read_lock()} 도 \co{rcu_read_unlock()} 도 메모리 배리어를 포함하고 있지
...
\co{list_del_rcu()} 와 \co{call_rcu()} 를 수행할 수 있는 가장 최신의 순간은
카운터 뒤집기를 알리는 schduling clock 인터럽트 직전입니다.
\co{call_rcu()} 호출은 콜백을 CPU~0 의 \co{next} 리스트에 놓고, 이 인터럽트는
이 콜백을 \co{next} 리스트에서 \co{wait[0]} 리스트로 옮길 겁니다.
이 콜백은 다음 카운터 뒤집기를 뒤따르는 첫번째 schduling-clock 인터럽트 때에
다시 (\co{wait[0]} 리스트에서 \co{wait[1]} 리스트로) 옮겨집니다.
비슷하게, 이 콜백은 값 3을 되게 하는 카운터 뒤집기를 뒤따르는 CPU~0 의 첫번째
scheduling-clock 인터럽트 때에 \co{wait[1]} 에서 \co{done} 리스트로 옮겨질
겁니다.
\iffalse
...
여기서의 \co{rcu_read_lock()} 이 (값을 1이 되게 하는) 첫번째 카운터 뒤집기를
뒤따라서, 이 \co{rcu_read_lock()} 이 CPU~1 의 \co{rcu_flipctr[1]} 카운터의 값을
증가시킨다고 가정해 봅시다.
\co{rcu_read_lock()} 은 어떤 메모리 배리어도 포함하지 않기 때문에, 이 크리티컬
섹션의 내용은 이 CPU 에 의해 더 먼저 수행될 수 있다는 점을 기억하세요.
하지만, 그림에 보인 것처럼, 이 더 빠른 수행은 CPU~1 에 의해 수행되는 앞의
메모리 배리어를 앞설 수는 없습니다.
그러나 이는 \co{rcu_dereference()} 가 CPU~0 의 \co{list_del_rcu()} 에 의해
...
이 \co{rcu_read_lock()} 은 index-1 카운터의 값을 증가시켰으므로, 연관된
\co{rcu_read_unlock()} 은 인덱스 1을 위한 ``기존 카운터들이 0이 되는'' 이벤트를
앞서야만 합니다.
하지만, \co{rcu_read_unlock()} 은 메모리 배리어를 포함하고 있지 않기에, 연관된
...
\QuickQuiz{}
	\co{rcu_read_lock()} 의 irq 비활성화가 preemption 비활성화로
	대체되었다고 생각해 봅시다.
	\co{GP_STAGES} 에 어떤 영향이 있을 수 있을까요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	\co{GP_STAGES} 의 어떤 유한한 값도 충분치 않습니다.
	Oleg Nesterov 에 감사컨대, 다음의 시나리오가 이를 보입니다:

	낮은 우선순위의 Task~A 가 CPU~0 에서 \co{rcu_read_lock()} 을 수행했고,
	따라서 \co{per_cpu(rcu_flipctr, 0)[0]} 의 값을 증가시켜서, 이제 값이
	1이 된다고 생각해 봅시다.
	더 나아가서 Task~A 가 이제 무한정 preemption 당한다고 생각해 봅시다.

	이 상황에서 다음의 이벤트들이 발생한다고 생각해 봅시다:
	\iffalse
	...
	\fi
	\begin{enumerate}
	\item	Task~B 가 역시 CPU~0 에서 \co{rcu_read_lock()} 을 수행하기
		시작해서, 여전히 0의 값을 갖는 \co{rcu_ctrlblk.completed} 의
		낮은 쪽 bit 을 가져옵니다.
	\item	Task~B 가 현재의 grace-period stage 가 완료되기에 충분한, 또한
		RCU grace-period state machine 이 \co{rcu_ctrlblk.complete}
		카운터가 진행되기에 충분한 수의 scheduling-clock 인터럽트들에
		인터럽트 당해서 그 바닥 bit 이 이제 1의 값이 되고 모든 CPU 들이
		이 값 증가 오퍼레이션을 확인합니다.
	\item	CPU~1 이 index==0 카운터들의 값의 합을 계산하기를 시작하는데
		이제 Task~A 의 값 증가에 의해 1의 값을 갖는
		\co{per_cpu(rcu_flipctr, 0)[0]} 부터 시작합니다.
		따라서 CPU~1 의 지역 변수 \co{sum} 은 1이 됩니다.
	\iffalse
	...
	\fi
	\item	Task~B 가 인터럽트로부터 돌아와서 \co{rcu_read_lock()} 수행을
		재개하고, \co{per_cpu(rcu_flipctr, 0)[0]} 의 값을 증가해서 그
		값은 이제 2가 됩니다.
	\item	Task~B 가 CPU~2 로 옮겨집니다.
	\item	Task~B 가 RCU read-side 크리티컬 섹션 수행을 완료하고,
		\co{rcu_read_unlock()} 을 수행하는데, 이는
		\co{per_cpu(rcu_flipctr, 2)[0]} 의 값을 감소시켜서 그 값은 이제
		-1이 됩니다.
	\item	CPU~1 은 이제 \co{per_cpu(rcu_flipctr, 1)[0]} 의 값과
		\co{per_cpu(rcu_flipctr, 2)[0]} 의 값을 자신의 로컬 변수
		\co{sum} 에 더해서 값 0을 얻습니다.
	...
	이 이벤트들은 무한정 반복될 수 있어서, 어떤 유한한 값의 \co{GP_STAGES}
	로 Task~A 를 망쳐버리는 걸 막을 수 없습니다.
	이 이벤트들은 \co{rcu_ctrlblk.completed} 의 값 증가를 확인하는 CPU 들에
	의해 만들어지는 약속의 중요성을 잘 보입니다.

	따라서, \co{rcu_rad_lock()} 이 irq 비활성화 없이 있을 수 있기 위애서는
	grace-period state 로의 더 많은 변화가 필요합니다.
	\iffalse
...
\QuickQuiz{}
	왜 \co{rcu_dereference()} 가 메모리 배리어를 앞설 수 없나요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	해당 메모리 배리어는 인터럽트 핸들러 안에서 수행되고, 인터럽트들은 PC
	의 하나의 값만큼 정확하기에 인터럽트는 정확히 코드의 한 순간에서
	발생하기 때문입니다.
	따라서, 만약 \co{rcu_dereference()} 가 해당 메모리 배리어를 앞선다면,
	해당 인터럽트는 \co{rcu_dereference()} 뒤에 발생했을 것이고, 따라서 이
	인터럽트는 또한 이 RCU read-side 크리티컬 섹션을 시작한
	\co{rcu_read_lock()} 뒤에 발생했을 것입니다.
	이는 \co{rcu_read_lock()} 이 grace-period 카운터의 이전 값을 사용하도록
	할 수 있는데, 이는 결국 연관된 \co{rcu_read_unlock()} 이 두번째가
	아니라 첫번째 `` 기존 카운터들이 0 [0]'' 이벤트를 앞설 수도 있음을
	의미합니다.
	이는 결국 read-side 크리티컬 섹션이 훨씬 짧았을 것을---이 방법의 핵심은
	가장 긴 RCU read-side 크리티컬 섹션을 판별해 내고자 했던 것이란 점에서
	생산적이지 않았을 것을 의미하게 됩니다.
	\iffalse
...
\QuickQuiz{}
	...
	\fi
\QuickQuizAnswer{
	첫째로, 부정확한 말의 문제는 그게 적어도 대중적인
	마이크로프로세서들에서는 실제로는 존재하지 않는, 하나의 전역적인
	시간대가 존재하는 것 같은 인상을 준다는 것임을 알아두는게 중요합니다.
	둘째로, 메모리 배리어들은 인지의 순서에 대한 것이지, 시간에 대한 것이
	아니라는 것을 알아두는게 중요합니다.
	마지막으로, 앞의 말을 더 정확히 하려면 다음과 같을 겁니다: ``만약 CPU~0
	가 CPU~1 의 값 증가로 만들어지는 값을 로드한다면, CPU~0 의 모든 이를
	뒤따르는 로드 오퍼레이션은 CPU~1 의 연관된 스토어 오퍼레이션들이 CPU~1
	의 앞의 마지막 메모리 배리어를 앞섰다면 해당 스토어 오퍼레이션들로 인한
	값들을 읽게 됩니다.''
	\iffalse
	...
	이 더 정확한 버전도 좀 움찔거리는 공간을 남겨두고 있습니다.
	``뒤따르는'' 이란 단어는 명시적인 메모리배리어나 해당 CPU 의 메모리
	순서규칙에 의해 ``뒤로 순서지어진'' 이라는 의미가 되어야만 합니다.
	또한, 메모리 배리어들은 연관된 오퍼레이션들을 순서짓기 충분할 만큼
	강력해야만 합니다.
	예를 들어, CPU~1 의 기존의 마지막 메모리 배리어는 스토어 오퍼레이션들을
	순서지어야 합니다 (예를 들어, \co{smp_wmb()} 나 \co{smp_mb()} 여야
	합니다).
	비슷하게, 만약 CPU~0 가 자신의 마지막 로드가 CPU~1 의 값 증가를 본
	로드를 뒤따른다는 걸 보장하기 위해 명시적 메모리 배리어가 필요하다면,
	이 메모리 배리어는 \co{smp_rmb()} 이거나 \co{smp_mb()} 여야 합니다.

	일반적으로, 병렬 알고리즘을 제공할 땐 상당한 신경을 써야 합니다.
	\iffalse
