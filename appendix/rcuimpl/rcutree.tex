...
CPU-online 이벤트는 \co{rcu_cpu_notify()} 가 \co{rcu_online_cpu()} 를 호출하게
하는데, 이 함수는 새로 켜지는 CPU 의 dynticks state 를 초기화 하며, 이어서 새로
들어온 CPU 의 \co{rcu_data} 구조체를 초기화 하고 이 CPU 의 (global 한
\co{->onofflock} 으로 보호되는) bit 들을 나중의 grace period 들이 이 CPU 의
quiescent state 를 기다리도록 하게 설정하는 \co{rcu_init_percpu_data()} 를
호출합니다.
마지막으로, \co{rcu_online_cpu()} 는 이 CPU 를 위한 RCU softirq vector 를
설정합니다.
\iffalse
...
\QuickQuiz{}
	Global 한 \co{->onofflock} 의 획득을 위한 동작들을 모두 보면, 수천개의
	CPU 들이 동작하는 경우에는 이게 상당한 락 경쟁이 되지 않을까요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	실제로, grace period 당 이 락에 대한 세개의 획득 동작만이 있을 것이고,
	각각의 grace period 는 수 밀리세컨드 동안 유지됩니다.
	이런 획득들 가운데 하나는 현재 grace period 를 위한 CPU 초기화에 의해서
	이루어지고, 나머지 두개는 CPU 의 online 화와 offline 화입니다.
	이 나중 두개의 동작은 CPU-hotplug 락킹으로 인해 동시에 수행될 수는
	없으므로, 언제든 이 락에 대해 최대 두개의 CPU 들만이 경쟁하게 됩니다.

	따라서 \co{->onofflock} 에 대한 락 경쟁은 수천개의 CPU 를 가진
	시스템에서도 문제가 되지 않습니다.
	\iffalse
...
\QuickQuiz{}
	Dyntick-idle CPU 탐지와 offline CPU 탐지를 합쳐서 코드를 단순화 시키는
	건 어떤가요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	그런 병합 작업이 최종적으로는 옳은 일이 될겁니다.
	하지만, 그러기 위해서는 다음과 같은 도전이 존재합니다:
	\iffalse
	...
	\begin{enumerate}
	\item	CPU 들은 그것들이 유보된 RCU 콜백들을 가지고 있는 동안은
		dyntick-idle 모드에 빠질 수 없는데, CPU 는 유보된 콜백들을
		가진채로 offline 으로 빠질 수 있습니다.
		이 말은 offline 이 되는 CPU 들은 다른 CPU 에게 자신의 콜백들을
		옮겨야 한다는 의미이며, 따라서 우린 CPU 들이 그냥 offline 으로
		빠지도록 할 수 없습니다.
	\item	오늘날의 리눅스 시스템은 실제 CPU 갯수보다 훨씬 많은
		\co{NR_CPUS} 를 가지고 동작합니다.
		따라서 통합을 하는 방법은 애초에 존재하지도 않으면서 offline 이
		아닌 CPU 들을 불필요하게 기다리는 것이 될 수 있습니다.
	\item	RCU 는 이미 부팅 중간에 CPU 들이 하나씩 online 이 되는
		상황에서도 동작할 수 있으며, 따라서 online 과정을 처리해야만
		합니다.
		이 온라인 과정은 grace-period 초기화와 배타적으로 수행되어야
		하며, 따라서 \co{->onofflock} 은 여전히 사용되어져야 합니다.
	\item	CPU 들은 종종 매우 빈번하게 dyntick-idle 모드를 들어갔나 나왔다
		하며, 따라서 dyntick-idle 모드에 들어가고 나오는 코드 경로를
		무겁게 하는건 합리적이지 않습니다.
	\iffalse
...
\co{CONFIG_RCU_CPU_STALL_DETECTOR} 커널 패러미터가 정의된 경우,
\co{record_gp_stall_check_time()} 함수는 시간과 이후의 3초간의 타임스탬프를
기록합니다.
현재의 grace period 가 그 시간까지 종료되지 않았다면, \co{check_cpu_stall()}
함수는 문제가 있는 부분을 체크해보고, 현재 CPU 가 붙잡고 있다면
\co{check_cpu_stall()} 을 호출하고, 다른 CPU 가 그러고 있다면
\co{print_cpu_stall()} 을 호출합니다.
...
시키는 간단한 스크립트를 사용합니다.
이 모듈은 dynticks idle 모드를 스트레스 테스트 하기 위해 \co{test_no_idle_hz}
모듈 패러키터를 받습니다.
...
\item    \co{CONFIG_TREE_RCU}: 거대한 SMP 시스템에서의 Classic RCU.
\item    \co{CONFIG_RCU_FANOUT}: 각 \co{rcu_node} 의 자식의 수.
\item    \co{CONFIG_RCU_FANOUT_EXACT}: \co{rcu_node} 트리의 밸런스를 맞춤.
\item    \co{CONFIG_HOTPLUG_CPU}: CPU 들이 offline 되고 online 되는 것을 허용.
\item    \co{CONFIG_NO_HZ}: dyntick-idle 모드를 활성화.
\item    \co{CONFIG_SMP}: multi-CPU 오퍼레이션을 활성화.
\item    \co{CONFIG_RCU_CPU_STALL_DETECTOR}: RCU 가 CPU 들이 언제 extended
	quiescent-state 를 들어가는지 파악할 수 있게 함.
\item    \co{CONFIG_RCU_TRACE}: Debugfs 안에 RCU trace 파일을 생성함.
\iffalse
...
우리는 heierarchical RCU 는 잘못된 lockdep 을 갖지는 않을 것이라는 아마도
자연스러운 가정 하에 \co{CONFIG_DEBUG_LOCK_ALLOC} 설정은 무시합니다.
10개의 환경 구성 변수들이 있는데, 이것들이 비의존적인 boolean 변수들이라면
1,024 개의 조합을 만들 겁니다.
다행히도 앞의 세개는 상호 배타적이어서 조합의 수를 384 개로 줄입니다만,
\co{CONFIG_RCU_FANOUT} 은 그 값을 2 부터 64 까지 가질 수 있어서, 조합의 수를
12,096 까지 늘립니다.
이건 상당한 수의 조합입니다.

주요한 발견 중 하나는 \co{CONFIG_NO_HZ} 와 \co{CONFIG_PREEMPT} 는
\co{CONFIG_CLASSIC_RCU} 나 \co{CONFIG_PREEMPT_RCU} 가 켜져 있다면 변경된 동작을
취할 수 있다는 것으로, 이 두개의 기존부터 존재한 RCU 구현들만이 앞의 두개의
환경구성 구현과 함께 변경되었기 때문입니다.
이는 가능한 조합 가운데 거의 3분의 2를 제거합니다.

더 나아가서, \co{CONFIG_RCU_FANOUT} 의 모든 설정 가능한 값이 상당히 다른 결과를
...
\item	2단계 밸런스 잡힌 트리.
\item	3단계 밸런스 잡힌 트리.
\item	\co{CONFIG_RCU_FANOUT} 은 밸런스 잡히지 않은 트리를 명시하지만
	\co{CONFIG_RCU_FANOUT_EXACT} 없이는 자동으로 밸런스 잡히게 되는
	자동으로 밸런스 잡힌 트리.
\item	밸런스 잡히지 않은 트리.
\iffalse
...
더 보자면, \co{CONFIG_HOTPLUG_CPU} 는 \co{CONFIG_SMP} 가 주어졌을 때에만 의미를
갖고, \co{CONFIG_RCU_CPU_STALL_DETECTOR} 는 의존성이 없으며, 한번만 테스트 되면
됩니다 (저보다 더 편집증적인 누군가는 이것도 \co{CONFIG_SMP} 를 켰을때와 껐을
때에 대해 모두 테스트할 수도 있겠습니다).
비슷하게, \co{CONFIG_RCU_TRACE} 는 한번만 테스트 되면 됩니다만, (저처럼) 정말로
편집증에 걸린 사람은 \co{CONFIG_NO_HZ} 를 켰을때와 껐을 때에 대해 모두 테스트
할 수 있을 겁니다.

이건 15개의 테스트 케이스만을 가지고도 RCU 의 상당한 부분을 커버할 수 있게
해줍니다.
모든 테스트 케이스들은 rcutorture 를 돌리기 위해, 그리고
\co{CONFIG_HOTPLUG_CPU=n} 이 실제로 영향을 끼치게 하기 위해 다음과 같은 환경
구성 패러미터들을 명시하게 됩니다.
\iffalse
...
\co{CONFIG_HOTPLUG_CPU} 의 경우를 위해 CPU offlining 과 onlining 과 함께
...
소프트웨어 고문: Geneva Convection 은 (아직은) 이를 금지하지 않고 있으며, 전
이걸 강력히 추천합니다!
\iffalse
...
이 RCU 의 계층형 구현은 락 경쟁을 줄이고, 불필요하게 dyntick-idle 상태에서
잠자는 CPU 들을 깨우지 않으면서 리눅스의 hotplug-CPU 코드 경로를 디버깅 하는
것을 돕습니다.
이 구현은 수천개의 CPU 를 가진 단일 시스템을 다루기 위해 설계되었는데, 64-bit
시스템들은 25만개 CPU 로 구조적 한계를 가지고 있으므로, 제가 예상한 한계점은
차후 몇년동안은 충분할 겁니다.

이 RCU 구현은 물론 몇가지 한계점도 가지고 있습니다:
\iffalse
...
\begin{enumerate}
\item	\co{force_quiescent_state()} 는 irq 가 비활성화 된채로 모든 CPU 들을
	스캔할 수 있습니다.
	이는 real-time RCU 구현에서는 치명적일 수 있으며, 따라서 preemptible
	RCU 에 계층이 도입될 필요가 있다면, 다른 방법이 필요할 겁니다.
	이는 4,096-CPU 시스템에서는 문제가 될 수 있습니다만, 이 방법을 따라야
	할지 다른 방법을 찾아야 할지를 증명하려면 그런 시스템에서의 실제
	테스트가 필요합니다.

	바쁘게 돌아가는 시스템에서는 \co{force_quiescent_state()} 의 스캔은
	일어나지 않을 것이라 예측될 수 있는데, CPU 들은 quiescent state 들을
	quiescent state 의 시작으로부터 3 jiffies 안에 지나가야 하기
	때문입니다.  적당히 바쁘게 돌아가는 시스템에서는 dynticks-idle 모드의
	CPU 들만이 스캔되어야 할겁니다.
	예를 들어 dynticks-idle CPU 가 스캔 중간에 인터럽트를 처리하는 것과
	같은 일부 경우에는 뒤따르는 스캔이 필요합니다.
	하지만, 그런 스캔 각각은 개별적으로 수행되고, 따라서 스케쥴링
	응답시간은 그런 스캔 하나의 오버헤드 만큼만 나빠질 겁니다.

	이 스캔이 문제라고 증명된다면, 한가지 직선적인 해결책은 이 스캔을
	점진적으로 하는 겁니다.
	이는 약간 코드 복잡도를 높일 수 있고 grace period 를 종료하는데 걸리는
	시간을 늘릴 수 있습니다만, 그럼에도 해결책이 될 수 있을 겁니다.
\iffalse
...
\item	\co{rcu_node} 구조는 컴파일 시점에 만들어지며, 따라서 최악의 경우인
	\co{NR_CPUS} 갯수의 CPU 들을 가정해 크기가 정해집니다.
	하지만, 4,096 개의 CPU 들에서도, \co{rcu_node} 구조는 64-bit 머신에서
	65개의 캐시라인만을 사용합니다 (그리고 32-bit 머신에서 4,096 개 CPU 를
	넣어보세요!).
	물론, 16-CPU 머신에서 \co{NR_CPUS=4096} 으로 빌드된 커널을 수행시키면
	단일 노드 트리로도 잘 돌아갈 곳에 2단계 트리를 사용하게 될겁니다.
	이 구성이 추가적인 락킹 오버헤드를 가져오긴 하겠지만, hot-path
	read-side 코드에는 영향을 끼치지 않으므로, 실제로는 문제가 되지 않을
	겁니다.
\iffalse
...
\item	이 패치는 커널 text 와 data 를 약간 증가시킵니다:
	\co{NR_CPUS=4} 시스템에서 기존의 Classic RCU 구현은 1,757 바이트의 커널
	text 와 456 바이트의 커널 data 로 총 2,213 바이트만을 사용했습니다만,
	새로운 계층적 RCU 구현은 4,006 바이트의 커널 text 와 624 바이트의 커널
	data 를 사용해 총 4,630 바이트를 사용합니다.
	이는 수백 메가바이트의 메인 메모리만을 갖는 대부분의 임베디드
	시스템에서 조차도 문제가 될정도는 아닙니다.
	하지만, 아주 작은 임베디드 시스템에서 이게 문제가 된다면, RCU 의
	``scale up'' 과 ``scale down'' 구현을 모두 제공할 필요가 있을 수
	있습니다.
\iffalse
...
이 계층적 RCU 구현은 어쨌거나 수백개의 CPU 들에서는 Classic RCU 를 넘어선
커다란 개선입니다.
무엇보다도, Classic RCU 는 16-32 개 CPU 의 시스템들만을 위해 설계되었습니다.

언젠가는, preemptible RCU 구현에도 계층을 적용할 필요가 있을 수 있습니다.
이는 per-CPU 카운터 쌍에 사용된 modular 연산으로 인해 좀 도전적인 일이 될 수
있습니다만, 불가능한 일은 아닐겁니다.
\iffalse
