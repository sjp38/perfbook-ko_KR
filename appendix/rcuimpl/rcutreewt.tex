...
\item	CPU 가 online 이 되는 과정 중에 있고, RCU 는 해당 CPU 가 online 이라고
	...
	뿐더러, \co{cpu_online_map} 에 online 이라고 표시되지 않은 경우.
...
에서는 holdout CPU 들을 스캔하는 \co{rcu_process_dyntick()} 을 다루고,
...
강제하는 프로세스가 동작하게 하는 \co{force_quiescent_state()} 를 설명합니다.
\iffalse
...
는 \co{dyntick_record_completed()} 와 \co{dyntick_recall_completed()} 의 코드를
보입니다.
이 함수들은 dynticks 가 활성화된 경우 (달리 말해서, \co{CONFIG_NO_HZ} 커널
...
Line~1-6 은 \co{dyntick_record_completed()} 함수로, \co{comp} 인자로 명시된
값을 명시된 \co{rcu_state} 구조체의 \co{->dynticks_completed} 필드에
저장합니다.
Line~8-12 는 \co{dyntick_recall_completed()} 함수로, 가장 최근의
\co{dyntick_record_compledted()} 호출로 이 CPU 와 \co{rcu_state} 구조체의
조합으로 저장된 값을 리턴합니다.
\iffalse
...
는 offline CPU 들을 체크하고 online holdout CPU 들을 quiescent state 에
들어가도록 강제하는 \co{rcu_implicit_offline_qs()} 의 코드를 보입니다.

Line~3 는 명시된 CPU 가 offline 인지 체크하고, 만약 그렇다면 line~4 에서 통계적
카운터인 \co{->offline_fqs} (tracing 을 위해서만 사용됩니다) 의 값을
증가시키고, line~5 에서 해당 CPU 가 extended quiescent state 에 있다는 것을
알리기 위해 0이 아닌 값을 리턴합니다.
\iffalse
...
그렇지 않다면, 해당 CPU 는 online 이고 dynticks-idle 모드가 아니며 (만약 그렇지
않다면 일단 이 함수가 호출되지도 않았을 겁니다), 이 grace period 를 위한
quiescent state 를 아직 지나지 않은 겁니다.
Line~7 은 이 holdout CPU 가 현재 수행중인 CPU 인지 체크하고, 만약 그렇지 않다면
line~8 에서 이 holdout CPU 에게 reschedule IPI 를 날립니다.
그렇지 않다면, line~10 에서 현재의 task 를 위해 \co{TIF_NEED_RESCHED} 플래그를
세워서 현재 CPU 가 scheduler 에 들어가도록 강제합니다.
어느 경우든, 이 CPU 는 이제 빨리 quiescent state 에 들어가야 합니다.
Line~11 은 tracing 을 위해서만 사용되는 통계적 카운터인 \co{resched_ipi} 의
값을 증가시킵니다.
마지막으로, line~12 는 이 holdout CPU 가 여전히 quiescent state 를 지나가길
거부하고 있다는 걸 알리기 위해 0을 리턴합니다.
\iffalse
...
에 파란 화살표로 그려진대로 holdout CPU 들을 탐색하기 위해 leaf \co{rcu_node}
구조체들을 스캔하는 \co{rcu_process_dyntick()} 의 코드를 보입니다.
이 함수는 각각의 CPU 의 \co{rcu_data} 구조체에 대해 인자로 받은 함수 \co{f} 를
실행하고, \co{lastcomp} 인자로 명시된 grace period 가 종료되었다면 0이 아닌
값을 리턴합니다.

Line~13 과 14 는 첫번째와 마지막 leaf \co{rcu_node} 구조체로의 레퍼런스를 각각
얻어옵니다.
Line~15-38 의 루프의 각 단계는 leaf \co{rcu_node} 구조체들 중 하나를
처리합니다.
\iffalse
...
Line~16 은 지역 변수 \co{mask} 를 0 으로 값 설정합니다.
이 변수는 현재 leaf \co{rcu_node} 구조체의 extended quiescent state 에 있는 CPU
들을 모으는데 사용됩니다.
Line~17 은 현재 leaf \co{rcu_node} 구조체의 락을 얻어오고, line~18 은 현재의
grace period 가 완료되었는지 확인하며, 만약 그렇다면 line~19 에서 이 락을
해제하고 line~20 에서 0이 아닌 값을 리턴합니다.
그렇지 않다면, line~22 에서 이 \co{rcu_node} 구조체에 연관된 holdout CPU 들을
체크하고, 그런게 하나도 없다면, line~23 에서 이 락을 해제하고 line~24 에서
루프를 다음 leaf \co{rcu_node} 구조체부터 시작하도록 재시작 시킵니다.

이 \co{rcu_node} 구조체에 연관된 holdout CPU 가 최소 하나라도 있다면 실행은
line~26 에 도달합니다.
Line~26 과 27 은 지역 변수 \co{cpu} 와 \co{bit} 을 이 \co{rcu_node} 구조체에
연관된 가장 낮은 숫자의 CPU 를 가리키도록 설정합니다.
Line~28-32 의 루프의 각 단계는 현재의 \co{rcu_node} 구조체에 연관된 CPU 들 중
하나씩을 체크합니다.
Line~29 는 이 CPU 가 여전히 유지되고 있는지 아니면 이미 quiescent state 를
지나갔는지 확인합니다.
만약 여전히 유지되고 있다면, line~30 에서 명시된 함수 (호출자에 의해 명시되는,
\co{dyntick_save_progress_counter()} 또는 \co{rcu_implicit_dynticks_qs()}) 를
호출하고, 그 함수가 0이 아닌 값을 리턴한다면 (현재 CPU 가 extended quiescent
state 에 있음을 의미함), line~31 에서 \co{mask} 내의 현재 CPU 의 bit 의 값을
1로 세팅합니다.
\iffalse
...
이어서 Line~33 은 어떤 CPU 가 extended quiescent state 에 있었던 걸로
구분되었는지 그리고 현재의 grace period 가 여전히 강제되어 있는지 체크하고,
만약 그렇다면 line~34 에서 이 grace period 는 더이상 그런 CPU 들을 기다릴
필요가 없단 걸 알리기 위해 \co{cpu_quiet_msk()} 를 호출하고 line~35 에서 다음
\co{rcu_node} 구조체를 가지로 루프를 재시작합니다.
(\co{cpu_quiet_msk()} 는 현재 \co{rcu_node} 구조체의 락을 해제하며, 현재 grace
period 를 끝낼 수도 있음을 알아두시기 바랍니다.)
그렇지 않다면, 만약 모든 holdout CPU 들이 여전히 진짜로 holdout 상태라면,
line~37 에서 현재 \co{rcu_node} 구조체의 락을 해제합니다.

일단 모든 leaf \co{rcu_node} 구조체들이 처리되었다면, 루프를 빠져나가고 line~39
에서 현재의 grace period 는 여전히 진행되어야 함을 알리기 위해 0을 리턴합니다.
(line~20 은 현재 grace period 가 끝났음을 알리기 위해 0이 아닌 값을 리턴함을
다시 말해둡니다.)
\iffalse
...
는 \co{CONFIG_SMP} 를 위한 \co{force_quiescent_state()} 의 코드를
보이는데\footnote{
	non-\co{CONFIG_SMP} 에서는 \co{force_quiescent_state()} 는
	\co{set_need_resched()} 의 wrapper 일 뿐입니다.}, 이 함수는 RCU 가 현재
grace period 를 CPU 들을 quiescent state 너머로 진척시키고자 하는 필요를 느낄
때 호출됩니다.
RCU 는 다음 중 한 상태일 때 이 필요를 느낍니다:
\begin{enumerate}
\item	현재의 grace period 가 3 jiffy (또는 compile-time 의
	\co{RCU_JIFFIES_TILL_FORCE_QS} 값으로 명시된 만큼) 이상 진행되었거나,
\item	\co{call_rcu()} 또는 \co{call_rcu_bh()} 를 통해 RCU 콜백을 집어넣는 CPU
	가 10,000 개 (또는 boot-time 패러미터인 \co{qhimark} 로 명시된 양)
	이상의 콜백이 넣어져 있음을 보았을 때.
\end{enumerate}
\iffalse
...
Line~10-12 는 진행중인 grace period 가 존재하는지 보고, 존재하지 않는다면 그냥
조용히 종료합니다.
Line~13-16 은 하나의 grace period 를 진척시키려는 동시적 시도를 방지해주는
\co{->fqslock} 을 획득하려 시도합니다.
\co{->n_force_qs_lh} 카운터는 이 락이 이미 잡혀 있을 때 그 값이 증가되며,
\co{CONFIG_RCU_TRACE} 커널 패러미터가 활성화 되어 있으면 \co{rcuhier} debugfs
파일의 \co{fqlh=} 필드로 그 값을 볼 수 있습니다.
Line~17-21 은 현재의 grace period 가 정말로 진척될 필요가 있는지 확인해 보는데,
달리 말하자면 (1) 현재의 CPU 가 10,000 개의 대기중인 RCU 콜백들을 가지고
있는지, 또는 (2) 현재 grace peiod 의 시작으로부터 또는 현재 grace period 를
진척시키려는 마지막 시도로부터 \co{jiffies} 카운터 또는 \co{rcu_pending} 호출
횟수로 측정해서 3 jiffy 이상이 지났는지를 봅니다.
Line~22 는 이어서 grace period 들을 진척시키려 시도한 횟수를 셉니다.
\iffalse
...
Line~23-36 은 현재의 grace period 가 그것을 진척시키려 시도하자마자 종료되었을
때의 혼란을 막기 위해 root \co{rcu_node} 구조체의 락을 잡은 채로 실행됩니다.
Line~24 와 25 는 \co{->completed} 와 \co{->signaled} 필드의 스냅샷을 뜨고,
line~26-30 에서는 뒤따르는 non-relaced \co{force_quiescent_state()} 가 실제로
어떤 진척을 행할 수 있게 되기까지의 가장 빠른 시간을 설정하고, line~31-35 는 이
grace period 가 우리가 이 \co{rcu_node} 구조체의 락을 잡는 사이에 종료되었는지
확인하고, 만약 그렇다면 이 락을 해제하고 리턴합니다.

Line~37-59 는 \co{force_quiescent_state()} 의 state machine 을 이끕니다.
해당 grace period 가 아직 초기화 중이라면, line~41 과 42 dptj rmsid flxjsgotj
\co{force_quiescent_state()} 가 아마도 초기화가 끝난 나중에 다시 호출될 수 있게
합니다.
만약 dynticks 가 (\co{CONFIG_NO_HZ} 커널 패러미터를 통해) 활성화 되어 있다면,
해당 grace period 의 초기화 뒤의 첫번째 \co{force_quiescent_state()} 호출은
line~40-52 를 실행하고, 두번째와 그 이후의 호출들은 line~53-59 를 실행하게
됩니다.
반면에, dynticks 가 활성화 되어 있지 않다면, 모든 초기화 뒤의
\co{force_quiescent_state()} 호출은 line~53-59 를 실행하게 될겁니다.
\iffalse
...
Line~40-52 의 목적은 아직 quiescent state 를 지나지 않은 모든 CPU 들의 현재
dynticks-idle state 를 기록하고, 현재 dynticks-idle 상태에 있는 (하지만 irq 나
NMI 핸들러에 있지 않은) 모든 quiesent state 를 기록하는 것입니다.
Line~41-42 는 gcc 에게 이 switch 문의 branch 가 non-\co{CONFIG_NO_HZ}
커널에서는 사용되지 않는 코드임을 알리는 역할을 합니다.
Line~43-45 는 현재의 grace period 를 위한 quiescent state 를 아직 지나지 않은
각각의 CPU 를 위해 \co{dyntick_save_progress_counter()} 를 호출하고 (아직
quiescent state 를 지나지 않은 모든 CPU 들이 dyntick-idle 모드에서 잠들어
있었음을 발견하거나 함으로써) 이 grace period 가 그 사이 종료되었다면
\co{force_quiescent_state()} 를 빠져나가기 위해 \co{rcu_process_dyntick()} 을
호출합니다.
Line~46 과 51 은 현재 grace period 의 동시적 종료와의 혼란을 막기 위해 root
\co{rcu_node} 구조체의 락을 잡고 해제합니다.
Line~47 은 현재의 grace period 가 여전히 진행 중인지 체크하고, 만약 그렇다면
line~48 에서 이 상태를 \co{RCU_FORCE_QS} 상태로 진척시키고 line~49 에서 현재의
grace -period 숫자를 다음 \co{force_quiescent_state()} 호출 때를 위해 저장해
둡니다.
현재 grace-peirod 숫자를 저장하는 이유는 현재의 grace period 가 다음의
\co{force_quiescent_state()} 의 호출과 동시에 종료되는 경우의 race condition 을
올바르게 처리하기 위함입니다.
\iffalse
...
앞에서 이야기했듯, line~53-58 은 \co{CONFIG_NO_HZ} 커널에서의 두번째 이후
\co{force_quiescent_state()} 호출, 그리고 non-\co{CONFIG_NO_HZ} 커널에서의
\emph{모든} 호출에 대해서 수행됩니다.
Line~54 와 58 은 아직 quiescent state 를 지나지 않은 CPU 들 사이를 지나며
그것들에 대해 (\co{CONFIG_NO_HZ} 가 활성화 되어 있다면) 이 CPU 들 중 하나라도
dyntick-idle state 를 지났는지 체크하고, offline CPU 들을 기다리고 있진 않은지
확인하고, 마지막으로 앞의 두 그룹에 속하지 않은 모든 CPU 들에 대해선 reschedule
IPI 를 날려주는 \co{rcu_implicit_dynticks_qs()} 를 호출하는
\co{rcu_process_dyntick()} 을 호출합니다.
\iffalse
...
RCU 는 \co{CONFIG_RCU_STALL_DETECTOR} 커널 패러미터가 선택되어 있는 경우에는
stall 되어있는 CPU 들을 검사합니다.
``Stall 되어있는 CPU 들'' 은 커널에서 preemption 이 불능화 되어있는채로 spin
하고 있어서 응답시간을 나쁘게 만드는 것들입니다.
이것들은 \co{record_gp_stall_check_time()}, \co{check_cpu_stall()},
\co{print_cpu_stall()}, 그리고 \co{print_other_cpu_stall()} 함수의 구현을 통해
검사되며, 이것들 각각이 아래에 설명됩니다.
이 함수들은 \co{CONFIG_RCU_CPU_STALL_DETECTOR} 커널 패러미터가 선택되어 있지
않은 경우에는 모두 no-op 이 됩니다.
\iffalse
...
는 \co{record_gp_stall_check_time()} 의 코드를 보입니다.
Line~4 는 jiffy 로 (이 grace period 의 시작) 현재 시간을 기록하고, line~5-6 는
stall 된 CPU 가 체[크되어야 할 시간을 기록합니다.
\iffalse
...
는 grace period 가 너무 길게 늘어졌는지 체크하고, 그럴 경우 CPU-stall 경고
메세지를 출력하기 위해 \co{print_cpu_stall()} 이나 \co{print_other_cpu_stall()}
을 호출하는 \co{check_cpu_stall()} 의 코드를 보입니다.

Line~8 은 stall 경고가 출력되어야 하는 시간으로부터의 jiffy 의 수를 계산하는데,
이는 아직 warning 을 출력할 시간이 아니라면 음수가 됩니다.
Line~9 는 현재의 CPU 에 연관된 leaf \co{rcu_node} 구조체로의 포인터를 가져오고,
line~10 은 현재의 CPU 가 아직 quiescent state 를 지나지 않았는지, 그리고 이
grace period 가 너무 길게 연장되지 않았는지 (달리 말해서, 현재 CPU 가 stall
되었는지) 검사하고, 만약 그렇다면 line~11 에서 \co{print_cpu_stall()} 을
호출합니다.

그렇지 않다면, line~12-13 에서 이 grace period 가 아직 효력을 발휘하고 있는지,
그리고 이게 CPU-stall 경고 기간을 지나 두 jiffy 만큼 연자오디었는지 검사하고,
만약 그렇다면 line~14 에서 \co{print_other_cpu_stall()} 을 호출합니다.
\iffalse
...
\QuickQuiz{}
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for check-cpu-stall}
	의 line~12-13 에서는 왜 두 jiffy 를 더 기다리는 거죠?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	이 추가된 딜레이는 이 CPU 에게 스스로를 보고할 기회를 주어서, stall 된
	코드의 더 나은 스택 트레이스를 얻을 수 있게 합니다.
	물론, 이 CPU 가 인터럽트가 불능화 된 채 spin 하고 있다면 스스로를
	보고하지 않을 것이어서, 다른 CPU 들은 더 짧은 딜레이 만에 이 일을 해야
	할 겁니다.
	\iffalse
...
는 \co{print_cpu_stall()} 코드를 보입니다.

Line~6-11 은 콘솔 메세지를 출력하고 현재 CPU 의 스택의 dump 를 행하며,
line~12-17 은 다음 CPU stall 경고를 할 시간을 계산하는데, grace period 가
그만큼 길어지면 경고를 하게 됩니다.
\iffalse
...
\QuickQuiz{}
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for print-cpu-stall}
	에서 stall 경고가 출력되게 전에 grace period 가 종료되는 건 누가
	막나요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	이 함수의 호출자는 이 CPU 가 아직 quiescent state 를 보고하지 않았음을
	체크했고, preemption 은 꺼져있기 때문에, quiescent state 가 이 사이에
	보고될 방법은 없습니다.
	\iffalse
...
는 현재 수행중인 CPU 외의 CPU 들을 위한 stall 경고를 출력하는
\co{print_other_cpu_stall()} 의 코드를 보입니다.

Line~10 과 11 은 첫번째 leaf \co{rcu_node} 구조체와 마지막 leaf \co{rcu_node}
구조체 다음 것의 레퍼런스를 가져옵니다.
Line~12 는 root \co{rcu_node} 구조체의 락을 잡고, 또한 인터럽트를 불능화
시킵니다.
Line~13 은 얼마나 오래 전에 CPU-stall 경고 시간 (아직 있지 않았다면 음수가
됩니다) 이 있었는지 계산하고, line~14 와 15 는 이 CPU-stall 경고 시간이
지났는지 보고 이 grace period 가 아직 끝나지 않았는지 검사하며, 만약 그렇다면
line~16 에서 이 락을 해제하고 (또 인터럽트를 재활성화 시키고) line~17 에서
리턴합니다.
\iffalse
...
\QuickQuiz{}
	\co{print_cpu_stall()} 은 안그러는데
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for print-other-cpu-stall}
	의 \co{print_other_cpu_stall()} 은 왜 grace period 의 종료를 검사해야
	하나요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	다른 CPU 들은 언제든 quiescent state 를 지날 수 있고, 따라서 이 grace
	period 는 중간에 끝났을 수 있습니다.
	\iffalse
...
그렇지 않다면, line~19 와 20 은 (그만큼 grace period 가 길어진다면) CPU stall
경고가 출력되어야 할 다음 시간을 계산하고 line~21 에서 락을 해제하고 인터럽트를
재활성화 시킵니다.
Line~23-33 은 stall 된 CPU 들의 목록을 출력하고, 마지막으로 line~34 에서 문제의
CPU 들을 quiescent state 를 지날 수 있도록 살짝 찔러주기 위해
\co{force_quiescent_state()} 를 호출합니다.
\iffalse
...
이 글을 쓰는 시점까지에 있어
Hierarchical RCU 의 있을 수 있는 가장 큰 문제는 \co{force_quiescent_state()} 는
잠재적으로 모든 CPU 의 \co{rcu_data} 구조체들을 돌아봐야 할 수 있다는 겁니다.
이 스캔 작업이 인터럽트가 불능화 된채 이루어지기 때문에 수천개의 CPU 들을 가진
기계에서는, 이는 잠재적으로 스케쥴링 반응시간에 큰 영향을 끼칠 수 있습니다.

이게 실제로 문제가 될 수 있는데, 한가지 해결책은 현재의 per-\co{rcu_state}
\co{->signaled} 상태 변수처럼 per-leaf-\co{rcu_node} 기반으로
\co{force_quiescent_state()} 시퀀스를 분리시켜 유지하는 겁니다.
이는 per-leaf-\co{rcu_node} 기반으로 quiescent state 진행을 가능하게 할
것이어서 최악의 경우의 스케쥴링 응답시간의 최악의 경우의 문제를 크게 줄일
겁니다.

그런 수정이 가해지기 전까지는 스케쥴링 응답시간에 민감한 분들은 시스템의 CPU
갯수를 제한하거나 preemptible RCU 구현을 사용하시기 바랍니다.
\iffalse
