...
특정 \co{struct} \co{srcu_strcut} 의 grace period 의 진행을 모니터 하기 위해
\co{srcu_batches_completed()} 기능을 사용할 수 있을 겁니다.
...
특정한 \co{struct} \co{srcu_strcut} 에 대해 SRCU read-side 기능들을 사용합니다.
이런 경우에 있어서는 module 종료 코드는 해당 쓰레드들을 종료시키고, 그것들이
종료되길 기다린 후, 정리 작업을 진행하면 됩니다.

예를 들면 디바이스 드라이버들과 같은 다른 경우에 있어서는 시스템의 어떤
...
\co{readside()} 함수는 line~5-11 의 RCU 와 line~10-13 의 SRCU read-side
크리티컬 섹션을 겹치게 합니다.
이 RCU read-side 크리티컬 섹션은 \co{nomoresrcu} 변수의 값을 지키기 위해 순수한
RCU~\cite{PaulEdwardMcKenneyPhD} 를 사용합니다.
만약 이 변수에 값이 할당되면, 우린 정리작업을 하고 있는 것이고, 따라서 뒤의
SRCU read-side 크리티컬 섹션을 들어가지 않아야 하므로, 그대신 \co{-EINVAL} 을
리턴합니다.
...
\co{cleanup()} 함수는 먼저 line~19 에서 \co{nomoresrcu} 변수에 값을 할당하지만,
그러고나서는 모든 현재 존재하는 RCU read-side 크리티컬 섹션이 완료되기를
line~20 에서의 \co{synchronize_rcu()} 기능을 통해 기다립니다.
일단 \co{cleanup()} 함수가 line~21 에 다다르면, \co{nomoresrcu} 의 값이 0인
것을 봤을 수 있는 모든 \co{readside()} 실행 쓰레드들은 이미 line~11 에 이르렀을
것이고, 따라서 이미 각작의 SRCU read-side 크리티컬 섹션에 들어갔어야 합니다.
모든 이후의 \co{readside()} 호출은 line~8 에서 종료될 것이고, 따라서 read-side
...
따라서, 일단 \co{cleanup()} 이 line~21 에서의 \co{synchronize_srcu()} 호출을
완료한다면, 모든 SRCU read-side 크리티컬 섹션들은 이미 완료되었을 것이고,
새로운 크리티컬 섹션의 시작은 없을 겁니다.
따라서 line~22 에서의 \co{cleanup_srcu_struct()} 호출을 통한 정리작업은
안전합니다.
\iffalse
...
이 섹션은 SRCU 의 데이터 구조, 초기화와 정리 기능들, read-side 기능들, 그리고
...
설명되어 있습니다.
\co{completed} 필드는 \co{struct} \co{srcu} 가 초기화된 이후의 grace period
들의 횟수를 세고, 다이어그램에 보여진 것처럼, 그 아래쪽 비트는 \co{struct}
\co{srcu_strcut_array} 를 가리키는데에 사용됩니다.
\co{per_cpu_ref} 필드는 이 배열을 가리키고, \co{mutex} 필드는 한번에 하나의
\co{synchronize_src()} 만이 진행될 수 있도록 하는데에 사용됩니다.
\iffalse
...
SRCU 의 초기화 함수, \co{init_srcu_struct()} 가
Figure~\ref{fig:app:rcuimpl:SRCU Initialization} 에 보여져 있습니다.
이 함수는 단순히 \co{struct} \co{srcu_strcut} 의 필드들을 초기화 할 뿐이며,
초기화가 성공하면 0을 리턴하고 그렇지 않으면 \co{-ENOMEM} 을 리턴합니다.
\iffalse
...
주요 cleanup 함수인 \co{cleanup_srcu_struct()} 가 그림의 line~19-29 에 보여져
있는데, 이 함수는 이 \co{strcut} \co{srcu_Strcut} 를 사용하는 읽기 쓰레드들이
현재 존재하지 않음을 검증하기 위해 이 그림의 line~13-17 에 보여져 있는
\co{srcu_readers_active()} 를 곧바로 호출합니다.

\co{srcu_readers_active()} 함수는 단순히 각 노드의
\co{srcu_readers_active_idx()} 의 값의 합을 리턴할 뿐이며,
\co{srcu_readers_active_idx()} 는 line~1-11 에 보여진 것과 같이 해당 인덱스에
...
\co{srcu_readers_active()} 로부터 리턴받은 값이 0이 아니라면,
\co{cleanup_srcu_struct()} 는 line~24 에서 경고를 내고는 단순히 line~25 와 26
에서 리턴해서 여전히 사용중인 \co{strcut} \co{srcu_strcut} 의 해제 작업을 하지
않습니다.
그런 경고는 항상 버그에 연관되어 있으며, 버그가 레포트 되었으므로, 있을 법한
메모리 오염을 알리기보다는 약간의 메모리 누수와 함께 시스템이 지속되도록 하는게
낫습니다.

그렇지 않다면, \co{cleanup_srcu_struct()} 는 line~27 과 28 에서 per-CPU
카운터들의 배열을 해제하고 포인터들의 값을 \co{NULL} 로 만듭니다.
\iffalse
...
결정하는데 사용됩니다.
Line~7 에서의 \co{barrier()} 호출은 이 인덱스가 최소 한번은 가져와 질
것을\footnote{
	이 이름에도 불구하고, \co{barrier()} 는 CPU 가 코드와 메모리 액세스의
...
Line~8-9 는 현재 CPU 의 선택된 카운터를 증가시킵니다.\footnote{
	\co{smp_processor_id()} 기능은 preemption 이 불능화 되었을 때에만
	장기적 의미를 가짐을 알아 두시기 바랍니다.
	Preemption 이 불능화 되어 있지 않다면, 이 기능을 곧바로 뒤따르는
	preemption 이 뒤따르는 코드들은 다른 CPU 에서 수행되는 결과를 초래할 수
	있습니다.}
Line~10 은 non-\co{CONFIG_PREEMPT} 빌드의 어떠한 코드도 잘못 순서잡히는 것을
막기 위해, 뒤따르는 코드 수행이 line~8-9 \emph{뒤에} 수행되도록 강제합니다.
하지만, \co{CONFIG_PREEMPT} 커널에서는 여기 필요했던 \co{barrier()} 호출이
line~11 의 \co{preempt_enable()} 안에 내장되어 있으므로, 이 경우에 있어
\co{srcu_barrier()} 는 no-op 이 됩니다.
마지막으로, line~12 는 인덱스를 리턴해서 이게 연관된 \co{srcu_read_unlock()} 에
사용될 수 있도록 합니다.
\iffalse
...
\co{CONFIG_PREEMPT} 커널들에서는, line~3 의 \co{preempt_disable()} 이
\co{barrier()} 기능을 내장하고 있으며, 그렇지 않은 경우에는 line~4 에서
\co{barrier()} 가 제공됩니다.
다시 한번, 이 지시어는 뒤따르는 코드가 겹쳐지는 인터럽트 핸들러들의 관점에서 이
크리티컬 섹션 뒤에 수행되도록 강제합니다.
Line~5 와 6 은 이 CPU 를 위한 카운터의 값을 감소시키는데, 이 때 사용되는
인덱스는 연관된 \co{srcu_read_lock()} 에서 사용된 것과 동일한 것입니다.
\iffalse
...
SRCU 의 핵심 포인트는 \co{synchronize_sched()} 는 현재 수행중인 preemption
불능화된 영역의 코드가 완료될 때까지 블록된다는 것입니다.
\co{synchronize_srcu()} 기능은
...
그렇지 않다면, line~11 에서 \co{srcu_read_lock()} 에서 grace period 카운터의
증가된 값을 보는 다른 CPU 는 \co{synchronize_srcu()} 에 진입하기 전에 이 CPU 에
...
카운터의 값을 증가시킵니다.
Line~14 는 이제 모든 현재 수행중인 \co{srcu_read_lock()} 이 완료되길 기다리고,
이로 인해 우리가 line~15 에 다다랐을 때에는, 모든 \co{srcu_read_lock()} 의
현존하는 인스턴스들은 \co{sp->completed} 의 업데이트된 값을 사용하고 있게
됩니다.
따라서, line~15 에서 \co{srcu_readers_active_idx()} 로 얻어와지는 카운터들은
...
하지만, \co{srcu_read_unlock()} 기능에는 메모리 배리어가 없으므로, CPU 는
카운터 값 감소를 SRCU 크리티컬 섹션 안으로 재배열 할 수 있고, 따라서 SRCU 로
보호되는 데이터 구조로의 레퍼런스가 SRCU 크리티컬 섹션 바깥으로 ``새어나올''
수도 있습니다.
이 시나리오는 line~17 의 \co{synchronize_sched()} 에 의해서 해결되어지는데, 이
기능은 \co{preempt_disable()} 코드 (\co{srcu_read_unlock()} 과 같은) 내에서
수행되고 있는 모든 CPU 들이 그 코드 수행을 완료할 때까지 블록되도록 합니다.
특정 \co{preempt_disable()} 코드 의 완료는 그 코드를 수행중인 CPU 에 의해
보여지므로, 이 코드의 완료는 모든 앞의 SRCU read-side 크리티컬 섹션의 완료를
의미합니다.
필요한 모든 메모리 배리어들은 이 완료의 발견을 하는 코드에 의해 제공되어집니다.

따라서, 이 시점에서는 line~18 에서와 같이 mutex 를 해제하고 호출자에게
리턴하기에 안전하며, 호출자는 같은 \co{struct} \co{srcu_struct} 를 공유하고
있는 모든 SRCU read-side 크리티컬 섹션들이 \co{synchronize_srcu()} 호출 전에
...
\QuickQuiz{}
	\co{synchronize_sched()} 에 의해 구분되는 업데이트들이 순서대로 수행될
	...
	\fi
\QuickQuizAnswer{
	이 특성은 RCU 가 동작할 수 있기 위한 측면에서 \co{synchronize_sched()}
	에 필요한 것이기 때문입니다.
	예를 들어, 리스트에서 객체를 하나 제거하고, \co{synchronize_sched()} 를
	호출하고, 이어서 해당 객체를 메모리에서 해제시키는 코드 시퀀스를 생각해
	봅시다.
	이 속성이 주어지지 않는다면, 해당 객체는 그것이 리스트에서 제거되기도
	전에 메모리에서 해제된 것으로 나타날 수 있는데, 이는
	\co{synchronize_sched()} 가 막을 수 있다고 여겨지는 상황 그 자체입니다!
	\iffalse
...
\QuickQuiz{}
	\co{synchronize_srcu()}
	(Figure~\ref{fig:app:rcuimpl:Update-Side Implementation})
	의 line~17 은 왜 line~18 의 mutex 해제보다 앞서야 하죠?
	이 두 라인이 서로 바뀌게 하려면 어떤 변경이 있어야 할까요?
	그런 변경은 의미가 있을까요?
	왜 그럴까요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	그 순서가 반대가 된 상태에서, CPU~0 가 \co{synchronize_srcu()} 의
	line~13 에 도달했으며, CPU~1 과 CPU~2 가 또다른 \co{synchronize_srcu()}
	를 각각 수행하기 시작했으며, CPU~3 는 \co{srcu_read_lock()} 을 수행하기
	시작했다고 생각해 봅시다.
	CPU~0 가 line~13 에서 카운터의 값을 증가시키기 직전에 CPU~1 이
	\co{synchronize_srcu()} 의 line~6 에 도달했다고 생각해 봅시다.
	가장 중요한건, CPU~3 가 뒤따르는 SRCU read-side 크리티컬 섹션에서
	\co{srcu_read_lock()} 을 다른 순서로 수행해서 CPU~0 가
	\co{sp->completed} 를 증가시키기 \emph{전에} SRCU 로 보호되는 어떤
	데이터 구조로의 레퍼런스를 획득했지만, CPU~0 가 이 증가 작업을 한
	\emph{후에} \co{srcu_read_lock()} 을 수행합니다.

	이렇게 되면 CPU~0 는 line~15-16 에서의 ``while'' 루프를 빠져나오고
	mutex 를 해제하기 전에 CPU~3 가 SRCU read-side 크리티컬 섹션을
	완료하기를 기다리지 \emph{않습니다} (기억하세요, CPU 는 코드를 재배열할
	수 있습니다).
	\iffalse
	...
	이제 CPU~2 가 이 mutex 를 뒤이어서 획득하고, 또다시 \co{sp->completed}
	를 증가시킵니다.
	이 CPU 는 이제 CPU~3 가 SRCU read-side 크리티컬 섹션을 완료하기를
	line~15-16 에서의 루프를 빠져나가고 mutex 를 해제하기 전에 기다려야
	합니다.
	하지만 CPU~3 가 다시 순서를 뒤바꿔서, SRCU read-side 크리티컬 섹션을
	들어갈 때 얻어온 포인터로의 마지막 레퍼런스를 수행하기 전에
	\co{srcu_read_unlock()} 을 완료한다고 생각해 봅시다.

	CPU~1 은 이제 mutex 를 획득하지만, \co{sp->completed} 카운터가 둘
	...
	\co{synchronize_srcu()} 내에서의 마지막 \co{synchronize_sched()} 는
	반드시 mutex 해제를 앞서야 합니다.

	다른 해결 방안은 \co{synchronize_srcu()} 의 line~7 에서의 비교 구문이
	해당 카운터의 최소 세개의 값 증가를 체크하도록 바꾸는 것이 될겁니다.
	하지만, 그런 변경은 해시 테이블이 복수의 쓰레드들을 사용해 업데이트
	되거나 unload 되는 것과 같은 ``bulk update'' 시나리오의 응답시간을
	증가시킬 겁니다.
	현재의 코드에서, \co{synchronize_srcu()} 의 동시적 호출에 의한
