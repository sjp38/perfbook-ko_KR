...
PWQ 는 Listing~\ref{lst:SMPdesign:SEQ Pseudocode}(\path{maze_seq.c}) 에 있는
SEQ 에 기반합니다.
...
Listing~\ref{lst:SMPdesign:SEQ Helper Pseudocode}
(\path{maze.c}) 의 line~14-28 에 있습니다.
...
(\path{maze_part.c}) 에 보여진 파티션을 사용한 병렬 알고리즘 (PART) 는 SEQ 와
...
마지막으로, \co{maze_find_any_next_cell()} 함수는 한 셀을 방문된 것으로
...
이 알고리즘들은 모두 올바른 해결책을 찾아내고 있습니다만,
Figure~\ref{fig:SMPdesign:CDF of Solution Times For SEQ, PWQ, and PART} 에
...
무엇보다, 이것은 일을 쪼갤 수 있음이 쓰레드를 추가하는 것으로 인해 전체 연산
비용의 증가를 불러일으키지 않음을 당혹스러울 정도의 병렬성도 아닙니다.
그 대신, 이것은 \emph{굴욕적일 정도의 병렬성} 입니다: 쓰레드를 추가하는 것이
전체 연산 비용을 상당히 줄여줘서 커다란 알고리즘적인 선형적인 성능향상을
초월하는 결과를 이끌어낸 것입니다.
\iffalse
...
더 나아가서 들여다본 결과 PART 는 가끔 미로의 셀들 중 2\,\% 미만만을
방문했는데, SEQ 와 PWQ 는 9\,\% 미만을 방문한 적이 없었습니다.
이런 차이점에 대한 이유는
...
하게 되며, 따라서 우리가 관측한 굴욕적인 병렬성을 갖는 것입니다.
\iffalse
...
그러합니다.
작은 20x20 이나 50x50 미로에서라면, 정렬과 패딩이 PART 에 대해 성능을 40\,\%
까지 향상시킬 수 있습니다만 이런 작은 크기의 미로에 대해서는 PART 가 쓰레드를
...
병렬성 모의실험을 해볼 가치가 있음을 제시하는데, 예를 들어,
Listing~\ref{lst:SMPdesign:Partitioned Parallel Solver Pseudocode} 의 do-while
루프의 각 패스에서 수동으로 컨텍스트 스위칭을 해보는 겁니다.
이 컨텍스트 스위칭은 간단한데, 이 컨텍스트는 변수들 \co{c} 와 \co{vi} 로만
...
사용한 PART 의 30\,\% 입니다(\path{maze_2seq.c}).
\iffalse
...
와~\ref{fig:SMPdesign:Varying Maze Size vs. COPART} 는 미로의 크기 변화에 따른
효과를 두개의 쓰레드로 동작하는 PWQ 와 PART 를 SEQ 또는 COPART 와 각각 비교해
90\,\% 의 정확성 에러 막대와 함께 보여주고 있습니다.
PART 는 100행 100열 이상 크기의 미로들에서 SEQ 에 비해 선형성을 초월한 확장성을
보이고 COPART 에 비해서는 적당한 확장성을 보입니다.
에너지 소모가 높은 주파수에서는 대략 주파수의 제곱 정도로 에너지 소모가
증가한다는 가정~\cite{TrevorMudge2000Power} 에 기반해서 보면 두 쓰레드를
사용해서 1.4 배의 확장성을 갖는 것은 단일 쓰레드가 같은 해법 탐색 시간을 필요로
할 때 소모하는 에너지와 동일하므로 PART 는 COPART 에 비교했을때 대략 200행
200열 크기 미로에서 이론적인 에너지 효율성 손익분기를 넘어섭니다.
...
앞으로 해야할 일들이 많이 남아있습니다.
첫째로, 이 섹션은 사람이 미로 해법 탐색에 사용하는 방법 가운데 하나의 방법만을
적용해 보았습니다.
...
있습니다.
셋째로, PART 알고리즘의 첫번째 적용했던 방법인 두개 쓰레드 사용이 단순하긴
하지만, 다른 여분의 쓰레드들을 사용하기 위한 방법들이 여럿 있습니다.
최적의 쓰레드 사용은 시작지점과 끝지점에도 의존적일 겁니다.
...
이 섹션은 미로 해법 탐색 알고리즘의 병렬화를 선보이고 분석해 보았습니다.
...
더 중요함을 보였습니다.
그렇습니다, 이 특별한 미로의 타입에 대해서만큼은 현명하게 병렬성을 적용하는것이
훌륭한 검색 전략이었습니다만, 이런 부류의 행운은 검색 전략 그 자체에 대한
명확한 초점으로 충분하지 않습니다.

Section~\ref{sec:intro:Parallel Programming Goals} 에서 앞서 이야기 했듯이,
병렬성은 많은 최적화 방법 중 하나의 잠재적인 최적화 수단일 뿐입니다.
성공적인 설계는 가장 중요한 최적화에 집중 해야 합니다.
저는 다르게 주장하고 싶은 마음이 간절하긴 하지만, 그 최적화는 병렬성일 수도,
병렬성이 아닐 수도 있습니다.

하지만, 병렬성이 올바른 최적화인 많은 경우들을 위해서, 다음 섹션에서는 대부분의
경우 동기화 작업을 처리하는 도구인 락킹에 대해 다룹니다.
\iffalse
