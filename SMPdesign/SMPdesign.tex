...
이용해서 성능, 확장성, 그리고 응답 시간 사이의 균형을 맞추며 최신 상용 멀티코어
시스템의 장점을 끌어내도록 소프트웨어를 어떻게 설계해야 하는지 설명합니다.
...
이 순서를 바꾸는 행위는 낮은 성능과 좌절스러운 확장성을 이끌어 낼 겁니다.\footnote{
	물리 법칙, read-only 복사 등의 또다른 큰 문제는
	Chapter~\ref{chp:Deferred Processing} 에서 다루어집니다.}
\iffalse
...
동기화 오버헤드가 무시할만 하다면 (예를 들어, 프로그램이 커다란 크리티컬 섹션과
굵게 쪼개진 병렬성을 사용한다면), 그리고 그 크리티컬 섹션의 작은 부분들만이
데이터를 수정한다면, 여러개의 읽기 작업을 병렬로 수행될 수 있도록 하면 확장성이
크게 개선될 겁니다.
...
Snaman~\cite{Snaman87} 은 여러 클러스터 시스템에서 사용된, 더 화려한 여섯개
모드의 비대칭적 락킹 디자인을 설명합니다.
일반적인 락킹과 reader-writer 락킹은 Chapter~\ref{chp:Locking} 에서 자세히
설명됩니다.
\iffalse
...
계층적 락킹의 아이디어는 잘게 쪼개진 (fine-grained) 락을 잡는 동작을 하는
동안만 잡는 굵은 (coarse-grained) 락을 두자는 것입니다.
Listing~\ref{lst:SMPdesign:Hierarchical-Locking Hash Table Search}
는 해시 테이블 탐색에 계층적 락킹이 어떻게 사용될 수 있을지 보여주는데, 또한 이
방법의 커다란 단점 역시 보여줍니다:
두번째 락을 잡기 위한 오버헤드를 가져야 하지만, 그 락은 짧은 시간동안만
잡습니다.
...
\QuickQuizAnswer{
	...
	줄여줘서 그로부터 얻는 이익이 추가적인 \co{cur->node_lock} 의 획득과
	해제에 드는 오버헤드보다 커질 수도 있을 것입니다.
	\iffalse
...
이 섹션에서는 병렬로 동작하는 고정 크기 블럭 메모리 할당자의 간단한 개요를
제공합니다.
더 자세한 설명은
...
병렬 메모리 할당자가 마주하게 되는 기본적인 문제는 일반적인 경우에 고속의
메모리 할당과 해제 기능을 제공해야 하는 필요성과 선호되지 않는 할당 / 해제
패턴을 마주했을 때 효과적으로 메모리를 분산시켜야 하는 필요성 사이의
갈등입니다.
...
간단히 각 CPU 에 1 기가바이트씩 메모리를 할당해 주고 각 CPU 가 자신이 소유하고
있는 메모리 덩어리에 접근하도록 할 수 있는데, 이렇게 되면 락킹의 필요성과
복잡성, 그리고 오버헤드들이 없습니다.
...
일반적으로 사용되는 방법은 각각의 CPU 가 블럭들의 적당한 캐시를 소유하는 병렬적
빠른 수행 경로를 사용하고 추가적인 블럭들을 위한 공유 풀에는 커다란 코드 락킹을
사용해서 관리하는 방법입니다.
한 CPU 가 메모리 블럭을 독점하는 것을 막기 위해, 각 CPU 의 캐시에 존재할 수
있는 블럭의 갯수에 제한을 걸어 둡니다.
두개의 CPU 가 있는 시스템에서, 메모리 블럭들의 흐름은
Figure~\ref{fig:SMPdesign:Allocator Cache Schematic} 에 보여진 대로일 겁니다:
...
Listing~\ref{lst:SMPdesign:Allocator-Cache Data Structures} 에 할당자 캐시의
``간단한'' 구현을 위한 실제 데이터 구조체가 있습니다.
Figure~\ref{fig:SMPdesign:Allocator Cache Schematic} 의 ``Global Pool'' 은
\co{struct globalmempool} 타입의 \co{globalmem} 으로 구현되었고
\co{struct percpumempool} 타입의 per-CPU 변수 \co{percpumem} 으로 두개의 CPU 별
풀이 구현되었습니다.
이 데이터 구조체 둘 다 각자의 \co{pool} 필드 안의 블럭들로의 포인터 배열을
가지고 있는데, 이것들은 인덱스 0부터 위쪽으로 채워져 나갑니다.
따라서, 만약 \co{globalmem.pool[3]} 이 \co{NULL} 이라면, 인덱스 4부터 위쪽인 이
배열의 나머지들 역시 모두 \co{NULL} 이어야 합니다.
\co{cur} 필드는 \co{pool} 배열의 채워진 원소들 중 가장 높은 숫자의 인덱스를
갖는데, 만약 모든 원소들이 비어있다면 -1을 갖습니다.
\co{globalmem.pool[0]} 부터 \co{globalmem.pool[globalmem.cur]} 사이의 모든
원소들은 반드시 채워져 있어야 하고, 나머지 것들은 모두 비어있어야만
합니다.\footnote{
...
\co{NULL} 포인터들을 의미합니다.
좀 혼란스러울 수 있지만 중요한 점은, 이 데이터 구조체가 항상 지키게 되는 사실
(불변식, invariant) 은, \co{cur} 필드는 항상 \co{NULL} 이 아닌 포인터들의
수보다 하나 작을 것이란 점입니다.
\iffalse
...
Listing~\ref{lst:SMPdesign:Allocator-Cache Allocator Function} 에 보였습니다.
Line~7 에서는 현재 쓰레드의 per-thread 풀을 가져오고, line~8 에서 그게
비어있는지 확인합니다.

만약 그렇다면, line~9-16 의 코드는 line~9 에서 획득하고 line~16 에서 해제하는
스핀락의 보호 아래 글로벌 풀로부터 해당 per-thread 풀을 채우려 시도합니다.
...
함수를 보입니다.
Line~6 에서는 이 쓰레드의 풀을 가리키는 포인터를 얻어오고 line~7 에서 이
per-thread 풀이 꽉 차 있는지 확인합니다.

만약 그렇다면, line~8-15 에서 이 per-thread 풀의 절반을 글로벌 풀로 비워내는데,
이 때 line~8 과~14 에서 글로벌 풀을 위한 스핀락을 잡고 놓습니다.
...
\QuickQuiz{}
	이 자원 할당자 설계는
	Section~\ref{sec:count:Approximate Limit Counters} 에서 다뤄진 대략적
	한계 카운터의 것과 닮지 않았나요?
	\iffalse
...
\QuickQuizAnswer{
	실제로 그렇습니다!
	우리는 메모리를 할당하고 해제하는 것을 생각했습니다만,
	Section~\ref{sec:count:Approximate Limit Counters}
	에서의 그 알고리즘은 ``카운트''를 할당하고 해제하는 매우 비슷한 액션을
	위해 만들어졌습니다.
	\iffalse
...
대략적인 성능 결과\footnote{
	이 데이터는 통계적으로 유의미한 방식으로 수집되지는 않았으므로 많은
	비판적 시각과 의심을 가지고 봐야만 합니다.
...
데이터를 위한 성능 평가 실험은 1\,GHz 로 동작하는 듀얼코어 Intel x86 (CPU 당
4300 bogomips) 에서 각 CPU 의 캐시에 최대 여섯개의 블럭들을 주고
수행되었습니다.
...
Y 축은 마이크로세컨드당 성공한 할당/해제 횟수입니다 --- 실패한 할당들은 카운트
되지 않습니다.
...
할당 수행 시간 길이가 6일 때까지는 선형적으로 확장되고 훌륭한 성능을 보이지만,
...
상당히 합리적으로, 이 경우 전체 시간의 99\,\% 에서 할당과 해제 작업들이
per-thread 풀에서만 이루어질 것이 보장됩니다.
...
\QuickQuizAnswer{
	...
	할당 수행 시간 길이 12 의 경우는 글로벌 풀 락을 두번 잡아야 하는 반면,
	할당 수행 시간 길이 13은 글로벌 풀 락을 세번 잡아야 합니다.
	\iffalse
...
\QuickQuiz{}
	두개 쓰레드를 사용한 테스트에서 할당 수행 길이가 19 이상일 때 할당
	실패가 발견되었습니다.
	글로벌 풀 사이즈가 40 이고 per-thread 타겟 풀 사이즈 $s$ 가 3 이고,
	쓰레드의 갯수 $n$ 가 2 이며, per-thread 풀들은 초기에 비어있고 메모리를
	아무도 사용하고 있지 않다고 가정하는 조건 하에서, 할당 실패가 발생할 수
	있는 최소의 할당 수행 시간 길이 $m$ 은 무엇일까요?
	(각 쓰레드는 메모리의 $m$ 블럭을 할당받고 $m$ 블럭의 메모리를 해제하는
	것을 반복함을 다시 이야기 드립니다.)
	그 대신에, $n$ 개 쓰레드가 각각 풀 사이즈 $s$ 를 갖는다면, 그리고 각
	쓰레드가 먼저 메모리의 $m$ 개 블럭을 할당받고 그 $m$ 개 블럭을 해제하는
...
\QuickQuizAnswer{
	...
	쓰레드별 풀과 쓰레드별 할당은 가장 왼쪽의 두개의 박스로 그려져
	있습니다.
	이 초기화 쓰레드는 할당된 블럭을 가지고 있지 않습니다만, $i$ 개의
	블럭들이 초기화 쓰레드의 쓰레드별 풀에 고립되어 있습니다.
	가장 오른쪽의 두개의 박스는 최대 가능한 수의 블럭을 쥐고 있는
...
널리 사용되는 이에 대한 해결 방법 중 하나는 1980년대 후반 BSD 메모리
할당자~\cite{McKusick88} 같은 방식으로, 고정된 크기의 집합을 제공하는 것인데,
...
아이디어는 똑같습니다 ---
Table~\ref{fig:app:questions:Schematic of Real-World Parallel Allocator} 에
보인것과 같은 병렬 빠른 수행 경로의 반복적인 적용.
\iffalse
