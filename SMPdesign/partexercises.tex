...
Partitioned} 에 그려져 있는데, 이 방법을 좀 더 잘 보이기 위해 철학자들의 수를
다섯명에서 네명으로 바꿨습니다.
여기서 위쪽과 오른쪽의 철학자들이 한 짝의 포크를 공유하고, 아래쪽과 왼쪽의
철학자들이 다른 한 짝의 포크를 공유합니다.
...
이것은 ``수평적 병렬성''~\cite{Inman85} 또는 ``데이터 병렬성'' 의 한 예인데,
두명씩으로 이루어진 철학자 그룹들 사이에 종속성이 없기 때문입니다.
수평적으로 병렬적인 데이터 처리 시스템에서 데이터의 개별 아이템은 소프트웨어
컴포넌트 집합의 복제본들 중 하나에서만 처리될 겁니다.
\iffalse
...
Double-ended 큐는 원소가 리스트의 양쪽 끝 중 어디로든 삽입되고 삭제될 수 있는
자료구조입니다~\cite{Knuth73}.
양 끝으로 원소가 동시에 삽입 / 삭제될 수 있는 Double-ended 큐를 락 기반으로
구현하는 것은 어렵다고 알려져 있습니다~\cite{DanGrossman2007TMGCAnalogy}.
이 섹션은 파티셔닝 디자인 전략이 어떻게 상당히 간단한 구현을 가능하게 하는지를
다음 섹션들에서 세개의 일반적인 방법들을 살펴봄으로써 알아봅니다.
\iffalse
...
직관적이어 보이는 방법중 하나는 양방향 링크드 리스트 (Doubly linked list) 를
왼쪽 인큐와 디큐를 위해 왼쪽 락을, 오른쪽 의 같은 작업들을 위해 오른쪽 락을
사용하는 것으로,
Figure~\ref{fig:SMPdesign:Double-Ended Queue With Left- and Right-Hand Locks}
에 그림으로 그려져 있습니다.
하지만, 이 방법의 문제는 이 리스트에 원소가 네개 미만일 때 두 락의 도메인이
겹쳐야 한다는 것입니다.
이 겹침은 어떤 원소의 삭제가 그 원소만이 아니라, 그것의 왼쪽 또는 오른쪽 이웃
원소에게도 영향을 끼치기 때문입니다.
이 도메인들은 그림에 색으로 표시되었는데, 아래로 향하는 줄무늬의 파란색이 왼쪽
락의 도메인을, 위쪽 줄무늬의 붉은색이 오른쪽 락의 도메인을, 그리고 줄무늬 없는
보라색은 겹치는 도메인을 의미합니다.
이 방법으로 동작하는 알고리즘을 만드는 것도 가능하겠지만, 최소 다섯개의 특별한
케이스들이 있다는 사실은 커다란 문제인데, 특히나 리스트의 다른 끝에서
동시적으로 이루어지는 동작이 언제든지 이 큐의 상태를 하나의 특별한 케이스에서
다른 케이스로 언제든지 바꿀 수 있습니다.
...
락 도메인이 겹치지 않게 하는 한가지 방법이
Figure~\ref{fig:SMPdesign:Compound Double-Ended Queue} 에 그려져 있습니다.
두개의 별도의 double-ended 큐가 직렬로 연결되며, 각각은 각자의 락으로
보호됩니다.
...
\begin{enumerate}
\item	오른쪽 락을 잡고 있다면, 오른쪽 락을 놓고 왼쪽 락을 다시 잡습니다.
\item	오른쪽 락을 잡습니다.
\item	두 큐 사이의 원소들의 균형을 다시 잡습니다.
\item	원소가 존재하면 요청받은대로 원소를 제거합니다.
\item	두 락을 모두 놓습니다.
\iffalse
...
\QuickQuiz{}
	...
	과정 사이에 더이상 비어있지 않게 된다면 어떻게 해야 할까요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	이 경우, 단순히 더이상 비어있지 않은 그 큐의 원소를 디큐하고, 락들을
	풀고, 리턴하면 그만입니다.
	\iffalse
...
그 결과로 만들어지는 코드 (\path{locktdeq.c}) 는 매우 직관적입니다.
균형을 다시 잡는 작업은 원소를 두 큐 사이에서 옮길 수도 있어서 시간을 소모할 수
있으므로 최적의 성능을 위해선 실제 워크로드에 따른 휴리스틱이 필요할 수 있을
겁니다.
...
락 도메인은 겹치지 않고, 체인 락에 앞서 인덱스 락을 획득하고 한번에 한 타입
(인덱스 또는 체인) 의 락만을 획득하게 함으로써 데드락이 발생 불가능하게
했습니다.
\iffalse
...
위에는 하나의 원소 (``R$_1$'') 이 오른쪽으로 들어온 후의, 오른쪽 인덱스가 해시
...
마지막 상황에서, 왼쪽 꺼내기 오퍼레이션은 ``L$_{-2}$'' 를 리턴하고 왼쪽
인덱스가 이제는 하나의 원소 (``R$_2$'')만을 가지고 있는 해시 체인~2 를
가리키도록 할 겁니다.
...
Line~6 에서는 왼쪽 스핀락을 획득하고, line~7 에서 꺼내어질 인덱스를 계산합니다.
...
\QuickQuizAnswer{
	...
	해보시기를 권장합니다.
	주의해야 하는 한가지 이유는, 이 구현에서 각 오퍼레이션은 한개가 아니라
...
간단한 균형잡기 방법을 사용해서 compound double-ended 큐를 다시 생각해 봅니다.
\iffalse
...
\QuickQuiz{}
	...
	대체 어떤 세상에서는 이런 정신나간 방법이 어떤 이유로든 최적인 거죠???
	\iffalse
...
\QuickQuizAnswer{
	...
	물론 double-ended 큐가 양쪽에서 동시에 비워진다면 매우 잘못된 선택일
	겁니다.
...
\co{pdeq_pop_l()} 과 \co{pdeq_pop_r()} 구현을 각각 고려해야 합니다.
\iffalse
...
\QuickQuizAnswer{
	...
	그랬듯이, 락 계층구조를 사용해 데드락을 예방해야 하는 관계로 비대칭성이
...
Line~5 는 왼쪽 락을 얻어오고, line~14 에서 이 락을 해제합니다.
...
놓습니다.
Line~10 에서 디큐된 원소가 존재했다면, 그 원소가 리턴됩니다.
\iffalse
...
앞에서 그랬듯, line~22 에서 오른쪽 락을 잡고 (line~36 에서 놓습니다), line~23
...
\QuickQuiz{}
	...
	그런데
	Figure~\ref{lst:SMPdesign:Compound Parallel Double-Ended Queue Implementation}
	의 line~25 에서는 왜 오른쪽 락을 무조건적으로 놔버려야 하는거죠?
	\iffalse
...
\QuickQuiz{}
	하지만 데이터가 한방향으로만 흐르는 경우에는,
	Listing~\ref{lst:SMPdesign:Compound Parallel Double-Ended Queue Implementation}
	에 보인 알고리즘은 디큐를 하는쪽 끝이 그 아래에 있는 double-ended queue
	를 비워버리게 되면 두 끝단이 모두 같은 락을 획득하려 시도할 겁니다.
	이는 이 알고리즘이 어떤 경우에 있어서는 큐가 임의의 많은 수의 원소들을
	가지고 있을 때조차도 큐의 양끝에 동시의 액세스를 제공하는데 실패한다는
	의미가 아닌가요?
	\iffalse
...
\QuickQuizAnswer{
	...
	하지만 이 속성을 주장하는 다른 알고리즘들에 있어서도 같습니다.
	예를 들어, 소프트웨어 트랜잭셔널 메모리 메커니즘을 사용하는 해결책들은
	락들의 해시된 배열에 기반하는데, 가장 왼쪽과 가장 오른쪽 원소들의
	주소들은 간혹 같은 락으로 해시됩니다.
	이 해시 충돌 역시 동시의 액세스를 막을 겁니다.
	또다른 예를 들어보면, 하드웨어 트랜잭셔널 메모리를 소프트웨어 폴백
	(fallback) 과 함께 사용하는
	해결책들~\cite{Yoo:2013:PEI:2503210.2503232,RickMerrit2011PowerTM,ChristianJacobi2012MainframeTM}
	은 종종 이 소프트웨어 폴백 내에서 락킹을 사용하며, 따라서 (흔치 않을
	테지만) 이 락킹 솔루션이 겪는 동시성의 제약으로 고통을 받게 됩니다.

	따라서, 2017년 말 기준, compound double-ended queue 를 포함해 동시성을
	갖는 double-ended queue 문제에 대한 모든 현실적 해결책들은 최소 일부
	환경에서는 완전한 동시성을 제공하는데 실패합니다.
	\iffalse
	Indeed it does!
...
심지어 하드웨어의 도움을 받는
알고리즘들~\cite{DavidDice:2010:SCA:HTM:deque} 과 비교해서도 잘 동작한다고
알려져 있습니다.
그러나, 이런 방법에서 우리는 최대 2배의 확장성까지만 기대할 수 있는데, 최대
두개의 쓰레드가 디큐를 위한 락들을 동시에 잡고 있을 수 있기 때문입니다.
...
구현들보다도 성능이 좋습니다.
따라서, 공유된 큐에 enqueue 와 dequeue 를 하는 데에는 실제 구현과 관계 없이
커다란 오버헤드가 존재한다는 점이 핵심입니다.
Chapter~\ref{chp:Hardware and its Habits} 에서의 내용과 이 큐들의 FIFO 특성을
생각하면 크게 놀라운 일도 아닙니다.
\iffalse
...
크리티컬 섹션들 안에 있습니다.
개별적인 오퍼레이션들이 시작된 시점의 관점에서 보기에 이 큐들은 엄격한 FIFO 는
...
Section~\ref{sec:SMPdesign:Dining Philosophers Problem} 의 Quick Quiz 에 대한
...
파이프라이닝에 필요한 더 밀착된 협동은 일의 더 큰 단위가 특정 수준의 효율성을
가질 것을 필요로 합니다.
\iffalse
...
\QuickQuizAnswer{
	...
	몰아서 처리될 수 있어서 각 enqueue 와 dequeue 오퍼레이션이 더 큰 단위의
	일에 연관될 수 있는지 알아보는 것도 해볼 가치가 있습니다.
