...
에서의 50-qubit 시스템 전시는 중요한
포인트입니다~\cite{WillKnight2017IBM50qubits}.
더 나아가, 만약 QC 가 고전적 컴퓨팅에 의해 만들어진 기술을 잘 사용할 수 있다면,
qubit 의 갯수가 갑자기 수십수백배로 들어날 가능성도 있습니다.
...
Consumer Electronics Show (CES) is a data point in
favor~\cite{WillKnight2017IBM50qubits}.
% bc -l: 12*l(2)/l(16/5)
% bc -l: 7*l(2)/l(50/16) = 4.25828003905821307361
Furthermore, if QC can make good use of the process technology
...
겁니다.
기계적 객체들의 쌍을 맨눈으로도 볼 수 있을 정도로 크게 (비록 0.15\,K 이지만)
entangle 시키려는 목적의 최근
연구~\cite{CFOckeloen-Korppi2017MassiveEntanglement} 또한 우리에게 향수를
느끼게 할 수 있을 겁니다.
무엇보다도, 우리의 맨눈으로 볼 수 없는 bit 이나 qubit 을 믿는걸 정말 정당화
시킬 수 있을까요?  $\ddot\smile$
\iffalse
...
또한, classical computing 설계와 제작에의 거대한 투자는 특수 목적의 classical
하드웨어가 만들어질 수 있게 했습니다.
예를 들어, Fujitsu 는 Digital Annealer 에 있어서 이 문제의 2018년 양자 컴퓨터
수준 성능을 갖는 칩을 만들었다는 이야기가
있습니다~\cite{JohnBoyd2018ClassicalAnneal}.
어떤 사람들은 이 Digital Annealer 의 제한된 연결성으로 인해 QC 시스템이 다시
한번 시뮬레이션된 annealing 성능 점수판을 지배할 거라 이야기하지만, 이는 물론
두고 봐야 할 일입니다.

Quantum computing 이 언젠가는 이 경주에서 승리하게 될 수도 있겠습니다만,
classic-computing 이 인상적인 경쟁을 보일 것임에도 의심의 여지가 없습니다.
\iffalse

In addition, huge investments in classical computing design and
fabrication enable special-purpose classical hardware to be constructed.
For example, Fujitsu is said to have created a Digital Annealer chip
that matches the performance of 2018 quantum computers on this
problem~\cite{JohnBoyd2018ClassicalAnneal}.
Some believe that the limited connectivity of the Digital Annealer
will allow QC systems to once again dominate the simulated-annealing
performance scoreboard, but this of course remains to be seen.

Quantum computing might one day win this race, but there can be no doubt that
classic computing is putting up an impressive fight.
\fi
