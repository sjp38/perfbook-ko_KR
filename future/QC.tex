...
현재로써는, 이 알고리즘을 사용할 가치가 알려진 바 없습니다.
\iffalse
...
해결할 수 있다는 가정 아래입니다.
더 나아가서, Shor's algorithm 의 진짜 구현은 추가적인 qubit 들이 필요할
것이어서, RSA 의 수명을 훨씬 연장시킬 듯 보입니다.

살짝 편집증적인 사람일지라도 RSA 를 교체하기엔 좀 이르다고 느낄 겁니다.
\iffalse
...
안에 찾아냅니다.
하지만 이게 이뤄지기 전에, 이 리스트는 QC 시스템에 다운로드 되어야만 합니다.
이에 경쟁하는 classical system 은 데이터에 대한 색인 작업을 진행한 후에 $O(\log
N)$ 시간의 탐색을 수행할 수 있는데, 이는 Grover's algorithm 으로 약속되는
$O(\sqrt N)$ 시간보다 훨씬 빠릅니다.
물론, 이는 데이터가 QC system 에 있다면 달라지겠습니다.
하지만, classical 대용량 저장장치의 거대한 용량과 낮은 비용을 생각해 보면,
상당한 데이터가 가까운 미래까지는 classical system 에 머물러 있을 것을 예측할
수 있습니다.

어떤 사람들은 Grover's algorithm 이 최적화 문제의 해답에 대한 탐색과 같은, 더
일반적인 탐색에 적용될 수 있을 것이라 믿습니다.
QC 를 통한 최적화는 이 섹션 뒤에서 다루겠습니다.
\iffalse
...
또한, IBM 은 chemistry 를 있을 수 있는 QC 어플리케이션이라 부릅니다.
\iffalse
...
따라서 quantum chemistry 는 QC 의 최고의 킬러 어플리케이션이 될 수도 있을
겁니다.
\iffalse
...
Nevertheless, innovative combinations of physical chemistry and
classical computing might well produce even better results,
although a key word here is ``might''.

Thus, quantum chemistry might be QC's best killer-app bet.
\fi
