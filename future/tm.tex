...
이루어진 트랜잭셔널 메모리 구현 (STM) 을 제안했습니다~\cite{Shavit95}.
...
비슷한 문제들이 \co{exit()} 과 \co{kill()} 시스템 콜, \co{longjmp()} 나
트랜잭션 밖으로 빠져나갈 수 있는 exception 들에 대해서도 존재합니다.
(\co{longjmp()} 나 exception 은 어디서 올까요?)
\iffalse
...
이 섹션은 RCU 에 주로 집중합니다.
TM 을 레퍼런스 카운터나 해저드 포인터 같은 다른 deferred-reclamation 메커니즘과
조합할 때에도 비슷한 문제나 해결책이 존재합니다.
아래에서, 알려진 차이점은 명시적으로 이야기될 겁니다.
\iffalse
...
\item	RCU 읽기 쓰레드들을 트랜잭션으로 변환시킵니다.
	...
	대해서는 적절치 못합니다.
	이 방법은 읽기 쓰레드의 코드 섹션을 명시적으로 정의하지 않는 해저드
	포인터에 적용하기에는 더 어렵습니다.
\iffalse
...
와 해저드 포인터의 구현의 발전에 따라서 특히 그렇습니다.\footnote{
...
\item	트랜잭셔널 메모리에 완화된 semantic 을 가져옵니다.
	한가지 방법은 Section~\ref{sec:future:Deferred Reclamation} 에서 설명된
	RCU 와의 조합으로, Gramoli 와 Guerraoui 가 예를 들면 커다란 ``신축성
	있는'' 트랜잭션들을 작은 트랜잭션들로의 제한적 분할로 (맥빠지는 성능과
...
\item	Introduce weak semantics to transactional memory.
	...
	Section~\ref{sec:future:Deferred Reclamation},
