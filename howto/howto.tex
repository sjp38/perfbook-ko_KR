...
있겠죠.
\Cref{chp:Introduction} 은 병렬 프로그래밍에 대한 높은 수준에서의 개요를
제공합니다.

\Cref{chp:Hardware and its Habits} 는 공유 메모리 병렬 하드웨어를
소개합니다.
어쨌건, 아래에 깔려있는 하드웨어를 이해하지 않고서는 좋은 병렬 코드를
작성하기가 어렵습니다.
하드웨어는 지속적으로 발전하므로, 이 챕터는 항상 시대에 뒤떨어져 있을 겁니다.
우린 최대한 시대에 맞춰지도록 최선을 다하겠지만요.
\Cref{chp:Tools of the Trade} 는 이어서 일반적인 공유 메모리 병렬
...
\Cref{chp:Counting} 은 상상 가능한 가장 간단한 문제 중 하나인 카운팅의
병렬화를 들여다 봅니다.
...
\Cref{chp:Locking} 은 여전히 제품 품질 병렬 프로그래밍에 널리 사용될
뿐만 아니라 병렬 프로그래밍의 최악의 악당으로도 널리 여겨지는 락킹을 다룹니다.
\Cref{chp:Data Ownership} 은 종종 과소평가되지만 놀라울 정도로 널리
...
\Cref{chp:Data Structures} 는 앞의 챕터들에서 배운 것들을 해시 테이블에
...
규칙의 고급 주제를 다룹니다.
\Cref{chp:Ease of Use} 는 사용하기 쉬운 몇가지 조언들을 봅니다.
...
\IfQqzChpEnd{}{
\Cref{chp:Answers to Quick Quizzes} 는 악명 높은 Quick Quizz
...
됩니다.
\Cref{lst:howto:Generating an Updated PDF} 의 커맨드는 반드시
