...
\QuickQuiz{}
	시퀀스 락을 동시적인 추가, 삭제, 그리고 검색을 지원하는 링크드 리스트를
	보호하는 유일한 동기화 메커니즘으로 사용할 수 있을까요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	이를 가능하게 하는 한가지 간단한 방법은 읽기만 하는 액세스들을 포함해서
	모든 액세스들을 \co{write_seqlock()} 과 \co{write_sequnlock()} 으로
	감싸는 것입니다.
	물론, 이 방법은 모든 읽기 쓰레드들 간의 병렬성을 방해할 것이고, 더
	나아가서 간단한 락킹을 사용하는 것만큼이나 쉽게 구현될 수 있을
	것입니다.

	읽기 쪽 액세스를 보호하는데 \co{read_seqbegin()} 과
	\co{read_Seqretry()} 함수를 사용하는 해결책을 찾아낸다면, 다음과 같은
	일련의 이벤트들에 대해서도 정확히 처리를 해주고 있음을 분명히 해야
	합니다:
	\iffalse
	...
	\begin{enumerate}
	\item	CPU~0 가 링크드 리스트를 탐색하다가 리스트의 원소~A 로의
		포인터를 집어냄.
	\item	CPU~1 이 리스트에서 원소~A 를 삭제하고 메모리에서 해제시킴.
	\item	CPU~2 가 관계없는 데이터 구조체를 위해 메모리를 할당받는데, 이
		때 원소~A 에서 사용되던 메모리를 얻어오게 됨.
		이 관계없는 데이터 구조체에서, 원소~A 의 \co{->next} 포인터로
		사용되던 메모리 영역은 이제 하나의 소수 (floating-point number)
		를 저장하게 됨.
	\item	CPU~0 원소~A 의 \co{->next} 포인터로 사용되던 메모리를 집어오고
		알 수 없는 무작위의 비트들을 갖게 되고, 따라서 세그먼테이션
		폴트가 발생함.
	\iffalse
...
\QuickQuiz{}
	...
	\co{read_seqbegin()} 의 line~19 에서의 체크를 왜 하는거죠?
	새로운 쓰기를 하는 쓰레드는 언제든 시작될 수 있다는 점을 생각해서, 왜
	간단하게 \co{read_seqretry()} 함수의 line~31 과 결합시키지 않는건가요?
	\iffalse
...
\QuickQuizAnswer{
	그것도 일리 있는 구현이 될겁니다.
	하지만, 그 어떤 것도 그 검사를 \co{read_seqretry()} 로 내려가게 하는
	것을 막지 못할 것입니다: 결국은 대략 같은 수의 인스트럭션들이 존재하게
	될겁니다.
	더욱이, 읽기를 하는 쓰레드의, 벌써 망한 read-side 크리티컬 섹션에서의
	액세스는 캐시 미스의 형태로 쓰기를 하는 쪽에 오버헤드를 가져다 줄수도
	있습니다.
	이 검사를
	Figure~\ref{fig:defer:Sequence-Locking Implementation} 의 line~19 에
	보여진 것처럼 \co{read_seqbegin()} 안에 위치시킴으로써 이런 캐시 미스를
	막을 수 있을 겁니다.
	\iffalse
...
Line~24-32 는 \co{read_seqretry()} 함수인데, 이 함수는 연관된
\co{read_seqbegin()} 의 실행 이래로부터 쓰기를 하는 쓰레드가 존재하지 않았다면
true 를 리턴합니다.
Line~29 에서는 호출자의 앞의 크리티컬 섹션을 line~30 에서의 시퀀스 카운터의
새로운 스냅샷을 얻어오는 작업 이전에 완료되도록 순서를 맞춥니다.
마지막으로, line~31 에서는 시퀀스 카운터가 변하지 않았음을 검사하는데, 달리
...
\QuickQuiz{}
	Figure~\ref{fig:defer:Sequence-Locking Implementation} 의 line~29
...
\QuickQuiz{}
	Figure~\ref{fig:defer:Sequence-Locking Implementation} 의 코드에서 좀
	더 완화된 형태의 메모리 배리어를 사용할 수는 없을까요?
	\iffalse
...
\QuickQuiz{}
	무엇이 시퀀스 락킹의 업데이트를 하는 쓰레드들이 읽기를 하는 쓰레드들이
	진행 못하게 하는걸 막을까요?
	\iffalse
...
Line~34-39 는 \co{write_seqlock()} 함수로, 단순히 락을 획득하고, 시퀀스 넘버를
증가시키고, 이 값 증가 연산이 호출자의 크리티컬 섹션 전으로 순서맞춰짐을 분명히
하도록 메모리 배리어를 실행합니다.
Line~41-46 은 \co{write_sequnlock()} 함수를 보여주는데, 이 함수는 호출자의
...
\QuickQuiz{}
	뭔가 다른 것들이 쓰기를 하는 쓰레드들을 직렬화 시켜서 락이 필요치
	않다면 어떻게 되죠?
	\iffalse
...
수 있지만, 그 반대는 불가능합니다.
이는 공정하지 않고 심지어는 쓰기가 대부분인 워크로드에서는 스타베이션을 낼수도
있습니다.
반면에, 쓰기를 하는 쓰레드가 없다면, 시퀀스 락을 사용하는 읽기를 하는
쓰레드들은 합리적인 수준으로 빠르고 선형적으로 확장이 가능할 것입니다.
두가지 모두 최선인 경우를 원하는 게 사람입니다: 읽는 쪽의 실패나 스타베이션의
가능성이 없는 빠른 읽기.
%또한, 시퀀스 락킹의 포인터들에 대한 제약점들 역시 극복된다면 좋을 겁니다.
%다음 섹션은 정확히 이런 속성을 가진 동기화 메커니즘을 소개합니다.
\iffalse
