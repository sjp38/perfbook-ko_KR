...
Figure~\ref{fig:defer:RCU Pre-BSD Routing Table Lookup}
...
% TODO: translate upstream change
...
에 있습니다.
RCU QSBR 은 바랬던 대로 이상적인 동기화를 아예 하지 않는 워크로드와 거의 동일한
성능과 확장성을 갖습니다.
\iffalse
...
\QuickQuiz{}
	...
	\fi
\QuickQuizAnswer{
	\co{rcu_dereference()} 도구는 컴파일러의 최적화를 조금 제약할 수
	있어서, 아주 약간 느린 코드를 만들어낼 수 있습니다.
...
\QuickQuiz{}
	...
	\fi
\QuickQuizAnswer{
	RCU QSBR 은 경우에 따라서는 불가능한 제약을 어플리케이션에 강제하는데,
	예를 들어 어플리케이션 내의 각각의 모든 쓰레드가 quiescent state 를
	정규적으로 통과해야 하는 식입니다.
...
물론, RCU 는 LKML 에서 논의되고 Section~\ref{sec:defer:Reference Counting} 에
요약되어 있는 것처럼, 전통적인 레퍼런스 카운팅과 함께 조합될 수도 있습니다.
\iffalse
