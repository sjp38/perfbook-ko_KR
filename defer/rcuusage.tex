...
논의를 위해선
Figure~\ref{fig:defer:QSBR: Waiting for Pre-Existing Readers} 를
참고하세요.)
\iffalse
...
QRCU 와
Appendix~\ref{chp:app:``Toy'' RCU Implementations} 에 설명된 일부 ``장난감''
RCU 구현의 예외가 있지만, RCU grace period 는 수 밀리세컨드까지 늘어납니다.
사용 가능한 비동기적인 인터페이스 (\co{call_rcu()} 와 \co{call_rcu_bh()}) 를
사용하는 방법을 포함해, 그런 긴 딜레이를 위험하지 않게 만드는 몇가지 테크닉들이
있긴 하지만, 이 상황이 RCU 가 읽기가 대부분인 상황에서만 사용되어야 한다는
규칙의 주요 이유입니다.
\iffalse
...
가장 적합한 경우, reader-writer 락킹에서 RCU 로의 변경은
...
이 \co{rcu_read_lock()} 은 \co{p} 로의 레퍼런스를 얻어오는 것으로 볼 수 있는데,
\co{rcu_dereference()} 를 통한 \co{p} 로의 값 할당 후 시작되는 grace period 는
연관되는 \co{rcu_read_unlock()} 전까지는 끝나지 않을 것이기 때문입니다.
이 레퍼런스 카운팅 방식은 RCU read-side 크리티컬 섹션 안에서는 블락이 허용되지
않고, 다른 태스크로 RCU read-side 크리티컬 섹션을 넘겨줄 수도 없다는 점에서
제한적입니다.
...
\QuickQuizAnswer{
	...
	이 중요성에 대해 한가지만 예를 들어 보자면, RCU 를 제한적인 레퍼런스
	카운팅 방법으로 생각하고 있다면, 업데이트가 RCU read-side 크리티컬
	섹션들과 배타적으로 수행될거라 생각하는 실수를 하지 않을 겁니다.

	하지만 RCU 를 reader-writer 락킹의 대체제로 생각하는 건 종종
	쓸모있는데, 예를 들면 reader-writer 락킹을 RCU 로 대체할 때입니다.
	\iffalse
...
하지만 왜 이런 신경을 쓰는 걸까요?
다시 말하지만, 그에 대한 답 중 하나는 성능으로, 16-CPU 3GHz Intel x86
시스템에서의 데이터가
Figure~\ref{fig:defer:Performance of RCU vs. Reference Counting} 에 그려져
있습니다.
\iffalse
...
\QuickQuiz{}
	6 CPU 근처에서 refcnt 오버헤드의 증가도가 확 떨어지는건 왜그렇죠?
	\iffalse
...
그리고, reader-writer 락킹처럼, RCU 의 성능상 이득은
...
하지만, 다양한 데이터 구조체에 하나의 글로벌 레퍼런스 카운터를 사용하는 것은 이
레퍼런스 카운트를 담고 있는 캐시 라인의 바운싱 (cache line bouncing) 을
초래합니다.
...
물론, SRCU 도 그 자신의 제약을 가지는데, \co{srcu_read_lock()} 의 리턴값은
...
RCU 를 처음 공부하는 사람들이 갖는, 희귀하지 않은 느낌 가운데 하나는 ``RCU 는
가비지 콜렉터 같은 것이다!'' 입니다.
이 느낌은 커다란 진실을 품고 있기는 하지만, 오해가 될 수도 있습니다.
\iffalse
...
RCU 와 자동화된 가비지 콜렉터 (GC) 사이의 관계를 생각하는 가장 좋은 방법은
RCU 는 콜렉션의 \emph{타이밍} 이 자동으로 정해진다는 점에서 RCU 가 GC 와
닮았다는 점입니다만, RCU 는 GC 와 다음과 같은 면에서 다릅니다: (1) 프로그래머는
언제 특정 데이터 구조체가 정리되어도 좋은지를 일일이 알려야만 하고, (2)
프로그래머는 레퍼런스들이 합법적으로 붙잡힐 수 있는 RCU read-side 크리티컬
섹션들을 일일이 표시해야만 합니다.
\iffalse
...
위해 가비지 콜렉터를 사용했습니다.
하지만, RCU 를 생각하는 더 나은 방법이 다음 섹션에 설명되어 있습니다.
\iffalse
...
Section~\ref{sec:locking:Lock-Based Existence Guarantees} 에서는 락킹을
사용해서 어떻게 존재 보장을 하는지를 그렇게 하는 것의 단점과 함께
이야기했습니다.
만약 RCU 로 보호되는 데이터 항목이 RCU read-side 크리티컬 섹션 내에서
...
은 RCU 기반의 존재 보장이 어떻게 원소별 락킹을 가능하게 하는지를 해시
...
\QuickQuizAnswer{
	...
	이 해시 테이블을 완전한 체이닝을 사용하도록 바꿔보는 건 독자 여러분의
	몫으로 두겠습니다.
	\iffalse
...
여러 lockless 알고리즘들이 데이터 원소가 그것을 레퍼런스 하고 있는
...
이와 같은, 학술적 문맥에서는 ``type-safe memory'' 라 불리는~\cite{Cheriton96a}
보장사항은 앞 섹션에서 이야기한 존재 보장보다 더 완화된 형태이고, 따라서 이걸
가지고  일을 처리하기는 약간 더 어렵습니다.
리눅스 커널 안의 Type-safe 메모리 알고리즘들은 slab 캐시들을 사용하는데, 특히
이런 캐시들을 \co{SLAB_TYPESAFE_BY_RCU} 로 표시해서 사용되지 않는 slab 을 시스템
메모리로 반환할 때 RCU 가 사용되게 합니다.
이런 RCU 의 사용은 slab 의 사용중인 원소들은 그 slab 안에 남아 있을 것을
보장해서 앞서 존재한 RCU read-side 크리티컬 섹션들의 기간동안은 그 타입을
유지하게 됩니다.
\iffalse
...
\QuickQuiz{}
	하지만 여러 쓰레드에 상당히 긴 RCU read-side 크리티컬 섹션이 존재해서
	어떤 특정한 시점에든 시스템의 최소 하나의 쓰레드는 RCU read-side
	크리티컬 섹션을 수행하고 있으면 어떡하죠?
	그게 어떤 데이터가 \co{SLAB_TYPESAFE_BY_RCU} 슬랩에서 시스템으로
...
\QuickQuizAnswer{
	...
	특정 RCU read-side 크리티컬 섹션은 컨셉적으로 그 크리티컬 섹션의 시작
...
분명히 하기 위해 일반적으로 검증 단계를
갖습니다~\cite[Section 2.5]{LaninShasha1986TSM}.
이런 검증은 데이터 구조체의 특정 부분이 메모리 해제-재할당 프로세스에서
수정되지 않았을 것을 요구합니다.
그런 검증은 일반적으로 잘 수행되기가 매우 어렵고, 애매하고 어려운 버그들을 숨길
수 있습니다.
...
Section~\ref{sec:defer:RCU Fundamentals} 에서 이야기했듯이, RCU 의 중요한 부분
중 하나는 RCU 읽기 쓰레드들이 끝나기를 기다리는 방법입니다.
RCU 의 큰 장점 가운데 하나는 수천개의 서로 다른 것들이 각자 끝나기를 기다리면서
그것들 각각의 정보를 명시적으로 추적할 필요가 없고, 명시적인 정보 추적
방식에서는 심각한 성능 저하, 확장성 제한, 복잡한 데드락 시나리오, 그리고 메모리
누수 문제를 걱정할 필요도 없다는 겁니다.
\iffalse
...
\begin{enumerate}
\item	예를들면 OS 가 NMI 에 반응하는 것과 같은 방식으로 변경을 만듭니다.
...
\item	예를 들어 메모리를 반환하는 작업은 변경이 성공적으로 만들어졌음을
	알리는 상태를 반환합니다.
\iffalse
...
사용합니다.
이 코드의 간략화된 버전이
Listing~\ref{lst:defer:Using RCU to Wait for NMIs to Finish} 에 있습니다.
\iffalse
...
\begin{enumerate}
\item	새로운 데이터를 추가하기 위한 publish-subscribe 메커니즘,
...
그러나, 앞의 섹션들에서 선보인 reader-writer 락킹, 레퍼런스 카운팅, 그리고 존재
보장 등의 더 높은 수준의 것을 RCU 위에 만드는 것이 가능합니다.
...
일관적인 데이터가 필요하며 읽기가 대부분인 워크로드라면, RCU 는 초록색의
``read-mostly, need consistent data'' 박스로 보인 것처럼 잘 동작합니다.
이런 케이스의 한가지 예는 리눅스 커널의 사용자 레벨 System-V 세마포어 ID 에서
...
사용은 읽기가 대부분인 경우에 비해 대부분의 경우 더 복잡합니다.
\iffalse
...
이야기했듯, 리눅스 커널 내에서 \co{SLAB_TYPESAFE_BY_RCU} 슬랩 얼로케이터
