...
하는데, 이 캐쉬들을 \co{SLAB_TYPESAFE_BY_RCU} 로 표시하여 메모리 해제된 slab 을
...
\QuickQuiz{
	하지만 여러 쓰레드에 임의의 긴 RCU read-side 크리티컬 섹션들이 존재하여
	어느 시점에서든 시스템 내에 최소 하나의 RCU read-side 크리티컬 섹션을
	수행하는 쓰레드가 존재하면 어떻게 되죠?
	그건 \co{SLAB_TYPESAFE_BY_RCU} slab 의 어떤 데이터든 시스템에 반환되는
	것을 막아서 OOM 상황에 이르지 않을까요?
	...
	최소 하나의 쓰레드가 RCU read-side 크리티컬 섹션에 있는 무한정 긴
	시간이 분명 존재할 수 있습니다.
	하지만,
	Section~\ref{sec:defer:RCU Provides Type-Safe Memory}
	의 설명 중 핵심 단어는 ``사용중'' 그리고 ``앞서서부터 존재한'' 입니다.
	특정 RCU read-side 크리티컬 섹션은 이론상 그 크리티컬 섹션의 시작
	시점에 사용 중이던 데이터 원소로의 참조만을 얻을 수 있습니다.
	더 나아가, slab 은 그 데이터 원소가 모두 메모리 해제되기 전까지는
	시스템에 반환 될 수 없으며, 실제로 RCU grace period 는 그것들이 모두
	메모리 해제되기 전까지는 시작될 수 없음을 기억하십시오.

	따라서, slab cache 는 해당 slab 의 마지막 원소가 메모리 해제되기 시작된
	RCU read-side 크리티컬 섹션들만을 기다려야 합니다.
	이는 결국 이 마지막 원소가 메모리 해제된 후 시작된 모든 RCU grace
	period 만 카운트 됨을 의미합니다---이 slab 은 그 grace period 가 종료된
	후 시스템으로 반환될 수 있을 겁니다.
...
것이 중요합니다!
실제로, \co{rcu_dereference()} 로 막 반환된, \co{SLAB_TYPESAFE_BY_RCU} 로
보호되는 데이터 구조는 임의의 많은 횟수동안 메모리 해제되고 재할당 되었을 수
있는데, \co{rcu_read_lock()} 의 보호 아래에서도 그렇습니다.
그 대신, \co{SLAB_TYPESAFE_BY_RCU} 는 \co{kmem_cache_free()} 가 완전히 메모리
해제된 데이터 구조들의 slab 을 RCU grace period 가 지나기 전에 시스템에
반환하는 것을 막습니다.
요약하자면, 주어진 RCU read-side 크리티컬 섹션이 특정 \co{SLAB_TYPESAFE_BY_RCU}
...
이 섹션에서는 어떻게 \co{synchronize_sched()} 의 읽기 쪽 비슷한 (하드웨어
...
\item	모든 앞서서부터 존재해온 read-side 크리티컬 섹션이 완전히 종료하길
	기다립니다 (예를 들면, \co{synchronize_sched()} 기능을 이용해서).
...
이 예에서, \co{nmi_stop()} 함수는 모든 진행중인 NMI 알림이 연관된 리소스를
메모리 해제하기 전에 완료되었음을 보장하기 위해 \co{synchronize_sched()} 를
사용합니다.
이 코드의 간략화 된 코드가
...
the Linux kernel.
In this example, the \co{nmi_stop()} function uses
...
\Clnrefrange{b}{e} 는 크기와 길이가 정해지지 않은 엔트리의 배열을 담는
\co{profile_buffer} 구조체를 정의합니다.
라인~\lnref{buf} 는 profile 버퍼 하나를 가리키는 포인터를 정의하는데, 이는
어딘가에서 동적으로 할당된 메모리 영역을 가리키도록 초기화 될 겁니다.
\end{fcvref}
...
\Clnrefrange{b}{e} 는 \co{nmi_profile()} 함수를 정의하는데, 이 함수는 NMI
핸들러 내에서 호출됩니다.
따라서, 이 함수는 preemption 될 수도 없고 평범한 인터럽트 핸들러에 의해
인터럽트될 수도 없습니다만, 캐쉬 미스, ECC 에러, 그리고 같은 코어의 다른
하드웨어 쓰레드에 의해 빼앗기는 사이클로 지연될 수는 있습니다.
라인~\lnref{rcu_deref} 는 profile 버퍼로의 지역 포인터를 DEC Alpha 에서의
메모리 순서를 보장하기 위해 \co{rcu_deref()} 기능을 이용해 가져오고,
라인~\lnref{if_NULL} 과~\lnref{ret:a} 는 현재 할당된 profile 버퍼가 없으면 이
함수에서 빠져나가며, 라인~\lnref{if_oor} 과~\lnref{ret:b} 는 \co{pcvalue}
인자가 범위 밖이면 이 함수에서 나갑니다.
그렇지 않다면, 라인~\lnref{inc} 는 \co{pcvalue} 인자에 의해 인덱스 되는 profile
버퍼 엔트리를 증가시킵니다.
...
\Clnrefrange{b}{e} 는 \co{nmi_stop()} 함수를 정의하는데, 이 함수의 호출자는
상호 배타적일 (예를 들어, 올바른 락을 잡고 있을) 필요가 있습니다.
라인~\lnref{fetch} 는 이 profile 버퍼로의 포인터를 가져오고,
라인~\lnref{if_NULL} 과~\lnref{ret} 는 버퍼가 없으면 이 함수를 나갑니다.
그렇지 않다면, 라인~\lnref{NULL} 은 이 profile 버퍼 포인터를 \co{NULL} 로
만들고 (완화된 메모리 순서 규칙 기계에서의 메모리 순서를 유지하기 위해
\co{rcu_assign_pointer()} 기능을 사용합니다.) 라인~\lnref{sync_sched} 는 하나의
RCU Sched grace period 가 지나가길 기다리는데, 특히, NMI 핸들러를 포함한 모든
preemption 불가한 코드 영역들이 완료되길 기다립니다.
수행이 라인~\lnref{kfree} 까지 이어진다면, 기존 버퍼로의 포인터를 얻은
\co{nmi_profile()} 인스턴스는 모두 리턴했을 것이 보장됩니다.
따라서 이 버퍼를 메모리 해제해도 안전하며, 여기선 \co{kfree()} 를 사용합니다.
\end{fcvref}
...
\QuickQuiz{
	\co{nmi_profile()} 함수가 preemption 가능했다고 쳐 봅시다.
	이 예제가 올바르게 동작하려면 무엇이 바뀌어야 할까요?
	...
	한가지 방법은 \co{nmi_profile()} 에서 \co{rcu_read_lock()} 과
	\co{rcu_read_unlock()} 을 사용하고 \co{synchronize_sched()} 를
	\co{synchronize_rcu()} 로 교체하는 것으로,
...
요약하자면, RCU 는 동적인 profile 버퍼들 간의 교체를 쉽게 해줍니다 (이걸 어토믹
오퍼레이션 또는 락킹을 사용해서 이걸 효율적으로 하려고 \emph{시도} 해 보세요!).
하지만, 앞의 섹션에서 보았듯 RCU 는 보통 더 높은 수준의 추상화 단계에서
사용됩니다.
...
마지막으로, 이 그림의 바닥에 빨간 상자로 표시되었듯, 업데이트가 대부분이며
일관적인 데이터를 필요로 하는 워크로드는 RCU 를 사용하기 좋은 경우가
드뭅니다만, 몇가지 예외가 있긴 합니다~\cite{MathieuDesnoyers2012URCU}.
또한,
Section~\ref{sec:defer:RCU Provides Type-Safe Memory} 에서 이야기 되었듯,
리눅스 커널 내에서는 \co{SLAB_TYPESAFE_BY_RCU} 슬랩 할당자 플래그가 type-safe
메모리를 RCU 읽기 쓰레드에게 제공하는데, 이는 non-blocking 동기화와 다른
lockless 알고리즘들을 상당히 단순화 시킬 수 있습니다.

요약하자면, RCU 는 새로운 데이터를 추가하기 위한 발행-구독 메커니즘, 앞서서부터
조재해온 RCU 읽기 쓰레드들이 끝나길 기다리는 한가지 방법, 그리고 동시의 RCU
읽기 쓰레드에게 해를 입히거나 지연시키지 않으면서 업데이트를 하기 위한 여러
버전을 유지하는 규칙을 제공하는 API 입니다.
이 RCU API 는 읽기가 대부분인 상황에 가장 잘 동작하며, 특히 오래되어 더이상
유효하지 않고 비일관적인 데이터가 어플리케이션에 의해 통제될 수 있을 때 더욱
그렇습니다.
