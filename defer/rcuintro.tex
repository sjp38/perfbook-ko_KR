...
\co{WRITE_ONCE()} 또는 나중에 설명되겠지만 \co{rcu_assign_pointer()} 를
사용해야만 합니다.
\iffalse
...
보입니다.
모든 걸 멈추기는 응답 시간을 상당히 악화시킬 수 있는데, 이는 동시적 가비지
...
이 관찰은 네번째 방법을 이끄는데, 하나의 CPU 또는 쓰레드를 충분한 시간동안
멈추는 것으로, 이 방법은 모든 응답 시간을 악화시키진 않고, 멈춰진 하나의 것의
응답시간만 악화시킵니다.
