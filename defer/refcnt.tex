...
있습니다~\cite{Weizenbaum:1963:SLP:367593.367617}.\footnote{
	...
	이 기계에 들어가기 전에, 각 일꾼은 이 기계의 on/off 스위치에 자물쇠를
	채워서 이 일꾼이 기계 안에 있는 동안 기계가 켜지지 못하게 했습니다.}
...
사용을 체크하는 필드인 \co{->re_freed} 를 추가하며, line~9 에서는 동시의
업데이트들을 동기화 시키는데 사용될 \co{routelock} 을 추가하며, line~11-15 는
\co{re_free()} 를 추가하는데, 이 함수는 \co{->re_freed} 의 값을 넣어서
\co{route_lookup()} 이 해제 후 사용 버그를 체크할 수 있게 합니다.
\co{route_lookup()} 내에서, line~29-31 은 앞의 원소의 레퍼런스 카운트 값을
...
Listing~\ref{lst:defer:Reference-Counted Pre-BSD Routing Table Add/Delete}
에서, line~12, 16, 25, 33, 그리고~40 은 동시의 업데이트 작업들을 동기화 시키기
위한 락킹을 보이고 있습니다.
...
아닙니다:
레퍼런스 카운트 획득과 해제는 read-only 워크로드에서도 빈번한 공유 메모리로의
쓰기를 추가했고, 따라서 물리 법칙의 상당한 보복을 이끌어냈습니다.
...
\QuickQuiz{}
	...
	\fi
\QuickQuizAnswer{
	레퍼런스 카운팅의 끔찍한 확장성을 고려해보면, 누가 여덟개보다 많은 CPU
	를 필요로 하겠어요?
...
\item	Thread~A 가 address~42 를 탐색해서
	...
	이 원소로의 레퍼런스를 획득하진 못한 상태입니다.
\item	Thread~B 가 address~42 의 route 원소를 지우려고
	Listing~\ref{lst:defer:Reference-Counted Pre-BSD Routing Table Add/Delete}
	의 \co{route_del()} 을 호출합니다.
	이 호출이 성공적으로 완료되어, 이 원소의 \co{->re_refcnt} 필드는 그
	값이 1이므로, \co{->re_freed} 필드를 설정하고 원소를 해제하기 위해
	\co{re_free()} 를 호출합니다.
...
쉽고~\cite{MagedMichael95a} 처참한 수준의 성능과 확장성만을
제공합니다~\cite{ThomasEHart2007a}.

한마디로, 동시성은 레퍼런스 카운팅의 유용성을 분명히 저하시켰습니다!
\iffalse
...
\QuickQuiz{}
	동시성이 ``레퍼런스 카운팅의 유용성을 분명히 저하시켰다''면, 리눅스
...
\QuickQuizAnswer{
	...
	리눅스 커널이 높은 동시성이 존재하는 환경에서 레퍼런스 카운팅의 장점을
...
그렇다곤 하지만, 가끔은 문제를 성공적으로 해결하기 위해선 문제를 완전히 다른
방법으로 바라볼 필요가 있습니다.
