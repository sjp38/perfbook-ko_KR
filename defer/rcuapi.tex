...
(\co{hlist_nulls} \co{NULL} 포인터를 받으면 \co{true} 를 리턴합니다) 를,
\co{NULL} 포인터의 타입을 가져오기 위해 \co{get_nulls_value()} (이 함수는
인자의 \co{NULL} 포인터 식별자를 리턴합니다) 를 사용할 수 있습니다.
...
that reader with it.
The reader can use the \co{is_a_nulls()} function (which returns \co{true}
...
사용될 수 있습니다.
RCU 리스트 순회 기능들은
\co{list_for_each_entry_rcu()}, \co{hlist_for_each_entry_rcu()} 등을
포함합니다.
비슷하게, RCU 리스트 업데이트 기능들은 \co{list_add_rcu()},
\co{hlist_del_rcu()} 등을 포함합니다.

RCU 의 다른 제품군에서의 기능들이 서로를 대체할 수 있는데, 예를 들어
\co{srcu_read_lock()} 은 \co{rcu_read_lock()} 이 사용될 수 있는 모든 context
에서 사용될 수 있습니다.
