...
Table~\ref{tab:defer:RCU Wait-to-Finish APIs} 와
...
크리티컬 섹션을 구분짓고, \co{call_rcu_bh()} 는 다음의 grace period 후에 특정
함수를 특정 인자와 함께 호출해 줍니다.
RCU BH 는 필요한 사람은 쉽게 구현할 수 있겠지만, 동기적인
\co{synchronize_rcu_bh()} 인터페이스를 갖지 않는다는데 주의하세요.
\iffalse
...
\QuickQuiz{}
	왜 SRCU 와 QRCU 둘 다 비동기적인 \co{call_srcu()} 나 \co{call_qrcu()}
	인터페이스가 없나요?
	\iffalse
	...
	\fi
\QuickQuizAnswer{
	비동기 인터페이스가 있다면, 하나의 태스크는 무수히 많은 SRCU 나 QRCU
	콜백들을 가질 수 있고, 따라서 무한히 큰 메모리를 소모할 수 있습니다.
	반면에, 현재의 동기적인 \co{synchronize_srcu()} 와
	\co{synchronize_qrcu()} 인터페이스들을 가지고는 특정 태스크는 다음
	grace period 를 기다리기 시작하기 전에 현재 grace period 를 기다리기를
	끝내야만 합니다.
	\iffalse
...
Table~\ref{tab:defer:Sleepable RCU Wait-to-Finish APIs} ``SRCU'' 행은 RCU
...
``QRCU'' 행은 SRCU 와 똑같은 API 구조를 갖지만 읽기 쓰레드들이 존재하지 않을
때에는 grace period 들의 극단적으로 낮은 대기시간을 위해 최적화된, 다른곳에서
설명된~\cite{PaulEMcKenney2007QRCUspin} RCU 구현을 소개합니다.
SRCU 에서와 마찬가지로, QRCU read-side 크리티컬 섹션 내에서의
\co{synchronize_qrcu()} 사용은 스스로의 deadlock 을 유발할 수 있으므로 그런
일은 없어야만 합니다.
QRCU 가 아직 리눅스 커널에 들어가지는 않았지만 마이크로세컨드 내의 grace-period
대기시간을 자랑할 수 있는 유일한 커널 수준의 RCU 구현이기 때문에 언급할 가치가
있습니다.
\iffalse
...
\item	RCU (Classic 과 Real-time 둘 다): read-side 크리티컬 섹션들은 NMI
	...
	보장해야 합니다.
	RCU 는 범위내에서 글로벌합니다.
\item	SRCU 와 QRCU: read-side 크리티컬 섹션들은 다른 태스크가 연관된 grace
	...
	이내에는 완료되어야 합니다 (그리고 더 빠르면 더 좋습니다).\footnote{
		...
		말씀을 드립니다.}
	SRCU 그리고 QRCU 의 범위는 각각 연관된 \co{srcu_struct} 나
	\co{qrcu_struct} 의 사용에 따라 정의됩니다.
\iffalse
...
달리 말하자면, SRCU 와 QRCU 는 개발자가 그 범위를 제한할 수 있도록 하는 것으로
