...
함을 의미합니다.
그런 한가지 구현이
Listing~\ref{lst:defer:Hazard-Pointer Storage and Erasure} 에 보여져 있는데,
line~1-14 에서 \co{hp_store()} 를 보이고 line~16-21 에서 \co{hp_erase()} 를
보이고 있습니다.
\co{smp_mb()} 기능은
...
거부하고 0을 리턴함으로써 호출자는 처음부터 데이터 접근을 다시 시작해야 함을
알립니다.
...
\QuickQuiz{}
	...
	\fi
\QuickQuizAnswer{
	\co{hp_store()} 는 동시의 수정에 대해서 체크를 해봐야 하기 때문입니다.
...
\QuickQuizAnswer{
	...
	포인터로 보호되는 링크드 리스트가 다음의 일련의 이벤트들을 겪는다고
	생각해 봅시다:
	\iffalse
	...
	쓰레드~0 는 이제는 제거된 원소~C 에 액세스를 시도할 수 있으며, 이는
...
해저드 포인터들을 사용하는 알고리즘들은 데이터 구조체를 탐색하는 중 어떤
...
반면에, 해저드 포인터는 동시의 업데이트가 존재할 때에도 올바르게 동작하며,
2019년초 기준으로 Facebook 과 MongoDB 에서 사용되고 있습니다.
\iffalse
