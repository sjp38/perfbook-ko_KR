...
일을 뒤로 미루는 전략은 역사의 기록이 시작되기 전부터 있었습니다.
이것은 미루기나 완전한 게으름으로 여겨져 종종 비웃음을 받아왔습니다.
하지만, 사람들은 지난 수십년간 병렬 알고리즘의 단순화와 능률화에 있어서의 이
전략의 가치를 깨달았습니다~\cite{Kung80,HMassalinPhD}.
이걸 믿든 믿지 않든, 병렬 프로그래밍에서의 ``게으름'' 은 많은 경우 근면성에
비해 성능과 확장성이 좋습니다!
...
에서는 이 챕터에서 다루어진 일 뒤로 미루기 전략들 가운데 선택을 어떻게 해야
하는지 이야기 하고, 
...
이 챕터는 이 접근법들의 가치를 보이고 또 그것들을 서로 비교할 수 있게끔
단순화된 패킷 라우팅 알고리즘을 사용할 겁니다.
라우팅 알고리즘은 운영체제 커널에서 각각의 바깥으로 나가는 TCP/IP 패킷들을
알맞는 네트워크 인터페이스로 전달하는데 사용됩니다.
여기서 사용하는 알고리즘은 고전적인 1980년대의 packet-train-optimized
알고리즘으로 BSD UNIX~\cite{VanJacobson88} 에서 사용되었으며, 단순한 링크드
리스트로 구성되었습니다.\footnote{
...
우리는 더 나아가서 출발지와 목적지 IP 주소와 포트, 네가지 정보로 구성되는 검색
...
외부의 네트워크는 안정적일 것을 가정하면, 이 리스트는 매우 자주 검색되고 아주
가끔씩만 업데이트 될 것입니다.
Chapter~\ref{chp:Hardware and its Habits} 에서 우리는 빛의 제한된 속도와 물질의
원자성의 자연적 법칙과 같은 불편한 물리 법칙을 회피하는 가장 좋은 방법은
...
Figure~\ref{fig:defer:Pre-BSD Packet Routing List} 의 간단한 싱글 쓰레드 구현을
보입니다.
...
\co{route_entry} 구조체가 존재한다면 제거하고 그렇지 않다면 \co{-ENOENT} 를
리턴합니다.
